...
: carma_ros2_utils::CarmaLifecycleNode(options), gsm_(this->get_node_logging_interface())
{
...
rcl_interfaces::msg::SetParametersResult GuidanceWorker::parameter_update_callback(
...
carma_ros2_utils::CallbackReturn GuidanceWorker::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  add_on_set_parameters_callback(
    std::bind(&GuidanceWorker::parameter_update_callback, this, std_ph::_1));

  // Setup subscribers
  robot_status_subscriber_ = create_subscription<carma_driver_msgs::msg::RobotEnabled>(
    "robot_status", 5, std::bind(&GuidanceWorker::robot_status_cb, this, std_ph::_1));
  route_event_subscriber_ = create_subscription<carma_planning_msgs::msg::RouteEvent>(
    "route_event", 5, std::bind(&GuidanceWorker::route_event_cb, this, std_ph::_1));
  vehicle_status_subscriber_ = create_subscription<autoware_msgs::msg::VehicleStatus>(
    "vehicle_status", 5, std::bind(&GuidanceWorker::vehicle_status_cb, this, std_ph::_1));
  ...
  guidance_activate_service_server_ = create_service<carma_planning_msgs::srv::SetGuidanceActive>(
    "set_guidance_active",
...
carma_ros2_utils::CallbackReturn GuidanceWorker::handle_on_activate(
  const rclcpp_lifecycle::State & prev_state)
{
  // Setup timer
  auto guidance_spin_period_ms = int(
    (1 / config_.spin_rate_hz) *
    1000);  // Conversion rom frequency (Hz) to milliseconds time period
  spin_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(guidance_spin_period_ms),
    std::bind(&GuidanceWorker::spin_cb, this));

  // Update Guidance State Machine since node activation by the lifecycle system indicates that all
  // required drivers are ready
  gsm_.onGuidanceInitialized();

  return CallbackReturn::SUCCESS;
}

carma_ros2_utils::CallbackReturn GuidanceWorker::handle_on_shutdown(
  const rclcpp_lifecycle::State & prev_state)
{
...
bool GuidanceWorker::guidance_activation_cb(
  ...
  std::shared_ptr<carma_planning_msgs::srv::SetGuidanceActive::Response> resp)
{
  ...
  if (!req->guidance_active) {
    auto enable_robotic_request =
      std::make_shared<carma_driver_msgs::srv::SetEnableRobotic::Request>();
