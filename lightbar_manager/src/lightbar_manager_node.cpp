...
carma_ros2_utils::CallbackReturn LightBarManager::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("lightbar_manager"), "Initalizing lightbar manager node...");
  ...
  request_control_server_ = create_service<carma_msgs::srv::RequestIndicatorControl>(
    "request_control",
    std::bind(&LightBarManager::requestControlCallBack, this, std_ph::_1, std_ph::_2, std_ph::_3));
  release_control_server_ = create_service<carma_msgs::srv::ReleaseIndicatorControl>(
    "release_control",
    std::bind(&LightBarManager::releaseControlCallBack, this, std_ph::_1, std_ph::_2, std_ph::_3));
  set_indicator_server_ = create_service<carma_driver_msgs::srv::SetLightBarIndicator>(
    "set_indicator",
    std::bind(&LightBarManager::setIndicatorCallBack, this, std_ph::_1, std_ph::_2, std_ph::_3));
  indicator_control_publisher_ =
    create_publisher<carma_msgs::msg::LightBarIndicatorControllers>("indicator_control", 5);
  guidance_state_subscriber_ = create_subscription<carma_planning_msgs::msg::GuidanceState>(
    "state", 5, std::bind(&LightBarManager::stateChangeCallBack, this, std_ph::_1));
  turn_signal_subscriber_ = create_subscription<automotive_platform_msgs::msg::TurnSignalCommand>(
    "turn_signal_command", 5, std::bind(&LightBarManager::turnSignalCallback, this, std_ph::_1));
  ...
  // Initialize indicator control map. Fills with supporting indicators with empty string name as
  // owners.
  lbm_->setIndicatorControllers();

  // Initialize indicator representation of lightbar status to all OFF
  for (int i = 0; i < INDICATOR_COUNT; i++) lbm_->light_status.push_back(OFF);
  ...
  if (denied_list.size() != 0) {
    if (config_.normal_operation) {
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("lightbar_manager"),
        "In fuction "
          << __FUNCTION__
          << ", LightBarManager was not able to take control of all green indicators."
          << ".\n Please check priority list rosparameter, and ensure " << node_name_
...
carma_ros2_utils::CallbackReturn LightBarManager::handle_on_activate(
  const rclcpp_lifecycle::State &)
{
  pub_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds((int)(1 / config_.spin_rate_hz * 1000)),
...
void LightBarManager::turnOffAll()
{
  ...
  for (auto indicator : all_indicators) {
    ...
    if (response_code != 0)
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("lightbar_manager"),
        "In Function " << __FUNCTION__ << ": LightBarManager was not able to turn off indicator ID:"
                       << indicator << ". Response code: " << response_code);
  }

  // Once forced them off, release controls in case carma is still running so that lightbar_manager
  // does not hog the control
  all_indicators.erase(
    std::remove(all_indicators.begin(), all_indicators.end(), GREEN_FLASH), all_indicators.end());
  all_indicators.erase(
    std::remove(all_indicators.begin(), all_indicators.end(), GREEN_SOLID), all_indicators.end());
  lbm_->releaseControl(all_indicators, node_name_);
}

bool LightBarManager::requestControlCallBack(
  ...
  std::shared_ptr<carma_msgs::srv::RequestIndicatorControl::Response> resp)
{
  ...
  if (req->cda_list.size() != 0) {
    for (auto cda_type : req->cda_list) {
      ...
      if (static_cast<uint8_t>(cda_type.type) >= INDICATOR_COUNT) return false;
      ind_list.push_back(
        lbm_->getIndicatorFromCDAType(static_cast<LightBarCDAType>(cda_type.type)));
    }
  } else {
    for (auto indicator : req->ind_list) {
      // return false if invalid indicator number
      if (static_cast<uint8_t>(indicator.indicator) >= INDICATOR_COUNT) return false;
...
bool LightBarManager::releaseControlCallBack(
  const std::shared_ptr<rmw_request_id_t>,
...
void LightBarManager::stateChangeCallBack(
  carma_planning_msgs::msg::GuidanceState::UniquePtr msg_ptr)
{
  // Relay the msg to state machine
  LightBarState prev_lightbar_state = lbm_->getCurrentState();
  lbm_->handleStateChange(*msg_ptr);

  // Change green lights depending on states, no need to check if its current owner, as it will
  // always be for green lights
...
void LightBarManager::turnSignalCallback(
  automotive_platform_msgs::msg::TurnSignalCommand::UniquePtr msg_ptr)
{
  return processTurnSignal(*msg_ptr);
}

void LightBarManager::processTurnSignal(
  const automotive_platform_msgs::msg::TurnSignalCommand & msg)
{
  ...
  std::vector<lightbar_manager::LightBarIndicator> changed_turn_signal =
  ...
  if (msg.turn_signal == automotive_platform_msgs::msg::TurnSignalCommand::NONE) {
    ...
    prev_owners_before_turn_ =
      lbm_->getIndicatorControllers();  // save the owner if new turn is starting
  }
  if (lbm_->requestControl(changed_turn_signal, node_name_).empty()) {
    ...
    std::string turn_string =
      msg.turn_signal == automotive_platform_msgs::msg::TurnSignalCommand::LEFT ? "left" : "right";
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("lightbar_manager"),
      "Lightbar was not able to take control of lightbar to indicate " << turn_string << "turn!");
    return;
  }
...
std::shared_ptr<LightBarManagerWorker> LightBarManager::getWorker() { return lbm_; }

int LightBarManager::setIndicator(
  LightBarIndicator ind, IndicatorStatus ind_status, const std::string & requester_name)
{
  ...
  if (requester_name == "" || current_controller != requester_name) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("lightbar_manager"),
      requester_name << " failed to set the LightBarIndicator ID" << ind
                     << " as this was already controlled by " << current_controller);
    response_code = 1;
    return response_code;
  }

  std::vector<IndicatorStatus> light_status_proposed =
...
bool LightBarManager::setIndicatorCallBack(
  const std::shared_ptr<rmw_request_id_t>,
  ...
  std::shared_ptr<carma_driver_msgs::srv::SetLightBarIndicator::Response> resp)
{
  ...
  response_code =
    setIndicator(indicator, static_cast<IndicatorStatus>(req->state), req->requester_name);
