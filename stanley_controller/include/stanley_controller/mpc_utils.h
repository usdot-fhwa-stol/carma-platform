...
#ifndef STANLEY_CONTROLLER_MPC_UTILS_H
#define STANLEY_CONTROLLER_MPC_UTILS_H
 ...
 */
void convertEulerAngleToMonotonic(std::vector<double>* const a);
...
bool interp1d(const T1& index, const T2& values, const double& ref, double* const ret);
 ...
 * @return bool to check whether it was interpolated properly
 */
bool interp1dMPCTraj(const std::vector<double>& index, const MPCTrajectory& values, const std::vector<double>& ref,
                     MPCTrajectory* const ret);

/**
 * @brief calculate yaw angle in MPCTrajectory from xy vector
 * @param [inout] traj object trajectory
 */
void calcTrajectoryYawFromXY(MPCTrajectory* const traj);
 ...
 *  * @param [inout] traj object trajectory@param [inout] traj object trajectory
 */
void calcTrajectoryCurvature(const int curvature_smoothing_num, MPCTrajectory* const traj);
 ...
 */
void convertWaypointsToMPCTraj(const autoware_msgs::Lane& path, MPCTrajectory* const mpc_traj);
 ...
 * @param [out] ref_traj converted reference trajectory
 */
void convertWaypointsToMPCTrajWithDistanceResample(const autoware_msgs::Lane& path,
...
void convertWaypointsToMPCTrajWithTimeResample(const autoware_msgs::Lane& path, const std::vector<double>& path_time,
                                               const double& dt, MPCTrajectory* const ref_traj_);
 ...
 */
void calcPathRelativeTime(const autoware_msgs::Lane& path, std::vector<double>* const path_time);
...
*/
bool filt_vector(const int num, std::vector<double>* const u);

};  // namespace MPCUtils
#endif  // STANLEY_CONTROLLER_MPC_UTILS_H
