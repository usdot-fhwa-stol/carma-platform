...
TEST(YieldPluginTest, test_polynomial_calc)
{
  ...
  YieldPlugin plugin(
    nh, wm, config, [](const auto & msg) {}, [](const auto & msg) {});
...
TEST(YieldPluginTest, MaxTrajectorySpeed)
{
  ...
  double result =
    plugin.max_trajectory_speed(trajectory_points, rclcpp::Time(point_7.target_time).seconds());
  EXPECT_EQ(5, result);
}
...
TEST(YieldPluginTest, test_update_traj)
{
  ...
  tp.trajectory_points = {trajectory_point_1, trajectory_point_2, trajectory_point_3,
                          trajectory_point_4, trajectory_point_5, trajectory_point_6,
                          trajectory_point_7};
...
TEST(YieldPluginTest, get_collision)
{
  ...
  EXPECT_EQ(
    plugin.update_traj_for_object(tp, {}, 0.0),
  ...
  ASSERT_TRUE(collision_result != std::nullopt);
  ASSERT_TRUE(
    collision_result.value().collision_time ==
    rclcpp::Time(2, 0, collision_result.value().collision_time.get_clock_type()));
  ...
  // HOWEVER, AS A NAIVE TEST, IT SHOULD STILL RETURN COLLISION (can be removed if algorithm gets
  // improved)
...
TEST(YieldPluginTest, is_object_behind_vehicle)
{
  ...
  EXPECT_NO_THROW(plugin.update_traj_for_object(tp, {}, 0.0));
  // not actually passed
  ASSERT_FALSE(plugin.is_object_behind_vehicle(
    rwo_1.id, rclcpp::Time(9, 0), 12.0, 11.0));  // dummy time and id
  ...
  ASSERT_FALSE(plugin.is_object_behind_vehicle(rwo_1.id, rclcpp::Time(9, 0), 14.0, 11.0));
  ASSERT_FALSE(plugin.is_object_behind_vehicle(rwo_1.id, rclcpp::Time(9, 0), 14.0, 11.0));
  ASSERT_TRUE(plugin.is_object_behind_vehicle(
    rwo_1.id, rclcpp::Time(9, 0), 14.0,
    11.0));  // need consecutive 5 times required to confirm vehicle passed
}
...
TEST(YieldPluginTest, test_update_traj2)
{
  ...
  original_tp.trajectory_points = {trajectory_point_1, trajectory_point_2, trajectory_point_3,
                                   trajectory_point_4, trajectory_point_5, trajectory_point_6,
                                   trajectory_point_7};
  ...
  std::vector<double> values = quintic_coefficient_calculator::quintic_coefficient_calculator(
    initial_pos, goal_pos, current_speed_, goal_velocity, initial_accel, goal_accel, initial_time,
    tp);
  ...
  for (size_t i = 1; i < original_tp.trajectory_points.size(); i++) {
    ...
    new_tpp.target_time =
      rclcpp::Time(new_trajectory_points[0].target_time) +
      rclcpp::Duration::from_nanoseconds((original_traj_downtracks[i] / dv) * 1e9);
...
TEST(YieldPluginTest, test_update_traj_stop)
{
  ...
  for (size_t i = 1; i < original_tp.trajectory_points.size(); i++) {
    ...
    if (dv >= 1.0) {
      ...
      // trajectory point is copied to move all the available information, then its target time is
      // updated
      new_tpp = original_tp.trajectory_points[i];
      new_tpp.target_time =
        rclcpp::Time(new_trajectory_points[i - 1].target_time) +
        rclcpp::Duration::from_nanoseconds((original_traj_downtracks[i] / dv) * 1e9);
      new_trajectory_points.push_back(new_tpp);
    } else {
      RCLCPP_WARN(rclcpp::get_logger("yield_plugin"), "target speed is zero");
      new_tpp = new_trajectory_points[i - 1];
      new_tpp.target_time = rclcpp::Time(new_trajectory_points[0].target_time) +
                            rclcpp::Duration::from_nanoseconds(traj_target_time * 1e9);
      new_trajectory_points.push_back(new_tpp);
    }
    new_speeds.push_back(dv);
  }
  ...
  EXPECT_TRUE(
    rclcpp::Time(new_trajectory_points[5].target_time) >
    rclcpp::Time(new_trajectory_points[4].target_time));
}
...
TEST(YieldPluginTest, jmt_traj)
{
  ...
  carma_planning_msgs::msg::TrajectoryPlan jmt_traj = plugin.generate_JMT_trajectory(
    original_tp, initial_pos, goal_pos, initial_velocity, goal_velocity, tp,
    rclcpp::Time(original_tp.trajectory_points.back().target_time).seconds());

  EXPECT_EQ(jmt_traj.trajectory_points.size(), original_tp.trajectory_points.size());
  EXPECT_LE(
    rclcpp::Time(jmt_traj.trajectory_points[2].target_time),
    rclcpp::Time(original_tp.trajectory_points[2].target_time));
}
...
TEST(YieldPluginTest, min_digital_gap)
{
  ...
  auto regulatory_element =
    std::make_shared<lanelet::DigitalMinimumGap>(lanelet::DigitalMinimumGap::buildData(
      lanelet::utils::getId(), min_gap, {ll_1200}, {}, {lanelet::Participants::VehicleCar}));
  ...
  original_tp.trajectory_points = {
    trajectory_point_1, trajectory_point_2, trajectory_point_3, trajectory_point_4};
