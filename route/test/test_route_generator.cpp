...
TEST(RouteGeneratorTest, testRouteVisualizerCenterLineParser)
{
  ...
  route::RouteGeneratorWorker worker(std::make_shared<tf2_ros::Buffer>(tf2_buffer));
  ...
  // If the output is an error about the geoReference field in the osm file, here is a correct
  // example. If the lat/lon coordinates are already correct, simply add: <geoReference>+proj=tmerc
  // +lat_0=0 +lon_0=0 +k=1 +x_0=0 +y_0=0 +datum=WGS84 +units=m +geoidgrids=egm96_15.gtx +vunits=m
  // +no_defs</geoReference>
  ...
  // https://github.com/usdot-fhwa-stol/carma-platform/blob/develop/carma_wm_ctrl/test/MapToolsTest.cpp
  lanelet::io_handlers::AutowareOsmParser::parseMapParams(file, &projector_type, &target_frame);
  lanelet::projection::LocalFrameProjector local_projector(target_frame.c_str());
  lanelet::LaneletMapPtr map = lanelet::load(file, local_projector, &load_errors);

  // Grabs lanelet elements from the start and end IDs. Fails the unit test if there is no lanelet
  ...
  try {
    ...
    FAIL() << "The specified starting lanelet Id of " << start_id
           << " does not exist in the provided map.";
  }
  try {
    end_lanelet = map->laneletLayer.get(end_id);
  } catch (const lanelet::NoSuchPrimitiveError & e) {
    FAIL() << "The specified ending lanelet Id of " << end_id
           << " does not exist in the provided map.";
  }
  ...
  lanelet::routing::RoutingGraphUPtr map_graph =
  ...
  // Computes the shortest path and prints the list of lanelet IDs to get from the start to the end.
  // Can be manually confirmed in JOSM
...
TEST(RouteGeneratorTest, testLaneletRoutingTown02VectorMap)
{
  ...
  /***
   * VALID PATHs (consists of lanenet ids): (This is also the shortest path because certain Lanelets
   * missing) 159->160->164->136->135->137->144->121; 159->160->164->136->135->137->144->118;
   * 168->170->111
   * 159->161->168->170->111
   * 167->169->168->170->111
...
TEST(RouteGeneratorTest, test_crosstrack_error_check)
{
  ...
  ASSERT_EQ(
    worker.crosstrackErrorCheck(mpt, start_lanelet),
    true);  // The vehicle will show crosstrack error, so the value should return true
  ...
  // Test 3: Vehicle is out of bounds, and has exceeded the maximum number of consecutive timesteps
  // outside of the route allowable before triggering LEFT_ROUTE//
  ...
  ASSERT_EQ(
    worker.crosstrackErrorCheck(mpt, start_lanelet),
    false);  // The vehicle will show no crosstrack error, so the value should return false
  ASSERT_EQ(
    worker.crosstrackErrorCheck(mpt, start_lanelet),
    true);  // The vehicle will show crosstrack error, so the value should return true
}
...
TEST(RouteGeneratorTest, test_set_active_route_cb)
{
    ...
    // Set georeference
    std::string proj = "+proj=tmerc +lat_0=4.9000000000000000e+1 +lon_0=8.0000000000000000e+0 +k=1
+x_0=0 +y_0=0 +datum=WGS84 +units=m +geoidgrids=egm96_15.gtx +vunits=m +no_defs";
    std_msgs::msg::String str_msg;
    str_msg.data = proj;
    std::unique_ptr<std_msgs::msg::String> msg_ptr =
std::make_unique<std_msgs::msg::String>(str_msg); worker.georeferenceCb(move(msg_ptr));
    ...
    for(auto i = 0; i < resp_ptr->available_routes.size();i++)
    {
        if(resp_ptr->available_routes[i].route_id  == "Test_town01_route_1")
        {
            ...
            auto gps_points =
worker.loadRouteDestinationGpsPointsFromRouteId("Test_town01_route_1"); auto map_points =
worker.loadRouteDestinationsInMapFrame(gps_points);
    ...
    for(auto i: resp_ptr->available_routes)
    {
        if(i.route_id  == "Test_town01_route_1")
        {
            ...
            auto req_ptr2 =
std::make_shared<carma_planning_msgs::srv::SetActiveRoute::Request>(req2); auto resp_ptr2 =
std::make_shared<carma_planning_msgs::srv::SetActiveRoute::Response>(resp2);
    ...
    ////////////
    // Conduct tests for set_active_route_cb() using an array of destination points in the service
request
    ////////////
    route::RouteGeneratorWorker worker2(tf2_buffer);
    std::unique_ptr<std_msgs::msg::String> msg_ptr2 =
std::make_unique<std_msgs::msg::String>(str_msg);
...
TEST(RouteGeneratorTest, test_duplicate_lanelets_in_shortest_path)
{
  ...
  auto route_with_duplicates =
    map_graph->getRouteVia(start_lanelet, via_lanelets_vector, end_lanelet);
  ...
  // Change the ending lanelet ID and update the route so the shortest path does not contain
  // duplicate lanelet IDs
  end_id = 170;
  end_lanelet = map->laneletLayer.get(end_id);
  auto route_without_duplicates =
    map_graph->getRouteVia(start_lanelet, via_lanelets_vector, end_lanelet);

  // Test that the shortest path in 'route_without_duplicates' does not contain duplicate Lanelet
  // IDs The shortest path is 111 -> 101 -> 100 -> 104 -> 167 -> 169 -> 168 -> 170
