...
carma_planning_msgs::msg::TrajectoryPlan PlanDelegator::planTrajectory()
{
  ...
  while (current_maneuver_index < latest_maneuver_plan_.maneuvers.size()) {
    ...
    auto future_status = future_response.wait_for(
    ...
    if (future_status == std::future_status::ready) {
      ...
      if (latest_trajectory_plan.trajectory_points.size() != 0) {
        if (
          ...
          plan_response->trajectory_plan.trajectory_points.front().target_time) {
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("plan_delegator"),
            "Removing duplicate point for planner: " << maneuver_planner);
          plan_response->trajectory_plan.trajectory_points.erase(
            plan_response->trajectory_plan.trajectory_points.begin());
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("plan_delegator"),
            "plan_response->trajectory_plan size: "
              << plan_response->trajectory_plan.trajectory_points.size());
        }
      }
      ...
      if (first_trajectory_plan == true) {
        latest_trajectory_plan.initial_longitudinal_velocity =
          plan_response->trajectory_plan.initial_longitudinal_velocity;
