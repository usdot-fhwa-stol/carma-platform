...
struct InLaneCruisingPluginConfig
{
  ...
  double lateral_accel_limit_multiplier = 0.5;        // Lateral acceleration limit multiplier
  double max_accel_multiplier = 0.75;        // Longitudinal acceleration limit multiplier
  ...
  friend std::ostream& operator<<(std::ostream& output, const InLaneCruisingPluginConfig& c)
  {
    output << "InLaneCruisingPluginConfig { " << std::endl
           ...
           << "max_deccel: " << c.max_decel << std::endl
