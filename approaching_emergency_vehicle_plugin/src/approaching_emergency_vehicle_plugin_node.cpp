...
{
/**
 * \brief Anonymous function to extract maneuver end speed which cannot be obtained with
 * GET_MANEUVER_PROPERY calls due to it missing in stop and wait plugin
 */
...
ApproachingEmergencyVehiclePlugin::ApproachingEmergencyVehiclePlugin(
...
: carma_guidance_plugins::StrategicPlugin(options)
{
  ...
  // Declare parameters
  config_.passing_threshold =
    declare_parameter<double>("passing_threshold", config_.passing_threshold);
  config_.approaching_threshold =
    declare_parameter<double>("approaching_threshold", config_.approaching_threshold);
  config_.finished_passing_threshold =
    declare_parameter<double>("finished_passing_threshold", config_.finished_passing_threshold);
  config_.min_lane_following_duration_before_lane_change = declare_parameter<double>(
    "min_lane_following_duration_before_lane_change",
    config_.min_lane_following_duration_before_lane_change);
  config_.bsm_processing_frequency =
    declare_parameter<double>("bsm_processing_frequency", config_.bsm_processing_frequency);
  config_.speed_limit_reduction_during_passing = declare_parameter<double>(
    "speed_limit_reduction_during_passing", config_.speed_limit_reduction_during_passing);
  config_.minimum_reduced_speed_limit =
    declare_parameter<double>("minimum_reduced_speed_limit", config_.minimum_reduced_speed_limit);
  config_.default_speed_limit =
    declare_parameter<double>("default_speed_limit", config_.default_speed_limit);
  config_.reduced_speed_buffer =
    declare_parameter<double>("reduced_speed_buffer", config_.reduced_speed_buffer);
  config_.timeout_check_frequency =
    declare_parameter<double>("timeout_check_frequency", config_.timeout_check_frequency);
  config_.timeout_duration =
    declare_parameter<double>("timeout_duration", config_.timeout_duration);
  config_.minimal_plan_duration =
    declare_parameter<double>("minimal_plan_duration", config_.minimal_plan_duration);
  config_.buffer_distance_before_stopping = declare_parameter<double>(
    "buffer_distance_before_stopping", config_.buffer_distance_before_stopping);
  config_.stopping_accel_limit_multiplier = declare_parameter<double>(
    "stopping_accel_limit_multiplier", config_.stopping_accel_limit_multiplier);
  config_.vehicle_acceleration_limit =
    declare_parameter<double>("vehicle_acceleration_limit", config_.vehicle_acceleration_limit);
  config_.route_end_point_buffer =
    declare_parameter<double>("route_end_point_buffer", config_.route_end_point_buffer);
  config_.approaching_erv_status_publication_frequency = declare_parameter<double>(
    "approaching_erv_status_publication_frequency",
    config_.approaching_erv_status_publication_frequency);
  config_.warning_broadcast_frequency =
    declare_parameter<double>("warning_broadcast_frequency", config_.warning_broadcast_frequency);
  config_.max_warning_broadcasts =
    declare_parameter<int>("max_warning_broadcasts", config_.max_warning_broadcasts);
  config_.vehicle_length = declare_parameter<double>("vehicle_length", config_.vehicle_length);
  config_.lane_following_plugin =
    declare_parameter<std::string>("lane_following_plugin", config_.lane_following_plugin);
  config_.lane_change_plugin =
    declare_parameter<std::string>("lane_change_plugin", config_.lane_change_plugin);
...
ApproachingEmergencyVehiclePlugin::parameter_update_callback(
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  auto error_2 =
    update_params<int>({{"max_warning_broadcasts", config_.max_warning_broadcasts}}, parameters);
...
{
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger(logger_name), "ApproachingEmergencyVehiclePlugin trying to configure");
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger(logger_name), "ApproachingEmergencyVehiclePlugin Config: " << config_);

  // Register runtime parameter update callback
  add_on_set_parameters_callback(
    std::bind(&ApproachingEmergencyVehiclePlugin::parameter_update_callback, this, std_ph::_1));

  // Setup subscribers
  incoming_bsm_sub_ = create_subscription<carma_v2x_msgs::msg::BSM>(
    "incoming_bsm", 1,
  ...
  route_state_sub_ = create_subscription<carma_planning_msgs::msg::RouteState>(
    "route_state", 10,
  ...
  incoming_emergency_vehicle_ack_sub_ =
    create_subscription<carma_v2x_msgs::msg::EmergencyVehicleAck>(
      "incoming_emergency_vehicle_ack", 10,
      std::bind(
        &ApproachingEmergencyVehiclePlugin::incomingEmergencyVehicleAckCallback, this, std_ph::_1));

  guidance_state_sub_ = create_subscription<carma_planning_msgs::msg::GuidanceState>(
    "state", 1,
    std::bind(&ApproachingEmergencyVehiclePlugin::guidanceStateCallback, this, std_ph::_1));

  route_sub_ = create_subscription<carma_planning_msgs::msg::Route>(
    "route", 1, std::bind(&ApproachingEmergencyVehiclePlugin::routeCallback, this, std_ph::_1));

  // Setup publishers
  outgoing_emergency_vehicle_response_pub_ =
    create_publisher<carma_v2x_msgs::msg::EmergencyVehicleResponse>(
      "outgoing_emergency_vehicle_response", 10);

  approaching_erv_status_pub_ =
    create_publisher<carma_msgs::msg::UIInstructions>("approaching_erv_status", 10);
...
carma_ros2_utils::CallbackReturn ApproachingEmergencyVehiclePlugin::on_activate_plugin()
{
  ...
  int erv_timeout_check_period_ms =
    (1 / config_.timeout_check_frequency) *
    1000;  // Conversion from frequency (Hz) to milliseconds time period
  erv_timeout_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(erv_timeout_check_period_ms),
    std::bind(&ApproachingEmergencyVehiclePlugin::checkForErvTimeout, this));

  // Timer setup for broadcasting EmergencyVehicleResponse warning message to an approaching ERV
  // when the ego vehicle cannot change lanes out of the ERV's path
  int emergency_vehicle_response_period_ms =
    (1 / config_.warning_broadcast_frequency) *
    1000;  // Conversion from frequency (Hz) to milliseconds time period
  warning_broadcast_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(emergency_vehicle_response_period_ms),
  ...
  int approaching_erv_status_period_ms =
    (1 / config_.approaching_erv_status_publication_frequency) *
    1000;  // Conversion from frequency (Hz) to milliseconds time period
  approaching_emergency_vehicle_status_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(approaching_erv_status_period_ms),
  ...
  int hazard_light_status_ms =
    (1 / 30) * 1000;  // Conversion from frequency 30(Hz) to milliseconds time period
  hazard_light_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(hazard_light_status_ms),
...
{
  if (
    transition_table_.getState() == ApproachingEmergencyVehicleState::SLOWING_DOWN_FOR_ERV &&
    has_tracked_erv_ && tracked_erv_.lane_index == ego_lane_index_) {
...
void ApproachingEmergencyVehiclePlugin::broadcastWarningToErv()
{
  if (has_tracked_erv_ && should_broadcast_warnings_) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name), "Sending a warning message to " << tracked_erv_.vehicle_id);
    ...
    if (num_warnings_broadcasted_ >= config_.max_warning_broadcasts) {
      RCLCPP_DEBUG_STREAM(
        ...
        "Maximum number of warning messages sent to " << tracked_erv_.vehicle_id);
...
ApproachingEmergencyVehiclePlugin::generateApproachingErvStatusMessage()
{
   ...
   *       |--------0----------------------1----------------------2--------------|
   *       Index 0: (Boolean) Value indicating whether the ego vehicle is tracking an approaching
   * ERV Index 1: (Double; rounded to first decimal place)  If an approaching ERV exists, this
   * indicates the estimated seconds until the ERV passes the ego vehicle. Index 2: (String)  If an
   * approaching ERV exists, this describes the current action of the ego vehicle in response to the
   * approaching ERV. NOTE: The values of indexes 1 and 2 can be ignored if index 0 indicates that
   * no approaching ERV is being tracked.
   */
  boost::format fmter(APPROACHING_ERV_STATUS_PARAMS);

  // Index 0 of formatted string; indicates whether an ERV that is approaching the ego vehicle is
  // being tracked
  ...
  switch (transition_table_.getState()) {
    ...
    case ApproachingEmergencyVehicleState::SLOWING_DOWN_FOR_ERV:
      // Add ego vehicle action based on whether the current speed is near the reduced target speed
      // of the latest maneuver plan's first maneuver
      if (!latest_maneuver_plan_.maneuvers.empty()) {
        // Extract the target speed in m/s from the latest maneuver plan's first maneuver and
        // convert it to mph
        ...
        if (ego_lane_index_ == tracked_erv_.lane_index) {
          if (!is_maintaining_non_reduced_speed_) {
            if (abs(current_speed_ - target_speed_ms) <= config_.reduced_speed_buffer) {
              fmter % ("Approaching ERV is in our lane and a lane change is not possible. "
                       "Remaining in the current lane at a reduced speed of " +
                       std::to_string(target_speed_mph) + " mph.");
            } else {
              fmter % ("Approaching ERV is in our lane and a lane change is not possible. "
                       "Remaining in the current lane and slowing down to a reduced speed of " +
                       std::to_string(target_speed_mph) + " mph.");
            }
          } else {
            int non_reduced_speed_to_maintain_mph =
              std::round(non_reduced_speed_to_maintain_ * METERS_PER_SEC_TO_MILES_PER_HOUR);
            fmter % ("Approaching ERV is in our lane and a lane change is not possible. Remaining "
                     "in the current lane and maintaining a speed of " +
                     std::to_string(non_reduced_speed_to_maintain_mph) + " mph.");
          }
        } else {
          if (abs(current_speed_ - target_speed_ms) <= config_.reduced_speed_buffer) {
            fmter % ("Approaching ERV is in adjacent lane. Remaining in the current lane at a "
                     "reduced speed of " +
                     std::to_string(target_speed_mph) + " mph.");
          } else {
            fmter % ("Approaching ERV is in adjacent lane. Remaining in the current lane and "
                     "slowing down to a reduced speed of " +
                     std::to_string(target_speed_mph) + " mph.");
          }
        }
      } else {
...
boost::optional<lanelet::BasicPoint2d> ApproachingEmergencyVehiclePlugin::getErvPositionInMap(
  const double & current_latitude, const double & current_longitude)
{
  ...
  auto erv_current_position_in_map_vec = lanelet::utils::transform(
    erv_current_position_projected_vec, [](auto a) { return lanelet::traits::to2D(a); });
...
boost::optional<ErvInformation> ApproachingEmergencyVehiclePlugin::getErvInformationFromBsm(
  carma_v2x_msgs::msg::BSM::UniquePtr msg)
{
  // Initialize ErvInformation object, which will be populated with information from this BSM if it
  // is from an ERV
  ...
  if (msg->presence_vector & carma_v2x_msgs::msg::BSM::HAS_PART_II) {
    ...
    if (msg->part_ii.size() > 0) {
      for (size_t i = 0; i < msg->part_ii.size(); ++i) {
        ...
        if (
          msg->part_ii[i].part_ii_id ==
          carma_v2x_msgs::msg::BSMPartIIExtension::SPECIAL_VEHICLE_EXT) {
          carma_v2x_msgs::msg::SpecialVehicleExtensions special_vehicle_ext =
            msg->part_ii[i].special_vehicle_extensions;

          if (
            special_vehicle_ext.presence_vector &
            carma_v2x_msgs::msg::SpecialVehicleExtensions::HAS_VEHICLE_ALERTS) {
            if (
              special_vehicle_ext.vehicle_alerts.siren_use.siren_in_use ==
                j2735_v2x_msgs::msg::SirenInUse::IN_USE &&
              special_vehicle_ext.vehicle_alerts.lights_use.lightbar_in_use ==
                j2735_v2x_msgs::msg::LightbarInUse::IN_USE) {
  ...
  if (!has_active_lights_and_sirens) {
    // BSM is not a valid ERV BSM since the lights and sirens are not both active; return an empty
    // object
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name),
      "BSM is not a valid ERV BSM since the lights and sirens are not both active; return an empty "
  ...
  if (erv_information.current_speed <= current_speed_) {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger(logger_name), "Ignoring received BSM since ERV speed of "
                                         << erv_information.current_speed
                                         << " is less than ego speed of " << current_speed_);
    return boost::optional<ErvInformation>();
  }
  ...
  boost::optional<lanelet::BasicPoint2d> erv_position_in_map =
    getErvPositionInMap(erv_information.current_latitude, erv_information.current_longitude);
  ...
  if (msg->presence_vector & carma_v2x_msgs::msg::BSM::HAS_REGIONAL) {
    // Parse BSM Regional Extensions to determine whether BSM includes the ERV's route destination
    // points
    if (msg->regional.size() > 0) {
      for (size_t i = 0; i < msg->regional.size(); ++i) {
        if (
          msg->regional[i].regional_extension_id ==
          carma_v2x_msgs::msg::BSMRegionalExtension::ROUTE_DESTINATIONS) {
  ...
  if (erv_destination_points.empty()) {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger(logger_name), "ERV's BSM does not contain any future destination points");

    // BSM is not a valid ERV BSM since it does not include destination points; return an empty
    // object
    return boost::optional<ErvInformation>();
  }
  ...
  lanelet::Optional<lanelet::routing::Route> erv_future_route = generateErvRoute(
    erv_information.current_latitude, erv_information.current_longitude, erv_destination_points);
  ...
  // Note: For 'lane index', 0 is rightmost lane, 1 is second rightmost, etc.; Only the current
  ...
  if (!erv_future_route.get().shortestPath().empty()) {
    ...
    if (is_same_direction_.find(erv_information.vehicle_id) == is_same_direction_.end()) {
      ...
      for (auto llt :
           erv_future_route.get().shortestPath())  // checks if ERV is on the same path assuming CMV
                                                   // got all of its planned route when detected
      {
        if (wm_->getRoute()->contains(llt)) {
          ...
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger(logger_name),
            "Detected that ERV: " << erv_information.vehicle_id
    ...
    // A currently-tracked ERV must report the same lane index twice in a row before it is assigned
    // the new lane index
    if (has_tracked_erv_) {
      if (lane_index == tracked_erv_.previous_lane_index) {
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger(logger_name), "Detected first new lane index of "
                                             << lane_index << ", ERV's lane index will remain "
                                             << tracked_erv_.previous_lane_index);
      }
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name), "ERV's lane index is " << erv_information.lane_index);
  ...
  boost::optional<lanelet::ConstLanelet> intersecting_lanelet =
    getRouteIntersectingLanelet(erv_future_route.get());
  ...
  if (intersecting_lanelet) {
    ...
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger(logger_name),
      "No intersecting lanelet between the ERV's future route and the ego vehicle's future route "
      "was found.");
  ...
  boost::optional<double> seconds_until_passing = getSecondsUntilPassing(
    erv_future_route, erv_information.current_position_in_map, erv_information.current_speed,
  ...
  if (seconds_until_passing) {
    ...
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger(logger_name), "Detected approaching ERV that is passing ego vehicle in "
                                         << seconds_until_passing.get() << " seconds");
  } else {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger(logger_name), "Detected ERV is not approaching the ego vehicle");

    // ERV will not be tracked since it is not considered to be approaching the ego vehicle; return
    // an empty object
    return boost::optional<ErvInformation>();
  }
...
void ApproachingEmergencyVehiclePlugin::georeferenceCallback(
  const std_msgs::msg::String::UniquePtr msg)
{
  // Build projector from proj string
  map_projector_ = msg->data;
}

void ApproachingEmergencyVehiclePlugin::incomingEmergencyVehicleAckCallback(
  const carma_v2x_msgs::msg::EmergencyVehicleAck::UniquePtr msg)
{
  // Only process message if it is from the currently-tracked ERV and it is intended for the ego
  // vehicle
  if (
    has_tracked_erv_ && (msg->m_header.sender_id == tracked_erv_.vehicle_id) &&
    (msg->m_header.recipient_id == config_.vehicle_id)) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name),
      "EmergencyVehicleAck received from ERV " << tracked_erv_.vehicle_id);
...
lanelet::Optional<lanelet::routing::Route> ApproachingEmergencyVehiclePlugin::generateErvRoute(
  ...
  std::vector<carma_v2x_msgs::msg::Position3D> erv_destination_points)
{
  ...
  auto erv_destination_points_in_map = lanelet::utils::transform(
    erv_destination_points_projected, [](auto a) { return lanelet::traits::to2D(a); });

  auto cmv_location = lanelet::traits::to2D(projector.forward(current_erv_location));
  auto shortened_erv_destination_points_in_map =
    filter_points_ahead(cmv_location, erv_destination_points_in_map);

  if (shortened_erv_destination_points_in_map.empty()) {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger(logger_name), "ERV has passed all the destination points!");
  ...
  for (size_t i = 0; i < shortened_erv_destination_points_in_map.size(); ++i) {
    ...
    if ((wm_->getLaneletsFromPoint(shortened_erv_destination_points_in_map[i])).empty()) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(logger_name),
        "ERV destination point " << i << " is not contained in a lanelet map; x: " << pt.x()
                                 << " y: " << pt.y());
  ...
  auto starting_lanelet_vector =
    lanelet::geometry::findNearest(wm_->getMap()->laneletLayer, cmv_location, 1);
  ...
  auto ending_lanelet_vector = lanelet::geometry::findNearest(
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name), "starting_lanelet: " << starting_lanelet.id());

  // Obtain ERV's via lanelets
  std::vector<lanelet::BasicPoint2d> via = std::vector<lanelet::BasicPoint2d>(
    shortened_erv_destination_points_in_map.begin(),
    shortened_erv_destination_points_in_map.end() - 1);
  ...
  for (const auto & point : via) {
    ...
    if (chosen_lanelet_to_emplace.id() != starting_lanelet.id())  // if id is same, it fails to
                                                                  // route
...
std::vector<lanelet::BasicPoint2d> ApproachingEmergencyVehiclePlugin::filter_points_ahead(
  const lanelet::BasicPoint2d & reference_point,
  const std::vector<lanelet::BasicPoint2d> & original_points) const
{
  ...
  lanelet::BasicPoint2d extended_point = {
    (original_points.end() - 1)->x() + last_dx, (original_points.end() - 1)->y() + last_dy};
  ...
  while (i < extended_points.size() - 1) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name),
      "reference_point x: " << reference_point.x() << ", y: " << reference_point.y());
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name),
      "extended_points x: " << extended_points[i].x() << ", y: " << extended_points[i].y());
    ...
    if (angleRad >= M_PI / 2) {
      ...
      if (distance < closest_dist)  // get closest point to the reference point from the multiple
                                    // possible points
      {
  ...
  return std::vector<lanelet::BasicPoint2d>(
    original_points.begin() + closest_idx, original_points.end());
}
void ApproachingEmergencyVehiclePlugin::incomingBsmCallback(carma_v2x_msgs::msg::BSM::UniquePtr msg)
{
  ...
  if (has_tracked_erv_) {
    // If there is already an ERV approaching the ego vehicle, only process this BSM further if it
    // is from that ERV and enough time has passed since the previously processed BSM

    if (erv_vehicle_id == tracked_erv_.vehicle_id) {
      double seconds_since_prev_processed_bsm =
        (this->now() - tracked_erv_.latest_update_time).seconds();

      if (seconds_since_prev_processed_bsm < (1.0 / config_.bsm_processing_frequency)) {
        // Do not process ERV's BSM further since not enough time has passed since its previously
        // processed BSM
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger(logger_name), "Ignoring BSM from tracked ERV "
                                             << erv_vehicle_id
                                             << " since a BSM from it was processed "
                                             << seconds_since_prev_processed_bsm << " seconds ago");
        return;
      }
    } else {
    ...
    // If BSM is from a detected active ERV, only process it further if enough time has passed since
    // the previously processed BSM from this ERV
    if (latest_erv_update_times_.find(erv_vehicle_id) != latest_erv_update_times_.end()) {
      double seconds_since_prev_processed_bsm =
        (this->now() - latest_erv_update_times_[erv_vehicle_id]).seconds();

      if (seconds_since_prev_processed_bsm < (1.0 / config_.bsm_processing_frequency)) {
        // Do not process ERV's BSM further since not enough time has passed since its previously
        // processed BSM
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger(logger_name), "Ignoring BSM from non tracked ERV "
                                             << erv_vehicle_id
                                             << " since a BSM from it was processed "
                                             << seconds_since_prev_processed_bsm << " seconds ago");
        return;
  ...
  // Get ErvInformation object with information from the BSM if it is from an active ERV that is
  // approaching the ego vehicle
  ...
  if (!erv_information) {
    ...
    // Update tracked ERV information since this is an active ERV that is approaching the ego
    // vehicle
...
boost::optional<double> ApproachingEmergencyVehiclePlugin::getSecondsUntilPassing(
  lanelet::Optional<lanelet::routing::Route> & erv_future_route,
  const lanelet::BasicPoint2d & erv_position_in_map, const double & erv_current_speed,
  lanelet::ConstLanelet & intersecting_lanelet)
{
  // Obtain ego vehicle and ERV distances to the end of the intersecting lanelet so neither vehicle
  // will currently be past that point
  lanelet::ConstLineString2d intersecting_centerline =
    lanelet::utils::to2D(intersecting_lanelet.centerline());
  ...
  double ego_dist_to_lanelet = wm_->routeTrackPos(intersecting_end_point).downtrack -
                               (latest_route_state_.down_track - config_.vehicle_length);

  // Set erv_world_model_ route to the erv_future_route
  lanelet::routing::Route route = std::move(*erv_future_route);
  carma_wm::LaneletRoutePtr erv_future_route_ptr =
    std::make_shared<lanelet::routing::Route>(std::move(route));
  erv_world_model_->setRoute(erv_future_route_ptr);

  // Get ERV's distance to the intersecting lanelet's centerline endpoint
  double erv_dist_to_lanelet = erv_world_model_->routeTrackPos(intersecting_end_point).downtrack -
                               erv_world_model_->routeTrackPos(erv_position_in_map).downtrack;

  if (erv_dist_to_lanelet < ego_dist_to_lanelet) {
    // When ERV is in front of the ego vehicle, only process further if the ERV is actively passing
    // the ego vehicle

    if (transition_table_.getState() != ApproachingEmergencyVehicleState::SLOWING_DOWN_FOR_ERV) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(logger_name), "Detected ERV is in front of the ego vehicle");
      return boost::optional<double>();
    } else {
      // This ERV is actively passing the ego vehicle until its distance in front of the ego vehicle
      // is at least config_.finished_passing_threshold
    ...
    // When ERV is behind the ego vehicle, only process further if the ERV is travelling faster than
    // the ego vehicle

    if (erv_current_speed < current_speed_) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(logger_name),
        "Detected ERV is travelling slower than the ego vehicle, and will not pass the ego "
        "vehicle");
      return boost::optional<double>();
    } else {
...
boost::optional<lanelet::ConstLanelet>
ApproachingEmergencyVehiclePlugin::getRouteIntersectingLanelet(
  const lanelet::routing::Route & erv_future_route)
{
  if (future_route_lanelet_ids_.empty()) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger(logger_name),
      "Remaining route lanelets for the ego vehicle not found; plugin cannot compute the "
      "intersecting lanelet.");
  ...
  // Find first successful intersecting lanelet between ERV route and ego route with a lanelet
  // starting downtrack greater than current downtrack. Additionally, remove lanelets from
  // future_route_lanelet_ids_ that the ego vehicle has passed.
  for (auto it = future_route_lanelet_ids_.begin(); it != future_route_lanelet_ids_.end();) {
    ...
    lanelet::BasicPoint2d ego_route_lanelet_centerline_end =
      lanelet::utils::to2D(ego_route_lanelet.centerline()).back();
    double ego_route_lanelet_centerline_end_downtrack =
    ...
    if (current_downtrack > ego_route_lanelet_centerline_end_downtrack) {
      // If current downtrack is greater than lanelet ending downtrack, remove lanelet from future
      // route lanelet and continue NOTE: Iterator is not updated since an element is being erased
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(logger_name), "Removing passed lanelet " << ego_route_lanelet_id);
      ...
      if (erv_future_route.contains(ego_route_lanelet)) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger(logger_name), "Found intersecting lanelet " << ego_route_lanelet_id);
...
void ApproachingEmergencyVehiclePlugin::routeStateCallback(
  carma_planning_msgs::msg::RouteState::UniquePtr msg)
{
  latest_route_state_ = *msg;
}

void ApproachingEmergencyVehiclePlugin::twistCallback(
  geometry_msgs::msg::TwistStamped::UniquePtr msg)
{
  current_speed_ = msg->twist.linear.x;
}

void ApproachingEmergencyVehiclePlugin::guidanceStateCallback(
  carma_planning_msgs::msg::GuidanceState::UniquePtr msg)
{
...
void ApproachingEmergencyVehiclePlugin::routeCallback(
  carma_planning_msgs::msg::Route::UniquePtr msg)
{
  ...
  for (size_t i = 0; i < msg->route_path_lanelet_ids.size(); ++i) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name), "New route lanelet: " << msg->route_path_lanelet_ids[i]);
...
double ApproachingEmergencyVehiclePlugin::getLaneletSpeedLimit(
...
rclcpp::Duration ApproachingEmergencyVehiclePlugin::getManeuverDuration(
  const carma_planning_msgs::msg::Maneuver & maneuver, double epsilon) const
{
  ...
  RCLCPP_ERROR_STREAM(
    rclcpp::get_logger(logger_name),
    "maneuver_end_dist: " << maneuver_end_dist << ", maneuver_start_dist: " << maneuver_start_dist
                          << ", sum_start_and_end_speed: " << sum_start_and_end_speed);

  maneuver_duration = rclcpp::Duration::from_nanoseconds(
    (maneuver_end_dist - maneuver_start_dist) / (0.5 * sum_start_and_end_speed) * 1e9);

  return maneuver_duration;
}

carma_planning_msgs::msg::Maneuver
ApproachingEmergencyVehiclePlugin::composeLaneFollowingManeuverMessage(
  double start_dist, double end_dist, double start_speed, double target_speed, int lanelet_id,
  rclcpp::Time & start_time) const
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name),
    "Composed lane follow maneuver for lanelet ID:" << lanelet_id << " with duration "
                                                    << maneuver_duration.seconds());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name),
    "start speed: " << start_speed << ", end speed: " << target_speed);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name), "start dist: " << start_dist << ", end dist: " << end_dist);

  return maneuver_msg;
}

carma_planning_msgs::msg::Maneuver
ApproachingEmergencyVehiclePlugin::composeLaneChangeManeuverMessage(
  double start_dist, double end_dist, double start_speed, double target_speed,
  lanelet::Id starting_lane_id, lanelet::Id ending_lane_id, rclcpp::Time & start_time) const
{
  carma_planning_msgs::msg::Maneuver maneuver_msg;
  ...
  maneuver_msg.lane_change_maneuver.parameters.presence_vector =
    carma_planning_msgs::msg::ManeuverParameters::HAS_TACTICAL_PLUGIN;
  maneuver_msg.lane_change_maneuver.parameters.planning_tactical_plugin =
    config_.lane_change_plugin;
  ...
  // Preserve lane change maneuver ID from previous plan to maintain identical trajectory generation
  // across separate maneuver plans
  if (has_planned_upcoming_lc_) {
    ...
    maneuver_msg.lane_change_maneuver.parameters.maneuver_id =
      boost::lexical_cast<std::string>(gen());  // generate uuid and convert to string
  }

  RCLCPP_DEBUG_STREAM(
    get_logger(), "Creating lane change id: "
                    << maneuver_msg.lane_change_maneuver.parameters.maneuver_id
                    << "start dist: " << start_dist << " end dist: " << end_dist
                    << " Starting llt: " << starting_lane_id << " Ending llt: " << ending_lane_id);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name),
    "start speed: " << start_speed << ", end speed: " << target_speed
...
carma_planning_msgs::msg::Maneuver
ApproachingEmergencyVehiclePlugin::composeStopAndWaitManeuverMessage(
  double start_dist, double end_dist, double start_speed, lanelet::Id starting_lane_id,
  lanelet::Id ending_lane_id, double stopping_deceleration, rclcpp::Time & start_time) const
{
  carma_planning_msgs::msg::Maneuver maneuver_msg;
  maneuver_msg.type = carma_planning_msgs::msg::Maneuver::STOP_AND_WAIT;
  maneuver_msg.stop_and_wait_maneuver.parameters.negotiation_type =
    carma_planning_msgs::msg::ManeuverParameters::NO_NEGOTIATION;
  ...
  maneuver_msg.stop_and_wait_maneuver.parameters.planning_tactical_plugin =
    config_.stop_and_wait_plugin;
  ...
  // Set the meta-data for the StopAndWait Maneuver to define the buffer in the route end point
  // stopping location
  maneuver_msg.stop_and_wait_maneuver.parameters.float_valued_meta_data.push_back(
    config_.route_end_point_buffer);
  maneuver_msg.stop_and_wait_maneuver.parameters.float_valued_meta_data.push_back(
    stopping_deceleration);

  static auto gen = boost::uuids::random_generator();  // Initialize uuid generator
  maneuver_msg.stop_and_wait_maneuver.parameters.maneuver_id =
    boost::lexical_cast<std::string>(gen());  // generate uuid and convert to string

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name), "Composed stop and wait maneuver for with start lanelet:"
                                       << starting_lane_id << ", end lanelet: " << ending_lane_id
...
void ApproachingEmergencyVehiclePlugin::addStopAndWaitToEndOfPlan(
  carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp, double downtrack_progress,
  double stop_maneuver_beginning_downtrack, double end_of_route_downtrack,
  double stopping_entry_speed, double stopping_deceleration,
  double current_lanelet_ending_downtrack, lanelet::ConstLanelet current_lanelet,
  rclcpp::Time time_progress)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name), "Maneuver plan is reaching the end of the route");

  // Add a lane follow maneuver before the stop and wait maneuver if the distance before
  // stop_maneuver_beginning_downtrack is too large
  if (
    (stop_maneuver_beginning_downtrack - downtrack_progress) >=
    config_.buffer_distance_before_stopping) {
    ...
    resp->new_plan.maneuvers.push_back(composeLaneFollowingManeuverMessage(
      downtrack_progress, stop_maneuver_beginning_downtrack, stopping_entry_speed,
      stopping_entry_speed, current_lanelet.id(), time_progress));
  ...
  while (current_lanelet_ending_downtrack < end_of_route_downtrack) {
    ...
    // Break from loop if lanelet's ending downtrack is after the route end downtrack; this is the
    // maneuver's ending lanelet
  ...
  resp->new_plan.maneuvers.push_back(composeStopAndWaitManeuverMessage(
    downtrack_progress, end_of_route_downtrack, stopping_entry_speed, start_lane_id,
    current_lanelet.id(), stopping_deceleration, time_progress));
}
...
void ApproachingEmergencyVehiclePlugin::generateReducedSpeedLaneFollowingeManeuverPlan(
  ...
  lanelet::ConstLanelet current_lanelet, double downtrack_progress,
  double current_lanelet_ending_downtrack, double speed_progress, double target_speed,
  rclcpp::Time time_progress, bool is_maintaining_non_reduced_speed)
{
  ...
  // Update maneuver target speed to be a speed being maintained or a reduced speed below the speed
  // limit
  if (is_maintaining_non_reduced_speed) {
    // ERV is close enough that if the ego vehicle slows down it could cause a safety hazard; set
    // target speed to maintain a non-reduced speed
    ...
    target_speed = std::max(
      (target_speed - config_.speed_limit_reduction_during_passing),
      config_.minimum_reduced_speed_limit);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(logger_name), "Maneuver target speed reduced to " << target_speed);
  }

  double maneuver_plan_ending_downtrack =
    downtrack_progress + config_.minimal_plan_duration * target_speed;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name),
    "Ending downtrack based on plan duration: " << maneuver_plan_ending_downtrack);

  maneuver_plan_ending_downtrack =
    std::min(maneuver_plan_ending_downtrack, wm_->getRouteEndTrackPos().downtrack);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name),
    "Ending downtrack based on end of route: " << wm_->getRouteEndTrackPos().downtrack);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name),
  ...
  double stopping_deceleration =
    config_.vehicle_acceleration_limit * config_.stopping_accel_limit_multiplier;
  ...
  while (downtrack_progress < maneuver_plan_ending_downtrack) {
    if (begin_stopping_downtrack <= current_lanelet_ending_downtrack) {
      // Complete maneuver plan with a stop and wait maneuver if the current lanelet intercepts the
      // stopping downtrack
      addStopAndWaitToEndOfPlan(
        resp, downtrack_progress, begin_stopping_downtrack, wm_->getRouteEndTrackPos().downtrack,
        speed_progress, stopping_deceleration, current_lanelet_ending_downtrack, current_lanelet,
        time_progress);
      return;
    } else {
      // Compose lane following maneuver and add it to the response's maneuver plan
      resp->new_plan.maneuvers.push_back(composeLaneFollowingManeuverMessage(
        downtrack_progress, current_lanelet_ending_downtrack, speed_progress, target_speed,
        current_lanelet.id(), time_progress));
      ...
      // Update maneuver target speed to be a speed being maintained or a reduced speed below the
      // speed limit
      if (is_maintaining_non_reduced_speed) {
        // ERV is close enough that if the ego vehicle slows down it could cause a safety hazard;
        // set target speed to maintain a non-reduced speed
        target_speed = non_reduced_speed_to_maintain_;
      } else {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(logger_name), "Next maneuver starting downtrack is "
                                           << downtrack_progress << ", end of plan is at "
                                           << maneuver_plan_ending_downtrack);
    }
  }
}
...
void ApproachingEmergencyVehiclePlugin::generateMoveOverManeuverPlan(
  ...
  lanelet::ConstLanelet current_lanelet, double downtrack_progress,
  double current_lanelet_ending_downtrack, double speed_progress, double target_speed,
  rclcpp::Time time_progress, int ego_lane_index, int erv_lane_index)
{
  ...
  if (has_planned_upcoming_lc_) {
    while (downtrack_progress < maneuver_plan_ending_downtrack) {
      if (begin_stopping_downtrack <= current_lanelet_ending_downtrack) {
        // Complete maneuver plan with a stop and wait maneuver if the current lanelet intercepts
        // the stopping downtrack
        addStopAndWaitToEndOfPlan(
          resp, downtrack_progress, begin_stopping_downtrack, wm_->getRouteEndTrackPos().downtrack,
          speed_progress, stopping_deceleration, current_lanelet_ending_downtrack, current_lanelet,
          time_progress);
        ...
        // Plan lane change maneuver if current downtrack progress is between the starting and
        // ending downtracks of the planned upcoming lane change, otherwise plan lane follow
        // maneuver
        if (
          ((upcoming_lc_params_.start_dist - epsilon_) <= downtrack_progress) &&
          (downtrack_progress < (upcoming_lc_params_.end_dist - epsilon_))) {
          resp->new_plan.maneuvers.push_back(composeLaneChangeManeuverMessage(
            upcoming_lc_params_.start_dist, upcoming_lc_params_.end_dist,
            upcoming_lc_params_.start_speed, upcoming_lc_params_.end_speed,
            upcoming_lc_params_.starting_lanelet.id(), upcoming_lc_params_.ending_lanelet.id(),
            time_progress));
    ...
    bool completed_initial_lane_following =
      false;  // Flag to indicate whether sufficient lane following has been planned before the lane
              // change
    double initial_lane_following_duration =
      0.0;  // Object to store the duration of lane following planned before the lane change
    ...
    while (downtrack_progress < maneuver_plan_ending_downtrack) {
      if (begin_stopping_downtrack <= current_lanelet_ending_downtrack) {
        ...
        // First maneuver(s) are lane-following to enable sufficient time to activate turn signals
        // before conducting lane change
        if (!completed_initial_lane_following) {
          ...
          initial_lane_following_duration +=
            getManeuverDuration(resp->new_plan.maneuvers.back(), epsilon_).seconds();
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger(logger_name),
          ...
          if (!has_planned_upcoming_lc_) {
            // Initialize UpcomingLaneChangeParameters object to store information regarding the
            // planned lane change maneuver
            UpcomingLaneChangeParameters new_lc_params;

            // Determine if left lane change or right lane change required, and get the target
            // lanelet
            lanelet::Optional<lanelet::ConstLanelet> target_lanelet;
            if (ego_lane_index != erv_lane_index) {
              RCLCPP_WARN_STREAM(
                rclcpp::get_logger(logger_name),
                "Planning a move-over maneuver plan when the ego vehicle is not in the ERV's path! "
                "Returning an empty maneuver plan");
              resp->new_plan.maneuvers = {};
              return;
            } else if ((ego_lane_index == 0) && (erv_lane_index == 0)) {
              // Ego vehicle and ERV are both in the rightmost lane so a left lane change is
              // required
            ...
            // Only compose lane change maneuver if the target lanelet exists, otherwise compose
            // lane follow maneuver
            if (target_lanelet) {
              resp->new_plan.maneuvers.push_back(composeLaneChangeManeuverMessage(
                downtrack_progress, current_lanelet_ending_downtrack, speed_progress, target_speed,
                current_lanelet.id(), target_lanelet.get().id(), time_progress));
              ...
              new_lc_params.maneuver_id =
                resp->new_plan.maneuvers.back().lane_change_maneuver.parameters.maneuver_id;
...
boost::optional<lanelet::ConstLanelet>
ApproachingEmergencyVehiclePlugin::getLaneletOnEgoRouteFromMapPosition(
  const double & x_position, const double & y_position)
{
  // Get lanelet(s) that the provided map coordinates are contained within
  std::vector<lanelet::ConstLanelet> ego_route_lanelets =
  ...
  if (ego_route_lanelets.empty()) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger(logger_name), "Given vehicle position is not on the road.");
    return boost::optional<lanelet::ConstLanelet>();
  }

  // Get the lanelet that is on the ego vehicle's route in case vehicle is located on overlapping
  // lanelets
  ...
  carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp)
{
  ...
  // Check whether ego vehicle is currently in the middle of a lane change, or whether its planned
  // lane change has been completed
  bool is_currently_lane_changing = false;
  if (has_planned_upcoming_lc_) {
    if (
      (upcoming_lc_params_.start_dist < req->veh_downtrack) &&
      (req->veh_downtrack < upcoming_lc_params_.end_dist)) {
  ...
  boost::optional<lanelet::ConstLanelet> ego_current_lanelet_optional =
    getLaneletOnEgoRouteFromMapPosition(req->veh_x, req->veh_y);
  ...
  // Update state machine if there is currently an ERV being tracked and if ego vehicle is not
  // currently in the middle of a lane change
  if (has_tracked_erv_ && !is_currently_lane_changing) {
    if (ego_lane_index_ == tracked_erv_.lane_index) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(logger_name),
        "Ego vehicle and ERV are both in lane index " << ego_lane_index_);

      if (tracked_erv_.seconds_until_passing >= config_.approaching_threshold) {
        transition_table_.event(ApproachingEmergencyVehicleEvent::NO_APPROACHING_ERV);
        has_planned_upcoming_lc_ = false;
      } else if (
        (config_.passing_threshold <= tracked_erv_.seconds_until_passing) &&
        (tracked_erv_.seconds_until_passing < config_.approaching_threshold)) {
        transition_table_.event(ApproachingEmergencyVehicleEvent::APPROACHING_ERV_IN_PATH);
      } else if (
        (0.0 <= tracked_erv_.seconds_until_passing) &&
        (tracked_erv_.seconds_until_passing < config_.passing_threshold)) {
        ...
        if (tracked_erv_.seconds_until_passing >= MAINTAIN_SPEED_THRESHOLD) {
          ...
          // ERV is close enough (in the same lane) that if the ego vehicle slows down it could
          // cause a safety hazard; set internal members for ego vehicle to maintain its current
          // speed
          if (!is_maintaining_non_reduced_speed_) {
        ...
        // Set flag to broadcast EmergencyVehicleResponse warning messages to ERV if they have not
        // already been broadcasted
        ...
        RCLCPP_WARN_STREAM(
          rclcpp::get_logger(logger_name), "Unsupported seconds_until_passing of "
                                             << tracked_erv_.seconds_until_passing
                                             << ", returning empty maneuver plan");
        resp->new_plan.maneuvers = {};
        return;
      }
    } else {
  ...
  switch (transition_table_.getState()) {
    case ApproachingEmergencyVehicleState::NO_APPROACHING_ERV:
      resp->new_plan.maneuvers = {};
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(logger_name), "No approaching ERV. Returning empty maneuver plan");
      is_maintaining_non_reduced_speed_ =
        false;  // Reset flag since ego vehicle does not need to maintain a non-reduced speed
      break;
    ...
    case ApproachingEmergencyVehicleState::MOVING_OVER_FOR_APPROACHING_ERV:
      ...
      generateMoveOverManeuverPlan(
        resp, ego_current_lanelet_optional.get(), downtrack_progress,
        current_lanelet_ending_downtrack, speed_progress, target_speed, time_progress,
        ego_lane_index_, tracked_erv_.lane_index);
      break;

    case ApproachingEmergencyVehicleState::SLOWING_DOWN_FOR_ERV:
      // Generate a maneuver plan consisting of only lane-following maneuvers at a reduced speed
      // since the ERV is actively passing the ego vehicle
...
bool ApproachingEmergencyVehiclePlugin::get_availability() { return true; }

std::string ApproachingEmergencyVehiclePlugin::get_version_id() { return "v1.0"; }
