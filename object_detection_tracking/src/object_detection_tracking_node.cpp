...
carma_ros2_utils::CallbackReturn ObjectDetectionTrackingNode::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  this->add_on_set_parameters_callback([this](auto param_vec) {
  ...
  // Setup pub/sub
  autoware_obj_sub_ = create_subscription<autoware_auto_msgs::msg::TrackedObjects>(
    "detected_objects", 10,
    std::bind(&ObjectDetectionTrackingWorker::detectedObjectCallback, &object_worker_, _1));

  carma_obj_pub_ =
    create_publisher<carma_perception_msgs::msg::ExternalObjectList>("external_objects", 10);

  return CallbackReturn::SUCCESS;
}

void ObjectDetectionTrackingNode::publishObject(
  const carma_perception_msgs::msg::ExternalObjectList & obj_msg)
{
  carma_obj_pub_->publish(obj_msg);
}

boost::optional<geometry_msgs::msg::TransformStamped> ObjectDetectionTrackingNode::lookupTransform(
