...
var divTab1Message = document.getElementById('divTab1Message');
...
function showRouteOptions (){
      ...
      divTab1Message.innerHTML = 'Please select a route.';
      ...
      getAvailableRoutesClient.callService(request, function(result) {
       ...
       for (i=0; i < myRoutes.length; i++) {
            createRadioElement(divRoutes, myRoutes[i].routeID, myRoutes[i].routeName, myRoutes.length);
        }

        if (myRoutes.length == 0) {
            divTab1Message.innerHTML = '<p> Sorry, there are no available routes, and cannot proceed without one. </p> <p> Please contact the System Admin.</p>';
        }
...
function setRoute(id){
    ...
    setActiveRouteClient.callService(request, function(result) {
      ...
      {
          divTab1Message.innerHTML = '<p> Activating the route failed, please try it again.</p>';
      ...
      else
      {
           ...
           divTab1Message.innerHTML = 'Please select one or more capabilities to activate.';
      }
    });
}
