...
LightControlledIntersectionTransitPluginNode::LightControlledIntersectionTransitPluginNode(
...
: carma_guidance_plugins::TacticalPlugin(options)
{
  ...
  config_.default_downsample_ratio =
    declare_parameter<int>("default_downsample_ratio", config_.default_downsample_ratio);
  ...
  config_.lat_accel_multiplier =
    declare_parameter<double>("lat_accel_multiplier", config_.lat_accel_multiplier);
  ...
  config_.algorithm_evaluation_distance = declare_parameter<double>(
    "algorithm_evaluation_distance", config_.algorithm_evaluation_distance);
  config_.algorithm_evaluation_period =
    declare_parameter<double>("algorithm_evaluation_period", config_.algorithm_evaluation_period);

  config_.lateral_accel_limit =
    declare_parameter<double>("vehicle_lateral_accel_limit", config_.lateral_accel_limit);
  ...
  config_.enable_object_avoidance =
    declare_parameter<bool>("enable_object_avoidance", config_.enable_object_avoidance);
}
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  auto error = update_params<double>(
    ...
    parameters);  // Global acceleration limits not allowed to dynamically update
...
carma_ros2_utils::CallbackReturn LightControlledIntersectionTransitPluginNode::on_configure_plugin()
{
  ...
  add_on_set_parameters_callback(std::bind(
    &LightControlledIntersectionTransitPluginNode::parameter_update_callback, this, std_ph::_1));

  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("light_controlled_intersection_tactical_plugin"),
    "Loaded params: " << config_);

  // Initialize worker object
  worker_ = std::make_shared<LightControlledIntersectionTacticalPlugin>(
    get_world_model(), config_, get_plugin_name(), shared_from_this());

  yield_client_ =
    create_client<carma_planning_msgs::srv::PlanTrajectory>("yield_plugin/plan_trajectory");
  worker_->set_yield_client(yield_client_);
  RCLCPP_INFO(
    rclcpp::get_logger("light_controlled_intersection_tactical_plugin"), "Yield Client Set");

  // Return success if everything initialized successfully
  return CallbackReturn::SUCCESS;
}

bool LightControlledIntersectionTransitPluginNode::get_availability() { return true; }
