  ...
  config_(InLaneCruisingPluginConfig())
{
  ...
  config_.max_accel_multiplier =
    declare_parameter<double>("max_accel_multiplier", config_.max_accel_multiplier);
...
{
  trajectory_debug_pub_ = create_publisher<carma_debug_ros2_msgs::msg::TrajectoryCurvatureSpeeds>(
  ...
  add_on_set_parameters_callback(
    std::bind(&InLaneCruisingPluginNode::parameter_update_callback, this, std_ph::_1));

  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("inlanecruising_plugin"), "InLaneCruisingPlugin Params" << config_);
  ...
  RCLCPP_INFO_STREAM(
    ...
    "InLaneCruisingPlugin Params After Accel Change" << config_);

  worker_ = std::make_shared<InLaneCruisingPlugin>(
    ...
    plugin_name_, version_id_);

  // TODO: Update yield client to use the Plugin Manager capabilities query, in case someone else
  // wants to add an alternate yield implementation
  yield_client_ =
...
rcl_interfaces::msg::SetParametersResult InLaneCruisingPluginNode::parameter_update_callback(
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  auto error_bool =
    update_params<bool>({{"enable_object_avoidance", config_.enable_object_avoidance}}, parameters);
...
bool InLaneCruisingPluginNode::get_availability() { return true; }

std::string InLaneCruisingPluginNode::get_version_id() { return version_id_; }
