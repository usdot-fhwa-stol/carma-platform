...
WMBroadcaster::WMBroadcaster(
  const PublishMapCallback & map_pub, const PublishMapUpdateCallback & map_update_pub,
  const PublishCtrlRequestCallback & control_msg_pub, const PublishActiveGeofCallback & active_pub,
  std::shared_ptr<carma_ros2_utils::timers::TimerFactory> timer_factory,
  const PublishMobilityOperationCallback & tcm_ack_pub)
: map_pub_(map_pub),
  map_update_pub_(map_update_pub),
  control_msg_pub_(control_msg_pub),
  active_pub_(active_pub),
  scheduler_(timer_factory),
  tcm_ack_pub_(tcm_ack_pub)
{
...
void WMBroadcaster::baseMapCallback(autoware_lanelet2_msgs::msg::MapBin::UniquePtr map_msg)
{
  ...
  // This function should generally only ever be called one time so log a warning if it occurs
  // multiple times
  if (firstCall) {
    firstCall = false;
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
  ...
  lanelet::MapConformer::ensureCompliance(
    base_map_, config_limit);  // Update map to ensure it complies with expectations
  ...
  lanelet::traffic_rules::TrafficRulesUPtr traffic_rules_car =
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"), "Done building routing graph for base map");

  // Publish map
  current_map_version_ +=
    1;  // Increment the map version. It should always start from 1 for the first map
...
/*!
 * \brief Populates the schedules member of the geofence object from given TrafficControlMessageV01
 * message \param gf_ptr geofence pointer \param msg_v01 TrafficControlMessageV01 (geofence msg)
 */
void WMBroadcaster::addScheduleFromMsg(
  std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::TrafficControlMessageV01 & msg_v01)
{
  ...
  if (!msg_schedule.end_exists) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "No end time for geofence, using rclcpp::Time::max()");
  ...
  GeofenceSchedule::DayOfTheWeekSet week_day_set = {
    0, 1, 2, 3, 4, 5, 6};  // Default to all days  0==Sun to 6==Sat
  ...
  if (msg_schedule.between_exists) {
    for (auto daily_schedule : msg_schedule.between) {
      if (msg_schedule.repeat_exists) {
        gf_ptr->schedules.push_back(GeofenceSchedule(
          {msg_schedule.start, clock_type}, end_time, rclcpp::Duration(daily_schedule.begin),
          rclcpp::Duration(daily_schedule.duration), rclcpp::Duration(msg_schedule.repeat.offset),
          rclcpp::Duration(msg_schedule.repeat.span), rclcpp::Duration(msg_schedule.repeat.period),
          week_day_set));
      } else {
        gf_ptr->schedules.push_back(GeofenceSchedule(
          {msg_schedule.start, clock_type}, end_time, rclcpp::Duration(daily_schedule.begin),
          rclcpp::Duration(daily_schedule.duration), rclcpp::Duration(0.0, 0.0),  // No offset
          rclcpp::Duration(daily_schedule.duration) -
            rclcpp::Duration(daily_schedule.begin),  // Compute schedule portion end time
          rclcpp::Duration(daily_schedule.duration) -
            rclcpp::Duration(daily_schedule.begin),  // No repetition so same as portion end time
          week_day_set));
      }
    }
  } else {
    if (msg_schedule.repeat_exists) {
      gf_ptr->schedules.push_back(GeofenceSchedule(
        {msg_schedule.start, clock_type}, end_time, rclcpp::Duration::from_nanoseconds(0.0),
        rclcpp::Duration::from_nanoseconds(86400.0e9),  // 24 hr daily application
        rclcpp::Duration(msg_schedule.repeat.offset), rclcpp::Duration(msg_schedule.repeat.span),
        rclcpp::Duration(msg_schedule.repeat.period), week_day_set));
    } else {
      gf_ptr->schedules.push_back(GeofenceSchedule(
        {msg_schedule.start, clock_type}, end_time, rclcpp::Duration::from_nanoseconds(0.0),
        rclcpp::Duration::from_nanoseconds(86400.0e9),  // 24 hr daily application
...
std::vector<std::shared_ptr<Geofence>> WMBroadcaster::geofenceFromMapMsg(
  std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::MapData & map_msg)
{
  ...
  sim_->createIntersectionFromMapMsg(
    intersections, traffic_signals, map_msg, current_map_, current_routing_graph_);

  if (*sim_ == *sim_copy)  // if no change
  {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      ">>> Detected no change from previous, ignoring duplicate message! with gf id: "
        << gf_ptr->id_);
    return {};
  }
...
void WMBroadcaster::geofenceFromMsg(
  std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::TrafficControlMessageV01 & msg_v01)
{
  bool detected_workzone_signal =
    msg_v01.package.label_exists && msg_v01.package.label.find("SIG_WZ") != std::string::npos;
  ...
  if (gf_ptr->affected_parts_.size() == 0) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "There is no applicable component in map for the new geofence message received by "
      "WMBroadcaster with id: "
        << gf_ptr->id_);
  ...
  for (auto llt_or_area : gf_ptr->affected_parts_) {
    ...
    if (llt_or_area.isArea())
      affected_areas.push_back(current_map_->areaLayer.get(llt_or_area.area()->id()));
  }
  ...
  // - digital speed limit, passing control line, digital minimum gap, region access rule, and
  // series of workzone related messages
  lanelet::Velocity sL;
  ...
  if (msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::MAXSPEED_CHOICE) {
    ...
    std::string reason = "";
    if (msg_v01.package.label_exists) reason = msg_v01.package.label;

    if (config_limit > 0_mph && config_limit < 80_mph && config_limit < sL)  // Accounting for the
                                                                             // configured speed
                                                                             // limit, input zero
                                                                             // when not in use
      sL = config_limit;
    ...
    if (sL > 80_mph) {
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"),
        "Digital maximum speed limit is invalid. Value capped at max speed limit.");  // Output
                                                                                      // warning
                                                                                      // message
    ...
    if (sL < 0_mph) {
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"), "Digital  speed limit is invalid. Value set to 0mph.");
      sL = 0_mph;
    }  // @SONAR_START@

    gf_ptr->regulatory_element_ =
      std::make_shared<lanelet::DigitalSpeedLimit>(lanelet::DigitalSpeedLimit::buildData(
        lanelet::utils::getId(), sL, affected_llts, affected_areas, participantsChecker(msg_v01),
        reason));
  }
  if (msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::MINSPEED_CHOICE) {
    ...
    if (config_limit > 0_mph && config_limit < 80_mph)  // Accounting for the configured speed
                                                        // limit, input zero when not in use
      sL = config_limit;
  ...
  if (
    msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::LATPERM_CHOICE ||
    msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::LATAFFINITY_CHOICE) {
  ...
  if (msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::MINHDWY_CHOICE) {
    ...
    if (min_gap < 0) {
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"), "Digital min gap is invalid. Value set to 0 meter.");
      min_gap = 0;
    }
  ...
  if (
    detected_workzone_signal &&
    msg_detail.choice !=
      carma_v2x_msgs::msg::TrafficControlDetail::MAXSPEED_CHOICE)  // if workzone message detected,
                                                                   // save to cache to process later
  {
    gf_ptr->label_ = msg_v01.package.label;  // to extract intersection, and signal group id
    if (
      msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::CLOSED_CHOICE &&
      (msg_detail.closed == carma_v2x_msgs::msg::TrafficControlDetail::CLOSED ||
      ...
      msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::DIRECTION_CHOICE &&
      msg_detail.direction == carma_v2x_msgs::msg::TrafficControlDetail::REVERSE) {
    ...
    msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::CLOSED_CHOICE &&
    msg_detail.closed ==
      carma_v2x_msgs::msg::TrafficControlDetail::CLOSED)  // if stand-alone closed signal apart from
                                                          // Workzone
  {
...
std::shared_ptr<Geofence> WMBroadcaster::createWorkzoneGeofence(
  std::unordered_map<uint8_t, std::shared_ptr<Geofence>> work_zone_geofence_cache)
{
  std::shared_ptr<std::vector<lanelet::Lanelet>> parallel_llts =
    std::make_shared<std::vector<lanelet::Lanelet>>(std::vector<lanelet::Lanelet>());
  ...
  // Create geofence and rest of the required lanelets along with traffic light for completing
  // workzone area
  auto gf_ptr = createWorkzoneGeometry(
    work_zone_geofence_cache, parallel_llts->front(), parallel_llts->back(),
    middle_opposite_lanelets);

  // copy static info from the existing workzone
  gf_ptr->id_ = work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]
                  ->id_;  // using taperright's id as the whole geofence's id

  // schedule
  gf_ptr->schedules =
    work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]
      ->schedules;  // using taperright's schedule as the whole geofence's schedule

  // erase cache now that it is processed
  for (auto pair : work_zone_geofence_cache) {
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "Workzone geofence finished processing. Therefore following geofence id is being dropped "
      "from cache as it is processed as part of it: "
        << pair.second->id_);
  }
...
std::shared_ptr<Geofence> WMBroadcaster::createWorkzoneGeometry(
  std::unordered_map<uint8_t, std::shared_ptr<Geofence>> work_zone_geofence_cache,
  lanelet::Lanelet parallel_llt_front, lanelet::Lanelet parallel_llt_back,
  std::shared_ptr<std::vector<lanelet::Lanelet>> middle_opposite_lanelets)
{
  ...
  lanelet::Lanelet front_llt_diag = createLinearInterpolatingLanelet(
    parallel_llt_front.leftBound3d().back(), parallel_llt_front.rightBound3d().back(),
    middle_opposite_lanelets->back().rightBound3d().back(),
    middle_opposite_lanelets->back().leftBound3d().back());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"), "Created diag front_llt_diag id:" << front_llt_diag.id());
  for (auto regem : middle_opposite_lanelets->back()
                      .regulatoryElements())  // copy existing regem into the new llts
  {
  ...
  lanelet::Lanelet back_llt_diag = createLinearInterpolatingLanelet(
    middle_opposite_lanelets->front().rightBound3d().front(),
    middle_opposite_lanelets->front().leftBound3d().front(),
    parallel_llt_back.leftBound3d().front(), parallel_llt_back.rightBound3d().front());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"), "Created back_llt_diag diag id:" << back_llt_diag.id());
  for (auto regem :
       parallel_llt_back.regulatoryElements())  // copy existing regem into the new llts
  {
  ...
  for (int i = middle_opposite_lanelets->size() - 1; i >= 0;
       i--)  // do no use size_t as it might overflow
  {
    lanelet::Lanelet middle_llt(
      lanelet::utils::getId(), (*(middle_opposite_lanelets.get()))[i].rightBound3d().invert(),
      (*(middle_opposite_lanelets.get()))[i].leftBound3d().invert());
  ...
  //////////////////////////////
  lanelet::LineString3d parallel_stop_ls(
    lanelet::utils::getId(),
    {parallel_llt_front.leftBound3d().back(), parallel_llt_front.rightBound3d().back()});
  // controlled lanelet:
  ...
  controlled_taper_right.insert(
    controlled_taper_right.end(), middle_llts.begin(), middle_llts.end());

  controlled_taper_right.push_back(back_llt_diag);

  lanelet::CarmaTrafficSignalPtr tfl_parallel =
    std::make_shared<lanelet::CarmaTrafficSignal>(lanelet::CarmaTrafficSignal::buildData(
      lanelet::utils::getId(), {parallel_stop_ls}, {controlled_taper_right.front()},
      {controlled_taper_right.back()}));

  gf_ptr->traffic_light_id_lookup_.push_back(
    {generate32BitId(work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]->label_),
     tfl_parallel->id()});
  ...
  //////////////////////////////
  std::shared_ptr<std::vector<lanelet::Lanelet>> opposite_llts_with_stop_line =
    std::make_shared<std::vector<lanelet::Lanelet>>(std::vector<lanelet::Lanelet>());

  auto old_opposite_llts = splitOppositeLaneletWithPoint(
    opposite_llts_with_stop_line,
    work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]->gf_pts.back().basicPoint2d(),
    current_map_->laneletLayer.get(work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]
                                     ->affected_parts_.back()
                                     .lanelet()
                                     .get()
                                     .id()),
    error_distance_);

  lanelet::LineString3d opposite_stop_ls(
    lanelet::utils::getId(), {opposite_llts_with_stop_line->front().leftBound3d().back(),
                              opposite_llts_with_stop_line->front().rightBound3d().back()});

  std::vector<lanelet::Lanelet> controlled_open_right;

  controlled_open_right.insert(
    controlled_open_right.end(), opposite_llts_with_stop_line->begin(),
    opposite_llts_with_stop_line->end());
  ;  // split lanelet one of which has light
  ...
  lanelet::CarmaTrafficSignalPtr tfl_opposite =
    std::make_shared<lanelet::CarmaTrafficSignal>(lanelet::CarmaTrafficSignal::buildData(
      lanelet::utils::getId(), {opposite_stop_ls}, {controlled_open_right.front()},
      {controlled_open_right.back()}));

  gf_ptr->traffic_light_id_lookup_.push_back(
    {generate32BitId(work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]->label_),
     tfl_opposite->id()});
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "Created TF_LIGHT of Id: " << tfl_opposite->id()
                               << ", to opposite_llts_with_stop_line->front() id:"
                               << opposite_llts_with_stop_line->front().id());
  ...
  //////////////////////////////
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"), "Added parallel_llt_front id:" << parallel_llt_front.id());
  gf_ptr->lanelet_additions_.push_back(parallel_llt_front);

  gf_ptr->lanelet_additions_.push_back(front_llt_diag);

  gf_ptr->lanelet_additions_.insert(
    gf_ptr->lanelet_additions_.end(), middle_llts.begin(), middle_llts.end());

  gf_ptr->lanelet_additions_.push_back(back_llt_diag);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"), "Added parallel_llt_back id:" << parallel_llt_back.id());
  gf_ptr->lanelet_additions_.push_back(parallel_llt_back);

  gf_ptr->lanelet_additions_.insert(
    gf_ptr->lanelet_additions_.end(), opposite_llts_with_stop_line->begin(),
    opposite_llts_with_stop_line->end());
  ;
  ...
  j2735_v2x_msgs::msg::TrafficControlVehClass
    participant;  // sending all possible VEHICLE will be processed as they are not accessuble by
                  // regionAccessRule
  ...
  participant.vehicle_class =
    j2735_v2x_msgs::msg::TrafficControlVehClass::TWO_AXLE_SIX_TIRE_SINGLE_UNIT_TRUCK;
  ...
  std::vector<lanelet::Lanelet>
    old_or_blocked_llts;  // this is needed to addRegionAccessRule input signatures
  ...
  for (auto llt : work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]->affected_parts_) {
    if (
      std::find(old_or_blocked_llts.begin(), old_or_blocked_llts.end(), llt) ==
      old_or_blocked_llts.end()) {
  ...
  gf_ptr->affected_parts_.push_back(
    old_opposite_llts[0]);  // block old lanelet in the opposing lanelet that will be replaced with
                            // split lanelets that have trafficlight
...
void WMBroadcaster::setErrorDistance(double error_distance) { error_distance_ = error_distance; }

void WMBroadcaster::preprocessWorkzoneGeometry(
  std::unordered_map<uint8_t, std::shared_ptr<Geofence>> work_zone_geofence_cache,
  std::shared_ptr<std::vector<lanelet::Lanelet>> parallel_llts,
  std::shared_ptr<std::vector<lanelet::Lanelet>> opposite_llts)
{
  if (!current_map_ || current_map_->laneletLayer.size() == 0) {
    throw lanelet::InvalidObjectStateError(
      std::string("Base lanelet map is not loaded to the WMBroadcaster"));
  }
  ////////////////////////////////////
  /// PARALLEL FRONT (TAPERRIGHT side)
  ////////////////////////////////////
  std::vector<lanelet::Lanelet> new_taper_right_llts;
  auto taper_right_first_pt =
    work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]->gf_pts.front().basicPoint2d();
  auto taper_right_first_llt =
    current_map_->laneletLayer.get(work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]
                                     ->affected_parts_.front()
                                     .lanelet()
                                     .get()
                                     .id());

  new_taper_right_llts =
    splitLaneletWithPoint({taper_right_first_pt}, taper_right_first_llt, error_distance_);
  double check_dist_tpr = lanelet::geometry::distance2d(
    taper_right_first_llt.centerline2d().front().basicPoint2d(), taper_right_first_pt);

  // if no splitting happened and taperright's first point is close to starting boundary, we need to
  // create duplicate of previous lanelet of TAPERRIGHT's first lanelet to match the output expected
  // of this function as if split happened
  if (new_taper_right_llts.size() == 1 && check_dist_tpr <= error_distance_) {
    ...
    auto previous_lanelets =
      current_routing_graph_->previous(work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]
                                         ->affected_parts_.front()
                                         .lanelet()
                                         .get());
    if (previous_lanelets.empty())  // error if bad match
    {
      RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"),
        "Workzone area starts from lanelet with no previous lanelet (Id : "
          << work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]
               ->affected_parts_.front()
               .lanelet()
               .get()
    ...
    // get previous lanelet of affected part of TAPERRIGHT (doesn't matter which previous, as the
    // new lanelet will only be duplicate anyways)
    auto prev_lanelet_to_copy = current_map_->laneletLayer.get(previous_lanelets.front().id());

    // parallel_llts will have a copy of `prev_lanelet_to_copy` with new id to be used as part of
    // workzone area
    new_taper_right_llts = splitLaneletWithPoint(
      {prev_lanelet_to_copy.centerline2d().back()}, prev_lanelet_to_copy, error_distance_);
  }
  parallel_llts->insert(
    parallel_llts->end(), new_taper_right_llts.begin(), new_taper_right_llts.end());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
  ...
  auto open_right_last_pt =
    work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]->gf_pts.back().basicPoint2d();
  auto open_right_last_llt =
    current_map_->laneletLayer.get(work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]
                                     ->affected_parts_.back()
                                     .lanelet()
                                     .get()
  ...
  new_open_right_llts =
    splitLaneletWithPoint({open_right_last_pt}, open_right_last_llt, error_distance_);
  double check_dist_opr = lanelet::geometry::distance2d(
    open_right_last_llt.centerline2d().back().basicPoint2d(), open_right_last_pt);

  // if no splitting happened, we need to create duplicate of next lanelet of OPENRIGHT's last
  // lanelet to match the output expected of this function as if split happened
  if (new_open_right_llts.size() == 1 && check_dist_opr <= error_distance_) {
    ...
    auto next_lanelets = current_routing_graph_->following(
      work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]->affected_parts_.back().lanelet().get());
    if (next_lanelets.empty())  // error if bad match
    {
      RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"),
        "Workzone area ends at lanelet with no following lanelet (Id : "
          << work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]
               ->affected_parts_.back()
               .lanelet()
               .get()
               .id()
    ...
    // get next lanelet of affected part of OPENRIGHT (doesn't matter which next, as the new lanelet
    // will only be duplicate anyways)
    auto next_lanelet_to_copy = current_map_->laneletLayer.get(next_lanelets.front().id());

    // parallel_llts will have a copy of `next_lanelet_to_copy` with new id to be used as part of
    // workzone area
    new_open_right_llts = splitLaneletWithPoint(
      {next_lanelet_to_copy.centerline2d().back()}, next_lanelet_to_copy, error_distance_);
  }
  parallel_llts->insert(
    parallel_llts->end(), new_open_right_llts.begin(), new_open_right_llts.end());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
  ...
  auto reverse_back_llts = carma_wm::query::getLaneletsFromPoint(
    current_map_, work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.back().basicPoint2d());
  auto reverse_back_llt = reverse_back_llts[0];
  auto reverse_front_llts = carma_wm::query::getLaneletsFromPoint(
    current_map_,
    work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.front().basicPoint2d());
  auto reverse_front_llt = reverse_front_llts[0];

  if (reverse_back_llt.id() == reverse_front_llt.id())  // means there is only 1 middle lanelet,
                                                        // which needs to be split into 3 lanelets
  {
    std::vector<lanelet::Lanelet> temp_llts;
    temp_llts = splitLaneletWithPoint(
      {work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.front().basicPoint2d(),
       work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.back().basicPoint2d()},
      current_map_->laneletLayer.get(work_zone_geofence_cache[WorkZoneSection::REVERSE]
                                       ->affected_parts_.front()
                                       .lanelet()
                                       .get()
                                       .id()),
    ...
    if (temp_llts.size() < 2)  // if there is only 1 lanelet
    {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"),
        "Ended preprocessWorkzoneGeometry with opposite_llts.size()"
          << opposite_llts->size() << ", and parallel_llts.size()" << parallel_llts->size());
      return;
    } else if (temp_llts.size() == 2)  // determine which
    {
      ...
      if (
        lanelet::geometry::distance2d(
          work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.back().basicPoint2d(),
          reverse_front_llt.centerline2d().back().basicPoint2d()) >
        lanelet::geometry::distance2d(
          work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.front().basicPoint2d(),
          reverse_front_llt.centerline2d().front().basicPoint2d())) {
  ...
  } else  // if there are two or more lanelets
  {
    ...
    auto reverse_first_pt =
      work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.front().basicPoint2d();
    auto reverse_first_llt =
      current_map_->laneletLayer.get(work_zone_geofence_cache[WorkZoneSection::REVERSE]
                                       ->affected_parts_.front()
                                       .lanelet()
                                       .get()
    ...
    temp_opposite_front_llts =
      splitLaneletWithPoint({reverse_first_pt}, reverse_first_llt, error_distance_);
    if (temp_opposite_front_llts.size() > 1) {
      opposite_llts->insert(
        opposite_llts->end(), temp_opposite_front_llts.begin() + 1, temp_opposite_front_llts.end());
    } else {
      opposite_llts->insert(
        opposite_llts->end(), temp_opposite_front_llts.begin(), temp_opposite_front_llts.end());
    }
    /// Fill in the middle part of middle lanelets
    if (work_zone_geofence_cache[WorkZoneSection::REVERSE]->affected_parts_.size() > 2) {
      for (int i = 1;
           i < work_zone_geofence_cache[WorkZoneSection::REVERSE]->affected_parts_.size() - 1;
           i++) {
        opposite_llts->push_back(current_map_->laneletLayer.get(
          work_zone_geofence_cache[WorkZoneSection::REVERSE]->affected_parts_[i].id()));
      }
    }
    /// OPPOSITE BACK (TAPERRIGHT side)

    auto reverse_last_pt =
      work_zone_geofence_cache[WorkZoneSection::REVERSE]->gf_pts.back().basicPoint2d();
    auto reverse_last_llt =
      current_map_->laneletLayer.get(work_zone_geofence_cache[WorkZoneSection::REVERSE]
                                       ->affected_parts_.back()
                                       .lanelet()
                                       .get()
    ...
    temp_opposite_back_llts =
      splitLaneletWithPoint({reverse_last_pt}, reverse_last_llt, error_distance_);
    if (temp_opposite_back_llts.size() > 1) {
      opposite_llts->insert(
        opposite_llts->end(), temp_opposite_back_llts.begin(), temp_opposite_back_llts.end() - 1);
    } else {
      opposite_llts->insert(
        opposite_llts->end(), temp_opposite_back_llts.begin(), temp_opposite_back_llts.end());
    }
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "Finished REVERSE processing of size: "
        << opposite_llts->size() << " from original of more than one REVERSE lanelet size");
  }
...
std::vector<lanelet::Lanelet> WMBroadcaster::splitLaneletWithPoint(
  const std::vector<lanelet::BasicPoint2d> & input_pts, const lanelet::Lanelet & input_llt,
  double error_distance)
{
  // get ratio of this point and split
  std::vector<lanelet::Lanelet> parallel_llts;
  double llt_downtrack =
    carma_wm::geometry::trackPos(input_llt, input_llt.centerline().back().basicPoint2d()).downtrack;
...
lanelet::Lanelets WMBroadcaster::splitOppositeLaneletWithPoint(
  std::shared_ptr<std::vector<lanelet::Lanelet>> opposite_llts,
  const lanelet::BasicPoint2d & input_pt, const lanelet::Lanelet & input_llt, double error_distance)
{
  // get ratio of this point and split
  auto point_downtrack = carma_wm::geometry::trackPos(input_llt, input_pt).downtrack;
  auto point_downtrack_ratio =
    point_downtrack /
    carma_wm::geometry::trackPos(input_llt, input_llt.centerline().back().basicPoint2d()).downtrack;
  ...
  if (opposing_llts.empty()) {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "WMBroadcaster was not able to find opposing lane for given point in geofence related to "
      "Work Zone! Returning");
    return {};
  }

  auto new_llts_opposite =
...
std::vector<lanelet::Lanelet> WMBroadcaster::splitLaneletWithRatio(
  std::vector<double> ratios, lanelet::Lanelet input_lanelet, double error_distance) const
{
  if (!current_map_ || current_map_->laneletLayer.size() == 0) {
    throw lanelet::InvalidObjectStateError(
      std::string("Base lanelet map is not loaded to the WMBroadcaster"));
  ...
  for (int i = 0; i < ratios.size(); i++) {
    ...
    if (
      lanelet::geometry::distance2d(
        input_lanelet.leftBound2d().front().basicPoint2d(),
        input_lanelet.leftBound2d()[left_next_pt_idx].basicPoint2d()) <= error_distance) {
      // assuming both linestrings have roughly the same number of points and
      // assuming distance between 0th and index-th points are small enough we can approximate the
      // curve between them as a line:
      RCLCPP_INFO_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"),
        "Ratio: " << ratios[i]
                  << ", is too close to the lanelet's front boundary! Therefore, ignoring... "
                     "Allowed error_distance: "
                  << error_distance << ", Distance: "
                  << lanelet::geometry::distance2d(
                       input_lanelet.leftBound2d().front().basicPoint2d(),
                       input_lanelet.leftBound2d()[left_next_pt_idx].basicPoint2d()));
      continue;
    }
    if (
      lanelet::geometry::distance2d(
        input_lanelet.leftBound2d().back().basicPoint2d(),
        input_lanelet.leftBound2d()[left_next_pt_idx].basicPoint2d()) <= error_distance) {
      RCLCPP_INFO_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"),
        "Ratio: " << ratios[i]
                  << ", is too close to the lanelet's back boundary! Therefore, ignoring... "
                     "Allowed error_distance: "
                  << error_distance << ", Distance: "
                  << lanelet::geometry::distance2d(
                       input_lanelet.leftBound2d().back().basicPoint2d(),
    ...
    left_pts.insert(
      ...
      current_map_->laneletLayer.get(input_lanelet.id()).leftBound3d().begin() + left_next_pt_idx +
        1);

    lanelet::LineString3d left_ls(
      lanelet::utils::getId(), left_pts,
    ...
    right_pts.insert(
      ...
      current_map_->laneletLayer.get(input_lanelet.id()).rightBound3d().begin() +
        right_next_pt_idx + 1);

    lanelet::LineString3d right_ls(lanelet::utils::getId(), right_pts);

    lanelet::Lanelet new_llt(
      lanelet::utils::getId(), left_ls, right_ls,
      current_map_->laneletLayer.get(input_lanelet.id()).rightBound3d().attributes());

    for (auto regem : current_map_->laneletLayer.get(input_lanelet.id())
                        .regulatoryElements())  // copy existing regem into new llts
    {
...
void WMBroadcaster::addPassingControlLineFromMsg(
  std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::TrafficControlMessageV01 & msg_v01,
  const std::vector<lanelet::Lanelet> & affected_llts) const
{
  ...
  if (
    msg_detail.latperm[0] == carma_v2x_msgs::msg::TrafficControlDetail::PERMITTED ||
  ...
  if (
    msg_detail.latperm[1] == carma_v2x_msgs::msg::TrafficControlDetail::PERMITTED ||
  ...
  gf_ptr->regulatory_element_ =
    std::make_shared<lanelet::PassingControlLine>(lanelet::PassingControlLine::buildData(
      lanelet::utils::getId(), pcl_bounds, left_participants, right_participants));
}

void WMBroadcaster::addRegionAccessRule(
  std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::TrafficControlMessageV01 & msg_v01,
  const std::vector<lanelet::Lanelet> & affected_llts) const
{
  ...
  auto regulatory_element =
    std::make_shared<lanelet::RegionAccessRule>(lanelet::RegionAccessRule::buildData(
      lanelet::utils::getId(), affected_llts, {}, invertParticipants(participantsChecker(msg_v01)),
      reason));

  if (
    !regulatory_element->accessable(lanelet::Participants::VehicleCar) ||
    !regulatory_element->accessable(lanelet::Participants::VehicleTruck)) {
...
void WMBroadcaster::addRegionMinimumGap(
  std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::TrafficControlMessageV01 & msg_v01,
  double min_gap, const std::vector<lanelet::Lanelet> & affected_llts,
  const std::vector<lanelet::Area> & affected_areas) const
{
  auto regulatory_element =
    std::make_shared<lanelet::DigitalMinimumGap>(lanelet::DigitalMinimumGap::buildData(
      lanelet::utils::getId(), min_gap, affected_llts, affected_areas,
      participantsChecker(msg_v01)));

  gf_ptr->regulatory_element_ = regulatory_element;
}

std::vector<std::string> WMBroadcaster::participantsChecker(
  const carma_v2x_msgs::msg::TrafficControlMessageV01 & msg_v01) const
{
  ...
  for (j2735_v2x_msgs::msg::TrafficControlVehClass participant : msg_v01.params.vclasses) {
    ...
    if (participant.vehicle_class == j2735_v2x_msgs::msg::TrafficControlVehClass::ANY) {
      participants = {
        lanelet::Participants::Vehicle, lanelet::Participants::Pedestrian,
        lanelet::Participants::Bicycle};
      break;
    } else if (
      participant.vehicle_class == j2735_v2x_msgs::msg::TrafficControlVehClass::PEDESTRIAN) {
      ...
      participant.vehicle_class == j2735_v2x_msgs::msg::TrafficControlVehClass::LIGHT_TRUCK_VAN ||
      participant.vehicle_class == j2735_v2x_msgs::msg::TrafficControlVehClass::PASSENGER_CAR) {
      participants.push_back(lanelet::Participants::VehicleCar);
    } else if (8 <= participant.vehicle_class && participant.vehicle_class <= 16)  // Truck enum
                                                                                   // definition
                                                                                   // range from
                                                                                   // 8-16 currently
    {
...
std::vector<std::string> WMBroadcaster::invertParticipants(
  const std::vector<std::string> & input_participants) const
{
  std::vector<std::string> participants;

  if (
    std::find(
      input_participants.begin(), input_participants.end(), lanelet::Participants::Pedestrian) ==
    input_participants.end())
    participants.emplace_back(lanelet::Participants::Pedestrian);
  if (
    std::find(
      input_participants.begin(), input_participants.end(), lanelet::Participants::Bicycle) ==
    input_participants.end())
    participants.emplace_back(lanelet::Participants::Bicycle);
  if (
    std::find(
      input_participants.begin(), input_participants.end(), lanelet::Participants::Vehicle) ==
    input_participants.end()) {
    if (
      std::find(
        input_participants.begin(), input_participants.end(),
        lanelet::Participants::VehicleMotorcycle) == input_participants.end())
      participants.emplace_back(lanelet::Participants::VehicleMotorcycle);
    if (
      std::find(
        input_participants.begin(), input_participants.end(), lanelet::Participants::VehicleBus) ==
      input_participants.end())
      participants.emplace_back(lanelet::Participants::VehicleBus);
    if (
      std::find(
        input_participants.begin(), input_participants.end(), lanelet::Participants::VehicleCar) ==
      input_participants.end())
      participants.emplace_back(lanelet::Participants::VehicleCar);
    if (
      std::find(
        input_participants.begin(), input_participants.end(),
        lanelet::Participants::VehicleTruck) == input_participants.end())
      participants.emplace_back(lanelet::Participants::VehicleTruck);
  }
  return participants;
}

std::vector<std::string> WMBroadcaster::combineParticipantsToVehicle(
  const std::vector<std::string> & input_participants) const
{
  std::vector<std::string> participants;
  ...
  if (
    ...
    std::find(
      ...
      input_participants.end() &&
    std::find(
      input_participants.begin(), input_participants.end(), lanelet::Participants::VehicleTruck) !=
      input_participants.end()) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "Not making any changes to the participants list");
...
void WMBroadcaster::externalMapMsgCallback(carma_v2x_msgs::msg::MapData::UniquePtr map_msg)
{
  ...
  if (!current_map_ || current_map_->laneletLayer.size() == 0) {
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "Map is not available yet. Skipping MAP msg");
    return;
  }
...
void WMBroadcaster::geofenceCallback(
  carma_v2x_msgs::msg::TrafficControlMessage::UniquePtr geofence_msg)
{
  std::lock_guard<std::mutex> guard(map_mutex_);
  ...
  if (geofence_msg->choice != carma_v2x_msgs::msg::TrafficControlMessage::TCMV01) {
    reason_ss << "Dropping received geofence for unsupported TrafficControl version: "
              << geofence_msg->choice;
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl"), reason_ss.str());
    pubTCMACK(
      geofence_msg->tcm_v01.reqid, geofence_msg->tcm_v01.msgnum,
      static_cast<int>(AcknowledgementStatus::REJECTED), reason_ss.str());
    return;
  }
  ...
  if (checked_geofence_ids_.find(boost::uuids::to_string(id)) != checked_geofence_ids_.end()) {
    reason_ss.str("");
    reason_ss << "Dropping received TrafficControl message with already handled id: "
              << boost::uuids::to_string(id);
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl"), reason_ss.str());
    pubTCMACK(
      geofence_msg->tcm_v01.reqid, geofence_msg->tcm_v01.msgnum,
      static_cast<int>(AcknowledgementStatus::ACKNOWLEDGED), reason_ss.str());
    return;
  }
  ...
  if (
    generated_geofence_reqids_.find(reqid) == generated_geofence_reqids_.end() &&
    reqid.compare("00000000") != 0) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "CARMA_WM_CTRL received a TrafficControlMessage with unknown TrafficControlRequest ID "
      "(reqid): "
        << reqid);
    return;
  }
...
void WMBroadcaster::scheduleGeofence(std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr)
{
  ...
  bool detected_workzone_signal = gf_ptr->msg_.package.label_exists &&
                                  gf_ptr->msg_.package.label.find("SIG_WZ") != std::string::npos;
  ...
  if (
    detected_workzone_signal &&
    msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::MAXSPEED_CHOICE) {
    // duplicate the messages with inverted points to support new lanelets created from workzone
    // as carma-cloud currently does not support geofence points with direction opposite to that of
    // the road
...
void WMBroadcaster::setIntersectionCoordCorrection(
  const std::vector<int64_t> & intersection_ids_for_correction,
  const std::vector<double> & intersection_correction)
{
  if (
    intersection_correction.size() % 2 != 0 ||
    intersection_ids_for_correction.size() != intersection_correction.size() / 2) {
  ...
  for (auto i = 0; i < intersection_correction.size(); i = i + 2) {
    sim_->intersection_coord_correction_[(uint16_t)intersection_ids_for_correction[i / 2]].first =
      intersection_correction[i];  // x
    sim_->intersection_coord_correction_[(uint16_t)intersection_ids_for_correction[i / 2]].second =
      intersection_correction[i + 1];  // y
  }
}
...
void WMBroadcaster::setConfigVehicleId(const std::string & vehicle_id) { vehicle_id_ = vehicle_id; }

void WMBroadcaster::setConfigACKPubTimes(int ack_pub_times) { ack_pub_times_ = ack_pub_times; }
...
std::string WMBroadcaster::getVehicleParticipationType() { return participant_; }
...
// currently only supports geofence message version 1: TrafficControlMessageV01
lanelet::Points3d WMBroadcaster::getPointsInLocalFrame(
  const carma_v2x_msgs::msg::TrafficControlMessageV01 & tcm_v01)
{
  ...
  if (base_map_georef_ == "")
    throw lanelet::InvalidObjectStateError(
      std::string("Base lanelet map has empty proj string loaded as georeference. Therefore, "
                  "WMBroadcaster failed to\n ") +
  ...
  // From the reference the message projection to map projection transformation is used to convert
  // the nodes in the TrafficControlMessage
  std::string projection = tcm_v01.geometry.proj;
  std::string datum = tcm_v01.geometry.datum;
  if (datum.empty()) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "Datum field not populated. Attempting to use WGS84");
    datum = "WGS84";
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "Traffic Control heading provided: "
      << tcm_v01.geometry.heading
      << " System understanding is that this value will not affect the projection and is only "
         "provided for supporting derivative calculations.");

  // Create the resulting projection transformation
  PJ * universal_to_target =
    proj_create_crs_to_crs(PJ_DEFAULT_CTX, universal_frame.c_str(), projection.c_str(), nullptr);
  if (universal_to_target == nullptr) {  // proj_create_crs_to_crs returns 0 when there is an error
                                         // in the projection

    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "Failed to generate projection between geofence and map with error number: "
        << proj_context_errno(PJ_DEFAULT_CTX) << " universal_frame: " << universal_frame
  ...
  PJ * target_to_map =
    proj_create_crs_to_crs(PJ_DEFAULT_CTX, projection.c_str(), base_map_georef_.c_str(), nullptr);

  if (target_to_map == nullptr) {  // proj_create_crs_to_crs returns 0 when there is an error in the
                                   // projection

    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "Failed to generate projection between geofence and map with error number: "
        << proj_context_errno(PJ_DEFAULT_CTX) << " target_to_map: " << target_to_map
        << " base_map_georef_: " << base_map_georef_);
  ...
  PJ_COORD c_init_latlong{
    {tcm_v01.geometry.reflat, tcm_v01.geometry.reflon, tcm_v01.geometry.refelv}};
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "In TCM's frame, initial Point X " << prev_pt.x << " Before conversion: Point Y " << prev_pt.y);
  for (auto pt : tcm_v01.geometry.nodes) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "Before conversion in TCM frame: Point X " << pt.x << " Before conversion: Point Y " << pt.y);
    ...
    gf_pts.push_back(
      lanelet::Point3d{current_map_->pointLayer.uniqueId(), c_out.xyz.x, c_out.xyz.y});
    prev_pt.x += pt.x;
    prev_pt.y += pt.y;

    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "After conversion in Map frame: Point X "
                                             << gf_pts.back().x() << " After conversion: Point Y "
                                             << gf_pts.back().y());
  }
...
lanelet::ConstLaneletOrAreas WMBroadcaster::getAffectedLaneletOrAreas(
  const lanelet::Points3d & gf_pts)
{
  return carma_wm::query::getAffectedLaneletOrAreas(
    gf_pts, current_map_, current_routing_graph_, max_lane_width_);
}

/*!
 * \brief This is a helper function that returns true if the provided regem is marked to be changed
 * by the geofence as there are usually multiple passing control lines are in the lanelet. \param
 * geofence_msg The ROS msg that contains geofence information \param el The LaneletOrArea that
 * houses the regem \param regem The regulatoryElement that needs to be checked NOTE: Currently this
 * function only works on lanelets. It returns true if the regem is not passingControlLine or if the
 * pcl should be changed.
 */
bool WMBroadcaster::shouldChangeControlLine(
  const lanelet::ConstLaneletOrArea & el, const lanelet::RegulatoryElementConstPtr & regem,
  std::shared_ptr<Geofence> gf_ptr) const
{
  // should change if if the regem is not a passing control line or area, which is not supported by
  // this logic
  if (
    regem->attribute(lanelet::AttributeName::Subtype)
        .value()
        .compare(lanelet::PassingControlLine::RuleName) != 0 ||
    !el.isLanelet()) {
    return true;
  }

  lanelet::PassingControlLinePtr pcl = std::dynamic_pointer_cast<lanelet::PassingControlLine>(
    current_map_->regulatoryElementLayer.get(regem->id()));
  // if this geofence's pcl doesn't match with the lanelets current bound side, return false as we
  // shouldn't change
  bool should_change_pcl = false;
  for (auto control_line : pcl->controlLine()) {
    if (
      (control_line.id() == el.lanelet()->leftBound2d().id() && gf_ptr->pcl_affects_left_) ||
...
/*!
  * \brief This is a helper function that returns true if signal in the lanelet should be changed
  according to the records of signalizer intersection manager Used in managing multiple
  signal_groups in a single entry lanelet for example
  * \param el The LaneletOrArea that houses the regem
  * \param regem The regulatoryElement that needs to be checked
  * \param sim The signalized intersection manager that has records regems and corresponding
  lanelets
  * NOTE: Currently this function only works on lanelets. It returns true if the regem is not
  CarmaTrafficSignal or if the signal should be changed.
  */
bool WMBroadcaster::shouldChangeTrafficSignal(
  const lanelet::ConstLaneletOrArea & el, const lanelet::RegulatoryElementConstPtr & regem,
  std::shared_ptr<carma_wm::SignalizedIntersectionManager> sim) const
{
  // should change if the regem is not a CarmaTrafficSignal, which is not supported by this logic
  if (
    regem->attribute(lanelet::AttributeName::Subtype)
        .value()
        .compare(lanelet::CarmaTrafficSignal::RuleName) != 0 ||
    !el.isLanelet() || !sim_) {
    return true;
  }

  lanelet::CarmaTrafficSignalPtr traffic_signal =
    std::dynamic_pointer_cast<lanelet::CarmaTrafficSignal>(
      current_map_->regulatoryElementLayer.get(regem->id()));
  uint8_t signal_id = 0;

  for (auto it = sim->signal_group_to_traffic_light_id_.begin();
       it != sim->signal_group_to_traffic_light_id_.end(); ++it) {
  ...
  if (
    sim->signal_group_to_entry_lanelet_ids_[signal_id].find(el.id()) !=
    sim->signal_group_to_entry_lanelet_ids_[signal_id].end())
...
void WMBroadcaster::addRegulatoryComponent(std::shared_ptr<Geofence> gf_ptr) const
{
  ...
  for (auto el : gf_ptr->affected_parts_) {
    for (auto regem : el.regulatoryElements()) {
      if (
        !shouldChangeControlLine(el, regem, gf_ptr) || !shouldChangeTrafficSignal(el, regem, sim_))
        continue;

      if (
        regem->attribute(lanelet::AttributeName::Subtype).value() ==
        gf_ptr->regulatory_element_->attribute(lanelet::AttributeName::Subtype).value()) {
        lanelet::RegulatoryElementPtr nonconst_regem =
          current_map_->regulatoryElementLayer.get(regem->id());
  ...
  for (auto el : gf_ptr->affected_parts_) {
    ...
    if (gf_ptr->regulatory_element_->id() != lanelet::InvalId) {
      ...
      gf_ptr->update_list_.push_back(std::pair<lanelet::Id, lanelet::RegulatoryElementPtr>(
        el.id(), gf_ptr->regulatory_element_));
    } else {
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"),
...
{
  // First loop is to remove the relation between element and regulatory element that this geofence
  // added initially
  ...
  for (auto pair : gf_ptr->prev_regems_) {
    if (
      pair.second->attribute(lanelet::AttributeName::Subtype).value() ==
      gf_ptr->regulatory_element_->attribute(lanelet::AttributeName::Subtype).value()) {
...
void WMBroadcaster::addGeofence(std::shared_ptr<Geofence> gf_ptr)
{
  ...
  bool detected_workzone_signal = gf_ptr->msg_.package.label_exists &&
                                  gf_ptr->msg_.package.label.find("SIG_WZ") != std::string::npos;
  bool detected_map_msg_signal = gf_ptr->msg_.package.label_exists &&
                                 gf_ptr->msg_.package.label.find("MAP_MSG") != std::string::npos;
  if (
    detected_workzone_signal && gf_ptr->msg_.params.detail.choice !=
                                  carma_v2x_msgs::msg::TrafficControlDetail::MAXSPEED_CHOICE) {
  ...
  for (auto update : updates_to_send) {
    // add marker to rviz
    tcm_marker_array_.markers.push_back(
      composeTCMMarkerVisualizer(update->gf_pts));  // create visualizer in rviz

    if (update->affected_parts_.empty()) continue;
    ...
    // If the geofence invalidates the route graph then recompute the routing graph now that the map
    // has been updated
    if (update->invalidate_route_) {
      ...
      current_routing_graph_ =
        lanelet::routing::RoutingGraph::build(*current_map_, *traffic_rules_car);
      ...
      RCLCPP_INFO_STREAM(
        rclcpp::get_logger("carma_wm_ctrl"), "Done creating routing graph message");
    }

    // Publish
    auto send_data = std::make_shared<carma_wm::TrafficControl>(carma_wm::TrafficControl(
      update->id_, update->update_list_, update->remove_list_, update->lanelet_additions_));
...
void WMBroadcaster::removeGeofence(std::shared_ptr<Geofence> gf_ptr)
{
  ...
  // Process the geofence object to populate update remove lists
  if (gf_ptr->affected_parts_.empty()) return;
  ...
  auto send_data = std::make_shared<carma_wm::TrafficControl>(
    carma_wm::TrafficControl(gf_ptr->id_, gf_ptr->update_list_, gf_ptr->remove_list_, {}));

  if (gf_ptr->invalidate_route_) {  // If a geofence initially invalidated the route it stands to
                                    // reason its removal should as well
    ...
    // Populate routing graph structure
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "Creating routing graph message for geofence removal");
...
carma_planning_msgs::msg::Route WMBroadcaster::getRoute() { return current_route; }
...
carma_v2x_msgs::msg::TrafficControlRequest WMBroadcaster::controlRequestFromRoute(
  const carma_planning_msgs::msg::Route & route_msg,
  std::shared_ptr<j2735_v2x_msgs::msg::Id64b> req_id_for_testing)
{
  ...
  if (path.size() == 0)
    throw lanelet::InvalidObjectStateError(std::string("No lanelets available in path."));
  ...
  while (path.size() != 0)  // Continue until there are no more lanelet elements in path
  {
    ...
    pathBox.push_back(lanelet::geometry::boundingBox2d(
      llt.back()));  // Create a bounding box of the added lanelet and add it to the vector
  ...
  if (target_frame.empty()) {
    ...
    throw lanelet::InvalidObjectStateError(
      std::string("Base georeference map may not be loaded to the WMBroadcaster"));
  }
  ...
  lanelet::GPSPoint gpsRoute = local_projector.reverse(
    localPoint);  // If the appropriate library is included, the reverse() function can be used to
                  // convert from local xyz to lat/lon

  // Create a local transverse mercator frame at the minimum point to allow us to get east,north
  // oriented bounds
  std::string local_tmerc_enu_proj =
    "+proj=tmerc +datum=WGS84 +h_0=0 +lat_0=" + std::to_string(gpsRoute.lat) +
    " +lon_0=" + std::to_string(gpsRoute.lon);

  // Create transform from map frame to local transform mercator frame at bounds min point
  PJ * tmerc_proj = proj_create_crs_to_crs(
    PJ_DEFAULT_CTX, target_frame.c_str(), local_tmerc_enu_proj.c_str(), nullptr);

  if (tmerc_proj == nullptr) {  // proj_create_crs_to_crs returns 0 when there is an error in the
                                // projection

    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "Failed to generate projection between request bounds frame and map with error number: "
        << proj_context_errno(PJ_DEFAULT_CTX) << " MapProjection: " << target_frame
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "Before conversion: Top Left: (" << minX << ", " << maxY << ")");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "Before conversion: Top Right: (" << maxX << ", " << maxY << ")");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "Before conversion: Bottom Left: (" << minX << ", " << minY << ")");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "Before conversion: Bottom Right: (" << maxX << ", " << minY << ")");
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "After conversion: MinPoint ( " << pj_min_tmerc.xyz.x << ", " << pj_min_tmerc.xyz.y << " )");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"),
    "After conversion: MaxPoint ( " << pj_max_tmerc.xyz.x << ", " << pj_max_tmerc.xyz.y << " )");

  carma_v2x_msgs::msg::TrafficControlRequest
    cR; /*Fill the latitude value in message cB with the value of lat */
  carma_v2x_msgs::msg::TrafficControlBounds
    cB; /*Fill the longitude value in message cB with the value of lon*/
  ...
  cB.offsets[0].deltax =
    pj_max_tmerc.xyz.x -
    pj_min_tmerc.xyz
      .x;  // Points in clockwise order min,min (lat,lon point) -> max,min -> max,max -> min,max
  ...
  tcr_polygon_ =
    composeTCRStatus(localPoint, cB, local_projector);  // TCR polygon can be visualized in UI

  cB.oldest = rclcpp::Time(
    0.0, 0.0, scheduler_.getClockType());  // TODO this needs to be set to 0 or an older value as
                                           // otherwise this will filter out all controls
  ...
  // take half as string
  std::string reqid = boost::uuids::to_string(uuid_id).substr(0, 8);
  std::string req_id_test =
    "12345678";  // TODO this is an extremely risky way of performing a unit test. This method needs
                 // to be refactored so that unit tests cannot side affect actual implementations
...
carma_v2x_msgs::msg::TrafficControlRequestPolygon WMBroadcaster::composeTCRStatus(
  const lanelet::BasicPoint3d & localPoint, const carma_v2x_msgs::msg::TrafficControlBounds & cB,
  const lanelet::projection::LocalFrameProjector & local_projector)
{
  ...
  while (i < 3)  // three offsets; offsets.size() doesn't return accurate size
  {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "TCR Vertex Lat: " << std::to_string(gps_vertex.lat));
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "TCR Vertex Lon: " << std::to_string(gps_vertex.lon));
...
visualization_msgs::msg::Marker WMBroadcaster::composeTCMMarkerVisualizer(
...
double WMBroadcaster::distToNearestActiveGeofence(const lanelet::BasicPoint2d & curr_pos)
{
  ...
  if (!current_map_ || current_map_->laneletLayer.size() == 0) {
    throw lanelet::InvalidObjectStateError(
      std::string("Lanelet map (current_map_) is not loaded to the WMBroadcaster"));
  }
  ...
  auto curr_lanelet =
    lanelet::geometry::findNearest(current_map_->laneletLayer, curr_pos, 1)[0].second;
  ...
  for (auto id : active_geofence_on_route) {
    carma_wm::TrackPos tp =
      carma_wm::geometry::trackPos(current_map_->laneletLayer.get(id), curr_pos);
...
bool WMBroadcaster::convertLightIdToInterGroupId(
  unsigned & intersection_id, unsigned & group_id, const lanelet::Id & lanelet_id)
{
  for (auto it = traffic_light_id_lookup_.begin(); it != traffic_light_id_lookup_.end(); ++it) {
    // Reverse of the logic for generating the lanelet_id. Reference function generate32BitId(const
    // std::string& label)
...
void WMBroadcaster::publishLightId()
{
  ...
  for (auto id : current_route.route_path_lanelet_ids) {
    ...
    if (!traffic_lights.empty()) {
      ...
      convert_success =
        convertLightIdToInterGroupId(intersection_id, group_id, traffic_lights.front()->id());
    }

    if (!convert_success) continue;

    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"),
      "Found Traffic Light with Intersection id: " << intersection_id << " Group id:" << group_id);
    bool id_exists = false;
    for (int idx = 0; idx < upcoming_intersection_ids_.data.size(); idx + 2) {
      if (
        upcoming_intersection_ids_.data[idx] == intersection_id &&
        upcoming_intersection_ids_.data[idx + 1] == group_id)  // check if already there
      {
        id_exists = true;
        break;
      }
...
carma_perception_msgs::msg::CheckActiveGeofence WMBroadcaster::checkActiveGeofenceLogic(
  const geometry_msgs::msg::PoseStamped & current_pos)
{
  if (!current_map_ || current_map_->laneletLayer.size() == 0) {
    throw lanelet::InvalidObjectStateError(
      std::string("Lanelet map 'current_map_' is not loaded to the WMBroadcaster"));
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("carma_wm_ctrl"), "Active geofence llt ids are loaded to the WMBroadcaster");
  ...
  if (boost::geometry::within(curr_pos, current_llt.polygon2d().basicPolygon())) {
    ...
    for (auto id : active_geofence_llt_ids_) {
      if (id == current_llt.id()) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("carma_wm_ctrl"),
          "Vehicle is on Lanelet " << current_llt.id() << ", which has an active geofence");
        ...
        for (auto regem : current_llt.regulatoryElements()) {
          ...
          if (
            regem->attribute(lanelet::AttributeName::Subtype)
              .value()
              .compare(lanelet::DigitalSpeedLimit::RuleName) == 0) {
            lanelet::DigitalSpeedLimitPtr speed =
              std::dynamic_pointer_cast<lanelet::DigitalSpeedLimit>(
                current_map_->regulatoryElementLayer.get(regem->id()));
          ...
          // Assign active geofence fields based on the minimum gap associated with this lanelet (if
          // it exists)
          if (
            regem->attribute(lanelet::AttributeName::Subtype)
              .value()
              .compare(lanelet::DigitalMinimumGap::RuleName) == 0) {
            lanelet::DigitalMinimumGapPtr min_gap =
              std::dynamic_pointer_cast<lanelet::DigitalMinimumGap>(
                current_map_->regulatoryElementLayer.get(regem->id()));
          ...
          // Assign active geofence fields based on whether the current lane is closed or is
          // immediately adjacent to a closed lane
          if (
            regem->attribute(lanelet::AttributeName::Subtype)
              .value()
              .compare(lanelet::RegionAccessRule::RuleName) == 0) {
            lanelet::RegionAccessRulePtr accessRuleReg =
              std::dynamic_pointer_cast<lanelet::RegionAccessRule>(
                current_map_->regulatoryElementLayer.get(regem->id()));

            // Update the 'type' and 'reason' for this active geofence if the vehicle is in a closed
            // lane
            if (
              !accessRuleReg->accessable(lanelet::Participants::VehicleCar) ||
              !accessRuleReg->accessable(lanelet::Participants::VehicleTruck)) {
              RCLCPP_DEBUG_STREAM(
                rclcpp::get_logger("carma_wm_ctrl"), "Active geofence is a closed lane.");
              RCLCPP_DEBUG_STREAM(
                rclcpp::get_logger("carma_wm_ctrl"),
            ...
            // Otherwise, update the 'type' and 'reason' for this active geofence if the vehicle is
            // in a lane immediately adjacent to a closed lane with the same travel direction
            else {
              // Obtain all same-direction lanes sharing the right lane boundary (will include the
              // current lanelet)
              auto right_boundary_lanelets =
                ...
                for (auto lanelet : right_boundary_lanelets) {
                  ...
                  if (lanelet.id() != current_llt.id()) {
                    for (auto rightRegem : lanelet.regulatoryElements()) {
                      if (
                        rightRegem->attribute(lanelet::AttributeName::Subtype)
                          .value()
                          .compare(lanelet::RegionAccessRule::RuleName) == 0) {
                        lanelet::RegionAccessRulePtr rightAccessRuleReg =
                          std::dynamic_pointer_cast<lanelet::RegionAccessRule>(
                            current_map_->regulatoryElementLayer.get(rightRegem->id()));
                        if (
                          !rightAccessRuleReg->accessable(lanelet::Participants::VehicleCar) ||
                          !rightAccessRuleReg->accessable(lanelet::Participants::VehicleTruck)) {
                          RCLCPP_DEBUG_STREAM(
                            rclcpp::get_logger("carma_wm_ctrl"),
                            "Right adjacent Lanelet " << lanelet.id() << " is CLOSED");
                          RCLCPP_DEBUG_STREAM(
                            rclcpp::get_logger("carma_wm_ctrl"),
                          ...
                          outgoing_geof.type =
                            carma_perception_msgs::msg::CheckActiveGeofence::LANE_CLOSED;
                        }
                      }
                    }
                  }
                ...
                for (auto lanelet : left_boundary_lanelets) {
                  ...
                  if (lanelet.id() != current_llt.id()) {
                    for (auto leftRegem : lanelet.regulatoryElements()) {
                      if (
                        leftRegem->attribute(lanelet::AttributeName::Subtype)
                          .value()
                          .compare(lanelet::RegionAccessRule::RuleName) == 0) {
                        lanelet::RegionAccessRulePtr leftAccessRuleReg =
                          std::dynamic_pointer_cast<lanelet::RegionAccessRule>(
                            current_map_->regulatoryElementLayer.get(leftRegem->id()));
                        if (
                          !leftAccessRuleReg->accessable(lanelet::Participants::VehicleCar) ||
                          !leftAccessRuleReg->accessable(lanelet::Participants::VehicleTruck)) {
                          RCLCPP_DEBUG_STREAM(
                            rclcpp::get_logger("carma_wm_ctrl"),
                            "Left adjacent Lanelet " << lanelet.id() << " is CLOSED");
                          RCLCPP_DEBUG_STREAM(
                            rclcpp::get_logger("carma_wm_ctrl"),
                            "Assigning LANE_CLOSED type to active geofence");
                          RCLCPP_DEBUG_STREAM(
...
lanelet::LineString3d WMBroadcaster::createLinearInterpolatingLinestring(
  const lanelet::Point3d & front_pt, const lanelet::Point3d & back_pt, double increment_distance)
{
  ...
  while (sum < distance) {
    points.push_back(lanelet::Point3d(
      lanelet::utils::getId(), front_pt.x() + sum * cos, front_pt.y() + sum * sin, 0.0));
...
lanelet::Lanelet WMBroadcaster::createLinearInterpolatingLanelet(
  const lanelet::Point3d & left_front_pt, const lanelet::Point3d & right_front_pt,
  const lanelet::Point3d & left_back_pt, const lanelet::Point3d & right_back_pt,
  double increment_distance)
{
  return lanelet::Lanelet(
    lanelet::utils::getId(),
    createLinearInterpolatingLinestring(left_front_pt, left_back_pt, increment_distance),
    createLinearInterpolatingLinestring(right_front_pt, right_back_pt, increment_distance));
}
...
void WMBroadcaster::updateUpcomingSGIntersectionIds()
{
  ...
  if (isLightFound && sim_) {
    for (auto itr = sim_->signal_group_to_traffic_light_id_.begin();
         itr != sim_->signal_group_to_traffic_light_id_.end(); itr++) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("carma_wm_ctrl"), "NO matching Traffic lights along the route");
  }  // END Traffic signals
  ...
  if (intersections.empty()) {
    ...
    if (intersection_id != lanelet::InvalId) {
      for (auto itr = sim_->intersection_id_to_regem_id_.begin();
           itr != sim_->intersection_id_to_regem_id_.end(); itr++) {
...
void WMBroadcaster::pubTCMACK(
  j2735_v2x_msgs::msg::Id64b tcm_req_id, uint16_t msgnum, int ack_status,
  const std::string & ack_reason)
{
  ...
  ss.str("");
  ss << "traffic_control_id:" << tcmv01_req_id_hex << ", msgnum:" << msgnum
     << ", acknowledgement:" << ack_status << ", reason:" << ack_reason;
