...
cd ..
docker build --no-cache -t $USERNAME/$IMAGE:$FULL_VERSION_STRING --build-arg SSH_PRIVATE_KEY="$(cat ~/.ssh/id_rsa)" --build-arg EXTRA_PACKAGES=git@github.com:fhwa-saxton/CARMASensitive.git .
