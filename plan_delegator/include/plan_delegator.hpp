                     ...
                     : ((mvr).type == carma_planning_msgs::msg::Maneuver::STOP_AND_WAIT           \
                          ...
                          : ((mvr).type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING     \
                               ...
                               : throw std::invalid_argument(                                     \
                                   "ADJUST_MANEUVER_PROPERTY (property) called on maneuver with " \
                                   "invalid type id " +                                           \
...
struct Config
{
  ...
  double duration_to_signal_before_lane_change =
    2.5;  // (Seconds) If an upcoming lane change will begin in under this time threshold, a turn
          // signal activation command will be published.
  int tactical_plugin_service_call_timeout =
    100;  // (Milliseconds) The maximum duration that Plan Delegator will wait after calling a
          // tactical plugin's trajectory planning service; if trajectory generation takes longer
          // than this, then planning will immediately end for the current trajectory planning
          // iteration.
  ...
  friend std::ostream & operator<<(std::ostream & output, const Config & c)
  {
    output << "PlanDelegator::Config { " << std::endl
           ...
           << "duration_to_signal_before_lane_change: " << c.duration_to_signal_before_lane_change
           << std::endl
           << "}" << std::endl;
    return output;
  }
...
struct LaneChangeInformation
{
  ...
  bool is_right_lane_change;  // Flag to indicate whether lane change is a right lane change; false
                              // if it is a left lane change
};
...
public:
   ...
   */
  carma_ros2_utils::ClientPtr<carma_planning_msgs::srv::PlanTrajectory> getPlannerClientByName(
   ...
   */
  std::shared_ptr<carma_planning_msgs::srv::PlanTrajectory::Request> composePlanTrajectoryRequest(
  ...
  /**
   * \brief Update the starting downtrack, ending downtrack, and maneuver-specific Lanelet ID
   * parameters associated with a given maneuver. These updates are required since the starting and
   * ending downtrack values of each maneuver are shifted based on the distance between the
   * base_link frame and the vehicle_front frame. \param maneuver The maneuver to be updated.
   */
...
protected:
  ...
  std::unordered_map<
    std::string, carma_ros2_utils::ClientPtr<carma_planning_msgs::srv::PlanTrajectory>>
...
private:
  ...
  carma_ros2_utils::PubPtr<carma_planning_msgs::msg::UpcomingLaneChangeStatus>
  ...
  // Object to store information regarding the current active lane change in latest_maneuver_plan_;
  // empty if first maneuver in latest_maneuver_plan_ is not a lane change
  ...
  /**
   * \brief Function for generating a LaneChangeInformation object from a provided lane change
   * maneuver. \param lane_change_maneuver The lane change maneuver that a LaneChangeInformation
   * object shall be generated from. \return A LaneChangeInformation object containing information
   * on the provided lane change maneuver.
   */
  LaneChangeInformation getLaneChangeInformation(
    const carma_planning_msgs::msg::Maneuver & lane_change_maneuver);

  /**
   * \brief Function for processing an optional LaneChangeInformation object pertaining to an
   * upcoming lane change. If not empty, an UpcomingLaneChangeStatus message is created and
   * published based on the contents of the LaneChangeInformation. The published
   * UpcomingLaneChangeStatus message is stored in upcoming_lane_change_status_.
   * \param upcoming_lane_change_information An optional LaneChangeInformation object. Empty if no
   * upcoming lane change exists.
   */
  void publishUpcomingLaneChangeStatus(
    const boost::optional<LaneChangeInformation> & upcoming_lane_change_information);

  /**
   * \brief Function for processing an optional LaneChangeInformation object pertaining to the
   * currently-occurring lane change and an UpcomingLaneChangeStatus message. If the optional object
   * pertaining to the currently-occurring lane change is not empty, then a turn signal command is
   * published based on the current lane change direction. Otherwise, a turn signal command in the
   * direction of the UpcomingLaneChangeStatus message is published if the vehicle is estimated to
   * begin that lane change in under the time threshold defined by
   * config_.duration_to_signal_before_lane_change. The published TurnSignalComand message is stored
   * in latest_turn_signal_command_. \param current_lane_change_information An optional
   * LaneChangeInformation object pertaining to the current lane change. Empty if vehicle is not
   * currently changing lanes. \param upcoming_lane_change_status An UpcomingLaneChangeStatus
   * message containing the lane change direction of an upcoming lane change, along with the
   * downtrack distance to that lane change.
   */
