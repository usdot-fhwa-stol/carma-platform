...
namespace yield_plugin
{
  ...
  std::vector<lanelet::BasicPoint2d> YieldPlugin::convert_eceftrajectory_to_mappoints(const carma_planning_msgs::msg::Trajectory& ecef_trajectory) const
  {
  ...
  carma_v2x_msgs::msg::MobilityResponse YieldPlugin::compose_mobility_response(const std::string& resp_recipient_id, const std::string& req_plan_id, bool response) const
  {
    ...
    out_mobility_response.m_header.timestamp = rclcpp::Time::now().toSec()*1000;
  ...
  void YieldPlugin::mobilityrequest_cb(const carma_v2x_msgs::msg::MobilityRequestConstPtr& msg)
  {
    ...
    {
      if (!map_projector_) {
        RCLCPP_ERROR_STREAM(get_logger(),"Cannot process mobility request as map projection is not yet set!");
        return;
      }
      ...
      {
        RCLCPP_DEBUG_STREAM(get_logger(),"Cooperative Lane Change Request Received");
        ...
        {
          RCLCPP_DEBUG_STREAM(get_logger(),"CLC Request correctly received");
        }
        ...
        RCLCPP_DEBUG_STREAM(get_logger(),"received urgency: " << clc_urgency_);
        ...
        RCLCPP_DEBUG_STREAM(get_logger(),"req_traj_speed" << req_traj_speed);
        RCLCPP_DEBUG_STREAM(get_logger(),"start_lanelet_id" << start_lanelet_id);
        RCLCPP_DEBUG_STREAM(get_logger(),"end_lanelet_id" << end_lanelet_id);
        ...
        double current_time_sec = rclcpp::Time::now().toSec();
        ...
        if (req_expiration_sec - current_time_sec >= config_.tpmin && cooperative_request_acceptable_)
        {
          ...
          RCLCPP_DEBUG_STREAM(get_logger(),"CLC accepted"); 
        }
        else
        {
          ...
          RCLCPP_DEBUG_STREAM(get_logger(),"CLC rejected"); 
        }
        ...
        RCLCPP_DEBUG_STREAM(get_logger(),"response sent"); 
      }
    }
  ...
  void YieldPlugin::set_incoming_request_info(std::vector <lanelet::BasicPoint2d> req_trajectory, double req_speed, double req_planning_time, double req_timestamp)
  {
    ...
    RCLCPP_DEBUG_STREAM(get_logger(),"req_target_plan_time_" << req_target_plan_time_); 
    req_timestamp_ = req_timestamp;
  }


  void YieldPlugin::bsm_cb(const carma_v2x_msgs::msg::BSMConstPtr& msg)
  {
  ...
  carma_planning_msgs::srv::PlanTrajectory::Response::SharedPtr resp)
{
    ...
    {
      RCLCPP_DEBUG_STREAM(get_logger(),"Only consider high urgency clc");
      if (timesteps_since_last_req_ < config_.acceptable_passed_timesteps)
      {
        RCLCPP_DEBUG_STREAM(get_logger(),"Yield for CLC. We haven't received an updated negotiation this timestep");
      ...
      {
        RCLCPP_DEBUG_STREAM(get_logger(),"unreliable CLC communication, switching to object avoidance");
        yield_trajectory = update_traj_for_object(original_trajectory, req.vehicle_state.longitudinal_vel); // Compute the trajectory
      }    
    }
    else
    {
      RCLCPP_DEBUG_STREAM(get_logger(),"Yield for object avoidance");
      yield_trajectory = update_traj_for_object(original_trajectory, req.vehicle_state.longitudinal_vel); // Compute the trajectory
    }
    yield_trajectory.header.frame_id = "map";
    yield_trajectory.header.stamp = rclcpp::Time::now();
  ...
  carma_planning_msgs::msg::TrajectoryPlan YieldPlugin::update_traj_for_cooperative_behavior(const carma_planning_msgs::msg::TrajectoryPlan& original_tp, double current_speed)
  {
    ...
    if (!intersection_points.empty())
    {
      ...
      RCLCPP_DEBUG_STREAM(get_logger(),"digital_gap: " << digital_gap);
      goal_pos = sqrt(dx*dx + dy*dy) - config_.x_gap;
      RCLCPP_DEBUG_STREAM(get_logger(),"Goal position (goal_pos): " << goal_pos);
      double collision_time = req_timestamp_ + (intersection_points[0].first * ecef_traj_timestep_) - config_.safety_collision_time_gap;
      RCLCPP_DEBUG_STREAM(get_logger(),"req time stamp: " << req_timestamp_);
      RCLCPP_DEBUG_STREAM(get_logger(),"Collision time: " << collision_time);
      RCLCPP_DEBUG_STREAM(get_logger(),"intersection num: " << intersection_points[0].first);
      RCLCPP_DEBUG_STREAM(get_logger(),"Planning time: " << planning_time);
      ...
      RCLCPP_DEBUG_STREAM(get_logger(),"goal_velocity: " << goal_velocity);
      RCLCPP_DEBUG_STREAM(get_logger(),"incoming_trajectory_speed: " << incoming_trajectory_speed);
      ...
      else
      {
        ...
        RCLCPP_DEBUG_STREAM(get_logger(),"The incoming requested trajectory is rejected, due to insufficient gap");
    ...
    {
      cooperative_request_acceptable_ = true;
      RCLCPP_DEBUG_STREAM(get_logger(),"The incoming requested trajectory does not overlap with host vehicle's trajectory");
      cooperative_trajectory = original_tp;
    }
  ...
  carma_planning_msgs::msg::TrajectoryPlan YieldPlugin::generate_JMT_trajectory(const carma_planning_msgs::msg::TrajectoryPlan& original_tp, double initial_pos, double goal_pos, double initial_velocity, double goal_velocity, double planning_time)                                                         
  {
    ...
    RCLCPP_DEBUG_STREAM(get_logger(),"original_max_speed" << original_max_speed);
    ...
    for(size_t i = 1; i < original_tp.trajectory_points.size(); i++ )
    {
      ...
      if (current_speed >= config_.max_stop_speed)
      {
        ...
        jmt_tpp.target_time = jmt_trajectory_points[i-1].target_time + rclcpp::Duration(dt);
        jmt_trajectory_points.push_back(jmt_tpp);
      }
      else
      {
        RCLCPP_DEBUG_STREAM(get_logger(),"target speed is zero");
        // if speed is zero, the vehicle will stay in previous location.
        jmt_tpp = jmt_trajectory_points[i-1];
        jmt_tpp.target_time = jmt_trajectory_points[0].target_time + rclcpp::Duration(traj_target_time);
        jmt_trajectory_points.push_back(jmt_tpp);
      }
  ...
  carma_planning_msgs::msg::TrajectoryPlan YieldPlugin::update_traj_for_object(const carma_planning_msgs::msg::TrajectoryPlan& original_tp, double initial_velocity) 
  {
    ...
    RCLCPP_DEBUG_STREAM(get_logger(),"vehicle_downtrack");
    RCLCPP_DEBUG_STREAM(get_logger(),vehicle_downtrack);
    ...
    for (auto i : rwol2.roadway_obstacles){
      ...
      RCLCPP_DEBUG_STREAM(get_logger(),"object_down_track");
      RCLCPP_DEBUG_STREAM(get_logger(),object_down_track);
      
      double dist_to_object = object_down_track - vehicle_downtrack;
      RCLCPP_DEBUG_STREAM(get_logger(),"object_down_track - vehicle_downtrack");
      RCLCPP_DEBUG_STREAM(get_logger(),dist_to_object);
      
      RCLCPP_DEBUG_STREAM(get_logger(),"i.object.velocity.twist.linear.x");
      RCLCPP_DEBUG_STREAM(get_logger(),i.object.velocity.twist.linear.x);

      if(current_velocity.linear.x > 0.0) {
          RCLCPP_DEBUG_STREAM(get_logger(),"(object_down_downtrack - vehicle_downtrack)/current_velocity.linear.x");
          RCLCPP_DEBUG_STREAM(get_logger(),dist_to_object/current_velocity.linear.x);
    ...
    RCLCPP_DEBUG_STREAM(get_logger(),"Roadway Object List (rwol) size: " << rwol.size());

    // correct the input types
    if(!rwol_collision.empty())
    {
      RCLCPP_WARN_STREAM(get_logger(),"Collision Detected!");
      ...
      if (goal_velocity <= config_.min_obstacle_speed){
        RCLCPP_WARN_STREAM(get_logger(),"The obstacle is not moving");
      }
      ...
      RCLCPP_DEBUG_STREAM(get_logger(),"Object avoidance planning time: " << tp);
    ...
    RCLCPP_DEBUG_STREAM(get_logger(),"No collision detection, so trajectory not modified.");
    return original_tp;
  }
  ...
  double YieldPlugin::check_traj_for_digital_min_gap(const carma_planning_msgs::msg::TrajectoryPlan& original_tp) const
  {
    ...
    for (size_t i = 0; i < original_tp.trajectory_points.size(); i++)
    {
      ...
      if (llts.empty())
      {
        RCLCPP_WARN_STREAM(get_logger(),"Trajectory point: " << original_tp.trajectory_points[i]);
      ...
      if (!digital_min_gap.empty()) 
      {
        ...
        RCLCPP_DEBUG_STREAM(get_logger(),"Digital Gap found with value: " << digital_gap);
