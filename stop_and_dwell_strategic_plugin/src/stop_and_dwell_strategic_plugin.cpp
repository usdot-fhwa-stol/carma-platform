...
double getManeuverEndSpeed(const carma_planning_msgs::msg::Maneuver & mvr)
{
  switch (mvr.type) {
    ...
    default:
      RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("stop_and_dwell_strategic_plugin"),
...
: carma_guidance_plugins::StrategicPlugin(options), config_(StopAndDwellStrategicPluginConfig())
{
  ...
  config_.bus_line_exit_zone_length =
    declare_parameter<double>("bus_line_exit_zone_length", config_.bus_line_exit_zone_length);
  ...
  config_.activation_distance =
    declare_parameter<double>("activation_distance", config_.activation_distance);
...
carma_ros2_utils::CallbackReturn StopAndDwellStrategicPlugin::on_configure_plugin()
{
  ...
  add_on_set_parameters_callback(
    std::bind(&StopAndDwellStrategicPlugin::parameter_update_callback, this, std_ph::_1));

  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("stop_and_dwell_strategic_plugin"), "Done loading parameters: " << config_);

  // Current Pose Subscriber
  current_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "current_pose", 1, std::bind(&StopAndDwellStrategicPlugin::currentPoseCb, this, std_ph::_1));
...
rcl_interfaces::msg::SetParametersResult StopAndDwellStrategicPlugin::parameter_update_callback(
...
VehicleState StopAndDwellStrategicPlugin::extractInitialState(
  const carma_planning_msgs::srv::PlanManeuvers::Request & req) const
{
  VehicleState state;
  if (!req.prior_plan.maneuvers.empty()) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("stop_and_dwell_strategic_plugin"), "Provided with initial plan...");
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("stop_and_dwell_strategic_plugin"), "No initial plan provided...");
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_and_dwell_strategic_plugin"), "state.downtrack : " << state.downtrack);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_and_dwell_strategic_plugin"), "state.speed: " << state.speed);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_and_dwell_strategic_plugin"), "state.lane_id: " << state.lane_id);

  return state;
}

void StopAndDwellStrategicPlugin::guidance_state_cb(
...
std::vector<lanelet::ConstLanelet> StopAndDwellStrategicPlugin::getLaneletsBetweenWithException(
  ...
  carma_planning_msgs::srv::PlanManeuvers::Request::SharedPtr req) const
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name_), "extractInitialState >>>> state.speed: " << state.speed);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name_), "extractInitialState >>>> state.lane_id: " << state.lane_id);
  ...
  carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp)
{
  ...
  if (!wm_->getRoute()) {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger(logger_name_), "Could not plan maneuvers as route was not available");
    return;
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name_), "Found bus stops of size: " << bus_stop_list.size());
  ...
  double bus_stop_downtrack_ =
    wm_->routeTrackPos(nearest_bus_stop->stopAndWaitLine().front().front().basicPoint2d())
      .downtrack;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name_), "bus_stop_downtrack_ : " << bus_stop_downtrack_);
  ...
  if (distance_remaining_to_bus_stop < -config_.bus_line_exit_zone_length) {
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger(logger_name_), "Already passed bus stop, sending empty maneuvers");
    return;
  }
  ...
  if (
    ...
    fabs(bus_stop_downtrack_ - current_state.downtrack) < config_.stop_line_buffer) {
    if (first_stop_) {
    ...
    if (time_to_move_ <= now()) {
      std::vector<lanelet::ConstLanelet> crossed_lanelets =
        getLaneletsBetweenWithException(current_state.downtrack, bus_stop_downtrack_, true, true);
      ...
      resp->new_plan.maneuvers.push_back(composeStopAndWaitManeuverMessage(
        current_state.downtrack, bus_stop_downtrack_, current_state.speed, starting_lane_id,
        ending_lane_id, max_comfort_decel_norm_, now(),
        now() + rclcpp::Duration::from_nanoseconds(config_.min_maneuver_planning_period * 1e9)));
    } else {
      double maneuver_end_distance = bus_stop_downtrack_ + config_.bus_line_exit_zone_length;
      std::vector<lanelet::ConstLanelet> crossed_lanelets =
        getLaneletsBetweenWithException(current_state.downtrack, maneuver_end_distance, true, true);
      ...
      resp->new_plan.maneuvers.push_back(composeLaneFollowingManeuverMessage(
        current_state.downtrack, maneuver_end_distance, current_state.speed, speed_limit_, now(),
        config_.min_maneuver_planning_period, lane_ids));
    }
    ...
    if (current_state.downtrack >= (bus_stop_downtrack_ - desired_distance_to_stop)) {
      ...
      resp->new_plan.maneuvers.push_back(composeStopAndWaitManeuverMessage(
        current_state.downtrack, bus_stop_downtrack_, current_state.speed, starting_lane_id,
        ending_lane_id, max_comfort_decel_norm_, start_time, end_time));
    } else {
      double time_to_stop =
        (distance_remaining_to_bus_stop - desired_distance_to_stop) / speed_limit_;
      rclcpp::Time timestamp_to_stop =
        now() + rclcpp::Duration::from_nanoseconds(time_to_stop * 1e9);
      std::vector<lanelet::ConstLanelet> crossed_lanelets = getLaneletsBetweenWithException(
        current_state.downtrack, (bus_stop_downtrack_ - desired_distance_to_stop), true, true);
      std::vector<lanelet::ConstLanelet> crossed_lanelets_stop = getLaneletsBetweenWithException(
        (bus_stop_downtrack_ - desired_distance_to_stop), bus_stop_downtrack_, true, true);
      std::vector<lanelet::Id> lane_ids =
        lanelet::utils::transform(crossed_lanelets, [](const auto & ll) { return ll.id(); });
      ...
      resp->new_plan.maneuvers.push_back(composeStopAndWaitManeuverMessage(
        (bus_stop_downtrack_ - desired_distance_to_stop), bus_stop_downtrack_, speed_limit_,
        starting_lane_id, ending_lane_id, max_comfort_decel_norm_, timestamp_to_stop,
        (timestamp_to_stop +
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(logger_name_),
    "ExecutionTime stop_and_dwell_strategic_plugin: "
      << std::chrono::duration<double>(execution_duration).count());
  return;
}

carma_planning_msgs::msg::Maneuver StopAndDwellStrategicPlugin::composeLaneFollowingManeuverMessage(
...
carma_planning_msgs::msg::Maneuver StopAndDwellStrategicPlugin::composeStopAndWaitManeuverMessage(
...
bool StopAndDwellStrategicPlugin::get_availability() { return true; }

std::string StopAndDwellStrategicPlugin::get_version_id() { return "v1.0"; }
