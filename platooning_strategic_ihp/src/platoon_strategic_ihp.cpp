...
PlatoonStrategicIHPPlugin::PlatoonStrategicIHPPlugin(
  carma_wm::WorldModelConstPtr wm, PlatoonPluginConfig config,
  MobilityResponseCB mobility_response_publisher, MobilityRequestCB mobility_request_publisher,
  MobilityOperationCB mobility_operation_publisher, PlatooningInfoCB platooning_info_publisher,
  std::shared_ptr<carma_ros2_utils::timers::TimerFactory> timer_factory)
: mobility_request_publisher_(mobility_request_publisher),
  mobility_response_publisher_(mobility_response_publisher),
  mobility_operation_publisher_(mobility_operation_publisher),
  platooning_info_publisher_(platooning_info_publisher),
  wm_(wm),
  config_(config),
...
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Top of PlatoonStrategicIHP ctor.");
  std::string hostStaticId = config_.vehicleID;  // static ID for this vehicle
  pm_.HostMobilityId = hostStaticId;
  // construct platoon member for host vehicle as the first element in the vector, since it starts
  // life as a solo vehicle
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
  ...
  geometry_msgs::msg::PoseStamped pose_msg)
{
  ...
  lanelet::BasicPoint3d ecef_point =
  ...
  // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("platooning_strategic_ihp"), "location.ecef_x: " <<
  // location.ecef_x); RCLCPP_DEBUG_STREAM(rclcpp::get_logger("platooning_strategic_ihp"),
  // "location.ecef_y: " << location.ecef_y);
  // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("platooning_strategic_ihp"), "location.ecef_z: " <<
  // location.ecef_z);
...
PlatoonManager PlatoonStrategicIHPPlugin::getHostPM() { return pm_; }
...
void PlatoonStrategicIHPPlugin::pose_cb(const geometry_msgs::msg::PoseStamped::UniquePtr msg)
{
  ...
  if (pm_.current_platoon_state != PlatoonState::STANDBY) {
    ...
    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("platooning_strategic_ihp"), "current_downtrack_ = "
    // << current_downtrack_ << ", current_crosstrack_ = " << current_crosstrack_);
...
int PlatoonStrategicIHPPlugin::findLaneletIndexFromPath(
  int target_id, lanelet::routing::LaneletPath & path)
{
...
double PlatoonStrategicIHPPlugin::findSpeedLimit(const lanelet::ConstLanelet & llt)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "target speed (limit) " << target_speed);
...
double PlatoonStrategicIHPPlugin::findLaneWidth()
{
  ...
  auto current_lanelets =
    lanelet::geometry::findNearest(wm_->getMap()->laneletLayer, current_loc, 10);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "calculated lane width: " << laneWidth);
...
// Check if target platoon is in front of the host vehicle, and within the same lane (downtrack is
// the DTD of the target vehicle).
bool PlatoonStrategicIHPPlugin::isVehicleRightInFront(double downtrack, double crosstrack)
{
  ...
  if (downtrack > currentDtd && samelane) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The front platoon dtd is " << downtrack << " and we are current at " << currentDtd);
    return false;
  }
}
...
lanelet::BasicPoint2d PlatoonStrategicIHPPlugin::ecef_to_map_point(
  carma_v2x_msgs::msg::LocationECEF ecef_point)
{
  ...
  lanelet::BasicPoint3d map_point = map_projector_->projectECEF(
    {(double)ecef_point.ecef_x / 100.0, (double)ecef_point.ecef_y / 100.0,
     (double)ecef_point.ecef_z / 100.0},
    -1);
...
carma_v2x_msgs::msg::MobilityOperation PlatoonStrategicIHPPlugin::composeMobilityOperationSTATUS()
{
  ...
  // TODO future: consider setting recipient_id to the platoon ID to make it obvious that anyone in
  // that group is intended reader.
...
carma_v2x_msgs::msg::MobilityOperation PlatoonStrategicIHPPlugin::composeMobilityOperationINFO()
{
   ...
   */
  carma_v2x_msgs::msg::MobilityOperation msg;
  msg.m_header.plan_id = pm_.currentPlatoonID;  // msg.m_header.plan_id is the platoon ID of the
                                                // request sender (rear join and frontal join).
  msg.m_header.recipient_id = "";
  std::string hostStaticId = config_.vehicleID;
  msg.m_header.sender_id = hostStaticId;
  msg.m_header.timestamp = timer_factory_->now().nanoseconds() / 1000000;
...
// Note: The function "find_target_lanelet_id" was used to test the IHP platooning logic and is only
// a pre-written scenario. The IHP platooning should provide necessary data in a maneuver plan for
// the arbitrary lane change module.

// This is the platoon leader's function to determine if the joining vehicle is closeby
bool PlatoonStrategicIHPPlugin::isJoiningVehicleNearPlatoon(
  double joining_downtrack, double joining_crosstrack)
{
  ...
  bool longitudinalCheck =
    joining_downtrack >= rearVehicleDtd - config_.longitudinalCheckThresold ||
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "The lateralCheck result is: " << lateralCheck);
  ...
  if (longitudinalCheck && lateralCheck) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The joining vehicle downtrack is "
        << joining_downtrack << " and the host (platoon leader) downtrack is " << frontVehicleDtd);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The joining vehicle crosstrack is " << joining_crosstrack
                                           << " and the host (platoon leader) crosstrack is "
                                           << frontVehicleCtd);
    return false;
  }
}
...
bool PlatoonStrategicIHPPlugin::isVehicleNearTargetPlatoon(
  double rearVehicleDtd, double frontVehicleDtd, double frontVehicleCtd)
{
  // lateral error for two lanes (lane width was calculated based on current lanelet)
  double two_lane_cross_error = 2 * config_.maxCrosstrackError + findLaneWidth();
  // add longitudinal check threshold in config file
  bool longitudinalCheck =
    current_downtrack_ >= rearVehicleDtd - config_.longitudinalCheckThresold ||
  ...
  if (longitudinalCheck && lateralCheck) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The platoon leader dtd is " << frontVehicleDtd << " and we are current at "
                                   << current_downtrack_);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The platoon leader ctd is " << frontVehicleCtd << " and we are current at "
                                   << current_crosstrack_);
    return false;
  }
}
...
{
  /**
   * Note: There is a difference between the "platoon info status" versus the the "platoon strategic
   * plugin states". The "platooning info status" reflect the overall operating status. The "platoon
   * strategic plugin states" manage the negotiation strategies and vehicle communication in a more
   * refined manner. A more detailed note can be found in the corresponding function declaration in
   * "platooning_strategic_ihp.h" file.
   */
  ...
  if (pm_.current_platoon_state == PlatoonState::STANDBY) {
    ...
    status_msg.state = pm_.getHostPlatoonSize() == 1
                         ? carma_planning_msgs::msg::PlatooningInfo::SEARCHING
                         : carma_planning_msgs::msg::PlatooningInfo::LEADING;
  ...
  if (pm_.current_platoon_state != PlatoonState::STANDBY) {
    ...
    if (pm_.current_platoon_state == PlatoonState::FOLLOWER) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "pm platoonsize: " << pm_.getHostPlatoonSize() << ", platoon_leader "
                           << platoon_leader.staticId);

      int numOfVehiclesGaps = pm_.getNumberOfVehicleInFront() - pm_.dynamic_leader_index_;
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The host vehicle have "
          << numOfVehiclesGaps << " vehicles between itself and its leader (includes the leader)");
      ...
      if (!llts.empty()) {
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "No lanelets in this location!!!: ");
      }
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "speed based gap: " << current_speed_ * config_.timeHeadway);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "max desired_headway " << desired_headway);
      // TODO: currently the average length of the vehicle is obtained from a config parameter. In
      // future, plugin needs to be updated to receive each vehicle's actual length through status
      // or BSM messages for more accuracy.
      status_msg.desired_gap =
        std::max(
          config_.standStillHeadway * numOfVehiclesGaps, desired_headway * numOfVehiclesGaps) +
        (numOfVehiclesGaps - 1) * 5.0;  // config_.averageVehicleLength;
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "regular_gap: " << regular_gap);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "current_speed_: " << current_speed_);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "speed based gap: " << desired_headway);
      if (pm_.isCreateGap) {
...
// Compose the Mobility Operation message for leader state. Message parameter types: STATUS and
// INFO.
carma_v2x_msgs::msg::MobilityOperation PlatoonStrategicIHPPlugin::composeMobilityOperationLeader(
  const std::string & type)
{
  ...
  // Unknown strategy param.
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger("platooning_strategic_ihp"), "UNKNOWN strategy param string!!!");
    msg.strategy_params = "";
  }
  return msg;
}
...
PlatoonStrategicIHPPlugin::composeMobilityOperationLeaderWaiting()
{
...
// Compose the Mobility Operation message for CandidateFollower state. Message parameter types:
// STATUS.
carma_v2x_msgs::msg::MobilityOperation
...
// UCLA: add compose msgs for LeaderAborting (inherited from candidate follower). Message parameter
// types: STATUS.
carma_v2x_msgs::msg::MobilityOperation
PlatoonStrategicIHPPlugin::composeMobilityOperationLeaderAborting()
{
  /*
...
// UCLA: add compose msgs for CandidateLeader (inherited from leader waiting). Message parameter
// types: STATUS.
carma_v2x_msgs::msg::MobilityOperation
PlatoonStrategicIHPPlugin::composeMobilityOperationCandidateLeader()
{
  /*
      UCLA Implementation note:
...
// UCLA: compose mobility message for prepare to join (cut-in join state, inherited from follower
// state's compose mob_op)
carma_v2x_msgs::msg::MobilityOperation
...
// UCLA: compose mobility message for leading with operation (cut-in join state, inherited from
// leader state's compose mob_op)
carma_v2x_msgs::msg::MobilityOperation
PlatoonStrategicIHPPlugin::composeMobilityOperationLeadWithOperation(const std::string & type)
{
  carma_v2x_msgs::msg::MobilityOperation msg;
...
carma_v2x_msgs::msg::LocationECEF PlatoonStrategicIHPPlugin::mob_op_find_ecef_from_STATUS_params(
...
void PlatoonStrategicIHPPlugin::mob_op_cb_STATUS(const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "strategyParams = " << strategyParams);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "platoonId = " << platoonId << ", sender ID = " << vehicleID);
  std::string statusParams = strategyParams.substr(OPERATION_STATUS_TYPE.size() + 1);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "pm_.currentPlatoonID = " << pm_.currentPlatoonID
                              << ", targetPlatoonID = " << pm_.targetPlatoonID);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "DTD calculated from ecef is: " << dtd);
  // read Crosstrack
  double ctd = wm_->routeTrackPos(incoming_pose).crosstrack;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "CTD calculated from ecef is: " << ctd);
  ...
  // else it represents an uninteresting platoon
    ...
    RCLCPP_DEBUG_STREAM(
      ...
      "Received mob op for platoon " << platoonId
                                     << " that doesn't match our platoon: " << pm_.currentPlatoonID
                                     << " or known neighbor platoon: " << pm_.targetPlatoonID);
  }
}

//
double PlatoonStrategicIHPPlugin::mob_op_find_platoon_length_from_INFO_params(
  std::string strategyParams)
{
  /**
   * Note: INFO param format:
...
carma_v2x_msgs::msg::LocationECEF PlatoonStrategicIHPPlugin::mob_op_find_ecef_from_INFO_params(
  std::string strategyParams)
{
  /**
   * Note: INFO param format:
...
void PlatoonStrategicIHPPlugin::mob_op_cb(
  const carma_v2x_msgs::msg::MobilityOperation::UniquePtr msg)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "mob_op_cb received msg with sender ID "
                                                      << msg->m_header.sender_id << ", plan ID "
                                                      << msg->m_header.plan_id);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "...strategy " << msg->strategy << ", strategy params " << msg->strategy_params);
  ...
  if (strategy.rfind(PLATOONING_STRATEGY, 0) != 0) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Ignoring mobility operation message for " << strategy << " strategy.");
    return;
  }
  ...
  if (current_speed_ < STOPPED_SPEED) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Ignoring message since host is stopped.");
    return;
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "strategyParams: " << strategyParams << "isPlatoonStatusMsg: " << isPlatoonStatusMsg
                       << "isPlatoonInfoMsg: " << isPlatoonInfoMsg);
  if (isPlatoonStatusMsg) {
    mob_op_cb_STATUS(*msg);
  } else if (isPlatoonInfoMsg) {
    // Note: this is where confusion will reign if multiple neighbor platoons are discovered; need
    // more sophisticated
    ...
    if (platoon_size > 1 && msg->m_header.plan_id.compare(pm_.currentPlatoonID) != 0) {
      // If platoon ID doesn't match our known target platoon then clear any old neighbor platoon
      // info and record the platoon ID and the sender as the leader (only leaders send INFO)
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "pm_.neighbor_platoon_leader_id_: " << pm_.neighbor_platoon_leader_id_);
      pm_.neighbor_platoon_info_size_ = platoon_size;
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "pm_.neighbor_platoon_info_size_: " << pm_.neighbor_platoon_info_size_);
    }
  }
...
void PlatoonStrategicIHPPlugin::mob_op_cb_standby(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
...
void PlatoonStrategicIHPPlugin::mob_op_cb_candidatefollower(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
...
// Handle STATUS operation message
void PlatoonStrategicIHPPlugin::mob_op_cb_follower(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  // std::string strategyParams = msg.strategy_params;
  // bool isPlatoonStatusMsg = (strategyParams.rfind(OPERATION_STATUS_TYPE, 0) == 0);
...
// Handle STATUS operation message
void PlatoonStrategicIHPPlugin::mob_op_cb_leaderwaiting(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
...
// UCLA: Handle both STATUS and INFO operation message. Front join and rear join are all handled if
// incoming operation message have INFO param.
void PlatoonStrategicIHPPlugin::mob_op_cb_leader(const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  /**
   * Note: This is the function to handle the mobility operation message. Vehicle in leader state is
   * either a single ADS vehicle or a platoon leader.
   *
   * Single ADS will send out INFO messages. Platoon leaders will send out both INFO and STATUS
   * messages.
   *
   * If the host is single vehicle, it should have "isPlatoonInfoMsg = true" and "isInNegotiation =
   * false". In such condition, single leader will start joining and send request to the platoon
   * leader. mob_op_cb_leader(
   *
   * If the host vehicle is platoon leader, then it should have "isPlatoonInfoMsg = true" and
   * "isInNegotiation = true". But existing platoon leader do not need to send out joining request.
   *
   * Both the platoon leader and single vehicle need to subscribe to the STATUS message to populate
   * the platoon manager with existing platoon members, so the PM can calculate dtd and ctd
   * corresponds to host vehicle's origin position, hence to be used for later calculation.
   */
  ...
  // In the current state, host vehicle care about the INFO heart-beat operation message if we are
  // not currently in a negotiation, and host also need to care about operation from members in our
  // current platoon.
  bool isPlatoonInfoMsg = strategyParams.rfind(OPERATION_INFO_TYPE, 0) ==
                          0;  // INFO message only broadcast by leader and single CAV.
  bool isInNegotiation = pm_.current_plan.valid ||
                         pm_.currentPlatoonID.compare(pm_.dummyID) !=
                           0;  // In negotiation indicates host is not available to become a joiner
  ...
  if (isPlatoonInfoMsg && !isInNegotiation) {
    ...
    //      (e.g. the shorter platoon should join the longer one, or rear joins front if same
    //      length)
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Neighbor platoon frontVehicleDtd from ecef: "
        << frontVehicleDtd << ", frontVehicleCtd from ecef: " << frontVehicleCtd);

    // use INFO param to find platoon rear vehicle DTD and CTD.
    double platoon_length = mob_op_find_platoon_length_from_INFO_params(
      strategyParams);  // length of the entire platoon in meters.
    /**
     *  note:
     *       [**veh3*] ---------- [*veh2*] ----------------- [*veh1*] ---------- [*veh0**]
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "target Platoon Size: " << targetPlatoonSize);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Found a vehicle/platoon with id = " << platoonId << " within range.");

    // TODO future: add logic here to assess closeness of the neighbor platoon, as well as its
    // speed, destination
    ...
    if (isVehicleRightInFront(rearVehicleDtd, rearVehicleCtd) && !config_.test_front_join) {
       ...
       */
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "Neighbor platoon is right in front of us");
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Publishing request to leader " << senderId << " with params " << request.strategy_params
                                        << " and plan id = " << request.m_header.plan_id);

      // Create a new join plan
      pm_.current_plan =
        ActionPlan(true, request.m_header.timestamp, request.m_header.plan_id, senderId);

      // If we are asking to join an actual platoon (not a solo vehicle), then save its ID for later
      // use
    ...
    // step 4. Request frontal join, if the neighbor is a real platoon
      ...
      (targetPlatoonSize > 1 || config_.test_front_join) &&
      isVehicleRightBehind(frontVehicleDtd, frontVehicleCtd)) {
      /**
       ...
       *                   |-------0------ --1---------2---------3---------4----------5-------|
       */
      boost::format fmter(JOIN_PARAMS);  // Note: Front and rear join uses same params, hence merge
                                         // to one param for both condition.
      int dummy_join_index = -2;         // not used for this message, but message spec requires it
      fmter % platoon_size;              //  index = 0
      fmter % current_speed_;            //  index = 1, in m/s
      fmter % pose_ecef_point_.ecef_x;   //  index = 2, in cm.
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Publishing front join request to the leader "
          << senderId << " with params " << request.strategy_params
          << " and plan id = " << request.m_header.plan_id);
      ...
      // If testing with a solo vehicle to represent the target platoon, then use the current join
      // plan ID for that platoon; otherwise, it will already have and ID so store it for future use
      if (config_.test_front_join) {
        pm_.targetPlatoonID = request.m_header.plan_id;
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Since neighbor is a fake platoon, storing " << pm_.targetPlatoonID
                                                       << " as its platoon ID");
      } else {
        pm_.targetPlatoonID = platoonId;
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Storing real neighbor platoon's ID as target: " << pm_.targetPlatoonID);
      }
    }

    // step 5. Request cut-in join (front, middle or rear, from adjacent lane)
    else if (
      (targetPlatoonSize > 1 || config_.test_cutin_join) && !config_.test_front_join &&
      isVehicleNearTargetPlatoon(rearVehicleDtd, frontVehicleDtd, frontVehicleCtd)) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "starting cut-in join process");
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "rearVehicleDtd " << rearVehicleDtd);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "rearVehicleCtd " << rearVehicleCtd);
      ...
      if (target_rear_pose) {
        ...
        auto target_lanelets =
          lanelet::geometry::findNearest(wm_->getMap()->laneletLayer, target_rear_pose.get(), 1);
        if (!target_lanelets.empty()) {
          ...
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"), "target_rear_pose_lanelet not found!!");
        }
      }
      ...
      // UCLA: Desired joining index for cut-in join, indicate the index of gap-leading vehicle. -1
      // indicate cut-in from front. Note: remove join_index to info param.
      request.plan_type.type = carma_v2x_msgs::msg::PlanType::PLATOON_CUT_IN_JOIN;

      // At this step all cut-in types start with this request, so the join_index at this point is
      // set to default, -2.
      int join_index = -2;
      boost::format fmter(JOIN_PARAMS);  // Note: Front and rear join uses same params, hence merge
                                         // to one param for both condition.
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Publishing request to the leader " << senderId << " with params "
    ...
    // step 6. Return none if no platoon nearby
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Ignore platoon with platoon id: " << platoonId << " because it is too far away to join.");
    }
  }
...
// UCLA: mob_op_cb for the new leader aborting state (inherited from candidate follower), handle
// STATUS message.
void PlatoonStrategicIHPPlugin::mob_op_cb_leaderaborting(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
...
void PlatoonStrategicIHPPlugin::mob_op_cb_candidateleader(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
...
void PlatoonStrategicIHPPlugin::mob_op_cb_leadwithoperation(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
...
void PlatoonStrategicIHPPlugin::mob_op_cb_preparetojoin(
  const carma_v2x_msgs::msg::MobilityOperation & msg)
{
  /*
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "pm_.is_neighbor_record_complete_" << pm_.is_neighbor_record_complete_);

  if (isPlatoonInfoMsg && pm_.is_neighbor_record_complete_) {
    // TODO: would be good to have a timeout here; if a neighbor platoon has been identified, and no
    // INFO messages
    //       from it are received in a while, then its record should be erased, and any in-work
    //       joining should be aborted.
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "rearVehicleIndex: " << rearVehicleIndex);
    ...
    // If lane change has not yet been authorized, stop here (this method will be running before the
    // negotiations with the platoon leader are complete)
    if (!safeToLaneChange_) {
      return;
    }

    // determine if the lane change is finished
    bool isSameLaneWithPlatoon =
      abs(frontVehicleCtd - current_crosstrack_) <= config_.maxCrosstrackError;
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Lane change has been authorized. isSameLaneWithPlatoon = " << isSameLaneWithPlatoon);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "crosstrack diff" << abs(frontVehicleCtd - current_crosstrack_));
    if (isSameLaneWithPlatoon) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "new platoon id: " << pm_.currentPlatoonID);
      pm_.current_plan =
        ActionPlan(true, request.m_header.timestamp, request.m_header.plan_id, senderId);
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "Lane Change not completed");
    }
  }
}
...
MobilityRequestResponse PlatoonStrategicIHPPlugin::handle_mob_req(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
...
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_standby(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
...
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_candidatefollower(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "Received mobility request with type " << msg.plan_type.type << " but ignored.");
  return MobilityRequestResponse::NO_RESPONSE;
}

MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_follower(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  /**
   * For cut-in join, the gap rear vehicle need to slow down once they received the request from
   * platoon leader. Note:1. (cut-in) When joiner is in position, the leader will send the request
   * to relevent platoon member. So no need to check for "in-position" one more time.
   *      2. (cut-in) If a new member is added in middle, "mob_op_cb_STATUS" will update and resort
   * all vehicle information. So the ordering of the platoon can be updated.
   *      3. (depart) If the current leader is departing, the closest follower (i.e., 2nd in
   * platoon) will become the new leader, and switch to candidate follower state. While the previous
   * leader depart and operating in single leader state.
   */
  ...
  if (isCutInJoin && isHostRecipent) {
    ...
    // 2. cut-in from middle
      ...
      req_sender_join_index >= 0 &&
      static_cast<size_t>(req_sender_join_index) < pm_.host_platoon_.size() - 1) {
    ...
    // 3. Abnormal join index
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "Abnormal cut-in index, abort operation.");
...
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_leaderwaiting(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "current_cross_track error = " << abs(obj_cross_track - current_crosstrack_));
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "inTheSameLane = " << inTheSameLane);
  ...
  if (isTargetVehicle && isCandidateJoin) {
    if (inTheSameLane) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Target vehicle " << pm_.current_plan.peerId << " is actually joining.");
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
      ...
      // If we are not already in a platoon, then use this activity plan ID for the newly formed
      // platoon
      if (pm_.currentPlatoonID.compare(pm_.dummyID) == 0) {
        pm_.currentPlatoonID = msg.m_header.plan_id;
      }

      // Add the joiner to our platoon record (ASSUMES that we have not yet received a mob_op STATUS
      // msg from joiner!)
      ...
      RCLCPP_DEBUG_STREAM(
        ...
        "New member being added to platoon vector whose size is currently "
          << pm_.host_platoon_.size());
      pm_.host_platoon_.push_back(newMember);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "pm_ now thinks platoon size is " << pm_.getHostPlatoonSize());
    ...
    } else  // correct vehicle and intent, but it's in the wrong lane
    {
      ...
      // {
      //     RCLCPP_DEBUG_STREAM(rclcpp::get_logger("platooning_strategic_ihp"), "Failed to remove
      //     candidate joiner from platoon record: " << msg.m_header.sender_id);
      // }
    }
  }
...
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_leader(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  /**
   *   UCLA implementation note:
   *   1. Here the mobility requests of joining platoon (front and rear) get processed and
   * responded.
   ...
   *      PLATOON_CUT_IN_JOIN indicate a cut-in join, which include three cut-in methods: cut-in
   * front, cut-in middle, and cut-in rear.
   */
  ...
  // Ignore the request if we are already working with a join/departure process or if no join type
  ...
  if (isFrontJoin || isRearJoin || isCutInJoin || isDepart) {
    if (pm_.current_plan.valid) {
      ...
      return MobilityRequestResponse::NO_RESPONSE;  // TODO: replace with NACK that indicates to ask
                                                    // me later
    }
  } else {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Received request with bogus message type " << plan_type.type << "; ignoring");
    return MobilityRequestResponse::NO_RESPONSE;
  }

  // TODO: Generalize - We currently ignore the lane information for now and assume the applicant is
  // in the same lane with us. Determine intra-platoon conditions We are currently checking two
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "Received mobility JOIN request from " << applicantId << " and PlanId = " << msgHeader.plan_id);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "The strategy parameters are " << params);
  if (params.length() == 0) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "applicantSize: " << applicantSize);
  ...
  if (isRearJoin) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "The received mobility JOIN request from "
                                                        << applicantId
    ...
    if (hasEnoughRoomInPlatoon && isInLane) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The gap between current platoon rear and applicant is " << currentGap << "m or "
                                                                 << currentTimeGap << "s");
      ...
      bool isDistanceCloseEnough =
        currentGap <= config_.maxAllowedJoinGap || currentTimeGap <= config_.maxAllowedJoinTimeGap;
      if (isDistanceCloseEnough) {
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Change to LeaderWaitingState and waiting for " << msg.m_header.sender_id << " to join");
  ...
  // front join; platoon leader --> leader aborting
    ...
    if (hasEnoughRoomInPlatoon && isInLane) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The gap between current platoon front and applicant is " << currentGap << "m or "
                                                                  << currentTimeGap << "s");
      ...
      // UCLA: add condition: only allow front join when host platoon size >= 2 (make sure when two
      // single vehicle join, only use back join)
      ...
      if (isDistanceCloseEnough && isPlatoonNotSingle) {
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Change to LeaderAborting state and waiting for "
            << msg.m_header.sender_id << " to join as the new platoon leader");

        // ----------------- give up leader position and look for new leader
        // --------------------------

        // adjust for frontal join. Platoon info is related to the platoon at back of the candidate
        // leader vehicle. Don't want an action plan here
        pm_.current_platoon_state = PlatoonState::LEADERABORTING;
        candidatestateStartTime = timer_factory_->now().nanoseconds() / 1000000;

        // If we are testing with a single vehicle representing this platoon, then we don't yet have
        // a platoon ID, so use the ID for the proposed joining action plan
        if (config_.test_front_join) {
        ...
        RCLCPP_DEBUG_STREAM(
          ...
          "The joining gap (" << currentGap
                              ...
                              << pm_.getHostPlatoonSize() << ") is one. NACK.");
        return MobilityRequestResponse::NACK;  // TODO: add reason & request to try again
      }
    } else {
      RCLCPP_DEBUG_STREAM(
  ...
  // UCLA: conditions for cut-in join; platoon leader --> leading with operation
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The received mobility JOIN request from "
        << applicantId << " and PlanId = " << msgHeader.plan_id << " is a CUT-IN-JOIN request !");

    // -- core condition to decided accept joiner or not. It is necessary leader only process the
    // first cut-in joining request. Note: The host is the platoon leader, need to use a different
    // method to determine if joining vehicle is nearby.
    if (
      hasEnoughRoomInPlatoon &&
      isJoiningVehicleNearPlatoon(applicantCurrentDtd, applicantCurrentCtd)) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The current platoon has enough room for the applicant with size " << applicantSize);
      RCLCPP_DEBUG_STREAM(
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The current platoon does not have enough room or the applicant is too far away from us. "
        "NACK the request.");
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The applicant downtrack is: " << current_downtrack_ << ".");
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The applicant crosstrack is: " << current_crosstrack_ << ".");
      return MobilityRequestResponse::NACK;
    }
  }
  ...
  // no response
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Received mobility request with type " << msg.plan_type.type << " and ignored.");
    return MobilityRequestResponse::NO_RESPONSE;
  }
}

// UCLA: mobility request leader aborting (inherited from candidate follower)
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_leaderaborting(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  // This state does not handle any mobility request for now
...
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_candidateleader(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  bool isTargetVehicle =
    msg.m_header.sender_id == pm_.current_plan.peerId;  // need to check: senderID (old leader)
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "current_cross_track error = " << abs(obj_cross_track - current_crosstrack_));
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "obj_cross_track = " << obj_cross_track);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "current_crosstrack_ = " << current_crosstrack_);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "inTheSameLane = " << inTheSameLane);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "isTargetVehicle = " << isTargetVehicle);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "isCandidateJoin = " << isCandidateJoin);
  if (isCandidateJoin && inTheSameLane && isTargetVehicle) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Old platoon leader " << pm_.current_plan.peerId << " has agreed to joining.");
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The request type is " << msg.plan_type.type << " and we choose to ignore");
    pm_.clearActionPlan();
...
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_leadwithoperation(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  /*
  *   Current leader change state to lead with opertaion once the cut-in join request is accepeted.
      ...
      The host leading vehicle will change to same-lane state once the lanechange completion plan
  was recieved.
  ...
  */
  ...
  if (plan_type.type == carma_v2x_msgs::msg::PlanType::PLATOON_CUT_IN_JOIN) {
    ...
    if (req_sender_join_index == -1) {
      ...
      // check dtd between current leader (host) and joining vehicle is far enough away to avoid a
      // collision
    ...
    // ----- CUT-IN rear -----
      ...
      // To pass, the joining vehicle need to be behind the last member, within three vehicle
      // length.
      // TODO: should it also test CTE to ensure vehicle is in same lane?
      double platoonEndVehicleDtd =
        pm_.host_platoon_[pm_.host_platoon_.size() - 1].vehiclePosition - config_.vehicleLength;
      double rearGap = platoonEndVehicleDtd - applicantCurrentDtd;
      bool isRearJoinerInPosition =
        rearGap >= 0 &&
        rearGap <= config_.maxCutinGap;  // 3*config_.vehicleLength; TODO: temporary increase

      if (isRearJoinerInPosition) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Published Mobility cut-in-rear-Join request to relavent platoon member, host is "
          "leader.");
        RCLCPP_WARN(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Published Mobility cut-in-rear-Join request to relavent platoon members to signal gap "
          "creation.");
        pm_.isCreateGap = true;
        return MobilityRequestResponse::ACK;
      } else {
    ...
    else  // any other join_index value
    {
      ...
      // To pass, the joining vehicle should be in front of the gap following vehicle, within three
      // vehicle length.
      ...
      bool isMidJoinerInPosition =
        gapFollowerDiff >= 0 && gapFollowerDiff <= 3 * config_.vehicleLength;
      ...
      if (isMidJoinerInPosition) {
        ...
        boost::format fmter(JOIN_PARAMS);  // Note: Front and rear join uses same params, hence
                                           // merge to one param for both condition.
        fmter % platoon_size;              //  index = 0
        fmter % current_speed_;            //  index = 1, in m/s
        fmter % pose_ecef_point_.ecef_x;   //  index = 2
        fmter % pose_ecef_point_.ecef_y;   //  index = 3
        ...
        request.urgency = 50;
        request.location = pose_to_ecef(pose_msg_);
        // note: for rear join, cut-in index == host_platoon_.size()-1; for join from front, index
        // == -1
        //       for cut-in in middle, index indicate the gap leading vehicle's index
        mobility_request_publisher_(request);
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Published Mobility cut-in-mid-Join request to relavent platoon members to signal gap "
          "creation, host is leader.");
        RCLCPP_WARN(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Published Mobility cut-in-mid-Join request to relavent platoon members to signal gap "
          "creation.");
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "The joining vehicle is cutting in at index: "
            << req_sender_join_index << ". Notify gap rear vehicle with ID: " << recipient_ID
            << " to slow down");
        return MobilityRequestResponse::ACK;
      } else {
        RCLCPP_DEBUG_STREAM(
  ...
  // task 5: if other joining vehicle send joning request, NACK it since there is already a cut-in
  // join going on.
  else {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "CUT-IN join maneuver is already in operation, NACK incoming join requests from other "
      "candidates.");
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Plan Type: " << plan_type.type);
    return MobilityRequestResponse::NACK;
  }

  // this statement should never be reached, but will ensure reasonable behavior in case of coding
  // error above
  RCLCPP_WARN_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "End of method reached! Apparent logic fault above.");  // since WARN doesn't always print
  return MobilityRequestResponse::NO_RESPONSE;
}

// UCLA: add request call-back function for prepare to join (for cut-in join)
MobilityRequestResponse PlatoonStrategicIHPPlugin::mob_req_cb_preparetojoin(
  const carma_v2x_msgs::msg::MobilityRequest & msg)
{
  // This state does not handle any mobility request for now
  // TODO: if joining vehicle need to adjust speed, the leader should request it and the request
...
void PlatoonStrategicIHPPlugin::mob_resp_cb(
  const carma_v2x_msgs::msg::MobilityResponse::UniquePtr msg)
{
  // Firstly, check eligibility of the received message.
  bool isCurrPlanValid =
    pm_.current_plan.valid;  // Check if current plan is still valid (i.e., not timed out).
  bool isForCurrentPlan =
    msg->m_header.plan_id == pm_.current_plan.planId;  // Check if plan Id matches.
  bool isFromTargetVehicle =
    msg->m_header.sender_id ==
    pm_.current_plan.peerId;  // Check if expected peer ID and sender ID matches.
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "sender ID = " << msg->m_header.sender_id << ", current peer ID = " << pm_.current_plan.peerId);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "incoming plan ID = " << msg->m_header.plan_id
                          << "current plan ID = " << pm_.current_plan.planId);

  if (!(isCurrPlanValid && isForCurrentPlan && isFromTargetVehicle)) {
    /**
     * If any of the three condition (i.e., isCurrPlanValid, isForCurrentPlan and
     * isFromTargetVehicle) was not satisfied, return ignore as this message was not intended for
     * the host.
     */
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_standby(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_candidatefollower(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Callback for candidate follower ");

  // Check if current plan is still valid (i.e., not timed out)
  if (pm_.current_plan.valid) {
    bool isForCurrentPlan = msg.m_header.plan_id == pm_.current_plan.planId;
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "isForCurrentPlan " << isForCurrentPlan);
    ...
    if (isForCurrentPlan) {
      if (msg.is_accepted) {
        ...
        // The platoon manager also need to change the platoon Id to the one that the target leader
        // is using
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "pm_.targetPlatoonID: " << pm_.currentPlatoonID);

        if (pm_.targetPlatoonID.compare(pm_.dummyID) != 0) {
          pm_.currentPlatoonID = pm_.targetPlatoonID;
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
            "pm_.currentPlatoonID now: " << pm_.currentPlatoonID);
          pm_.resetNeighborPlatoon();
        } else {
          pm_.currentPlatoonID = msg.m_header.plan_id;
        }
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_leaderwaiting(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  /**
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_follower(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  /**
   * UCLA Note:
  ...
  // TODO: when would this code block ever be used? A normal follower would have to talk to a front
  // joiner.
  ...
  if (isFrontJoin && msg.is_accepted) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Received positive response for front-join plan id = " << pm_.current_plan.planId);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_leader(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  /**
   *  UCLA implementation note:
   ...
   *  the response sender is the existing platoon leader (front join: aborting leader, back join:
   * waiting leader).
   *
   *  If the host is the current platoon leader, all three case will be false and no further action
   * is needed.
   *
   *  Disclaimer: Currently, if the host vehicle is platoon leader, there is no further action
   * needed when receiving the mobility response. However, future development may add functions in
   * this mehtod.
   */
  ...
  carma_v2x_msgs::msg::PlanType plan_type = msg.plan_type;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "plan_type.type = " << plan_type.type);

  // UCLA: determine joining type
  bool isCutInJoin = plan_type.type == carma_v2x_msgs::msg::PlanType::PLATOON_CUT_IN_JOIN &&
                     !config_.test_front_join;
  bool isRearJoin = plan_type.type == carma_v2x_msgs::msg::PlanType::JOIN_PLATOON_AT_REAR &&
                    !config_.test_front_join;
  bool isFrontJoin = plan_type.type == carma_v2x_msgs::msg::PlanType::JOIN_PLATOON_FROM_FRONT ||
                     config_.test_front_join;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Joining type: isRearJoin = " << isRearJoin);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Joining type: isFrontJoin = " << isFrontJoin);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Joining type: isCutInJoin = " << isCutInJoin);

  // Check if current plan is still valid (i.e., not timed out).
  if (pm_.current_plan.valid) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "My plan id = " << pm_.current_plan.planId
                      << " and response plan Id = " << msg.m_header.plan_id);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Expected peer id = " << pm_.current_plan.peerId
                            << " and response sender Id = " << msg.m_header.sender_id);

    // Check the response is received correctly (i.e., host vehicle is the desired receiver).
    if (
      pm_.current_plan.planId == msg.m_header.plan_id &&
      pm_.current_plan.peerId == msg.m_header.sender_id) {
      // rear join
      if (isRearJoin && msg.is_accepted) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Received positive response for plan id = " << pm_.current_plan.planId);
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
      ...
      // Current leader of an actual platoon (to be in this method host is in leader state)
        ...
        RCLCPP_WARN_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Host received response for joining vehicles, remain idling as the host is a current "
          "platoon leader.");
      } else {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "Received negative response for plan id = " << pm_.current_plan.planId
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "And peer id match " << (pm_.current_plan.peerId == msg.m_header.sender_id));
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Expected peer id = " << pm_.current_plan.peerId
                              << " and response sender Id = " << msg.m_header.sender_id);
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_leaderaborting(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  /**
   *  UCLA implementation note:
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Callback for leader aborting !");
  ...
  if (pm_.current_plan.valid) {
    ...
    if (msg.plan_type.type == carma_v2x_msgs::msg::PlanType::UNKNOWN) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "*** plan type JOIN_PLATOON_FROM_FRONT");
    } else if (msg.plan_type.type == carma_v2x_msgs::msg::PlanType::PLATOON_CUT_IN_JOIN) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "*** plan type PLATOON_CUT_IN_JOIN");
    } else {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "*** plan type not captured.");
    }
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Plan Type " << msg.plan_type.type);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "isForFrontJoin " << isForFrontJoin);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "isForCurrentPlan " << isForCurrentPlan);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
    ...
    if (isForCurrentPlan && isFromTargetVehicle && isForFrontJoin) {
      if (msg.is_accepted) {
        ...
        pm_.current_platoon_state = PlatoonState::LEADER;
        // We were already leading a platoon, so don't erase any of that info. But we need to remove
        // the erstwhile candidate leader from our platoon roster; we know it is in position 0, so
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_candidateleader(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_leadwithoperation(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
...
void PlatoonStrategicIHPPlugin::mob_resp_cb_preparetojoin(
  const carma_v2x_msgs::msg::MobilityResponse & msg)
{
  /*
  ...
  */
  ...
  bool isFinishLaneChangeMidorRear =
    plan_type.type == carma_v2x_msgs::msg::PlanType::CUT_IN_MID_OR_REAR_DONE;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "isCreatingGap = " << isCreatingGap
                       << ", is_neighbor_record_complete = " << pm_.is_neighbor_record_complete_);

  if (!msg.is_accepted) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Request " << msg.m_header.plan_id << " was rejected by leader.");
  ...
  if (isCreatingGap && pm_.is_neighbor_record_complete_) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Gap is now sufficiently large.");
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "in mob_resp_cb safeToLaneChange_: " << safeToLaneChange_);
  ...
  // UCLA: Revert to same-lane for cut-in front
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "pm_.targetPlatoonID: " << pm_.targetPlatoonID);
    if (pm_.targetPlatoonID.compare(pm_.dummyID) != 0) {
      pm_.currentPlatoonID = pm_.targetPlatoonID;
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
  ...
  // UCLA: Revert to same-lane operation for cut-in from middle/rear
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Cut-in from mid or rear, the lane change finished, the joining vehicle revert to same-lane "
      "maneuver.");
    pm_.current_platoon_state = PlatoonState::CANDIDATEFOLLOWER;
    candidatestateStartTime = timer_factory_->now().nanoseconds() / 1000000;
    pm_.current_plan.valid = false;  // but leave peerId intact for use in second request
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "End of mob_resp_cb_preparetojoin");
  }
}
...
void PlatoonStrategicIHPPlugin::mob_req_cb(
  const carma_v2x_msgs::msg::MobilityRequest::UniquePtr msg)
{
  ...
  if (req_response == MobilityRequestResponse::ACK) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), " NO response to mobility request. ");
  }
}
...
void PlatoonStrategicIHPPlugin::run_leader_waiting()
{
  ...
  if (tsStart - waitingStartTime > waitingStateTimeout * 1000) {
    // TODO if the current state timeouts, we need to have a kind of ABORT message to inform the
    // applicant
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
  ...
  // TODO this solution is not sim-time complient and should be replaced with one which is when
  // possible https://github.com/usdot-fhwa-stol/carma-platform/issues/1888
...
void PlatoonStrategicIHPPlugin::run_leader()
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "time since last heart beat: " << tsStart - prevHeartBeatTime_);
  ...
  if (pm_.current_plan.valid) {
    ...
    if (isCurrentPlanTimeout) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Give up current on waiting plan with planId: " << pm_.current_plan.planId);
      pm_.clearActionPlan();
    }
  }
...
void PlatoonStrategicIHPPlugin::run_follower()
{
  ...
  // 1. Check the state start time, if it exceeds a limit it will give up current plan and change
  // back to leader state
  // 2. Abort current request if we wait for long enough time for response from leader and change
  // back to leader state
  ...
  if (vehicleInFront == 0) {
    ...
    if (noLeaderUpdatesCounter >= LEADER_TIMEOUT_COUNTER_LIMIT) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "noLeaderUpdatesCounter = " << noLeaderUpdatesCounter << " and change to leader state");
...
void PlatoonStrategicIHPPlugin::run_candidate_follower()
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "timeout1: " << tsStart - candidatestateStartTime);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "waitingStateTimeout: " << waitingStateTimeout * 1000);
  ...
  if (pm_.current_plan.valid) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "pm_.current_plan.planStartTime: " << pm_.current_plan.planStartTime);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "timeout2: " << tsStart - pm_.current_plan.planStartTime);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
    ...
    if (isPlanTimeout) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "Changed the state back to Leader");
    }
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "Since we have max allowed gap as " << config_.desiredJoinGap << " m then max join gap became "
                                        << maxJoinGap << " m");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "The current gap from radar is " << currentGap << " m");
  if (currentGap <= maxJoinGap && !pm_.current_plan.valid) {
    ...
    request.strategy = PLATOONING_STRATEGY;
    request.strategy_params =
      "";  // params will not be read by receiver since this is 2nd msg in sequence
    request.urgency = 50;
    request.location = pose_to_ecef(pose_msg_);
    mobility_request_publisher_(request);
    RCLCPP_DEBUG_STREAM(
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "current plan peer id: " << pm_.current_plan.peerId);

    // Update the local record of the new activity plan and now establish that we have a platoon
    // plan as well, which allows us to start sending necessary op STATUS messages
  ...
  if (pm_.current_plan.valid) {
    // Don't want to do this until after the above MobReq message is delivered, otherwise recipient
    // will double-count us in their platoon
...
// UCLA: frontal join state (inherit from candidate follower: prepare to give up leading state and
...
void PlatoonStrategicIHPPlugin::run_leader_aborting()
{
  /*
      ...
      2. the sender of the plan (host vehicle) is the previous leader, it wil prepare to follow
     front joiner (new leader)
  */
  long tsStart = timer_factory_->now().nanoseconds() / 1000000;
  // Task 1: state timeout
  bool isCurrentStateTimeout = (tsStart - candidatestateStartTime) > waitingStateTimeout * 1000;
  ...
  if (isCurrentStateTimeout) {
    ...
    // clear plan validity & end; leave platoon info alone, as we may still be leading a valid
    // platoon
    pm_.clearActionPlan();
    return;
  }
  ...
  // preceding host is the candidate joiner. For this code to work, it depends on the candidate to
  // publish mobility operation STATUS messages so that host can include it in the pm_ platoon
  // membership.
  double currentGap =
    pm_.getDistanceToPredVehicle();  // returns 0 if we haven't received op STATUS from joiner yet
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "Based on desired join time gap, the desired join distance gap is " << desiredJoinGap2 << " m");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "Since we have max allowed gap as " << config_.desiredJoinGap << " m then max join gap became "
                                        << maxJoinGap << " m");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "The current gap to joiner is " << currentGap << " m");

  // NOTE: The front join depends upon the joiner to publish op STATUS messages with this platoon's
  // ID, then host receives at least one and thereby adds the joiner to the platoon record. This
  // process requires host's mob_req_cb_leader() to ACK the join request, then the remote vehicle to
  // handle that ACK and broadcast its first op STATUS, then host to receive and process it. All
  // that has to happen before the below code block runs, even though this method starts to spin
  // immediately after we send out the mentioned ACK. To avoid a race condition, we must wait a few
  // cycles to ensure the 2-way messaging has completed. The race is that above calculation for
  // current gap will be returned as 0 until we have received said STATUS message from the joiner,
  // which would prematurely trigger the process to move forward.

  // Check if gap is big enough and if there is no currently active plan and this method has been
  // called several times Add a condition to prevent sending repeated requests (Note: This is a
  // same-lane maneuver, so no need to consider lower bound of joining gap.)
  ++numLeaderAbortingCalls_;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "numLeaderAbortingCalls = " << numLeaderAbortingCalls_
                                << ", max = " << config_.maxLeaderAbortingCalls);
  if (
    currentGap <= maxJoinGap && !pm_.current_plan.valid &&
    numLeaderAbortingCalls_ > config_.maxLeaderAbortingCalls) {
    ...
    int platoon_size =
      pm_.getHostPlatoonSize();  // depends on joiner to send op STATUS messages while joining

    boost::format fmter(JOIN_PARAMS);  // Note: Front and rear join uses same params, hence merge to
                                       // one param for both condition.
  ...
  mobility_operation_publisher_(status);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Published platoon STATUS operation message");
...
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Run Candidate Leader State ");
  long tsStart = timer_factory_->now().nanoseconds() / 1000000;
...
void PlatoonStrategicIHPPlugin::run_prepare_to_join()
{
  ...
  if (isCurrentStateTimeout) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The current prepare to join state is timeout. Change back to leader state and abort lane "
      "change.");
    pm_.current_platoon_state = PlatoonState::LEADER;
    safeToLaneChange_ = false;
    pm_.clearActionPlan();
  ...
  // If we aren't already waiting on a response to one of these plans, create one once neighbor info
  // is available
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "current_plan.valid = " << pm_.current_plan.valid << ", is_neighbor_record_complete = "
                            << pm_.is_neighbor_record_complete_);

  if (!pm_.current_plan.valid && pm_.is_neighbor_record_complete_) {
    // Task 1: compose mobility operation (status)
    carma_v2x_msgs::msg::MobilityOperation status;
    status = composeMobilityOperationPrepareToJoin();  // TODO: I bet we could consolidate a lot of
                                                       // these compose methods
    mobility_operation_publisher_(status);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Published platoon STATUS operation message");

    // Task 3: Calculate proper cut_in index
    // Note: The cut-in index is zero-based and points to the gap-leading vehicle's index. For
    // cut-in from front, the join index = -1.
    double joinerDtD = current_downtrack_;
    target_join_index_ = pm_.getClosestIndex(joinerDtD);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "calculated join index: " << target_join_index_);
...
bool PlatoonStrategicIHPPlugin::onSpin()
{
  ...
  // UCLA: added prepare to join for CUT-IN join
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "standby state, nothing to do");
  }
  // coding oversight
  else {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "///// unhandled state " << pm_.current_platoon_state);
  }
  // TODO: Place holder for departure
...
carma_planning_msgs::msg::Maneuver PlatoonStrategicIHPPlugin::composeManeuverMessage(
  double current_dist, double end_dist, double current_speed, double target_speed, int lane_id,
  rclcpp::Time & current_time)
{
  ...
  maneuver_msg.lane_following_maneuver.parameters.negotiation_type =
    carma_planning_msgs::msg::ManeuverParameters::PLATOONING;
  ...
  maneuver_msg.lane_following_maneuver.end_time =
    current_time + rclcpp::Duration::from_nanoseconds(config_.time_step * 1e9);
  maneuver_msg.lane_following_maneuver.lane_ids = {std::to_string(lane_id)};

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "in compose maneuver lane id:" << lane_id);
  ...
  if (!wm_->getMapRoutingGraph()->following(current_lanelet, false).empty()) {
    auto next_lanelet_id =
      wm_->getMapRoutingGraph()->following(current_lanelet, false).front().id();
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "next_lanelet_id:" << next_lanelet_id);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "Duration:" << rclcpp::Time(maneuver_msg.lane_following_maneuver.end_time).seconds() -
                     rclcpp::Time(maneuver_msg.lane_following_maneuver.start_time).seconds());
  return maneuver_msg;
}

// UCLA: compose maneuver message for lane change
carma_planning_msgs::msg::Maneuver PlatoonStrategicIHPPlugin::composeLaneChangeManeuverMessage(
  double current_dist, double end_dist, double current_speed, double target_speed,
  int starting_lane_id, int ending_lane_id, rclcpp::Time & current_time)
{
  carma_planning_msgs::msg::Maneuver maneuver_msg;
  // UCLA: change to lane change maneuvers
  maneuver_msg.type = carma_planning_msgs::msg::Maneuver::LANE_CHANGE;
  maneuver_msg.lane_change_maneuver.parameters.negotiation_type =
    carma_planning_msgs::msg::ManeuverParameters::PLATOONING;
  maneuver_msg.lane_change_maneuver.parameters.presence_vector =
    carma_planning_msgs::msg::ManeuverParameters::HAS_TACTICAL_PLUGIN;
  maneuver_msg.lane_change_maneuver.parameters.planning_tactical_plugin = "cooperative_lanechange";
  maneuver_msg.lane_change_maneuver.parameters.planning_strategic_plugin =
    "platooning_strategic_ihp";
  ...
  // Generate a new maneuver ID for the lane change maneuver (not needed for lane following
  // maneuver).
  maneuver_msg.lane_following_maneuver.parameters.maneuver_id =
    boost::uuids::to_string(boost::uuids::random_generator()());

  // because it is a rough plan, assume vehicle can always reach to the target speed in a lanelet
  double cur_plus_target = current_speed + target_speed;
  if (cur_plus_target < 0.00001) {
    maneuver_msg.lane_change_maneuver.end_time =
      current_time + rclcpp::Duration::from_nanoseconds(config_.time_step * 1e9);
  } else {
    // maneuver_msg.lane_change_maneuver.end_time = current_time +
    // rclcpp::Duration::from_nanoseconds((end_dist - current_dist) / (0.5 * cur_plus_target));
    maneuver_msg.lane_change_maneuver.end_time =
      current_time + rclcpp::Duration::from_nanoseconds(20.0 * 1e9);
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "Duration:" << rclcpp::Time(maneuver_msg.lane_change_maneuver.end_time).seconds() -
                     rclcpp::Time(maneuver_msg.lane_change_maneuver.start_time).seconds());
  return maneuver_msg;
}

// update current status based on maneuver
void PlatoonStrategicIHPPlugin::updateCurrentStatus(
  carma_planning_msgs::msg::Maneuver maneuver, double & speed, double & current_progress,
  int & lane_id)
{
...
bool PlatoonStrategicIHPPlugin::plan_maneuver_cb(
  ...
  carma_planning_msgs::srv::PlanManeuvers::Response & resp)
{
  ...
  if (current_lanelets.size() == 0) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Cannot find any lanelet in map!");
    return true;
  }
  ...
  // Note: The desired gap will be adjusted and send to control plugin (via platoon_info_msg) where
  // gap creation will be handled.
  double target_speed;
  ...
  if (req.prior_plan.maneuvers.size() != 0) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Provided with initial plan...");
    time_progress = req.prior_plan.planning_completion_time;
    int end_lanelet = 0;
    updateCurrentStatus(
      req.prior_plan.maneuvers.back(), speed_progress, current_progress, end_lanelet);
  }

  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("platooning_strategic_ihp"), "Starting Loop");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "total_maneuver_length: " << total_maneuver_length << " route_length: " << route_length);

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "in mvr  callback safeToLaneChange: " << safeToLaneChange_);

  // Note: Use current_lanlet list (which was determined based on vehicle pose) to find current
  // lanelet ID.
  lanelet::Id current_lanelet_id = current_lanelet.id();

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "current_lanelet_id: " << current_lanelet_id);
  ...
  if (safeToLaneChange_) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "target_crosstrack: " << target_crosstrack);
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "crosstrackDiff: " << crosstrackDiff);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
    ...
    if (!isLaneChangeFinished) {
      ...
      while (current_progress < total_maneuver_length) {
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "current_progress: " << current_progress);
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "speed_progress: " << speed_progress);
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "target_speed: " << target_speed);
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "end_dist: " << end_dist);
        // consider calculate dtd_diff and ctd_diff
        double dist_diff = end_dist - current_progress;
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "dist_diff: " << dist_diff);

        // TODO: target_cutin_pose_ represents the platoon leader. It seems this may be the wrong
        // answer for mid- or rear-cutins? SAINA: currently, the functions do not provide the
        // correct point of rear vehicle of the platoon
        ...
        // TODO: target_cutin_pose_ represents the platoon leader. Is this the best pose to use
        // here?
        //  get the actually closest lanelets,
        auto target_lanelets =
          lanelet::geometry::findNearest(wm_->getMap()->laneletLayer, target_cutin_pose_, 1);
        if (target_lanelets.empty()) {
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
        ...
        if (lanechangePossible) {
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"), "Lane change possible, planning it.. ");
          resp.new_plan.maneuvers.push_back(composeLaneChangeManeuverMessage(
            current_downtrack_, lc_end_dist, speed_progress, target_speed, current_lanelet_id,
            target_lanelet_id, time_progress));
          ...
          resp.new_plan.maneuvers.push_back(composeManeuverMessage(
            current_downtrack_, end_dist, speed_progress, target_speed, current_lanelet_id,
            time_progress));
        }
    ...
    // lane change finished, use lane following plan
      ...
      while (current_progress < total_maneuver_length) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "Same Lane Maneuver for platoon join ! ");
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "current_progress: " << current_progress);
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "speed_progress: " << speed_progress);
        ...
        // Note: The previous plan was generated at the beginning of the trip. It is necessary to
        // update
  ...
  // same-lane maneuver
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Planning Same Lane Maneuver! ");
    while (current_progress < total_maneuver_length) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "Same Lane Maneuver for platoon join ! ");
      RCLCPP_DEBUG_STREAM(
      ...
      resp.new_plan.maneuvers.push_back(composeManeuverMessage(
        current_progress, end_dist, speed_progress, target_speed, current_lanelet_id,
        time_progress));
  ...
  if (pm_.getHostPlatoonSize() < 2 && !safeToLaneChange_) {
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "Platoon size 1 so Empty maneuver sent");
  } else {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("platooning_strategic_ihp"), "Planning maneuvers: ");
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "safeToLaneChange_: " << safeToLaneChange_);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "pm_.getHostPlatoonSize(): " << pm_.getHostPlatoonSize());
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "current_downtrack: " << current_downtrack_);

  return true;
}

bool PlatoonStrategicIHPPlugin::is_lanechange_possible(
  lanelet::Id start_lanelet_id, lanelet::Id target_lanelet_id)
{
  ...
  while (!shared_boundary_found) {
    ...
    if (current_lanelet.leftBound() == ending_lanelet.rightBound()) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Lanelet " << std::to_string(current_lanelet.id()) << " shares left boundary with "
                   << std::to_string(ending_lanelet.id()));
      shared_boundary_found = true;
    } else if (current_lanelet.rightBound() == ending_lanelet.leftBound()) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Lanelet " << std::to_string(current_lanelet.id()) << " shares right boundary with "
                   << std::to_string(ending_lanelet.id()));
      shared_boundary_found = true;
    }

    else {
      // If there are no following lanelets on route, lanechange should be completing before
      // reaching it
      if (wm_->getMapRoutingGraph()->following(current_lanelet, false).empty()) {
        // In this case we have reached a lanelet which does not have a routable lanelet ahead +
        // isn't adjacent to the lanelet where lane change ends
        return false;
      }
...
void PlatoonStrategicIHPPlugin::setConfig(const PlatoonPluginConfig & config) { config_ = config; }
