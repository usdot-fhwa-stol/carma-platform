...
TEST(pure_pursuit_jerk_wrapper, trajectoryPlanHandler)
{
  ...
  pure_pursuit_jerk_wrapper::PurePursuitJerkWrapper ppw(config, [&wp_msg](auto msg) { wp_msg = msg; },
...
TEST(pure_pursuit_wrapper, onSpin)
{
  ...
  ASSERT_EQ(0, msg.name.compare("Pure Pursuit Jerk"));
  ...
  ASSERT_EQ(0, msg.capability.compare("control_pure_pursuit_jerk_plan/plan_controls"));
}
