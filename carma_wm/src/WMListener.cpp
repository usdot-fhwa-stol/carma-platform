...
WMListener::WMListener(
  ...
  rclcpp::node_interfaces::NodeParametersInterface::SharedPtr node_params_, bool multi_thread)
  ...
  multi_threaded_(multi_thread)
{
  ...
  if (!node_params_->get_parameter("config_speed_limit", config_speed_limit_param)) {
    ...
    config_speed_limit_param_value =
      node_params_->declare_parameter("config_speed_limit", rclcpp::ParameterValue(0.0));
  }
  ...
  if (!node_params_->get_parameter("vehicle_participant_type", participant_param)) {
    ...
    participant_param_value =
      node_params_->declare_parameter("vehicle_participant_type", rclcpp::ParameterValue(""));
  }
  ...
  if (!node_params_->get_parameter("use_sim_time", use_sim_time_param)) {
    ...
    use_sim_time_param_value =
      node_params_->declare_parameter("use_sim_time", rclcpp::ParameterValue(false));
  }
  ...
  if (!node_params_->get_parameter("is_spat_wall_time", is_spat_wall_time_param)) {
    ...
    is_spat_wall_time_param_value =
      node_params_->declare_parameter("is_spat_wall_time", rclcpp::ParameterValue(true));
  }
  ...
  RCLCPP_INFO_STREAM(
    node_logging->get_logger(), "Is using simulation time? : " << use_sim_time_param.as_bool());
  RCLCPP_INFO_STREAM(
    node_logging->get_logger(), "Is SPaT using wall time? : " << is_spat_wall_time_param.as_bool());
  ...
  if (multi_threaded_) {
    RCLCPP_DEBUG_STREAM(
      node_logging_->get_logger(), "WMListener: Using a multi-threaded subscription");

    map_update_options.callback_group =
      node_base_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

    map_options.callback_group =
      node_base_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

    route_options.callback_group =
      node_base_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

    roadway_objects_options.callback_group =
      node_base_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

    traffic_spat_options.callback_group =
      node_base_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

    ros1_clock_options.callback_group =
      node_base_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

    sim_clock_options.callback_group =
      node_base_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  }

  // Setup subscribers
  route_sub_ = rclcpp::create_subscription<carma_planning_msgs::msg::Route>(
    node_topics_, "route", 1,
    [this](const carma_planning_msgs::msg::Route::SharedPtr msg) {
      this->worker_->routeCallback(msg);
    },
    route_options);

  ros1_clock_sub_ = rclcpp::create_subscription<rosgraph_msgs::msg::Clock>(
    node_topics_, "/clock", 1,
    [this](const rosgraph_msgs::msg::Clock::SharedPtr msg) {
      this->worker_->ros1ClockCallback(msg);
    },
    ros1_clock_options);

  sim_clock_sub_ = rclcpp::create_subscription<rosgraph_msgs::msg::Clock>(
    node_topics_, "/sim_clock", 1,
    [this](const rosgraph_msgs::msg::Clock::SharedPtr msg) {
  ...
  roadway_objects_sub_ =
    rclcpp::create_subscription<carma_perception_msgs::msg::RoadwayObstacleList>(
      node_topics_, "roadway_objects", 1,
  ...
  traffic_spat_sub_ = rclcpp::create_subscription<carma_v2x_msgs::msg::SPAT>(
    node_topics_, "incoming_spat", 1,
  ...
  map_update_options.use_intra_process_comm =
    rclcpp::IntraProcessSetting::Disable;  // Disable intra-process comms for the map update
                                           // subscriber
  auto map_update_sub_qos =
    rclcpp::QoS(rclcpp::KeepLast(10));  // Set the queue size for the map update subscriber
  map_update_sub_qos
    .transient_local();  // If it is possible that this node is a late-joiner to its topic, it must
                         // be set to transient_local to receive earlier messages that were missed.
                         // NOTE: The publisher's QoS must be set to transisent_local() as well for
                         // earlier messages to be resent to this later-joiner.

  // Create map update subscriber that will receive earlier messages that were missed ONLY if the
  // publisher is transient_local too
  map_update_sub_ = rclcpp::create_subscription<autoware_lanelet2_msgs::msg::MapBin>(
    node_topics_, "map_update", map_update_sub_qos,
    [this](const autoware_lanelet2_msgs::msg::MapBin::SharedPtr msg) {
      this->mapUpdateCallback(msg);
    },
    map_update_options);

  map_options.use_intra_process_comm =
    rclcpp::IntraProcessSetting::Disable;  // Disable intra-process comms for the semantic map
                                           // subscriber
  auto map_sub_qos =
    rclcpp::QoS(rclcpp::KeepLast(2));  // Set the queue size for the semantic map subscriber
  map_sub_qos
    .transient_local();  // If it is possible that this node is a late-joiner to its topic, it must
                         // be set to transient_local to receive earlier messages that were missed.
                         // NOTE: The publisher's QoS must be set to transisent_local() as well for
  ...
  // Create semantic mab subscriber that will receive earlier messages that were missed ONLY if the
  // publisher is transient_local too
  map_sub_ = rclcpp::create_subscription<autoware_lanelet2_msgs::msg::MapBin>(
    node_topics_, "semantic_map", map_sub_qos,
    [this](const autoware_lanelet2_msgs::msg::MapBin::SharedPtr msg) {
...
void WMListener::mapUpdateCallback(autoware_lanelet2_msgs::msg::MapBin::SharedPtr geofence_msg)
{
  ...
  RCLCPP_INFO_STREAM(
    node_logging_->get_logger(), "New Map Update Received. SeqNum: " << geofence_msg->seq_id);
