...
StopControlledIntersectionTacticalPlugin::StopControlledIntersectionTacticalPlugin(
  ...
  config_(StopControlledIntersectionTacticalPluginConfig())
{
  ...
  config_.centerline_sampling_spacing =
    declare_parameter<double>("centerline_sampling_spacing", config_.centerline_sampling_spacing);
...
StopControlledIntersectionTacticalPlugin::parameter_update_callback(
...
carma_ros2_utils::CallbackReturn StopControlledIntersectionTacticalPlugin::on_configure_plugin()
{
  ...
  add_on_set_parameters_callback(std::bind(
    &StopControlledIntersectionTacticalPlugin::parameter_update_callback, this, std_ph::_1));

  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
    "Done loading parameters: " << config_);
  ...
  carma_planning_msgs::srv::PlanTrajectory::Response::SharedPtr resp)
{
  ...
  if (req->maneuver_index_to_plan >= req->maneuver_plan.maneuvers.size()) {
    throw std::invalid_argument(
      "Stop Control Intersection Plugin asked to plan invalid maneuver index: " +
  ...
  for (size_t i = req->maneuver_index_to_plan; i < req->maneuver_plan.maneuvers.size(); i++) {
    if (
      (req->maneuver_plan.maneuvers[i].type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING ||
       req->maneuver_plan.maneuvers[i].type ==
         carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_STRAIGHT ||
       req->maneuver_plan.maneuvers[i].type ==
         carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_LEFT_TURN ||
       req->maneuver_plan.maneuvers[i].type ==
         carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_RIGHT_TURN) &&
      GET_MANEUVER_PROPERTY(
        req->maneuver_plan.maneuvers[i], parameters.string_valued_meta_data.front()) ==
  ...
  std::vector<PointSpeedPair> points_and_target_speeds =
    maneuvers_to_points(maneuver_plan, wm_, req->vehicle_state);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
    "Maneuver to points size:" << points_and_target_speeds.size());
  // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
  // "Printing points: ");
  ...
  trajectory.trajectory_points = compose_trajectory_from_centerline(
    points_and_target_speeds, req->vehicle_state, req->header.stamp);
...
std::vector<PointSpeedPair> StopControlledIntersectionTacticalPlugin::maneuvers_to_points(
  ...
  const carma_wm::WorldModelConstPtr & wm, const carma_planning_msgs::msg::VehicleState & state)
{
  ...
  for (const auto & maneuver : maneuvers) {
    if (
      ...
      maneuver.type != carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_STRAIGHT &&
    ...
    // std::min call here is a guard against starting_downtrack being within 1m of the maneuver
    // end_dist in this case the sampleRoutePoints method will return a single point allowing
    // execution to continue
    std::vector<lanelet::BasicPoint2d> route_points = wm->sampleRoutePoints(
      std::min(
        starting_downtrack + config_.centerline_sampling_spacing,
        GET_MANEUVER_PROPERTY(maneuver, end_dist)),
    ...
    if (case_num == 1) {
      points_and_target_speeds =
        create_case_one_speed_profile(wm, maneuver, route_points, starting_speed, state);
    } else if (case_num == 2) {
      points_and_target_speeds =
        create_case_two_speed_profile(wm, maneuver, route_points, starting_speed);
    } else if (case_num == 3) {
      points_and_target_speeds =
        create_case_three_speed_profile(wm, maneuver, route_points, starting_speed);
    } else {
      throw std::invalid_argument(
        "The stop controlled intersection tactical plugin doesn't handle the case number "
        "requested");
    }
  }
...
std::vector<PointSpeedPair> StopControlledIntersectionTacticalPlugin::create_case_one_speed_profile(
  const carma_wm::WorldModelConstPtr & wm, const carma_planning_msgs::msg::Maneuver & maneuver,
...
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"), "Planning for Case One");
  ...
  double a_dec =
    GET_MANEUVER_PROPERTY(maneuver, parameters.float_valued_meta_data[1]);  // a_dec is a -ve value
  ...
  double route_starting_downtrack =
    wm->routeTrackPos(state_point).downtrack;  // Starting downtrack based on geometry points
  ...
  for (size_t i = 1; i < route_geometry_points.size(); i++) {
    ...
    if (total_dist_covered <= dist_acc) {
      ...
      // Deceleration part
      speed_i = sqrt(std::max(
        pow(speed_before_decel, 2) + 2 * a_dec * (total_dist_covered - dist_acc),
        0.0));  // std::max to ensure negative value is not sqrt
      if (speed_i < epsilon_) {
        speed_i = 0.0;
      }
    }
...
std::vector<PointSpeedPair> StopControlledIntersectionTacticalPlugin::create_case_two_speed_profile(
  const carma_wm::WorldModelConstPtr & wm, const carma_planning_msgs::msg::Maneuver & maneuver,
  std::vector<lanelet::BasicPoint2d> & route_geometry_points, double starting_speed)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"), "Planning for Case Two");
  // Derive meta data values from maneuver message - Using order in sci_strategic_plugin
  double a_acc = GET_MANEUVER_PROPERTY(maneuver, parameters.float_valued_meta_data[0]);
  double a_dec =
    GET_MANEUVER_PROPERTY(maneuver, parameters.float_valued_meta_data[1]);  // a_dec is a -ve value
  ...
  double route_starting_downtrack = wm->routeTrackPos(route_geometry_points[0])
                                      .downtrack;  // Starting downtrack based on geometry points
  ...
  for (auto route_point : route_geometry_points) {
    ...
    if (total_dist_planned < dist_acc) {
      ...
      dist_cruise > 0 && total_dist_planned >= dist_acc &&
      total_dist_planned <= (dist_acc + dist_cruise)) {
      // Cruising part
      speed_i = prev_speed;
    } else {
      // Deceleration part
      speed_i = sqrt(std::max(
        pow(speed_before_decel, 2) + 2 * a_dec * (total_dist_planned - dist_acc - dist_cruise),
        0.0));  // std::max to ensure negative value is not sqrt
    }
...
std::vector<PointSpeedPair>
StopControlledIntersectionTacticalPlugin::create_case_three_speed_profile(
  const carma_wm::WorldModelConstPtr & wm, const carma_planning_msgs::msg::Maneuver & maneuver,
  std::vector<lanelet::BasicPoint2d> & route_geometry_points, double starting_speed)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"), "Planning for Case three");
  // Derive meta data values from maneuver message - Using order in sci_strategic_plugin
  ...
  for (size_t i = 0; i < route_geometry_points.size(); i++) {
    ...
    total_dist_covered += delta_d;
    // Find speed at dist covered
    double speed_i = sqrt(std::max(
      pow(starting_speed, 2) + 2 * a_dec * total_dist_covered,
      0.0));  // std::max to ensure negative value is not sqrt
...
StopControlledIntersectionTacticalPlugin::compose_trajectory_from_centerline(
  ...
  const rclcpp::Time & state_time)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
    "VehicleState: "
      << " x: " << state.x_pos_global << " y: " << state.y_pos_global
      << " yaw: " << state.orientation << " speed: " << state.longitudinal_vel);

  int nearest_pt_index =
    basic_autonomy::waypoint_generation::get_nearest_point_index(points, state);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
  ...
  auto time_bound_points = basic_autonomy::waypoint_generation::constrain_to_time_boundary(
    future_points, config_.trajectory_time_length);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
  ...
  std::vector<PointSpeedPair> back_and_future =
    attach_past_points(points, time_bound_points, nearest_pt_index, config_.back_distance);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
  ...
  std::unique_ptr<basic_autonomy::smoothing::SplineI> fit_curve =
    basic_autonomy::waypoint_generation::compute_fit(
      curve_points);  // Compute splines based on curve points
  if (!fit_curve) {
    throw std::invalid_argument("Could not fit a spline curve along the trajectory!");
  }

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"), "Got fit");
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
  ...
  auto total_step_along_curve =
    static_cast<int>(downtracks_raw.back() / config_.curve_resample_step_size);
  ...
  for (size_t steps_along_curve = 0; steps_along_curve < total_step_along_curve;
       steps_along_curve++)  // Resample curve at tighter resolution
  {
    ...
    double c = basic_autonomy::waypoint_generation::compute_curvature_at(
      (*fit_curve), scaled_steps_along_curve);
  ...
  std::vector<double> curvatures = basic_autonomy::smoothing::moving_average_filter(
    better_curvature, config_.curvature_moving_average_window_size, false);
  ...
  std::vector<double> constrained_speed_limits =
    basic_autonomy::waypoint_generation::apply_speed_limits(
      ideal_speeds, distributed_speed_limits);  // Speed min(ideal, calculated)
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("stop_controlled_intersection_tactical_plugin"),
  ...
  // Drop Past points
  nearest_pt_index = basic_autonomy::waypoint_generation::get_nearest_index_by_downtrack(
    all_sampling_points, wm_, state);
  std::vector<lanelet::BasicPoint2d> future_basic_points(
    all_sampling_points.begin() + nearest_pt_index + 1, all_sampling_points.end());
  std::vector<double> future_speeds(
    final_actual_speeds.begin() + nearest_pt_index + 1, final_actual_speeds.end());
  std::vector<double> future_yaw(
    final_yaw_values.begin() + nearest_pt_index + 1, final_yaw_values.end());
  ...
  future_basic_points.insert(
    future_basic_points.begin(),
  ...
  final_actual_speeds = basic_autonomy::smoothing::moving_average_filter(
    future_speeds, config_.speed_moving_average_window_size);
  ...
  std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> traj_points =
    basic_autonomy::waypoint_generation::trajectory_from_points_times_orientations(
      future_basic_points, times, future_yaw, state_time, "default");

  return traj_points;
}

bool StopControlledIntersectionTacticalPlugin::get_availability() { return true; }

std::string StopControlledIntersectionTacticalPlugin::get_version_id() { return "v1.0"; }
