...
{
ArbitratorNode::ArbitratorNode(const rclcpp::NodeOptions & options)
: carma_ros2_utils::CarmaLifecycleNode(options)
{
  ...
  config_.min_plan_duration =
    declare_parameter<double>("min_plan_duration", config_.min_plan_duration);
  config_.target_plan_duration =
    declare_parameter<double>("target_plan_duration", config_.target_plan_duration);
  config_.planning_frequency =
    declare_parameter<double>("planning_frequency", config_.planning_frequency);
  ...
  config_.plugin_priorities =
    plugin_priorities_map_from_json(declare_parameter<std::string>("plugin_priorities", ""));
}

carma_ros2_utils::CallbackReturn ArbitratorNode::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  auto png =
    std::make_shared<arbitrator::PluginNeighborGenerator<arbitrator::CapabilitiesInterface>>(ci);
  arbitrator::TreePlanner tp(
    cf, png, bss, rclcpp::Duration::from_nanoseconds(config_.target_plan_duration * 1e9));
  ...
  twist_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>(
    "current_velocity", 1,
    std::bind(&Arbitrator::twist_cb, arbitrator_.get(), std::placeholders::_1));
...
carma_ros2_utils::CallbackReturn ArbitratorNode::handle_on_activate(const rclcpp_lifecycle::State &)
{
  bumper_pose_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(100), [this]() { this->arbitrator_->bumper_pose_cb(); });

  arbitrator_run_ = create_timer(
    get_clock(),
    std::chrono::duration<double>(
      1 / (config_.planning_frequency * 2)),  // there is waiting state between each planning state
    [this]() { this->arbitrator_->run(); });
  // The intention is to keep the arbitrator planning at intended frequency - 1s. Looks like ROS2
  // conversion kept the WAITING state from ROS1, which occurs between PLANNING state in the state
  // machine. So if the timer calls state callback each 1 second, the arbitrator ends up planning
  // every 2s due to PLANNING > WAITING > PLANNING transition. That's why the frequency is doubled.
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("arbitrator"), "Arbitrator started, beginning arbitrator state machine.");
  return CallbackReturn::SUCCESS;
}

std::map<std::string, double> ArbitratorNode::plugin_priorities_map_from_json(
  const std::string & json_string)
{
  ...
  if (!d.HasMember("plugin_priorities")) {
    RCLCPP_WARN(
      rclcpp::get_logger("arbitrator"), "No plugin_priorities found in arbitrator config");
    return map;
  }
