...
DriverManager::DriverManager(
  const std::vector<std::string> & critical_driver_names, const long driver_timeout)
  ...
  driver_timeout_(driver_timeout)
{
  ...
  em_ = std::make_shared<EntryManager>(critical_driver_names);
}

carma_msgs::msg::SystemAlert DriverManager::handle_spin(
...
void DriverManager::update_driver_status(
  const carma_driver_msgs::msg::DriverStatus::SharedPtr msg, long current_time)
{
  // update driver status is only called in response to a message received on driver_discovery. This
  // topic is only being published in ros1
  Entry driver_status(
...
void DriverManager::evaluate_sensor(
...
std::string DriverManager::are_critical_drivers_operational(long current_time)
{
  ...
  for (auto i = driver_list.begin(); i < driver_list.end(); ++i) {
    RCLCPP_DEBUG_STREAM(
      ...
      "i->name_: " << i->name_ << ", current_time: " << current_time << ", i->timestamp_: "
                   << i->timestamp_ << ", difference: " << current_time - (i->timestamp_));
