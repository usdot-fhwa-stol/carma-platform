...
TEST(YieldPluginTest, compose_mobility_response)
{
  ...
  YieldPlugin plugin(
    nh, wm, config, [&](auto msg) {}, [&](auto msg) {});
  carma_v2x_msgs::msg::MobilityResponse resp =
    plugin.compose_mobility_response("recicpient_id", "plan_id", true);
...
TEST(YieldPluginTest, test_detect_trajectories_intersection)
{
  ...
  std::vector<std::pair<int, lanelet::BasicPoint2d>> output =
    plugin.detect_trajectories_intersection(v1, v2);
...
TEST(YieldPluginTest, test_update_clc_trajectory)
{
  ...
  carma_planning_msgs::msg::TrajectoryPlan yield_plan =
    plugin.update_traj_for_cooperative_behavior(original_tp, current_speed);
  ...
  EXPECT_TRUE(
    rclcpp::Time(yield_plan.trajectory_points[2].target_time) >
    rclcpp::Time(original_tp.trajectory_points[2].target_time));
  EXPECT_TRUE(
    rclcpp::Time(yield_plan.trajectory_points[3].target_time) >
    rclcpp::Time(original_tp.trajectory_points[3].target_time));
}
...
TEST(YieldPluginTest, test_traj_cb)
{
  ...
  std::shared_ptr<carma_planning_msgs::srv::PlanTrajectory::Request> req =
    std::make_shared<carma_planning_msgs::srv::PlanTrajectory::Request>();
  ...
  std::shared_ptr<carma_planning_msgs::srv::PlanTrajectory::Response> resp =
    std::make_shared<carma_planning_msgs::srv::PlanTrajectory::Response>();
