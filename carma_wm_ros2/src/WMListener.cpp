    ...
    :node_base_(node_base),node_logging_(node_logging),node_topics_(node_topics),worker_(std::unique_ptr<WMListenerWorker>(new WMListenerWorker)), multi_threaded_(multi_thread)
{
  ...
  // Get params
  config_speed_limit_param = node_params_->get_parameter("config_speed_limit", 0.0);
  participant_param = node_params_->get_parameter("vehicle_participant_type", "vehicle:car");
