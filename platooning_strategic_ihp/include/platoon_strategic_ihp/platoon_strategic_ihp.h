...
/**
 * \brief Class containing the logic for Strategic IHP Plugin. This class is responsible for all the
 * negotiations as well as generating a maneuver msg.
 */
...
public:
   ...
   * \brief Constructor
   *
   * \param wm Pointer to initalized instance of the carma world model for accessing semantic map
   * data \param config The configuration to be used for this object
   */
   ...
   */
  carma_planning_msgs::msg::Maneuver composeManeuverMessage(
    double current_dist, double end_dist, double current_speed, double target_speed, int lane_id,
    rclcpp::Time & current_time);

  /**
   * \brief Find start(current) and target(end) lanelet index from path to generate lane change
   * maneuver message.
   *
   ...
   * \param ending_lane_id: target lanelet id which is also the ending lanelet id that is in another
   * lane \param current_time: current time in seconds
   *
   * \return Maneuver message
   */
  ...
  *       include vehicles in different "platoon strategic plugin states" as long as the current
  state is relavent
  ...
  *       vehicle communication in a more refined manner that can achieve the "platooning info
  status'" objective. Hence,
  *       vehicles in different states will behave differently based on the corresponding roles and
  predefined rules.
  *       However, they can belong to the same "platooning info statu" for the same operation
  purpose.
  *
  *       [Example]
  *       Multiple strategic states, such as "leader aborting" and "candidate follower", can both be
  mapped to
  *       "connecting to new leader" platooning info status, as both states are serving the same
...
private:
   ...
   *        note: This is only applicable for same lane application, so it will check cross track
   * distance.
   *
   ...
   * \return ACK, NACK, or No response
   */
  MobilityRequestResponse mob_req_cb_candidatefollower(
    const carma_v2x_msgs::msg::MobilityRequest & msg);
  ...
  /**
   * \brief The method for platoon leader to determine if the joining vehicle is closeby. (Note:
   * Used when host vehicle is the platoon leader)
   *
   *
  ...
  /**
   * \brief Function to determine if the host vehicle is close to the target platoon (used for
   * cut-in join scenarios).
   *
   *
   ...
   * \return (boolean): if the host vehicle is close to the target platoon.
   */
  bool isVehicleNearTargetPlatoon(
    double rearVehicleDtd, double frontVehicleDtd, double frontVehicleCtd);

  /**
   * \brief Function to find the starting and ending lanelet ID for lane change in a two-lane
   * scenario (used for cut-in join scenarios).
   *
   * Note: This is a temporary function for internal test only. The scenario is not generalized. Can
   * only find adjacent lanletID based on predefined direction (left, right).
   *
   ...
   */
  carma_v2x_msgs::msg::MobilityOperation composeMobilityOperationLeadWithOperation(
  ...
  /**
   * \brief Function to process mobility operation INFO params to find platoon leader's ecef
   * location.
   *
   * \param strategyParams The parsed strategy params, used to find ecef locaton.
   *
   ...
   * \return ACK, NACK, or No response.
   */
  MobilityRequestResponse mob_req_cb_leaderaborting(
    const carma_v2x_msgs::msg::MobilityRequest & msg);
   ...
   * \return ACK, NACK, or No response.
   */
  MobilityRequestResponse mob_req_cb_candidateleader(
    const carma_v2x_msgs::msg::MobilityRequest & msg);
   ...
   * \return ACK, NACK, or No response
   */
  MobilityRequestResponse mob_req_cb_leadwithoperation(
    const carma_v2x_msgs::msg::MobilityRequest & msg);
   ...
   * \return ACK, NACK, or No response
   */
  MobilityRequestResponse mob_req_cb_preparetojoin(
    const carma_v2x_msgs::msg::MobilityRequest & msg);
  ...
  // UCLA: add member variable for state prepare to join (default -2, front join -1, mid/rear join
  // other integer)
