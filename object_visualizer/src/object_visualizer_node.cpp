...
Node::Node(const rclcpp::NodeOptions & options) : carma_ros2_utils::CarmaLifecycleNode(options)
{
  ...
  // Declare parameters
  config_.enable_external_objects_viz =
    declare_parameter<bool>("enable_external_objects_viz", config_.enable_external_objects_viz);
  config_.enable_roadway_objects_viz =
    declare_parameter<bool>("enable_roadway_objects_viz", config_.enable_roadway_objects_viz);
  config_.external_objects_viz_ns =
    declare_parameter<std::string>("external_objects_viz_ns", config_.external_objects_viz_ns);
  config_.roadway_obstacles_viz_ns =
    declare_parameter<std::string>("roadway_obstacles_viz_ns", config_.roadway_obstacles_viz_ns);
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  auto error3 = update_params<uint8_t>({{"marker_shape", config_.marker_shape}}, parameters);
...
carma_ros2_utils::CallbackReturn Node::handle_on_configure(const rclcpp_lifecycle::State &)
{
  ...
  external_objects_sub_ = create_subscription<carma_perception_msgs::msg::ExternalObjectList>(
    "external_objects", 10, std::bind(&Node::external_objects_callback, this, std_ph::_1));

  roadway_obstacles_sub_ = create_subscription<carma_perception_msgs::msg::RoadwayObstacleList>(
    "roadway_obstacles", 10, std::bind(&Node::roadway_obstacles_callback, this, std_ph::_1));

  // Setup publishers
  external_objects_viz_pub_ =
    create_publisher<visualization_msgs::msg::MarkerArray>("external_objects_viz", 10);
  roadway_obstacles_viz_pub_ =
    create_publisher<visualization_msgs::msg::MarkerArray>("roadway_obstacles_viz", 10);
...
void Node::external_objects_callback(carma_perception_msgs::msg::ExternalObjectList::UniquePtr msg)
{
  ...
  if (!config_.enable_external_objects_viz) {
    RCLCPP_DEBUG_STREAM(
      get_logger(), "external_objects_callback called, but visualization is not enabled.");
    return;
  }
  ...
  size_t id =
    0;  // We always count the id from zero so we can delete markers later in a consistent manner
  ...
  for (auto obj : msg->objects) {
    ...
    marker.pose.position.z =
      std::max(1.0, obj.size.z);  // lifts the marker above ground so that it doesn't clip

    marker.id = id;
    if (static_cast<int>(config_.marker_shape) < 1 || static_cast<int>(config_.marker_shape) > 3) {
      throw std::invalid_argument(
        "Marker shape is not valid: " + std::to_string(static_cast<int>(config_.marker_shape)) +
    ...
    marker.scale.x =
      std::max(1.0, obj.size.x) * 2;  // Size in carma is half the length/width/height
...
void Node::clear_and_update_old_objects(
  visualization_msgs::msg::MarkerArray & viz_msg, size_t & old_size)
{
...
void Node::roadway_obstacles_callback(
  carma_perception_msgs::msg::RoadwayObstacleList::UniquePtr msg)
{
  RCLCPP_DEBUG_STREAM(get_logger(), "roadway_obstacles_callback called");

  if (!config_.enable_roadway_objects_viz) {
    RCLCPP_DEBUG_STREAM(
      get_logger(), "roadway_obstacles_callback called, but visualization is not enabled.");
    return;
  }
  ...
  for (auto obj : msg->roadway_obstacles) {
    ...
    if (obj.object.object_type == carma_perception_msgs::msg::ExternalObject::PEDESTRIAN) {
      ...
      obj.connected_vehicle_type.type !=
