...
rcl_interfaces::msg::SetParametersResult MobilityPathPublication::parameter_update_callback(
...
carma_ros2_utils::CallbackReturn MobilityPathPublication::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  add_on_set_parameters_callback(
    std::bind(&MobilityPathPublication::parameter_update_callback, this, std_ph::_1));

  // Setup subscribers
  traj_sub_ = create_subscription<carma_planning_msgs::msg::TrajectoryPlan>(
    "plan_trajectory", 5, std::bind(&MobilityPathPublication::trajectory_cb, this, std_ph::_1));
  bsm_sub_ = create_subscription<carma_v2x_msgs::msg::BSM>(
    "bsm_outbound", 1, std::bind(&MobilityPathPublication::bsm_cb, this, std_ph::_1));

  guidance_state_sub_ = create_subscription<carma_planning_msgs::msg::GuidanceState>(
    "guidance_state", 5,
    std::bind(&MobilityPathPublication::guidance_state_cb, this, std::placeholders::_1));

  georeference_sub_ = create_subscription<std_msgs::msg::String>(
    "georeference", 1, std::bind(&MobilityPathPublication::georeference_cb, this, std_ph::_1));
...
void MobilityPathPublication::guidance_state_cb(
...
carma_ros2_utils::CallbackReturn MobilityPathPublication::handle_on_activate(
  const rclcpp_lifecycle::State & prev_state)
{
  // Timer setup
  int path_pub_period_millisecs =
    (1 / config_.path_pub_rate) *
    1000;  // Conversion from frequency (Hz) to milliseconds time period
  path_pub_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(path_pub_period_millisecs),
...
bool MobilityPathPublication::spin_callback()
{
  ...
  if (guidance_engaged_) path_pub_->publish(latest_mobility_path_);
  return true;
}
...
void MobilityPathPublication::trajectory_cb(
...
carma_v2x_msgs::msg::MobilityPath MobilityPathPublication::mobility_path_message_generator(
  const carma_planning_msgs::msg::TrajectoryPlan & trajectory_plan)
{
  carma_v2x_msgs::msg::MobilityPath mobility_path_msg;
  // TODO this caluclation uses a poor assumption of zero latency see
  // https://github.com/usdot-fhwa-stol/carma-platform/issues/1606
  uint64_t millisecs = get_clock()->now().nanoseconds() / 1000000;
  mobility_path_msg.m_header = compose_mobility_header(millisecs);

  if (!map_projector_) {
    RCLCPP_ERROR_STREAM(
      this->get_logger(), "MobilityPath cannot be populated as map projection is not available");
    return mobility_path_msg;
  }

  carma_v2x_msgs::msg::Trajectory mob_path_traj =
    trajectory_plan_to_trajectory(trajectory_plan.trajectory_points);
...
carma_v2x_msgs::msg::Trajectory MobilityPathPublication::trajectory_plan_to_trajectory(
  const std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> & traj_points) const
{
  ...
  carma_v2x_msgs::msg::LocationECEF ecef_location =
    trajectory_point_to_ECEF(traj_points[0]);  // m to cm to fit the msg standard
...
carma_v2x_msgs::msg::LocationECEF MobilityPathPublication::trajectory_point_to_ECEF(
