...
std::pair<bool, rclcpp::Time> GeofenceSchedule::getNextInterval(const rclcpp::Time& time, const rclcpp::Logger& logger) const
{
  auto clock_type = time.get_clock_type();
  if (scheduleExpired(time))
  {
    RCLCPP_DEBUG_STREAM(logger, "Geofence schedule expired");
  ...
  {
    RCLCPP_DEBUG_STREAM(logger, "Geofence wrong day of the week");
  ...
  {
    RCLCPP_DEBUG_STREAM(logger, "Geofence schedule too late in the day");
  ...
  while (cur_start < full_day && ros_time_of_day > rclcpp::Time(cur_start.nanoseconds(), clock_type))
  {
    ...
    {
      RCLCPP_DEBUG_STREAM(logger, "Geofence schedule active!");
  ...
  {
    RCLCPP_DEBUG_STREAM(logger, "Geofence schedule beyond end time");
