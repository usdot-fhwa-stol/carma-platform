...
{
  // Declare parameters
  base_config_.service_timeout_ms =
    this->declare_parameter<int>("service_timeout_ms", base_config_.service_timeout_ms);
  base_config_.call_timeout_ms =
    this->declare_parameter<int>("call_timeout_ms", base_config_.call_timeout_ms);
  base_config_.required_subsystem_nodes = this->declare_parameter<std::vector<std::string>>(
    "required_subsystem_nodes", base_config_.required_subsystem_nodes);
  base_config_.subsystem_namespace =
    this->declare_parameter<std::string>("subsystem_namespace", base_config_.subsystem_namespace);
  base_config_.full_subsystem_required =
    this->declare_parameter<bool>("full_subsystem_required", base_config_.full_subsystem_required);
  base_config_.unmanaged_required_nodes = this->declare_parameter<std::vector<std::string>>(
    "unmanaged_required_nodes", base_config_.unmanaged_required_nodes);

  // Handle fact that parameter vectors cannot be empty
  if (
    base_config_.required_subsystem_nodes.size() == 1 &&
    base_config_.required_subsystem_nodes[0].empty()) {
    base_config_.required_subsystem_nodes.clear();
  }

  if (
    base_config_.unmanaged_required_nodes.size() == 1 &&
    base_config_.unmanaged_required_nodes[0].empty()) {
...
{
  RCLCPP_INFO_STREAM(
    get_logger(), "Received SystemAlert message of type: "
                    << static_cast<int>(msg->type) << " with message: " << msg->description);
  ...
  if (msg->type == carma_msgs::msg::SystemAlert::FATAL) {
    ...
    if (
      std::find(
        base_config_.required_subsystem_nodes.begin(), base_config_.required_subsystem_nodes.end(),
        msg->source_node) != base_config_.required_subsystem_nodes.end() ||
      std::find(
        base_config_.unmanaged_required_nodes.begin(), base_config_.unmanaged_required_nodes.end(),
        msg->source_node) != base_config_.unmanaged_required_nodes.end()) {
      // Our subsystem has failed so notify the overall system controller

      RCLCPP_ERROR_STREAM(get_logger(), "Failure in required node: " << msg->source_node);

      carma_msgs::msg::SystemAlert alert;
      alert.type = carma_msgs::msg::SystemAlert::FATAL;
      alert.description =
        base_config_.subsystem_namespace + " subsytem has failed with error: " + msg->description;
    ...
    } else {  // Optional node has failed
      RCLCPP_WARN_STREAM(get_logger(), "Failure in optional node: " << msg->source_node);
    }
  }
}

carma_ros2_utils::CallbackReturn BaseSubsystemController::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  auto additional_nodes =
    get_non_intersecting_set(base_config_.required_subsystem_nodes, nodes_in_namespace);
  ...
  if (base_config_.full_subsystem_required) {
    RCLCPP_INFO_STREAM(
      get_logger(), "full_subsystem_required is True. Setting all namespace nodes as required");
  ...
  bool success =
    lifecycle_mgr_
      .configure(std_msec(base_config_.service_timeout_ms), std_msec(base_config_.call_timeout_ms))
      .empty();
...
carma_ros2_utils::CallbackReturn BaseSubsystemController::handle_on_activate(
  const rclcpp_lifecycle::State &)
{
  ...
  bool success =
    lifecycle_mgr_
      .activate(std_msec(base_config_.service_timeout_ms), std_msec(base_config_.call_timeout_ms))
      .empty();
...
carma_ros2_utils::CallbackReturn BaseSubsystemController::handle_on_deactivate(
  const rclcpp_lifecycle::State &)
{
  ...
  bool success =
    lifecycle_mgr_
      .deactivate(std_msec(base_config_.service_timeout_ms), std_msec(base_config_.call_timeout_ms))
      .empty();
...
carma_ros2_utils::CallbackReturn BaseSubsystemController::handle_on_cleanup(
  const rclcpp_lifecycle::State &)
{
  ...
  bool success =
    lifecycle_mgr_
      .cleanup(std_msec(base_config_.service_timeout_ms), std_msec(base_config_.call_timeout_ms))
      .empty();
...
carma_ros2_utils::CallbackReturn BaseSubsystemController::handle_on_error(
  const rclcpp_lifecycle::State &, const std::string &)
{
  RCLCPP_INFO_STREAM(get_logger(), "Subsystem trying to shutdown due to error");

  bool success =
    lifecycle_mgr_
      .shutdown(std_msec(base_config_.service_timeout_ms), std_msec(base_config_.call_timeout_ms))
      .empty();
...
carma_ros2_utils::CallbackReturn BaseSubsystemController::handle_on_shutdown(
  const rclcpp_lifecycle::State &)
{
  ...
  bool success =
    lifecycle_mgr_
      .shutdown(
        std_msec(base_config_.service_timeout_ms), std_msec(base_config_.call_timeout_ms), false)
      .empty();
...
std::vector<std::string> BaseSubsystemController::get_nodes_in_namespace(
  const std::string & node_namespace) const
{
  ...
  static const std::string this_full_name =
    std::string(get_namespace()) + "/" + std::string(get_name());
  ...
  for (const auto & node : all_nodes) {
    if (node == this_full_name) continue;  // no need for this node to try managing itself

    if (node.find(node_namespace) == 0) {  // The node is in the provided namespace

      ////
      // In the following section of code we check if the current node in the target namespace is
      // actually a lifecycle node This check is done by evaluating if that nodes exposes the
      // change_state and get_state lifecycle services. If the node does not expose these services
      // then it cannot be managed by this component. However, this does not result in an error as
      // the node could be wrapped by a lifecycle component wrapper
      ////

      // First extract the service names and types for the current node. This requires seperating
      // the node base name and namespace
      ...
      if (
        services_and_types.find(cs_srv) != services_and_types.end() &&
        services_and_types.find(gs_srv) != services_and_types.end() &&
        std::find(
          services_and_types.at(cs_srv).begin(), services_and_types.at(cs_srv).end(),
          CHANGE_STATE_TYPE) != services_and_types.at(cs_srv).end() &&
        std::find(
          services_and_types.at(gs_srv).begin(), services_and_types.at(gs_srv).end(),
          GET_STATE_TYPE) != services_and_types.at(gs_srv).end()) {
        nodes_in_namspace.emplace_back(node);

      } else {
        // Current node is not a lifecycle node so log a warning
        RCLCPP_WARN_STREAM(
          get_logger(), "Failed to find lifecycle services for node: "
                          << node
                          << " this node will not be managed. NOTE: If this node is wrapped by a "
                             "lifecycle component that is managed then this is not an issue.");
...
std::vector<std::string> BaseSubsystemController::get_non_intersecting_set(
  const std::vector<std::string> & set_a, const std::vector<std::string> & set_b) const
{
  ...
  for (const auto & string : set_a) {
    if (set_b_lookup.find(string) == set_b_lookup.end()) {  // No intersection so store
