...

{
  ...
  // Declare parameters
  config_.fitness_score_degraded_threshold = declare_parameter<double>(
    "fitness_score_degraded_threshold", config_.fitness_score_degraded_threshold);
  config_.fitness_score_fault_threshold = declare_parameter<double>(
    "fitness_score_fault_threshold", config_.fitness_score_fault_threshold);
  config_.ndt_frequency_degraded_threshold = declare_parameter<double>(
    "ndt_frequency_degraded_threshold", config_.ndt_frequency_degraded_threshold);
  config_.ndt_frequency_fault_threshold = declare_parameter<double>(
    "ndt_frequency_fault_threshold", config_.ndt_frequency_fault_threshold);
  config_.auto_initialization_timeout =
    declare_parameter<int>("auto_initialization_timeout", config_.auto_initialization_timeout);
  config_.gnss_only_operation_timeout =
    declare_parameter<int>("gnss_only_operation_timeout", config_.gnss_only_operation_timeout);
  config_.sequential_timesteps_until_gps_operation = declare_parameter<int>(
    "sequential_timesteps_until_gps_operation", config_.sequential_timesteps_until_gps_operation);
  config_.gnss_data_timeout =
    declare_parameter<int>("gnss_data_timeout", config_.gnss_data_timeout);
  config_.localization_mode =
    declare_parameter<int>("localization_mode", config_.localization_mode);
...
carma_ros2_utils::CallbackReturn Node::handle_on_configure(const rclcpp_lifecycle::State &)
{
  ...
  manager_.reset(new LocalizationManager(
    std::bind(&Node::publishPoseStamped, this, std_ph::_1),
    std::bind(&Node::publishStatus, this, std_ph::_1),
    std::bind(&Node::publishManagedInitialPose, this, std_ph::_1), config_,
  ...
  gnss_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "gnss_pose", 5, std::bind(&LocalizationManager::gnssPoseCallback, manager_.get(), std_ph::_1));
  ...
  pose_stats_synchronizer_->registerCallback(
    std::bind(&Node::poseAndStatsCallback, this, std_ph::_1, std_ph::_2));
  ...
  state_pub_ = create_publisher<carma_localization_msgs::msg::LocalizationStatusReport>(
  ...
  auto pub_qos_transient_local = rclcpp::QoS(rclcpp::KeepLast(
    1));  // A publisher with this QoS will store all messages that it has sent on the topic
  pub_qos_transient_local
    .transient_local();  // A publisher with this QoS will re-send all (when KeepAll is used)
                         ...
                         // the later-joiner.
  managed_initial_pose_pub_ = create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(
    "managed_initialpose", pub_qos_transient_local, intra_proc_disabled);

  // Setup timer
  pose_timer_ = create_timer(
