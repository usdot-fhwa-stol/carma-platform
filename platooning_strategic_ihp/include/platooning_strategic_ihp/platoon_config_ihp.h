...
struct PlatoonPluginConfig
{
  ...
  bool test_front_join =
    false;  // Flag to enable/disable front join functionality with two vehicles.
  ...
  bool test_cutin_join =
    false;  // Flag to enable/disable front/rear cutin join functionality with only 2 vehicles
  ...
  double significantDTDchange = 0.2;  // Ratio of dtd that is considered a significant change and
                                      // triggers a new sort of platoon vector
  int join_index = -1;  // target join index for cut-in join - used for testing purposes. -1: front
                        // cutin join, target_platoon.size()-1: cut-in rear
