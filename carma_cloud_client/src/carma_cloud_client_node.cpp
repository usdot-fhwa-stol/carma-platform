...
rcl_interfaces::msg::SetParametersResult CarmaCloudClient::parameter_update_callback(
...
carma_ros2_utils::CallbackReturn CarmaCloudClient::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  add_on_set_parameters_callback(
    std::bind(&CarmaCloudClient::parameter_update_callback, this, std_ph::_1));
  // Setup subscribers
  tcr_sub_ = create_subscription<carma_v2x_msgs::msg::TrafficControlRequest>(
    "outgoing_geofence_request", 10, std::bind(&CarmaCloudClient::tcr_callback, this, std_ph::_1));
  tcm_pub_ = create_publisher<j2735_v2x_msgs::msg::TrafficControlMessage>(
...
void CarmaCloudClient::XMLconversion(
  char * xml_str, carma_v2x_msgs::msg::TrafficControlRequest request_msg)
{
  ...
  //  get current time
  std::time_t tm =
    this->now().seconds() / 60 - config_.fetchtime * 24 * 60;  //  T minus fetchtime*24 hours in min
  ...
  while (cnt < totBounds) {
    ...
    int x = std::sprintf(
      bounds_str + strlen(bounds_str),
      "<bounds><TrafficControlBounds><oldest>%ld</oldest><reflon>%ld</reflon><reflat>%ld</"
      "reflat><offsets><OffsetPoint><deltax>%ld</deltax><deltay>%ld</deltay></"
      "OffsetPoint><OffsetPoint><deltax>%ld</deltax><deltay>%ld</deltay></"
      "OffsetPoint><OffsetPoint><deltax>%ld</deltax><deltay>%ld</deltay></OffsetPoint></offsets></"
      "TrafficControlBounds></bounds>",
      oldest, longg, lat, dtx0, dty0, dtx1, dty1, dtx2, dty2);
  ...
  sprintf(
    xml_str,
    "<?xml version=\"1.0\" encoding=\"UTF-8\"?><TrafficControlRequest port=\"%s\" "
    "list=\"%s\"><reqid>%s</reqid><reqseq>%ld</reqseq><scale>%ld</scale>%s</TrafficControlRequest>",
    port, list, reqid, reqseq, scale, bounds_str);

  RCLCPP_DEBUG_STREAM(get_logger(), "xml_str: " << xml_str);
}

int CarmaCloudClient::CloudSend(
  const std::string & local_msg, const std::string & local_url, const std::string & local_base,
  const std::string & local_method)
{
  ...
  if (req) {
    ...
    if (strcmp(local_method.c_str(), "POST") == 0) {
      ...
      curl_easy_setopt(
        req, CURLOPT_TIMEOUT_MS,
        1000L);  // Request operation complete within max millisecond timeout
...
void CarmaCloudClient::CloudSendAsync(
  const std::string & local_msg, const std::string & local_url, const std::string & local_base,
  const std::string & local_method)
{
...
void CarmaCloudClient::TCMHandler(QHttpEngine::Socket * socket)
{
  ...
  while (socket->bytesAvailable() > 0) {
    ...
    if (
      socket->headers().keys().contains(CONTENT_ENCODING_KEY) &&
      std::string(socket->headers().constFind(CONTENT_ENCODING_KEY).value().data()) ==
...
j2735_v2x_msgs::msg::TrafficControlMessage CarmaCloudClient::parseTCMXML(
  boost::property_tree::ptree & tree)
{
  ...
  if (!child_tcmv01) {
    ...
    tcm.choice = j2735_v2x_msgs::msg::TrafficControlMessage::TCMV01;
  ...
  for (std::size_t i = 0; i != reqid_string.size() / 2; ++i) {
    tcm.tcm_v01.reqid.id[i] =
      16 * parse_hex(reqid_string[2 * i]) + parse_hex(reqid_string[2 * i + 1]);
  }
...
int CarmaCloudClient::StartWebService()
{
  ...
  QObject::connect(
    handler.data(), &OpenAPI::OAIApiRequestHandler::requestReceived,
    [&](QHttpEngine::Socket * socket) { TCMHandler(socket); });

  QObject::connect(
    handler.data(), &OpenAPI::OAIApiRequestHandler::requestReceived,
    [&](QHttpEngine::Socket * socket) { router->processRequest(socket); });
...
j2735_v2x_msgs::msg::TrafficControlPackage CarmaCloudClient::parse_package(
  boost::property_tree::ptree & tree)
{
...
j2735_v2x_msgs::msg::TrafficControlSchedule CarmaCloudClient::parse_schedule(
  boost::property_tree::ptree & tree)
{
...
j2735_v2x_msgs::msg::TrafficControlDetail CarmaCloudClient::parse_detail(
  boost::property_tree::ptree & tree)
{
...
j2735_v2x_msgs::msg::TrafficControlParams CarmaCloudClient::parse_params(
  boost::property_tree::ptree & tree)
{
  ...
  for (auto & item : tree.get_child("vclasses")) {
    ...
    if (item.first == "any") {
      ...
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::THREE_AXLE_SINGLE_UNIT_TRUCK;
    } else if (item.first == "four-or-more-axle-single-unit-truck") {
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::FOUR_OR_MORE_AXLE_SINGLE_UNIT_TRUCK;
    } else if (item.first == "four-or-fewer-axle-single-trailer-truck") {
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::FOUR_OR_FEWER_AXLE_SINGLE_TRAILER_TRUCK;
    } else if (item.first == "five-axle-single-trailer-truck") {
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::FIVE_AXLE_SINGLE_TRAILER_TRUCK;
    } else if (item.first == "six-or-more-axle-single-trailer-truck") {
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::SIX_OR_MORE_AXLE_SINGLE_TRAILER_TRUCK;
    } else if (item.first == "five-or-fewer-axle-multi-trailer-truck") {
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::FIVE_OR_FEWER_AXLE_MULTI_TRAILER_TRUCK;
    } else if (item.first == "six-axle-multi-trailer-truck") {
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::SIX_AXLE_MULTI_TRAILER_TRUCK;
    } else if (item.first == "seven-or-more-axle-multi-trailer-truck") {
      vclass.vehicle_class =
        j2735_v2x_msgs::msg::TrafficControlVehClass::SEVEN_OR_MORE_AXLE_MULTI_TRAILER_TRUCK;
  ...
  if (bool_str == "false")
    tcm_params.regulatory = false;
  else
    tcm_params.regulatory = true;
...
j2735_v2x_msgs::msg::TrafficControlGeometry CarmaCloudClient::parse_geometry(
  boost::property_tree::ptree & tree)
{
