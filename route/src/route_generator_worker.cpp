...
void RouteGeneratorWorker::setLoggerInterface(
...
void RouteGeneratorWorker::setClock(rclcpp::Clock::SharedPtr clock) { this->clock_ = clock; }

lanelet::Optional<lanelet::routing::Route> RouteGeneratorWorker::routing(
  const lanelet::BasicPoint2d start, const std::vector<lanelet::BasicPoint2d> & via,
  const lanelet::BasicPoint2d end, const lanelet::LaneletMapConstPtr map_pointer,
  const carma_wm::LaneletRoutingGraphConstPtr graph_pointer) const
{
  ...
  if (start_lanelet_vector.empty()) {
    RCLCPP_ERROR_STREAM(
      logger_->get_logger(), "Found no lanelets in the map. Routing cannot be done.");
  ...
  for (lanelet::BasicPoint2d point : via) {
    ...
    via_lanelets_vector.emplace_back(
      lanelet::ConstLanelet(via_lanelet_vector[0].second.constData()));
  }
  // routing
...
bool RouteGeneratorWorker::getAvailableRouteCb(
  ...
  std::shared_ptr<carma_planning_msgs::srv::GetAvailableRoutes::Response> resp)
{
  ...
  for (boost::filesystem::directory_iterator itr(boost::filesystem::path(this->route_file_path_));
       itr != end_point; ++itr) {
  ...
  // after route path object is available to select, worker will able to transit state and provide
...
bool RouteGeneratorWorker::setActiveRouteCb(
  const std::shared_ptr<rmw_request_id_t>,
  ...
  std::shared_ptr<carma_planning_msgs::srv::SetActiveRoute::Response> resp)
{
  ...
  if (this->rs_worker_.getRouteState() != RouteStateWorker::RouteState::SELECTION) {
    ...
    resp->error_status =
      carma_planning_msgs::srv::SetActiveRoute::Response::ALREADY_FOLLOWING_ROUTE;
  ...
  if (!map_proj_) {
    RCLCPP_ERROR_STREAM(
      logger_->get_logger(), "Could not generate route as there was no map projection available");
    resp->error_status = carma_planning_msgs::srv::SetActiveRoute::Response::ROUTING_FAILURE;
    this->rs_worker_.onRouteEvent(RouteStateWorker::RouteEvent::ROUTE_GEN_FAILED);
    publishRouteEvent(carma_planning_msgs::msg::RouteEvent::ROUTE_GEN_FAILED);
    return true;
  ...
  if (req->choice == carma_planning_msgs::srv::SetActiveRoute::Request::ROUTE_ID) {
    RCLCPP_INFO_STREAM(
      logger_->get_logger(), "set_active_route_cb: Selected Route ID: " << req->route_id);
    std::vector<carma_v2x_msgs::msg::Position3D> gps_destination_points =
      loadRouteDestinationGpsPointsFromRouteId(req->route_id);
    destination_points = loadRouteDestinationsInMapFrame(gps_destination_points);
  } else if (
    req->choice == carma_planning_msgs::srv::SetActiveRoute::Request::DESTINATION_POINTS_ARRAY) {
    RCLCPP_INFO_STREAM(
      logger_->get_logger(),
  ...
  destination_points_in_map_ =
    lanelet::utils::transform(destination_points, [](auto a) { return lanelet::traits::to2D(a); });

  // Add vehicle as first destination point
  auto destination_points_in_map_with_vehicle = destination_points_in_map_;

  lanelet::BasicPoint2d vehicle_position(
    vehicle_pose_->pose.position.x, vehicle_pose_->pose.position.y);
  destination_points_in_map_with_vehicle.insert(
    destination_points_in_map_with_vehicle.begin(), vehicle_position);
  ...
  for (auto pt : destination_points_in_map_with_vehicle) {
    if ((world_model_->getLaneletsFromPoint(pt, 1)).empty()) {
      RCLCPP_ERROR_STREAM(
        logger_->get_logger(),
        "Route Generator: " << idx << "th destination point is not in the map, x: " << pt.x()
                            << " y: " << pt.y());
      resp->error_status = carma_planning_msgs::srv::SetActiveRoute::Response::ROUTE_FILE_ERROR;
      this->rs_worker_.onRouteEvent(RouteStateWorker::RouteEvent::ROUTE_GEN_FAILED);
      publishRouteEvent(carma_planning_msgs::msg::RouteEvent::ROUTE_GEN_FAILED);
  ...
  // generate a route
  auto route = routing(
    destination_points_in_map_with_vehicle.front(),
    std::vector<lanelet::BasicPoint2d>(
      destination_points_in_map_with_vehicle.begin() + 1,
      destination_points_in_map_with_vehicle.end() - 1),
    destination_points_in_map_with_vehicle.back(), world_model_->getMap(),
    world_model_->getMapRoutingGraph());
  ...
  if (checkForDuplicateLaneletsInShortestPath(route.get())) {
    RCLCPP_ERROR_STREAM(
      logger_->get_logger(),
      "At least one duplicate Lanelet ID occurs in the shortest path. Routing cannot be "
      "completed.");
    resp->error_status = carma_planning_msgs::srv::SetActiveRoute::Response::ROUTING_FAILURE;
    this->rs_worker_.onRouteEvent(RouteStateWorker::RouteEvent::ROUTE_GEN_FAILED);
    publishRouteEvent(carma_planning_msgs::msg::RouteEvent::ROUTE_GEN_FAILED);
  ...
  lanelet::Point3d end_point{
    lanelet::utils::getId(), destination_points_in_map_with_vehicle.back().x(),
    destination_points_in_map_with_vehicle.back().y(), 0};
...
bool RouteGeneratorWorker::checkForDuplicateLaneletsInShortestPath(
  const lanelet::routing::Route & route) const
{
  ...
  if (
    std::adjacent_find(shortest_path_lanelet_ids.begin(), shortest_path_lanelet_ids.end()) !=
    shortest_path_lanelet_ids.end()) {
...
std::vector<lanelet::BasicPoint3d> RouteGeneratorWorker::loadRouteDestinationsInMapFrame(
  const std::vector<carma_v2x_msgs::msg::Position3D> & destinations) const
{
  if (!map_proj_) {
    throw std::invalid_argument(
      "loadRouteDestinationsInMapFrame (using destination points array) before map projection was "
      "set");
  }

  lanelet::projection::LocalFrameProjector projector(
    map_proj_.get().c_str());  // Build map projector
...
std::vector<carma_v2x_msgs::msg::Position3D>
RouteGeneratorWorker::loadRouteDestinationGpsPointsFromRouteId(const std::string & route_id) const
{
...
visualization_msgs::msg::Marker RouteGeneratorWorker::composeRouteMarkerMsg(
  const lanelet::Optional<lanelet::routing::Route> & route)
{
  std::vector<lanelet::ConstPoint3d> points;
  auto end_point_3d = route.get().getEndPoint();
  auto last_ll = route.get().shortestPath().back();
  double end_point_downtrack =
    carma_wm::geometry::trackPos(last_ll, {end_point_3d.x(), end_point_3d.y()}).downtrack;
  double lanelet_downtrack =
    carma_wm::geometry::trackPos(last_ll, last_ll.centerline().back().basicPoint2d()).downtrack;
  // get number of points to display using ratio of the downtracks
  auto points_until_end_point =
    int(last_ll.centerline().size() * (end_point_downtrack / lanelet_downtrack));
...
carma_planning_msgs::msg::Route RouteGeneratorWorker::composeRouteMsg(
  const lanelet::Optional<lanelet::routing::Route> & route) const
{
...
bool RouteGeneratorWorker::abortActiveRouteCb(
  const std::shared_ptr<rmw_request_id_t>,
...
void RouteGeneratorWorker::bumperPoseCb()
{
  ...
  if (this->rs_worker_.getRouteState() == RouteStateWorker::RouteState::FOLLOWING) {
    ...
    current_loc_ =
      lanelet::BasicPoint2d(vehicle_pose_->pose.position.x, vehicle_pose_->pose.position.y);
    ...
    try {
      ...
      RCLCPP_WARN_STREAM(
        logger_->get_logger(), "Routing has finished but carma_wm has not receive it!");
      return;
    }
    ...
    if ((this->world_model_->getRouteName()).compare(route_msg_.route_name) != 0) {
      RCLCPP_WARN_STREAM(
        logger_->get_logger(), "Current active route name is "
                                 << route_msg_.route_name << ", WorldModel is using "
                                 << this->world_model_->getRouteName());
      return;
    }
    ...
    lanelet::Optional<carma_wm::TrafficRulesConstPtr> traffic_rules =
    ...
    std::shared_ptr<geometry_msgs::msg::PoseStamped> pose_ptr(
      new geometry_msgs::msg::PoseStamped(*vehicle_pose_));
    ...
    if (
      (current_downtrack_distance_ > route_length_2d - down_track_target_range_ &&
       current_speed_ < epsilon_) ||
      (current_downtrack_distance_ > route_length_2d)) {
...
void RouteGeneratorWorker::setPublishers(
...
lanelet::Optional<lanelet::routing::Route> RouteGeneratorWorker::rerouteAfterRouteInvalidation(
  const std::vector<lanelet::BasicPoint2d> & destination_points_in_map)
{
  ...
  for (const auto & i :
       destination_points_in_map)  // Identify all route points that we have not yet passed
  {
    ...
    if (current_downtrack_distance_ < destination_down_track) {
      ...
      RCLCPP_DEBUG_STREAM(
        logger_->get_logger(), "current_downtrack_distance_:" << current_downtrack_distance_);
      RCLCPP_DEBUG_STREAM(
        logger_->get_logger(), "destination_down_track:" << destination_down_track);
    }
  }
  ...
  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(),
    "New destination_points_in_map.size:" << destination_points_in_map_.size());

  auto route = routing(
    current_loc_,  // Route from current location through future destinations
    std::vector<lanelet::BasicPoint2d>(
      destination_points_in_map_.begin(), destination_points_in_map_.end() - 1),
    destination_points_in_map_.back(), world_model_->getMap(), world_model_->getMapRoutingGraph());
...
bool RouteGeneratorWorker::crosstrackErrorCheck(
  const std::shared_ptr<geometry_msgs::msg::PoseStamped> & msg, lanelet::ConstLanelet current)
{
  ...
  if (boost::geometry::within(
        position,
        current.polygon2d()))  // If vehicle is inside current_lanelet, there is no crosstrack error
  {
    cte_count_ = 0;
    return false;
  }
  ...
  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(), "LLt Polygon Dimensions1: " << current.polygon2d().front().x() << ", "
                                                       << current.polygon2d().front().y());
  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(), "LLt Polygon Dimensions2: " << current.polygon2d().back().x() << ", "
                                                       << current.polygon2d().back().y());
  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(), "Distance1: " << boost::geometry::distance(position, current.polygon2d())
                                         << " Max allowed Crosstrack: " << cross_track_dist_);

  if (
    boost::geometry::distance(position, current.polygon2d()) >
    cross_track_dist_)  // Evaluate lanelet crosstrack distance from vehicle
  {
    cte_count_++;

    if (cte_count_ > cte_count_max_)  // If the distance exceeds the crosstrack distance a certain
                                      // number of times, report that the route has been departed
    {
      cte_count_ = 0;
      return true;
    } else
...
lanelet::ConstLanelet RouteGeneratorWorker::getClosestLaneletFromRouteLanelets(
  lanelet::BasicPoint2d position) const
{
...
void RouteGeneratorWorker::setCrosstrackErrorCountMax(int cte_max) { cte_count_max_ = cte_max; }

void RouteGeneratorWorker::addLanelet(lanelet::ConstLanelet llt) { route_llts.push_back(llt); }
