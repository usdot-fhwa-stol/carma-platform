                     ...
                     : ((mvr).type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING       \
                          ...
                          : ((mvr).type == carma_planning_msgs::msg::Maneuver::STOP_AND_WAIT   \
                               ...
                               : throw std::invalid_argument(                                  \
                                   "GET_MANEUVER_PROPERTY (property) called on maneuver with " \
                                   "invalid type id " +                                        \
                                   std::to_string((mvr).type)))))))))

#define SET_MANEUVER_PROPERTY(mvr, property, value)                                            \
  (((mvr).type == carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_LEFT_TURN           \
      ? (mvr).intersection_transit_left_turn_maneuver.property = (value)                       \
      : ((mvr).type == carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_RIGHT_TURN     \
           ? (mvr).intersection_transit_right_turn_maneuver.property = (value)                 \
           : ((mvr).type == carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_STRAIGHT  \
                ? (mvr).intersection_transit_straight_maneuver.property = (value)              \
                : ((mvr).type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE               \
                     ? (mvr).lane_change_maneuver.property = (value)                           \
                     : ((mvr).type == carma_planning_msgs::msg::Maneuver::STOP_AND_WAIT        \
                          ? (mvr).stop_and_wait_maneuver.property = (value)                    \
                          : ((mvr).type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING  \
                               ? (mvr).lane_following_maneuver.property = (value)              \
                               : throw std::invalid_argument(                                  \
                                   "SET_MANEUVER_PROPERTY (property) called on maneuver with " \
                                   "invalid type id " +                                        \
                                   std::to_string((mvr).type)))))))))
...
private:
   ...
   * \param lane_ids List of lanelet IDs that the current maneuver traverses. Message expects these
   * to be contiguous and end to end \return A lane keeping maneuver message which is ready to be
   * published
   */
  carma_planning_msgs::msg::Maneuver composeLaneFollowingManeuverMessage(
   ...
   *        NOTE: The start and stop time are not set. This is because this is recomputed based on
   * requests from the arbitrator \param start_dist Start downtrack distance of the current maneuver
   * \param end_dist End downtrack distance of the current maneuver
   * \param start_speed Start speed of the current maneuver
   ...
   */
  carma_planning_msgs::msg::Maneuver composeLaneChangeManeuverMessage(
    double start_dist, double end_dist, double start_speed, double target_speed,
   ...
   * \param start_time The starting time for the first maneuver in the sequence, each consequent
   * maneuver is pushed ahead by same amount
   */
  ...
  /**
   * \brief Calculate maneuver plan for remaining route. This callback is triggered when a new route
   * has been received and processed by the world model \param route_shortest_path A list of
   * lanelets along the shortest path of the route using which the maneuver plan is calculated.
   */
  std::vector<carma_planning_msgs::msg::Maneuver> routeCb(
    const lanelet::routing::LaneletPath & route_shortest_path);

  /**
   * \brief Adds a StopAndWait maneuver to the end of a maneuver set stopping at the provided
   * downtrack value NOTE: The priority of this method is to plan the stopping maneuver therefore
   * earlier maneuvers will be modified or removed if required to allow the stopping behavior to be
   * executed
   *
   * \param input_maneuvers The set of maneuvers to modify to support the StopAndWait maneuver.
   * \param route_end_downtrack The target stopping point (normally the end of the route) which the
   * vehicle should stop before. Units meters \param stopping_entry_speed The expected entry speed
   * for stopping. This is used to compute the stopping distance. Units m/s \param
   * stopping_logitudinal_accel The target deceleration (unsigned) for the stopping operation. Units
   * m/s/s \param lateral_accel_limit The lateral acceleration limit allowed for lane changes. Units
   * m/s/s \param min_maneuver_length The absolute minimum allowable maneuver length for any
   * existing maneuvers in meters
   *
   * NOTE: Only min_maneuver_length can be a zero-valued input. All other parameters must be
   * positive values greater than zero.
   *
   * \throw std::invalid_argument If existing maneuvers cannot be modified to allow stopping
   * maneuver creation, or if the generated maneuvers do not overlap any lanelets in the map.
   *
   * \return A list of maneuvers which mirrors the input list but with the modifications required to
   * include a stopping maneuver at the end
   *
   * ASSUMPTION: At the moment the stopping entry speed is not updated because the assumption is
   * that any previous maneuvers which were slower need not be accounted for as planning for a
   * higher speed will always be capable of handling that case and any which were faster would
   * already have their speed reduced by the maneuver which this speed was derived from.
   */
  ...
  /**
   * \brief Identifies if a maneuver starts after the provided downtrack with compensation for a
   * dynamic buffer size based on the maneuver type
   *
   ...
   *
   * \return true if the provided maneuver plus the computed dynamic buffer starts after the
   * provided downtrack value
   */
  bool maneuverWithBufferStartsAfterDowntrack(
  ...
  /**
   * \brief This method re-routes the vehicle back to the shortest path, if the vehicle has left the
   * shortest path, but is still on the route Re-routing is performed by generating a new shortest
   * path via the closest lanelet on the original shortest path
   *
  ...
  /**
   * \brief Callback for the ManeuverPlan subscriber, will store the current maneuver plan received
   * locally. Used as part of the detection system for differentiating leaving the shortest path via
   * another plugin vs. control drift taking the vehicle's reference point outside of the intended
   * lane. \param msg Latest ManeuverPlan message
   */
  void current_maneuver_plan_cb(carma_planning_msgs::msg::ManeuverPlan::UniquePtr msg);

  /**
   * \brief returns duration as ros::Duration required to complete maneuver given its start dist,
   * end dist, start speed and end speed \param maneuver The maneuver message to calculate duration
   * for \param epsilon The acceptable min start_speed + target_speed in the maneuver message, under
   * which the maneuver is treated as faulty. Throws exception if sum of start and target speed of
   * maneuver is below limit defined by parameter epsilon
   */
  rclcpp::Duration getManeuverDuration(
