...
/**
 * Base class for a compound maneuver that exists in the longitudinal dimension.
 * A future maneuver is initially defined only in terms
...
public class FutureLongitudinalManeuver extends LongitudinalManeuver {
    ...
    public FutureLongitudinalManeuver(String plannerName, IManeuverInputs inputs, double startDist, double startSpeed, double endDist, double endSpeed) {
        super(plannerName);
        inputs_ = inputs;
    ...
    public void plan(IManeuverInputs inputs, IGuidanceCommands commands, double startDist) throws IllegalStateException {
        throw new IllegalStateException("FutureLongitudinalManeuver.plan is not implemented - should not be called.");
    }
    ...
    public double planToTargetDistance(IManeuverInputs inputs, IGuidanceCommands commands, double startDist, double endDist)
        ...
        throw new IllegalStateException("FutureLongitudinalManeuver.planToTargetDistance is not implemented - should not be called.");
    }

    @Override
    public boolean canPlan(IManeuverInputs inputs, double startDist, double endDist) throws IllegalStateException {
        throw new IllegalStateException("FutureLongitudinalManeuver.canPlan is not implemented - should not be called.");
    }
    ...
    public double addManeuver(ISimpleManeuver mvr) throws IllegalStateException {
        ...
        if (mvr.getEndDistance() > endDist_) {
            throw new IllegalStateException("Attempting to add a maneuver that extends past end of FutureLongitudinalManeuver.");
        }

        if (Math.abs(mvr.getStartDistance() - maneuversEnd_) > CONCATENATION_TOLERANCE) {
            throw new IllegalStateException("Attempting to add a maneuver to a FutureLongitudinalManeuver with a gap or overlap");
        }

        mvrs_.add(mvr);
        maneuversEnd_ = mvr.getEndDistance();
        log_.debug("MVR", "Added new concrete maneuver to FutureLongitudinalManeuver from " +
        ...
        if (currentLoc >= startDist_) {
            ...
            if (currentLoc > mvrs_.get(executingIdx_).getEndDistance()) {
                ...
                if (++executingIdx_ >= mvrs_.size()) {
                    throw new IllegalStateException("Attempting to execute a non-existent maneuver in FutureLongitudinalManeuver.");
                }
            }
            log_.debug("MVR", "Executing constituent maneuver " + executingIdx_ + " in FutureLongitudinalManeuver");
    ...
    public double generateSpeedCommand() {
        ...
        log_.warn("MVR", "\\\\ in FutureLongitudinalManeuver.generateSpeedCommand() - shouldn't be here! \\\\");
        return 0.0;
    }
