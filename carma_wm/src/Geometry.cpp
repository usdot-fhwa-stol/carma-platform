...
// This safeAcos implementation is based on Stack Overflow answer:
// https://stackoverflow.com/questions/8489792/is-it-legal-to-take-acos-of-1-0f-or-1-0f Asked by
// SirYakalot: https://stackoverflow.com/users/956689/siryakalot Answered by TonyK:
// https://stackoverflow.com/users/428857/tonyk Credited in accordance with Stack Overflow's CC-BY
// license
  ...
  const lanelet::BasicPoint2d & seg_end)
{
  ...
  double sign = d >= 0 ? 1.0 : -1.0;  // If d is positive then the point is to the right if it is
                                      // negative the point is to the left
...
/*! \brief Helper function to identify whether to select the preceeding or succeeding segment of a
 * linstring point that is nearest an external point when trying to find the TrackPos of the
 * external point. Of the 3 points comprising the 2 segment linestirng the external point must be
 * closest to the midpoint for this function to be valid
 *
 * ASSUMPTION: Of the 3 points comprising the 2 segment linestirng the external point is closest to
 * the midpoint
 *
 * Meant to be called in the matchSegment method. Logic is as follows
 * If the external point is within the downtrack bounds of one segment but not the other then use
 * the one it is within If the external point is within the downtrack bounds of both segments then
 * use the one with the smallest crosstrack distance If the external point is within the downtrack
 * bounds of both segments and has exactly equal crosstrack bounds with each segment then use the
 * preceeding segment If the external point is outside the downtrack bounds of both segments then
 * assign to the first segment as it will always have positive downtrack in this case while the
 ...
 *
 * \return True if the preceeding segment is the better choice. False if the succeeding segment is
 * the better choice.
 */

bool selectFirstSegment(
  const TrackPos & first_seg_trackPos, const TrackPos & second_seg_trackPos,
  double first_seg_length, double second_seg_length)
{
  ...
  if (first_seg_in_downtrack && !second_seg_in_downtrack) {  // If in the first segment but not the
                                                             // last segment
    ...
    return first_seg_trackPos.crosstrack <=
           second_seg_trackPos.crosstrack;  // Pick the first segment if the crosstrack values are
                                            // equal or the first segment is closer
  } else {  // Point lies outside the downtrack bounds of both segments (Remember According to
            // function contract the nearest point to external point is the midpoint of the two
            // segments)
    return true;  // Choose first segment as it will always have positive downtrack in this case
                  // while the second segment will always have negative downtrack
  }
}
  ...
  const lanelet::BasicPoint2d & p, const lanelet::BasicLineString2d & line_string)
{
  ...
  lanelet::BasicSegment2d best_segment = std::make_pair(
    line_string[0], line_string[1]);  // Default to starting segment if no match is found
  ...
  for (size_t i = 0; i < line_string.size();
       i++) {  // Iterate over line string to find nearest point
    double seg_length = 0;
    if (i < line_string.size() - 1) {
      seg_length = lanelet::geometry::distance2d(
        line_string[i], line_string[i + 1]);  // Compute segment length
    }

    double distance = lanelet::geometry::distance2d(
      p, line_string[i]);           // Compute from current point to external point
    if (distance < min_distance) {  // If this distance is below minimum discovered so far then
                                    // update minimum
      min_distance = distance;
      best_point_index = i;
      best_accumulated_length = accumulated_length;
      best_last_accumulated_length =
        last_accumulated_length;  // Record accumulated lengths to each segment
  ...
  // Minimum point has been found next step is to determine which segment it should go with using
  // the following rules. If the minimum point is the first point then use the first segment If the
  // minimum point is the last point then use the last segment If the minimum point is within the
  ...
  // crosstrack distance If the minimum point is within the downtrack bounds of both segments and
  // has exactly equal crosstrack bounds with each segment then use the first one
  ...
  if (best_point_index == 0) {
    ...
    best_segment =
      std::make_pair(line_string[line_string.size() - 2], line_string[line_string.size() - 1]);
  } else {
    TrackPos first_seg_trackPos =
      trackPos(p, line_string[best_point_index - 1], line_string[best_point_index]);
    TrackPos second_seg_trackPos =
      trackPos(p, line_string[best_point_index], line_string[best_point_index + 1]);
    if (selectFirstSegment(
          first_seg_trackPos, second_seg_trackPos, best_last_seg_length, best_seg_length)) {
      best_pos = first_seg_trackPos;
      best_pos.downtrack += best_last_accumulated_length;
      best_segment =
        std::make_pair(line_string[best_point_index - 1], line_string[best_point_index]);
    } else {
      best_pos = second_seg_trackPos;
      best_pos.downtrack += best_accumulated_length;
      best_segment =
        std::make_pair(line_string[best_point_index], line_string[best_point_index + 1]);
    }
  }
  ...
  const geometry_msgs::msg::Pose & pose, const geometry_msgs::msg::Vector3 & size)
{
  ...
  // 4 corners of the object starting with upper left and moving in clockwise direction in pose
  // frame
...
std::vector<double> templated_local_curvatures(const std::vector<P, A> & centerline_points)
{
  ...
  std::vector<Eigen::Vector2d> tangent_derivative =
    compute_finite_differences(normalized_tangent_vectors, arc_lengths);
...
lanelet::BasicLineString2d concatenate_line_strings(
  const lanelet::BasicLineString2d & a, const lanelet::BasicLineString2d & b)
{
  ...
  if (
    !a.empty() && !b.empty() &&
    compute_euclidean_distance(a.back(), b.front()) < SPATIAL_EPSILON_M) {
    start_offset = 1;
  }
...
{
  std::vector<double> out;
  if (centerline.empty()) return out;

  std::vector<Eigen::Vector2d> tangents =
    carma_wm::geometry::compute_templated_finite_differences(centerline);
