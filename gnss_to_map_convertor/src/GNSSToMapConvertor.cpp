...
{
GNSSToMapConvertor::GNSSToMapConvertor(
  PosePubCallback pose_pub, TransformLookupCallback tf_lookup, std::string map_frame_id,
...
{
  if (!baselink_in_sensor_)  // Extract the assumed static transform between baselink and the sensor
                             // if it was not already optained
  {
    auto tf_msg = tf_lookup_(fix_msg->header.frame_id, base_link_frame_id_);
    if (!tf_msg)  // Failed to get transform
    {
      RCLCPP_WARN_STREAM(
        logger_->get_logger(), "Ignoring fix message: Could not locate static transform between "
  ...
  if (!sensor_in_ned_heading_rotation_)  // Extract the assumed static transform between heading
                                         // frame and the position sensor frame if it was not
                                         // already optained
  {
    ...
    if (!tf_msg)  // Failed to get transform
    {
      RCLCPP_WARN_STREAM(
        logger_->get_logger(), "Ignoring fix message: Could not locate static transform between "
                                 << heading_frame_id_ << " and " << fix_msg->header.frame_id);
      return;
    }
    ...
    if (tf.getOrigin().x() != 0.0 || tf.getOrigin().y() != 0.0 || tf.getOrigin().z() != 0.0) {
      RCLCPP_WARN_STREAM(
        logger_->get_logger(),
        "Heading frame does not have rotation only transform with sensor frame. The translation "
        "will not "
  ...
  {
    RCLCPP_WARN_STREAM(
      logger_->get_logger(), "Ignoring fix message as no map projection has been recieved.");
    return;
  }

  geometry_msgs::msg::PoseWithCovarianceStamped pose_msg = poseFromGnss(
    baselink_in_sensor_.get(), sensor_in_ned_heading_rotation_.get(), *map_projector_,
...
void GNSSToMapConvertor::geoReferenceCallback(std_msgs::msg::String::UniquePtr geo_ref)
{
  if (georeference_ != geo_ref->data) {
    ...
    std::string axis = wgs84_utils::proj_tools::getAxisFromProjString(
      geo_ref->data);  // Extract axis for orientation calc
...
geometry_msgs::msg::PoseWithCovarianceStamped GNSSToMapConvertor::poseFromGnss(
  ...
  const tf2::Quaternion & ned_in_map_rotation, gps_msgs::msg::GPSFix fix_msg)
{
  ...
  if (fabs(map_point.x()) > 10000.0 || fabs(map_point.y()) > 10000.0) {  // Above 10km from map
                                                                         // origin earth curvature
  ...
  // This logic assumes that the orientation difference between an NED frame located at the map
  // origin and an NED frame located at the GNSS point are sufficiently small that they can be
  // ignored. Therefore it is assumed the heading report of the GNSS system reguardless of its
  // poition in the map without change in its orientation will give the same result (as far as we
  // are concered).
  ...
  tf2::Quaternion R_h_s =
    sensor_in_ned_heading_rotation;  // Rotation of heading report in sensor frame

  tf2::Quaternion R_m_s =
    R_m_n * R_n_h *
    R_h_s;  // Rotation of sensor in map frame under assumption that distance from map origin is
            // sufficiently small so as to ignore local changes in NED orientation

  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(), "R_m_n (x,y,z,w) : ( " << R_m_n.x() << ", " << R_m_n.y() << ", "
                                                  << R_m_n.z() << ", " << R_m_n.w());

  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(), "R_n_h (x,y,z,w) : ( " << R_n_h.x() << ", " << R_n_h.y() << ", "
                                                  << R_n_h.z() << ", " << R_n_h.w());

  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(), "R_h_s (x,y,z,w) : ( " << R_h_s.x() << ", " << R_h_s.y() << ", "
                                                  << R_h_s.z() << ", " << R_h_s.w());

  RCLCPP_DEBUG_STREAM(
    logger_->get_logger(), "R_m_s (x,y,z,w) : ( " << R_m_s.x() << ", " << R_m_s.y() << ", "
                                                  << R_m_s.z() << ", " << R_m_s.w());

  tf2::Transform T_m_s(
    R_m_s, tf2::Vector3(
             map_point.x(), map_point.y(), map_point.z()));  // Reported position and orientation
