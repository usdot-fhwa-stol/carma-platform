...
lanelet::ConstLaneletOrAreas getAffectedLaneletOrAreas(const lanelet::Points3d& gf_pts, const lanelet::LaneletMapPtr& lanelet_map, std::shared_ptr<const lanelet::routing::RoutingGraph> routing_graph, double max_lane_width)
{
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Get affected lanelets loop");
  std::unordered_set<lanelet::Lanelet> affected_lanelets;
  for (size_t idx = 0; idx < gf_pts.size(); idx ++)
  {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Index: " << idx << " Point: " << gf_pts[idx].x() << ", " << gf_pts[idx].y());
    ...
    while (continue_search) {
      ...
      for (const auto& ll_pair : lanelet::geometry::findNearest(lanelet_map->laneletLayer, gf_pts[idx].basicPoint2d(), nearest_count)) { // Get the nearest lanelets and iterate over them
        ...
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Distance to lanelet " << ll.id() << ": " << dist << " max_lane_width: " << max_lane_width);
    ...
    {
      RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Last point");
      ...
      RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Got successor lanelets of size: " << filtered.size());
      affected_lanelets.insert(filtered.begin(), filtered.end());


      if (affected_lanelets.empty() && !possible_lanelets.empty() && idx !=0 )
      {
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Checking if it is the edge case where only last point falls on a valid (correct direction) lanelet");
        for (auto llt: possible_lanelets)
        {
          RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Evaluating lanelet: " << llt.id());
          ...
          {
            RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Overlaps starting line... Picking llt: " << llt.id());
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Checking possible lanelets");
    ...
    for (auto llt: possible_lanelets)
    {
      ...
      if (boost::geometry::intersects(llt_boundary, gf_dir_line))
      {
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Overlaps end line");
        affected_lanelets.insert(llt);
      }
      ...
      { 
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Within new lanelet");
        ...
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "vec_to_median: " << vec_to_median.x() << ", " << vec_to_median.y());
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "vec_to_gf_start: " << vec_to_gf_start.x() << ", " << vec_to_gf_start.y());
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "vec_to_gf_end: " << vec_to_gf_end.x() << ", " << vec_to_gf_end.y());
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "start_to_median: " << start_to_median.x() << ", " << start_to_median.y());
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "start_to_end: " << start_to_end.x() << ", " << start_to_end.y());
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "interior_angle: " << interior_angle);
      ...
      {
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "------ Did not record anything...");
      }

    }
  }
  
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ros2"), "affected_lanelets size: " << affected_lanelets.size());
