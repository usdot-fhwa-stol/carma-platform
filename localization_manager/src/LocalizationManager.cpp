                                                ...
                                                const autoware_msgs::msg::NDTStat::ConstPtr stats)
{
    ...
    if (static_cast<LocalizerMode>(config_.localization_mode) == LocalizerMode::GNSS_WITH_NDT_INIT && state == LocalizationState::OPERATIONAL && last_raw_gnss_value_)
    {
        ...
        else
        {
            ...
            RCLCPP_DEBUG_STREAM(rclcpp::get_logger("localization.localization_manager"), "Switched to LIDAR_INITIALIZED_SWITCH_TO_GPS at lidar_init_sequential_timesteps_counter_: " << lidar_init_sequential_timesteps_counter_ 
    ...
    {
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("localization.localization_manager"),"Publishing mixed pose with lidar_init_sequential_timesteps_counter_: " << lidar_init_sequential_timesteps_counter_ << ", and state" << state);
        current_pose_ = *pose;
    }
