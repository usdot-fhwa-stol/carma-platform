...
For standalone CARMA Platform launch, add following to your docker-compose assuming carma-platform is built with the basic_travel_simulator.launch.py:

```
  basic_travel:
    image: local/carma-platform:latest
    network_mode: host
    container_name: basic_travel
    depends_on:
      - platform
    volumes_from:
