...
public class LaneChangePlugin extends AbstractPlugin implements ITacticalPlugin {
    ...
    public boolean planSubtrajectory(Trajectory traj, double startDistance, double endDistance) {
        ...
        if (targetLane_ > -1) {
            ...
            futureLatMvr_ = new FutureLateralManeuver(version.componentName(), inputs, startDistance, startSpeed_, endDistance, endSpeed_);
            futureLonMvr_ = new FutureLongitudinalManeuver(version.componentName(),inputs, startDistance, startSpeed_, endDistance, endSpeed_);
    ...
    private boolean plan(double startDist, double endDist, int targetLane, double startSpeed, double endSpeed)
        ...
        laneChangeMvr_ = new LaneChange(version.componentName());
    ...
    private void populateFutureManeuver() {
        ...
        try {
            ...
            if (endDist - startDist > 0) {
                LaneKeeping lk = new LaneKeeping(version.componentName());
            ...
            SteadySpeed ss = new SteadySpeed(version.componentName());
