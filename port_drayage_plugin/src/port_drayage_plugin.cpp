...
: carma_ros2_utils::CarmaLifecycleNode(options),
  pdw_(
    get_node_logging_interface(), get_clock(),
    ...
    std::bind(&PortDrayagePlugin::callSetActiveRouteClient, this, std_ph::_1))
{
  ...
  config_.enable_port_drayage =
    declare_parameter<bool>("enable_port_drayage", config_.enable_port_drayage);
  config_.starting_at_staging_area =
    declare_parameter<bool>("starting_at_staging_area", config_.starting_at_staging_area);
}

rcl_interfaces::msg::SetParametersResult PortDrayagePlugin::parameter_update_callback(
  const std::vector<rclcpp::Parameter> & parameters)
{
  auto error = update_params<std::string>(
    {{"vehicle_id", config_.cmv_id}, {"cargo_id", config_.cargo_id}}, parameters);
  auto error_2 = update_params<bool>(
    {{"enable_port_drayage", config_.enable_port_drayage},
     {"starting_at_staging_area", config_.starting_at_staging_area}},
    parameters);
...
carma_ros2_utils::CallbackReturn PortDrayagePlugin::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  add_on_set_parameters_callback(
    std::bind(&PortDrayagePlugin::parameter_update_callback, this, std_ph::_1));

  // Setup subscribers
  pose_subscriber_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "current_pose", 5, std::bind(&PortDrayageWorker::onNewPose, &pdw_, std_ph::_1));
  inbound_mobility_operation_subscriber_ =
    create_subscription<carma_v2x_msgs::msg::MobilityOperation>(
      "incoming_mobility_operation", 5,
  ...
  guidance_state_subscriber_ = create_subscription<carma_planning_msgs::msg::GuidanceState>(
    "guidance_state", 5, std::bind(&PortDrayageWorker::onGuidanceState, &pdw_, std_ph::_1));
  route_event_subscriber_ = create_subscription<carma_planning_msgs::msg::RouteEvent>(
    "route_event", 5, std::bind(&PortDrayageWorker::onRouteEvent, &pdw_, std_ph::_1));
  georeference_subscriber_ = create_subscription<std_msgs::msg::String>(
    "georeference", 1, std::bind(&PortDrayageWorker::onNewGeoreference, &pdw_, std_ph::_1));

  // Setup publishers
  outbound_mobility_operations_publisher_ =
    create_publisher<carma_v2x_msgs::msg::MobilityOperation>("outgoing_mobility_operation", 5);
  ui_instructions_publisher_ =
    create_publisher<carma_msgs::msg::UIInstructions>("ui_instructions", 5);

  // Setup service clients
  set_active_route_client_ =
    create_client<carma_planning_msgs::srv::SetActiveRoute>("set_active_route");
...
bool PortDrayagePlugin::callSetActiveRouteClient(
  std::shared_ptr<carma_planning_msgs::srv::SetActiveRoute::Request> req)
{
  ...
  if (future_status == std::future_status::ready) {
    if (
      route_result.get()->error_status ==
      carma_planning_msgs::srv::SetActiveRoute::Response::NO_ERROR) {
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Route Generation succeeded for Set Active Route service call.");
      return true;
    } else {
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Route Generation failed for Set Active Route service call.");
      return false;
    }
  } else {
