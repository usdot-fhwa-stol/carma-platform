...
: BaseSubsystemController(options)
{
  ...
  config_.required_plugins =
    declare_parameter<std::vector<std::string>>("required_plugins", config_.required_plugins);
  config_.auto_activated_plugins = declare_parameter<std::vector<std::string>>(
    "auto_activated_plugins", config_.auto_activated_plugins);
}

cr2::CallbackReturn GuidanceControllerNode::handle_on_configure(
  const rclcpp_lifecycle::State & prev_state)
{
  ...
  std::copy_if(
    base_managed_nodes.begin(), base_managed_nodes.end(), std::back_inserter(guidance_plugin_nodes),
    [plugin_namespace](const std::string & s) { return s.rfind(plugin_namespace, 0) == 0; });

  std::vector<std::string> non_plugin_guidance_nodes =
  ...
  plugin_manager_ = std::make_shared<PluginManager>(
    config_.required_plugins, config_.auto_activated_plugins, plugin_lifecycle_manager,
  ...
  get_registered_plugins_server_ = create_service<carma_planning_msgs::srv::PluginList>(
    ...
    std::bind(
      &PluginManager::get_registered_plugins, plugin_manager_, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));

  get_active_plugins_server_ = create_service<carma_planning_msgs::srv::PluginList>(
    "plugins/get_active_plugins",
    std::bind(
      &PluginManager::get_active_plugins, plugin_manager_, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));
  ...
  activate_plugin_server_ = create_service<carma_planning_msgs::srv::PluginActivation>(
    ...
    std::bind(
      &PluginManager::activate_plugin, plugin_manager_, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));

  get_strategic_plugins_by_capability_server_ =
    create_service<carma_planning_msgs::srv::GetPluginApi>(
      "plugins/get_strategic_plugins_by_capability",
      std::bind(
        &PluginManager::get_strategic_plugins_by_capability, plugin_manager_, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3));
  ...
  get_tactical_plugins_by_capability_server_ =
    create_service<carma_planning_msgs::srv::GetPluginApi>(
      ...
      std::bind(
        &PluginManager::get_tactical_plugins_by_capability, plugin_manager_, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3));
  ...
  get_control_plugins_by_capability_server_ =
    create_service<carma_planning_msgs::srv::GetPluginApi>(
      ...
      std::bind(
        &PluginManager::get_control_plugins_by_capability, plugin_manager_, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3));

  // With all of our non-plugin managed nodes now being tracked we can execute their configure
  // operations
  bool success =
    lifecycle_mgr_
  ...
  try {
    plugin_manager_
      ->configure();  // Only checking required nodes. Other node failure tracked by activity status
...
cr2::CallbackReturn GuidanceControllerNode::handle_on_activate(
  const rclcpp_lifecycle::State & prev_state)
{
  auto base_return = BaseSubsystemController::handle_on_activate(
    prev_state);  // This will activate all nodes in the namespace TODO what about the plugins?
  ...
  try {
    plugin_manager_
      ->activate();  // Only checking required nodes. Other node failure tracked by activity status
  } catch (const std::runtime_error & e) {
...
cr2::CallbackReturn GuidanceControllerNode::handle_on_deactivate(
  const rclcpp_lifecycle::State & prev_state)
{
  ...
  bool success = true;
  try {
    plugin_manager_->deactivate();  // Only checking required nodes. Other node failure tracked by
                                    // activity status
  } catch (const std::runtime_error & e) {
...
cr2::CallbackReturn GuidanceControllerNode::handle_on_cleanup(
  const rclcpp_lifecycle::State & prev_state)
{
  ...
  try {
    plugin_manager_
      ->cleanup();  // Only checking required nodes. Other node failure tracked by activity status
  } catch (const std::runtime_error & e) {
...
cr2::CallbackReturn GuidanceControllerNode::handle_on_shutdown(
  const rclcpp_lifecycle::State & prev_state)
{
