...
TEST(InLaneCruisingPluginTest, test_parse_strategy_params)
{
  ...
  plugin.parse_strategy_params(msg.strategy_params);

  EXPECT_EQ(timestamp, plugin.street_msg_timestamp_);
  EXPECT_EQ(est_stop_t, plugin.scheduled_stop_time_);
  EXPECT_EQ(false, plugin.is_allowed_int_);
}

TEST(InLaneCruisingPluginTest, test_cal_stop_time)
{
  InLaneCruisingPluginConfig config;
  config.default_downsample_ratio = 1;
  std::shared_ptr<carma_wm::CARMAWorldModel> wm = std::make_shared<carma_wm::CARMAWorldModel>();
  ...
  double stop_time = plugin.calc_estimated_stop_time(25, 13);

  EXPECT_NEAR(3.84, stop_time, 0.01);
}

TEST(InLaneCruisingPluginTest, test_cal_stop_speed)
{
  InLaneCruisingPluginConfig config;
  config.default_downsample_ratio = 1;
  std::shared_ptr<carma_wm::CARMAWorldModel> wm = std::make_shared<carma_wm::CARMAWorldModel>();
  ...
  double stop_speed = plugin.calc_stop_speed(33, 50, 15);

  EXPECT_NEAR(11, stop_speed, 0.2);
}

TEST(InLaneCruisingPluginTest, test_cal_case)
{
  InLaneCruisingPluginConfig config;
  config.default_downsample_ratio = 1;
  std::shared_ptr<carma_wm::CARMAWorldModel> wm = std::make_shared<carma_wm::CARMAWorldModel>();
  ...
  int case_num1 = plugin.determine_case(50, 15, 40, 10);

  EXPECT_EQ(3, case_num1);

  int case_num2 = plugin.determine_case(100, 13, 11, 10);

  EXPECT_EQ(2, case_num2);

  int case_num3 = plugin.determine_case(100, 13, 11, 20);
