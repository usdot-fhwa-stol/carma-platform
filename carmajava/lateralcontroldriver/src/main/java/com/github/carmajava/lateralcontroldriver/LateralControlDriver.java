...
public class LateralControlDriver extends SaxtonBaseNode implements ILateralControlDriver {
  ...
  LateralControlWorker worker_;

  // Topics
  // Publishers
  Publisher<cav_msgs.UIInstructions> uiInstructionsPub_;
  Publisher<cav_msgs.DriverStatus> discoveryPub_;
  // Subscribers
  Subscriber<cav_msgs.LateralControl> lateralControlSub_;
  ...
  @Override public GraphName getDefaultNodeName() {
    return GraphName.of("lateral_control_driver");
  }
  ...
  @Override public void onSaxtonStart(final ConnectedNode connectedNode) {
    ...
    double angleThreshold = params.getFloat("~angle_threshold", 2.0);

    // Topics
    // Publishers
    uiInstructionsPub_ = connectedNode.newPublisher("ui_instructions", cav_msgs.UIInstructions._TYPE);
    ...
    // Subscribers
    lateralControlSub_ = connectedNode.newSubscriber("lateral_control", cav_msgs.LateralControl._TYPE);
