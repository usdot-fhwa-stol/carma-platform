...
WMListenerWorker::WMListenerWorker(rclcpp::Logger logger)
: logger_(logger)
{
...
void WMListenerWorker::mapCallback(const autoware_lanelet2_msgs::msg::MapBin::SharedPtr map_msg)
{
  ...
  while(!map_update_queue_.empty() && more_updates_to_apply) {
    ...
    if (update->map_version < current_map_version_) { // Drop any so far unapplied updates for the previous map
      RCLCPP_WARN_STREAM(logger_, "There were unapplied updates in carma_wm when a new map was received.");
      continue;
    }
    if (update->map_version == current_map_version_) { // Current update goes with current map
      mapUpdateCallback(update); // Apply the update
    } else {
      RCLCPP_INFO_STREAM(logger_, "Done applying updates for new map. However, more updates are waiting for a future map.");
  ...
  if (delayed_route_msg_) {
    if (delayed_route_msg_.get().map_version == current_map_version_) { // If there is a delayed route message to apply then do so
      ...
      RCLCPP_WARN_STREAM(logger_, "Dropping delayed route message which was never applied as updated map was not recieved");
      delayed_route_msg_ = boost::none;
    } else {
      RCLCPP_INFO_STREAM(logger_, "There is a delayed route message still waiting to be applied in carma_wm");
    }
  }
}
...
void logSignalizedIntersectionManager(const carma_wm::SignalizedIntersectionManager& sim, const rclcpp::Logger& logger)
{
  ...
  for (auto const &[signal_id, entry_llt_ids] : sim.signal_group_to_entry_lanelet_ids_)
  {
    ...
    {
      RCLCPP_DEBUG_STREAM(logger, "signal id: " << (int)signal_id << ", entry llt id: " << entry_llt_id);
    }
  }
  for (auto const &[signal_id, exit_llt_ids] : sim.signal_group_to_exit_lanelet_ids_)
  {
    ...
    {
      RCLCPP_DEBUG_STREAM(logger, "signal id: " << (int)signal_id << ", exit llt id: " << exit_llt_id);
    }
  }
  for (auto const &[signal_id, regem_id] : sim.signal_group_to_traffic_light_id_)
  {
    RCLCPP_DEBUG_STREAM(logger, "signal id: " << (int)signal_id << ", regem id: " << regem_id);
  }
}

void WMListenerWorker::mapUpdateCallback(autoware_lanelet2_msgs::msg::MapBin::SharedPtr geofence_msg)
{
  RCLCPP_INFO_STREAM(logger_, "Map Update Being Evaluated. SeqNum: " << geofence_msg->seq_id);
  if (rerouting_flag_) // no update should be applied if rerouting
  {
    RCLCPP_INFO_STREAM(logger_, "Currently new route is being processed. Queueing this update. Received seq: " << geofence_msg->seq_id << " prev seq: " << most_recent_update_msg_seq_);
    map_update_queue_.emplace(geofence_msg);
    return;
  }
  if (geofence_msg->seq_id <= most_recent_update_msg_seq_) {
    RCLCPP_DEBUG_STREAM(logger_, "Dropping map update which has already been processed. Received seq: " << geofence_msg->seq_id << " prev seq: " << most_recent_update_msg_seq_);
    return;
    ...
    RCLCPP_DEBUG_STREAM(logger_, "Update received for newer map version than available. Queueing update until map is available.");
    map_update_queue_.emplace(geofence_msg);
    return;
  } else if (current_map_version_ > geofence_msg->map_version) { // If this update is for an older map
    RCLCPP_WARN_STREAM(logger_, "Dropping old map update as newer map is already available.");
    return;
  } else if (most_recent_update_msg_seq_ + 1 < geofence_msg->seq_id) {
    RCLCPP_INFO_STREAM(logger_, "Queuing map update as we are waiting on an earlier update to be applied. most_recent_update_msg_seq_: " << most_recent_update_msg_seq_ << "geofence_msg->seq_id: " << geofence_msg->seq_id);
    map_update_queue_.emplace(geofence_msg);
    return;
  }
  ...
  if(geofence_msg->invalidates_route==true && world_model_->getRoute())
  {
    ...
    RCLCPP_DEBUG_STREAM(logger_, "Received notice that route has been invalidated in mapUpdateCallback");

    if(route_node_flag_!=true)
    {
     RCLCPP_INFO_STREAM(logger_, "Route is not yet available. Therefore queueing the update");
     map_update_queue_.emplace(geofence_msg);
     return;
    }
  }
  ...
  RCLCPP_DEBUG_STREAM(logger_, "Geofence id" << gf_ptr->id_ << " requests addition of lanelets size: " << gf_ptr->lanelet_additions_.size());
  for (auto llt : gf_ptr->lanelet_additions_)
  {
    // world model here should blindly accept the map update received
    RCLCPP_DEBUG_STREAM(logger_, "Adding new lanelet with id: " << llt.id());
  ...
  RCLCPP_DEBUG_STREAM(logger_, "Geofence id" << gf_ptr->id_ << " sends record of traffic_lights_id size: " << gf_ptr->traffic_light_id_lookup_.size());
  for (auto const &[traffic_light_id, lanelet_id] : gf_ptr->traffic_light_id_lookup_)
  {
    RCLCPP_DEBUG_STREAM(logger_, "Adding new pair for traffic light ids: " << traffic_light_id << ", and lanelet::Id: " << lanelet_id);
    world_model_->setTrafficLightIds(traffic_light_id, lanelet_id);
  }

  RCLCPP_DEBUG_STREAM(logger_, "Geofence id" << gf_ptr->id_ << " sends record of intersections size: " << gf_ptr->sim_.intersection_id_to_regem_id_.size());
  if (gf_ptr->sim_.intersection_id_to_regem_id_.size() > 0)
  {
    world_model_->sim_ = gf_ptr->sim_;
    logSignalizedIntersectionManager(world_model_->sim_, logger_);
  }

  RCLCPP_DEBUG_STREAM(logger_, "Geofence id" << gf_ptr->id_ << " requests removal of size: " << gf_ptr->remove_list_.size());
  for (auto const &[lanelet_id, lanelet_to_remove] : gf_ptr->remove_list_)
  {
    ...
    RCLCPP_DEBUG_STREAM(logger_, "Regems found in lanelet: " << regems_copy_to_check.size());
    ...
    RCLCPP_DEBUG_STREAM(logger_, "Regems left in lanelet after removal: " << parent_llt.regulatoryElements().size());

  }

  RCLCPP_INFO_STREAM(logger_, "Geofence id" << gf_ptr->id_ << " requests update of size: " << gf_ptr->update_list_.size());
  ...
  for (auto const &[lanelet_id, lanelet_to_update]: gf_ptr->update_list_)
  {
    ...
    if (regemptr_it != world_model_->getMutableMap()->regulatoryElementLayer.end())
    {
      ...
      RCLCPP_DEBUG_STREAM(logger_, "Reapplying previously existing element for lanelet id:" << parent_llt.id() << ", and regem id: " << regemptr_it->get()->id());
    ...
    {
      RCLCPP_DEBUG_STREAM(logger_, "New regulatory element at lanelet: " << parent_llt.id() << ", and id: " << lanelet_to_update->id());
  ...
  RCLCPP_INFO_STREAM(logger_, "Finished Applying the Map Update with Geofence Id:" << gf_ptr->id_);

  // Call user defined map callback
  if (map_callback_)
  {
    RCLCPP_INFO_STREAM(logger_, "Calling user defined map update callback");
    map_callback_();
  }
}
...
void WMListenerWorker::newRegemUpdateHelper(lanelet::Lanelet parent_llt, lanelet::RegulatoryElement* regem) const
{
  ...
  switch(resolveGeofenceType(regem->attribute(lanelet::AttributeName::Subtype).value()))
  {
    ...
    default:
      RCLCPP_WARN_STREAM(logger_, "World Model instance received an unsupported geofence type in its map update callback!");
      break;
  }
}
...
LaneletRoutingGraphPtr WMListenerWorker::routingGraphFromMsg(const autoware_lanelet2_msgs::msg::RoutingGraph& msg, lanelet::LaneletMapPtr map) const {
  ...
  if (msg.participant_type.compare(getVehicleParticipationType()) != 0) {
    ...
    RCLCPP_ERROR_STREAM(logger_,"Received routing graph does not have matching vehicle type for world model. WM Type: "
  ...
  try {
    ...
    RCLCPP_ERROR_STREAM(logger_, "Received routing graph specifies lanelets which do not match the current map version. Actual exception: " << e.what());
  ...
  for (size_t i = 0; i < msg.lanelet_vertices.size(); ++i) {
    ...
    for (size_t j = 0; j < vertex.lanelet_or_area_ids.size(); ++j) {
      ...
      try {
        ...
        RCLCPP_ERROR_STREAM(logger_,"Received routing graph specifies lanelets which do not match the current map version. Not found lanelet or area: "
...
void WMListenerWorker::routeCallback(const carma_planning_msgs::msg::Route::SharedPtr route_msg)
{
  if (route_msg->map_version < current_map_version_) {
    RCLCPP_WARN_STREAM(logger_, "Route message rejected as it is for an older map");
    rerouting_flag_ = false; // Clear any blockers on map updates as the route we were waiting for is no longer valid
    return;
  }

  if (route_msg->map_version > current_map_version_) {
    RCLCPP_WARN_STREAM(logger_, "Route message received for future map. Delaying application until map is recieved");
...

  {
    ...
    while(!map_update_queue_.empty() && more_updates_to_apply) {
      ...
      if (update->map_version < current_map_version_) { // Drop any so far unapplied updates for the current map
        RCLCPP_WARN_STREAM(logger_, "Apply from reroute: There were unapplied updates in carma_wm when a new map was recieved.");
        continue;
      }
      if (update->map_version == current_map_version_) { // Current update goes with current map which is also the map used by this route
        RCLCPP_DEBUG_STREAM(logger_, "Applying queued update after route was recieved. ");

        if (update->invalidates_route == true) {
          RCLCPP_DEBUG_STREAM(logger_, "Applied queued map update has invalidated the route.");
        ...
        RCLCPP_INFO_STREAM(logger_, "Apply from reroute: Done applying updates for new map. However, more updates are waiting for a future map.");
        more_updates_to_apply = false; // If there is more updates queued that are not for this map version assume they are for a future map version
      }
  ...
  if (!world_model_->getMap()) { // This check is a bit redundant but still useful from a debugging perspective as the alternative is a segfault
    RCLCPP_ERROR_STREAM(logger_, "WMListener received a route before a map was available. Dropping route message.");
    return;
  }
  ...
  if (route_invalidated_by_queued_map_update && route_node_flag_){
    RCLCPP_DEBUG_STREAM(logger_,"At least one applied queued map update has invalidated the route. Routing graph will be recomputed.");
    world_model_->setMap(world_model_->getMutableMap(), current_map_version_, route_invalidated_by_queued_map_update);
    RCLCPP_DEBUG_STREAM(logger_,"Finished recomputing the routing graph for the applied queued map update(s)");
    ...
    if(route_opt.is_initialized()) {
      RCLCPP_DEBUG_STREAM(logger_, "Setting route in world model");
