...
{
/**
 * \brief geometry namespace contains utility geometry functions which do not require the map or
 * route provided by an active world model
 */
namespace geometry
{
/*! \brief Returns the TrackPos, computed in 2d, of the provided point relative to the centerline of
 * the provided lanelet. Positive crosstrack will be to the right. Points occuring before the
 * segment will have negative downtrack. See the matchSegment function for a description of the edge
 * cases associated with the max_crosstrack parameter
 *
 ...
 * \param max_crosstrack The maximum crosstrack distance allowed for a perfect centerline segment
 * match. See the matchSegment function for details
 *
...
/*! \brief Return the track position of a point relative to a line segment defined by two other
 * points. Positive crosstrack will be to the right. Points occuring before the segment will have
 * negative downtrack
 *
 ...
 */
TrackPos trackPos(
  const lanelet::BasicPoint2d & p, const lanelet::BasicPoint2d & seg_start,
  const lanelet::BasicPoint2d & seg_end);
/**
 * \brief Get the TrackPos of the provided point along the linestring and the segment of the line
 * string which the provided point should be considered in.
 *
 ...
 * Once the nearest point has been found next step is to determine which segment it should go with
 * using the following rules. If the minimum point is the first point then use the first segment If
 * the minimum point is the last point then use the last segment If the minimum point is within the
 * downtrack bounds of one segment but not the other then use the one it is within If the minimum
 * point is within the downtrack bounds of both segments then use the one with the smallest
 * crosstrack distance If the minimum point is within the downtrack bounds of both segments and has
 ...
 *
 * \return An std::tuple where the first element is the TrackPos of the point and the second element
 * is the matched segment
 */
std::tuple<TrackPos, lanelet::BasicSegment2d> matchSegment(
  const lanelet::BasicPoint2d & p, const lanelet::BasicLineString2d & line_string);
...
/*!
 * \brief Helper function to concatenate 2 linestrings together and return the result. Neither
 * LineString is modified in this function.
 */
lanelet::BasicLineString2d concatenate_line_strings(
  const lanelet::BasicLineString2d & l1, const lanelet::BasicLineString2d & l2);

/*!
 * \brief Helper function to a list of lanelets together and return the result. Neither LineString
...
/*!
 * \brief Use finite differences methods to compute the derivative of the input data set with
 * respect to index Compute the finite differences using the forward finite difference for the first
 * point, centered finite differences for the middle points and backwards finite difference for the
 * final point. This will result in the middle points being a better approximation of the actual
 * derivative than the endpoints.
 *
...
std::vector<Eigen::Vector2d> compute_finite_differences(
  const std::vector<lanelet::BasicPoint2d> & data);
...
/*!
 * \brief Use finite differences methods to compute the derivative of the input data set with
 * respect to the second paramter.
 *
 * Input x and y must be the same length. Compute the finite differences using the forward finite
 * difference for the first point, centered finite differences for the middle points and backwards
 * finite difference for the final point. This will result in the middle points being a better
 * approximation of the actual derivative than the endpoints.
 *
 ...
 */
std::vector<Eigen::Vector2d> compute_finite_differences(
  const std::vector<Eigen::Vector2d> & x, const std::vector<double> & y);
 ...
 *
 * This function is a direct copy of the function by the same name found in the lanelet2_validation
 * package which was not exposed for use The original function can be found here
 ...
 * The source function is copyrighted under BSD 3-Clause "New" or "Revised" License a copy of that
 * notice has been included with this package
 *
 ...
 */
[[deprecated(
  "computeCurvature is deprecated in favor of using the finite differences-based "
  "computeLocalCurvature for "
  "large curves")]] double
computeCurvature(
 ...
 *        The user should still make sure the input data is expected to be within the [-1, 1] range
 * for this method to give accurate results.
 *
...
/**
 * \brief Uses the provided pose and size vector of an ExternalObject to compute what the polygon
 * would be of that object would be if viewed from the same frame as the pose is defined relative
 * to.
 *
 * \param pose the pose of an external object
 * \param size The size vector of an external object
 *
 * \return A polygon of 4 points describing the object aligned bounds starting with the upper left
 * point in the object frame and moving clockwise.
 */
lanelet::BasicPolygon2d objectToMapPolygon(
 ...
 *
 * \returns A vector of doubles representing the yaw value in radians the same frame as the provided
 * centerline.
 *
 */
...
std::vector<double> compute_tangent_orientations(
  const std::vector<lanelet::BasicPoint2d> & centerline);

/**
 * \brief Builds a 2D Eigen coordinate frame transform with not applied scaling (only translation
 * and rotation) based on the provided position and rotation parameters
 *
 ...
 *
 * \return A 2D transformation which is defined such that given a parent frame F and a child frame
 * C, A point P_c in child frame C can be converted to the parent frame using the returned transform
 * T_f_c Therefore P_f = T_f_c * P_c
 */
Eigen::Isometry2d build2dEigenTransform(
  const Eigen::Vector2d & position, const Eigen::Rotation2Dd & rotation);

/**
 * \brief Builds a 3D Eigen coordinate frame transform with not applied scaling (only translation
 * and rotation) based on the provided position and rotation parameters
 *
 * \param position The origin of the child coordinate frame in the parent frame
 * \param rotation The rotation of the child coordinate frame in the parent frame
 *
 * \return A 3D transformation which is defined such that given a parent frame F and a child frame
 * C, A point P_c in child frame C can be converted to the parent frame using the returned transform
 * T_f_c Therefore P_f = T_f_c * P_c
 */
Eigen::Isometry3d build3dEigenTransform(
  const Eigen::Vector3d & position, const Eigen::Quaterniond & rotation);

Eigen::Isometry3d build3dEigenTransform(
  const Eigen::Vector3d & position, const Eigen::AngleAxisd & rotation);
 ...
 */
double point_to_point_yaw(
  const lanelet::BasicPoint2d & cur_point, const lanelet::BasicPoint2d & next_point);

/**
 * \brief Computes the curvature of a circular arc that passes through the two provided points.
 *        If the two points lie on a curve then larger distances between them will result in less
 * accurate, but also less noisy curvature calculations.
 *
 * \param cur_point The first point
 * \param next_point The second point
 *
 * \return The curvature of the circular arc that passes through both points. Curvature is always
 * positive.
 */
...
/**
 * \brief Computes the curvature of each point in the provided list by calling
 * circular_arc_curvature using that point and the point at index current point index + lookahead.
 *        In this way the curvature of a path is effectively low pass filtered by the size of the
 * lookahead value
 *
 ...
 *
 */
std::vector<double> local_circular_arc_curvatures(
  const std::vector<lanelet::BasicPoint2d> & points, int lookahead);
