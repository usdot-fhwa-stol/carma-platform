...
{
/**
 * \brief Anonymous function to extract maneuver end speed which can not be optained with
 ...
 */
void setManeuverLaneletIds(
  carma_planning_msgs::msg::Maneuver & mvr, lanelet::Id start_id, lanelet::Id end_id)
{
  switch (mvr.type) {
    case carma_planning_msgs::msg::Maneuver::LANE_CHANGE:
...
{
  // Declare parameters
  config_.min_plan_duration_ =
    declare_parameter<double>("minimal_plan_duration", config_.min_plan_duration_);
  config_.lane_change_plugin_ =
    declare_parameter<std::string>("lane_change_plugin", config_.lane_change_plugin_);
  config_.stop_and_wait_plugin_ =
    declare_parameter<std::string>("stop_and_wait_plugin", config_.stop_and_wait_plugin_);
  config_.lanefollow_planning_tactical_plugin_ = declare_parameter<std::string>(
    "lane_following_plugin", config_.lanefollow_planning_tactical_plugin_);
  config_.route_end_point_buffer_ = declare_parameter<double>(
    "guidance/route/destination_downtrack_range", config_.route_end_point_buffer_);
  config_.accel_limit_ =
    declare_parameter<double>("vehicle_acceleration_limit", config_.accel_limit_);
  config_.lateral_accel_limit_ =
    declare_parameter<double>("vehicle_lateral_accel_limit", config_.lateral_accel_limit_);
  config_.stopping_accel_limit_multiplier_ = declare_parameter<double>(
    "stopping_accel_limit_multiplier", config_.stopping_accel_limit_multiplier_);
  config_.vehicle_id = declare_parameter<std::string>("vehicle_id", config_.vehicle_id);
  config_.min_maneuver_length_ =
    declare_parameter<double>("min_maneuver_length", config_.min_maneuver_length_);
}
...
carma_ros2_utils::CallbackReturn RouteFollowingPlugin::on_configure_plugin()
{
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("route_following_plugin"), "RouteFollowingPlugin Config: " << config_);

  // Setup subscribers
  twist_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>(
    "current_velocity", 50, std::bind(&RouteFollowingPlugin::twist_cb, this, std_ph::_1));
  current_maneuver_plan_sub_ = create_subscription<carma_planning_msgs::msg::ManeuverPlan>(
    "maneuver_plan", 50,
  ...
  wml_->setMapCallback([this]() {
    if (wm_->getRoute()) {  // If this map update occured after a route was provided we need to
                            // regenerate maneuvers
...
{
  bumper_pose_timer_ = create_timer(
...
void RouteFollowingPlugin::current_maneuver_plan_cb(
  carma_planning_msgs::msg::ManeuverPlan::UniquePtr msg)
{
  current_maneuver_plan_ = std::move(msg);
}

std::vector<carma_planning_msgs::msg::Maneuver> RouteFollowingPlugin::routeCb(
  const lanelet::routing::LaneletPath & route_shortest_path)
{
  ...
  auto nearest_lanelets = lanelet::geometry::findNearest(
    wm_->getMap()->laneletLayer, current_loc_, 10);  // Return 10 nearest lanelets
  ...
  // Find lane changes in path - up to the second to last lanelet in path (till lane change is
  // possible)
  for (shortest_path_index = 0; shortest_path_index < route_shortest_path.size() - 1;
       ++shortest_path_index) {
    ...
    start_dist =
      (maneuvers.empty())
        ? wm_->routeTrackPos(route_shortest_path[shortest_path_index].centerline2d().front())
            .downtrack
        : GET_MANEUVER_PROPERTY(
            maneuvers.back(),
            end_dist);  // TODO_REFAC if there is no initial maneuver start distance and start speed
                        // should be derived from current state. Current state ought to be provided
                        // in planning request
    start_speed = (maneuvers.empty()) ? 0.0 : getManeuverEndSpeed(maneuvers.back());
    RCLCPP_DEBUG_STREAM(
      get_logger(), "start_dist:" << start_dist << ", start_speed:" << start_speed);
    ...
    if (std::fabs(start_dist - end_dist) < 0.1)  // TODO: edge case that was not recreatable.
                                                 // Sometimes start and end dist was same which
                                                 // crashes inlanecruising
    {
      RCLCPP_WARN_STREAM(
        get_logger(), "start and end dist are equal! shortest path id"
                        << shortest_path_index
                        << ", lanelet id:" << route_shortest_path[shortest_path_index].id()
    ...
    if (isLaneChangeNeeded(following_lanelets, route_shortest_path[shortest_path_index + 1].id())) {
      ...
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Recording lanechange start_dist <<"
                        << start_dist
                        << ", from llt id:" << route_shortest_path[shortest_path_index].id()
                        << " to llt id: " << route_shortest_path[shortest_path_index + 1].id());

      maneuvers.push_back(composeLaneChangeManeuverMessage(
        start_dist, end_dist, start_speed, target_speed_in_lanelet,
        route_shortest_path[shortest_path_index].id(),
        route_shortest_path[shortest_path_index + 1].id()));
      ++shortest_path_index;  // Since lane change covers 2 lanelets - skip planning for the next
                              // lanelet

    } else {
      RCLCPP_DEBUG_STREAM(get_logger(), "Lanechange NOT Needed ");
      maneuvers.push_back(composeLaneFollowingManeuverMessage(
        start_dist, end_dist, start_speed, target_speed_in_lanelet,
        {route_shortest_path[shortest_path_index].id()}));
    }
  }
  ...
  if (shortest_path_index < route_shortest_path.size()) {
    ...
    double stopping_entry_speed =
      maneuvers.empty() ? current_speed_ : getManeuverEndSpeed(maneuvers.back());

    // Add stop and wait maneuver based on deceleration target and entry speed
    maneuvers = addStopAndWaitAtRouteEnd(
      maneuvers, route_length, stopping_entry_speed, stopping_accel_limit,
      config_.lateral_accel_limit_, config_.min_maneuver_length_);
  }
  ////------------------
...
std::vector<carma_planning_msgs::msg::Maneuver> RouteFollowingPlugin::addStopAndWaitAtRouteEnd(
  ...
  double lateral_accel_limit, double min_maneuver_length) const
{
   ...
   * 3. If end downtrack - stop distance is > last maneuver end distance then fill delta with lane
   * follow followed by stop and wait
   * 4. If end downtrack - stop distance is < last maneuver end distance then reduce the existing
   * maneuver to allow for stopping maneuver to be planned
   * 5. Add maneuver to list
   */
  std::vector<carma_planning_msgs::msg::Maneuver> maneuvers =
    input_maneuvers;  // Output maneuvers which will be modified
  ...
  // Loop to drop any maneuvers which fully overlap our stopping maneuver while accounting for
  // minimum length maneuver buffers
  while (!maneuvers.empty() && maneuverWithBufferStartsAfterDowntrack(
                                 maneuvers.back(), required_start_downtrack, lateral_accel_limit,
                                 config_.min_maneuver_length_)) {
    RCLCPP_WARN_STREAM(
      get_logger(), "Dropping maneuver with id: " << GET_MANEUVER_PROPERTY(
                      maneuvers.back(), parameters.maneuver_id));

    if (maneuvers.back().type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE) {
      // TODO develop more robust approach for this case per:
      // https://github.com/usdot-fhwa-stol/carma-platform/issues/1350
      throw std::invalid_argument(
        "Stopping at the end of the route requires replanning a lane change. RouteFollowing cannot "
  ...
  double last_maneuver_end_downtrack =
    required_start_downtrack;  // Set default starting location for stop and wait maneuver

  if (!maneuvers.empty()) {  // If there are existing maneuvers we need to make sure stop and wait
    ...
    if (required_start_downtrack >= last_maneuver_end_downtrack) {
      ...
      if (required_start_downtrack - last_maneuver_end_downtrack > min_maneuver_length) {
        ...
        std::vector<lanelet::ConstLanelet> crossed_lanelets = wm_->getLaneletsBetween(
          last_maneuver_end_downtrack, required_start_downtrack, true, false);
        ...
        if (crossed_lanelets.empty()) {
          throw std::invalid_argument(
            ...
            " to: " + std::to_string(required_start_downtrack));
        }

        // Create the lane follow maneuver
        maneuvers.push_back(composeLaneFollowingManeuverMessage(
          last_maneuver_end_downtrack, required_start_downtrack, stopping_entry_speed,
          stopping_entry_speed,
          lanelet::utils::transform(crossed_lanelets, [](auto ll) { return ll.id(); })));

        // Update last maneuver end downtrack so the stop and wait maneuver can be properly
        // formulated
        last_maneuver_end_downtrack = required_start_downtrack;
      } else {
        RCLCPP_DEBUG_STREAM(
          get_logger(),
          "Stop and wait maneuver being extended to nearest maneuver which is closer than the "
          "minimum maneuver length");
      }
      ...
      std::vector<lanelet::ConstLanelet> crossed_lanelets = wm_->getLaneletsBetween(
        GET_MANEUVER_PROPERTY(maneuvers.back(), start_dist), required_start_downtrack, true, false);

      if (crossed_lanelets.empty()) {
        throw std::invalid_argument(
          "Updated maneuver does not cross any lanelets going from: " +
          std::to_string(GET_MANEUVER_PROPERTY(maneuvers.back(), start_dist)) +
          " to: " + std::to_string(required_start_downtrack));
      }

      // Set the impact lane ids for maneuvers
      if (maneuvers.back().type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING) {
        maneuvers.back().lane_following_maneuver.lane_ids = lanelet::utils::transform(
          crossed_lanelets, [](auto ll) { return std::to_string(ll.id()); });

      } else {
        setManeuverLaneletIds(
          maneuvers.back(), crossed_lanelets.front().id(), crossed_lanelets.back().id());
      }
  ...
  std::vector<lanelet::ConstLanelet> crossed_lanelets =
    wm_->getLaneletsBetween(last_maneuver_end_downtrack, route_end_downtrack, true, false);

  if (crossed_lanelets.empty()) {
    throw std::invalid_argument(
      "Stopping maneuver does not cross any lanelets going from: " +
      std::to_string(last_maneuver_end_downtrack) + " to: " + std::to_string(route_end_downtrack));
  }
  ...
  maneuvers.push_back(composeStopAndWaitManeuverMessage(
    last_maneuver_end_downtrack, route_end_downtrack, stopping_entry_speed, start_lane, end_lane,
    stopping_logitudinal_accel));

  return maneuvers;
}

bool RouteFollowingPlugin::maneuverWithBufferStartsAfterDowntrack(
  const carma_planning_msgs::msg::Maneuver & maneuver, double downtrack, double lateral_accel,
  double min_maneuver_length) const
{
  if (maneuver.type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE) {
    ...
    double min_lane_change_distance = std::max(
      min_maneuver_length,
      lane_change_time *
        (GET_MANEUVER_PROPERTY(maneuver, start_speed) + getManeuverEndSpeed(maneuver)) /
        2.0  // dist = v_avg * t
    );
...
bool RouteFollowingPlugin::get_availability() { return true; }

std::string RouteFollowingPlugin::get_version_id() { return "v1.0"; }
  ...
  carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp)
{
  ...
  if (!req->prior_plan.maneuvers.empty()) {
    ...
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Detected a prior plan! Using back maneuver's end_dist:" << current_downtrack);
  } else {
    current_downtrack = req->veh_downtrack;
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Detected NO prior plan! Using req.veh_downtrack: " << current_downtrack);
  }
  ...
  if (!req->prior_plan.maneuvers.empty()) {
    updateTimeProgress(
      new_maneuvers, GET_MANEUVER_PROPERTY(req->prior_plan.maneuvers.back(), end_time));
    RCLCPP_DEBUG_STREAM(
      get_logger(),
      "Detected a prior plan! Using back maneuver's end time:" << std::to_string(
        rclcpp::Time(GET_MANEUVER_PROPERTY(req->prior_plan.maneuvers.back(), end_time)).seconds()));
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Where plan_completion_time was:" << std::to_string(
                      rclcpp::Time(req->prior_plan.planning_completion_time).seconds()));
  } else {
    updateTimeProgress(new_maneuvers, rclcpp::Time(req->header.stamp));
    RCLCPP_DEBUG_STREAM(
      get_logger(),
      "Detected NO prior plan! Using this->now():" << std::to_string(this->now().seconds()));
  }
  ...
  if (!req->prior_plan.maneuvers.empty()) {
    ...
    switch (req->prior_plan.maneuvers.back().type) {
      ...
      case carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_STRAIGHT:
        start_speed =
          req->prior_plan.maneuvers.back().intersection_transit_straight_maneuver.end_speed;
        break;
      case carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_LEFT_TURN:
        start_speed =
          req->prior_plan.maneuvers.back().intersection_transit_left_turn_maneuver.end_speed;
        break;
      case carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_RIGHT_TURN:
        start_speed =
          req->prior_plan.maneuvers.back().intersection_transit_right_turn_maneuver.end_speed;
        break;
      default:
        throw std::invalid_argument(
    ...
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Detected a prior plan! Using back maneuver's end speed:" << start_speed);
  } else {
    updateStartingSpeed(new_maneuvers.front(), req->veh_logitudinal_velocity);
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Detected NO prior plan! Using req->veh_logitudinal_velocity:"
                      << req->veh_logitudinal_velocity);
  }
  // update plan
  ...
  RCLCPP_DEBUG_STREAM(
    get_logger(), "Returning total of maneuver size: " << resp->new_plan.maneuvers.size());
...
void RouteFollowingPlugin::bumper_pose_cb()
{
  ...
  if (
    current_maneuver_plan_ != nullptr &&
    GET_MANEUVER_PROPERTY(
      current_maneuver_plan_->maneuvers[0], parameters.planning_strategic_plugin) !=
    ...
    // !!! ASSUMPTION !!!:
    // Once non-route lanelets have been removed, it is assumed that our actual current lanelet is
    // the only one that can remain.
    // TODO: Verify that this assumption is true in all cases OR implement more robust logic to
    // track current lanelet when there are overlaps
    ...
    if (llts.size() > 1) {
      ...
      RCLCPP_ERROR_STREAM(
        get_logger(), "Vehicle has left the route entirely. Unable to compute new shortest path.");
    ...
    // Replan if vehicle currently located in a lane change planned by route_following_plugin, since
    // a lane change maneuver can't begin part way through
    for (size_t i = 0; i < latest_maneuver_plan_.size(); ++i) {
      if (
        (GET_MANEUVER_PROPERTY(latest_maneuver_plan_[i], start_dist) <= current_progress) &&
        (current_progress <= GET_MANEUVER_PROPERTY(latest_maneuver_plan_[i], end_dist))) {
        if (latest_maneuver_plan_[i].type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE) {
          RCLCPP_DEBUG_STREAM(
            get_logger(),
            "Generating a new shortest path since vehicle is positioned in a lane change but the "
            "previous maneuver plan was not generated by route_following_plugin");
          returnToShortestPath(current_lanelet);
        }
      }
    }
...
rclcpp::Duration RouteFollowingPlugin::getManeuverDuration(
  carma_planning_msgs::msg::Maneuver & maneuver, double epsilon) const
{
  ...
  RCLCPP_DEBUG_STREAM(
    get_logger(), "maneuver_end_dist: " << maneuver_end_dist
                                        << ", maneuver_start_dist: " << maneuver_start_dist
                                        << ", cur_plus_target: " << cur_plus_target);

  duration = rclcpp::Duration::from_nanoseconds(
    (maneuver_end_dist - maneuver_start_dist) / (0.5 * cur_plus_target) * 1e9);

  return duration;
}

void RouteFollowingPlugin::updateTimeProgress(
  std::vector<carma_planning_msgs::msg::Maneuver> & maneuvers, rclcpp::Time start_time) const
{
  ...
  for (auto & maneuver : maneuvers) {
    ...
    switch (maneuver.type) {
      ...
      case carma_planning_msgs::msg::Maneuver::STOP_AND_WAIT:
        ...
        maneuver.stop_and_wait_maneuver.end_time =
          start_time +
          rclcpp::Duration(
            86400, 0);  // Set maneuver time period as 24hrs since this is the end of the route
        break;
      default:
        throw std::invalid_argument(
...
void RouteFollowingPlugin::updateStartingSpeed(
  carma_planning_msgs::msg::Maneuver & maneuver, double start_speed) const
{
  switch (maneuver.type) {
    case carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING:
...
void RouteFollowingPlugin::setManeuverStartDist(
  carma_planning_msgs::msg::Maneuver & maneuver, double start_dist) const
{
  switch (maneuver.type) {
    case carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING:
...
carma_planning_msgs::msg::Maneuver RouteFollowingPlugin::composeLaneFollowingManeuverMessage(
  ...
  const std::vector<lanelet::Id> & lane_ids) const
{
  ...
  // NOTE: The maneuver id is set here because maneuvers are regenerated once per route, so it is
  // acceptable to regenerate them on route updates.
  //       If maneuvers were not generated only on route updates we would want to preserve the ids
  //       across plans
  maneuver_msg.lane_following_maneuver.parameters.maneuver_id = getNewManeuverId();

  std::stringstream ss;
  for (const auto & id : maneuver_msg.lane_following_maneuver.lane_ids) ss << " " << id;
...
carma_planning_msgs::msg::Maneuver RouteFollowingPlugin::composeLaneChangeManeuverMessage(
  double start_dist, double end_dist, double start_speed, double target_speed,
  lanelet::Id starting_lane_id, lanelet::Id ending_lane_id) const
{
  carma_planning_msgs::msg::Maneuver maneuver_msg;
...
carma_planning_msgs::msg::Maneuver RouteFollowingPlugin::composeStopAndWaitManeuverMessage(
  double start_dist, double end_dist, double start_speed, lanelet::Id starting_lane_id,
  lanelet::Id ending_lane_id, double stopping_accel) const
{
  carma_planning_msgs::msg::Maneuver maneuver_msg;
  ...
  maneuver_msg.stop_and_wait_maneuver.parameters.float_valued_meta_data.push_back(
    config_.route_end_point_buffer_);
  ...
  RCLCPP_DEBUG_STREAM(
    get_logger(), "Creating stop and wait maneuver id:"
                    << maneuver_msg.stop_and_wait_maneuver.parameters.maneuver_id << "start dist: "
                    << start_dist << " end dist: " << end_dist << " start_speed: " << start_speed
                    << " Starting llt: " << starting_lane_id << " Ending llt: " << ending_lane_id);

  return maneuver_msg;
}

bool RouteFollowingPlugin::isLaneChangeNeeded(
  lanelet::routing::LaneletRelations relations, lanelet::Id target_id) const
{
  // This method is constrained to the lanelet being checked against being accessible. A
  // non-accessible target lanelet would result in unspecified behavior
  for (auto & relation : relations) {
    if (
      relation.lanelet.id() == target_id &&
...
void RouteFollowingPlugin::returnToShortestPath(const lanelet::ConstLanelet & current_lanelet)
{
  ...
  // Obtain the lanelet following the current lanelet in this same lane, this lanelet must exist
  // since the new shortest path cannot begin with a lane change
  ...
  if (!current_lane_following_lanelets.empty()) {
    ...
    throw std::invalid_argument(
      "The current lanelet does not have a following lanelet. Vehicle cannot return to shortest "
      "path!");
  }

  // In order to return to the shortest path, the closest future lanelet on the shortest path needs
  // to be found. That is the adjacent lanelet of the following lanelet of the current lanelet.
  ...
  if (!adjacent_lanelets.empty()) {
    for (const auto & adjacent : adjacent_lanelets) {
      if (shortest_path_set_.find(adjacent.id()) != shortest_path_set_.end()) {
        ...
        RCLCPP_DEBUG_STREAM(
          get_logger(), "The target_following_lanelet id is: " << target_following_lanelet.id());
        ...
        // a new shortest path, via the the lanelets in 'interm' is calculated and used an
        // alternative shortest path
        auto new_shortestpath =
          routing_graph->shortestPathVia(current_lanelet, interm, original_shortestpath.back());
        RCLCPP_DEBUG_STREAM(
          get_logger(), "a new shortestpath is generated to return to original shortestpath");
        ...
        // a new shortest path, via the current_lanelet is calculated and used an alternative
        // shortest path
        lanelet::ConstLanelets new_interm;
        new_interm.push_back(static_cast<lanelet::ConstLanelet>(current_lane_following_lanelet));
        auto new_shortestpath =
          routing_graph->shortestPathVia(current_lanelet, new_interm, original_shortestpath.back());
        RCLCPP_DEBUG_STREAM(
          get_logger(),
          "Cannot return to the original shortestpath from adjacent lanes, so a new shortestpath "
          "is generated");
        // routeCb is called to update latest_maneuver_plan_
        if (new_shortestpath) this->latest_maneuver_plan_ = routeCb(new_shortestpath.get());
      }
