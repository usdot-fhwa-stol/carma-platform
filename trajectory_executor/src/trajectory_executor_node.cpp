...
: carma_ros2_utils::CarmaLifecycleNode(options)
{
  ...
  // Declare parameters
  config_.trajectory_publish_rate =
    declare_parameter<double>("trajectory_publish_rate", config_.trajectory_publish_rate);
  config_.default_control_plugin =
    declare_parameter<std::string>("default_control_plugin", config_.default_control_plugin);
  config_.default_control_plugin_topic = declare_parameter<std::string>(
    "default_control_plugin_topic", config_.default_control_plugin_topic);
}

rcl_interfaces::msg::SetParametersResult TrajectoryExecutor::parameter_update_callback(
  const std::vector<rclcpp::Parameter> & parameters)
{
  auto error = update_params<double>(
    {{"trajectory_publish_rate", config_.trajectory_publish_rate}}, parameters);
...
carma_ros2_utils::CallbackReturn TrajectoryExecutor::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  add_on_set_parameters_callback(
    std::bind(&TrajectoryExecutor::parameter_update_callback, this, std_ph::_1));

  // Setup subscribers
  plan_sub_ = create_subscription<carma_planning_msgs::msg::TrajectoryPlan>(
    "trajectory", 5, std::bind(&TrajectoryExecutor::onNewTrajectoryPlan, this, std_ph::_1));
  state_sub_ = create_subscription<carma_planning_msgs::msg::GuidanceState>(
    "state", 5, std::bind(&TrajectoryExecutor::guidanceStateMonitor, this, std_ph::_1));
  ...
  std::map<std::string, carma_ros2_utils::PubPtr<carma_planning_msgs::msg::TrajectoryPlan>>
    control_plugin_topics;
  ...
  for (auto it = discovered_control_plugins.begin(); it != discovered_control_plugins.end(); it++) {
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Trajectory executor discovered control plugin "
                      << it->first.c_str() << " listening on topic " << it->second.c_str());
    carma_ros2_utils::PubPtr<carma_planning_msgs::msg::TrajectoryPlan> control_plugin_pub =
      create_publisher<carma_planning_msgs::msg::TrajectoryPlan>(it->second, 1000);
...
carma_ros2_utils::CallbackReturn TrajectoryExecutor::handle_on_activate(
  const rclcpp_lifecycle::State &)
{
  // Create timer for publishing outbound trajectories to the control plugins
  int timer_period_ms = (1 / config_.trajectory_publish_rate) *
                        1000;  // Conversion from frequency (Hz) to milliseconds time period

  timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(timer_period_ms),
...
void TrajectoryExecutor::onTrajEmitTick()
{
  ...
  if (cur_traj_ != nullptr) {
    ...
    std::map<std::string, carma_ros2_utils::PubPtr<carma_planning_msgs::msg::TrajectoryPlan>>::
      iterator it = traj_publisher_map_.find(control_plugin);
    if (it != traj_publisher_map_.end()) {
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Found match for control plugin " << control_plugin.c_str() << " at point "
                                                        << timesteps_since_last_traj_
                                                        << " in current trajectory!");
      it->second->publish(*cur_traj_);
    } else {
      std::ostringstream description_builder;
      description_builder << "No match found for control plugin " << control_plugin << " at point "
...
void TrajectoryExecutor::onNewTrajectoryPlan(
  carma_planning_msgs::msg::TrajectoryPlan::UniquePtr msg)
{
  ...
  RCLCPP_DEBUG_STREAM(
    get_logger(), "New plan contains " << msg->trajectory_points.size() << " points");
...
void TrajectoryExecutor::guidanceStateMonitor(
