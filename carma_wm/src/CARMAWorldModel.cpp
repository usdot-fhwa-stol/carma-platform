...
namespace carma_wm
{
  ...
  TrackPos CARMAWorldModel::routeTrackPos(const lanelet::BasicPoint2d& point) const {
    ...
    lanelet::Points3d near_points = shortest_path_filtered_centerline_view_->pointLayer.nearest(point, 1); // Find the nearest points
    
    // Match point with linestring using fast map lookup
    auto lineString_1 = lanelet::utils::to2D(shortest_path_filtered_centerline_view_->lineStringLayer.findUsages(near_points[0]).front()); // Only need the first index due to nature of route centerline
...
// Find best route segment
    ...
    if (near_points[0].id() == lineString_1.front().id() ) { // Nearest point is at the start of a line string
        ...
        if (tp_next.downtrack >= 0 || ls_i == 0) {
          ...
          tp = trackPos(point, prev_centerline[prev_centerline.size() - 2].basicPoint(), prev_centerline[prev_centerline.size() - 1].basicPoint());
    ...
    } else if (near_points[0].id() == lineString_1.back().id()) { // Nearest point is the end of a line string
        ...
        double last_seg_length = shortest_path_distance_map_.distanceBetween(ls_i, lineString_1.size() - 2, lineString_1.size() - 1);
    ...
    } else { // The nearest point is in the middle of a line string 
      // Graph the two bounding points on the line string and call matchSegment using a 3 element segment
      // There is a guarantee from the earlier if statements that near_points[0] will always be located at an index within the exclusive range (0,lineString_1.size() - 1) so no need for range checks
      
      lanelet::BasicLineString2d subSegment = lanelet::BasicLineString2d({lineString_1[p_i - 1].basicPoint(), lineString_1[p_i].basicPoint(), lineString_1[p_i + 1].basicPoint()});
  ...
  std::vector<lanelet::ConstLanelet> CARMAWorldModel::getLaneletsBetween(double start, double end) const {
    ...
    for (lanelet::ConstLanelet lanelet : lanelet_map->laneletLayer) {
      ...
      if (std::max(min.downtrack, start) > std::min(max.downtrack, end)) { // Check for 1d intersection
  ...
  std::vector<std::tuple<size_t, std::vector<double>>> CARMAWorldModel::getLocalCurvatures(const std::vector<lanelet::ConstLanelet>& lanelets) const {
  ...
  void CARMAWorldModel::computeDowntrackReferenceLine() {
    ...
    lanelet::traffic_rules::TrafficRulesUPtr traffic_rules = lanelet::traffic_rules::TrafficRulesFactory::create(lanelet::Locations::Germany, lanelet::Participants::VehicleCar);
    lanelet::routing::RoutingGraphUPtr shortest_path_graph = lanelet::routing::RoutingGraph::build(*shortest_path_view_, *traffic_rules);

    std::vector<lanelet::LineString3d> lineStrings; // List of continuos line strings representing segments of the route reference line
    ...
    for(lanelet::ConstLanelet ll : shortest_path) {
      next_index++;
      if (first) { // For the first lanelet store its centerline and length
        lineStrings.push_back(copyConstructLineString(ll.centerline()));
        first = false;
      } 
      if (next_index < shortest_path.size()) { // Check for remaining lanelets
        ...
        if (connectionCount == 1) { // Get list of connected lanelets without lanechanges. On the shortest path this should only return 1 or 0
          // No lane change
    ...
    if (shortest_path_centerlines_.size() > shortest_path_distance_map_.size()) {
      shortest_path_distance_map_.pushBack(lanelet::utils::to2D(lineStrings.back())); // Record length of last continuous segment
    }
  ...
  LaneletRoutingGraphConstPtr CARMAWorldModel::getMapRoutingGraph() const {
    return std::static_pointer_cast< const lanelet::routing::RoutingGraph >(map_routing_graph_); // Cast pointer to const variant
  }

  // NOTE: See header file for details on source of logic in this function
  double CARMAWorldModel::computeCurvature(const lanelet::BasicPoint2d& p1, const lanelet::BasicPoint2d& p2, const lanelet::BasicPoint2d& p3) const {
    auto dp = 0.5 * (p3 - p1);
 ...
 TrackPos CARMAWorldModel::trackPos(const lanelet::BasicPoint2d& p, const lanelet::BasicPoint2d& seg_start, const lanelet::BasicPoint2d& seg_end) const {
    ...
    double sign = d >= 0 ? 1.0 : -1.0; // If d is positive then the point is to the right if it is negative the point is to the left
  ...
  bool CARMAWorldModel::selectFirstSegment(const TrackPos& first_seg_trackPos, const TrackPos& second_seg_trackPos, double first_seg_length, double second_seg_length) const {
    const bool first_seg_in_downtrack = (0 <= first_seg_trackPos.downtrack && first_seg_trackPos.downtrack < first_seg_length);
    const bool second_seg_in_downtrack = (0 <= second_seg_trackPos.downtrack && second_seg_trackPos.downtrack < second_seg_length);
    
    if (first_seg_in_downtrack && !second_seg_in_downtrack) { // If in the first segment but not the last segment
      ...
      return first_seg_trackPos.crosstrack <= second_seg_trackPos.crosstrack; // Pick the first segment if the crosstrack values are equal or the first segment is closer
    
    } else { // Point lies outside the downtrack bounds of both segments (Remember According to function contract the nearest point to external point is the midpoint of the two segments)
      return true; // Choose first segment as it will always have positive downtrack in this case while the second segment will always have negative downtrack
    }
  }

  std::tuple<TrackPos, lanelet::BasicSegment2d> CARMAWorldModel::matchSegment(const lanelet::BasicPoint2d& p, const lanelet::BasicLineString2d& line_string) const {

    if (line_string.size() < 2) {
      throw std::invalid_argument("Provided with linestring containing fewer than 2 points");
    }
    
    lanelet::BasicSegment2d best_segment = std::make_pair(line_string[0], line_string[1]); // Default to starting segment if no match is found
    ...
    for (size_t i = 0; i < line_string.size(); i++) { // Iterate over line string to find nearest point
      double seg_length = 0;
      ...
      if (distance < min_distance) { // If this distance is below minimum discovered so far then update minimum
