...
namespace carma_wm
{
                                                              ...
                                                              const std::shared_ptr<lanelet::LaneletMap>& map, std::shared_ptr<const lanelet::routing::RoutingGraph> current_routing_graph)
  {
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Reference node in map frame x: " << ref_node.x() << ", y: " << ref_node.y());
    ...
    for (auto lane : intersection.lane_list)
    {
      ...
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Lane id: " << (int)lane.lane_id << ", is not a lane for vehicle. Only vehicle road is currently supported. Skipping..." );
        continue;
      }
      ...
      if (intersection_coord_correction_.find(intersection.id.id) != intersection_coord_correction_.end())
      {
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Applied reference point correction, delta_x: " <<  intersection_coord_correction_[intersection.id.id].first <<
                          ", delta_y: " << intersection_coord_correction_[intersection.id.id].second << ", to intersection id: " << (int)lane.lane_id);
      }

      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Processing Lane id: " << (int)lane.lane_id);
      ...
      for (size_t i = 0; i < size_of_available_points; i ++ )
      {
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Current node x: " << curr_x << ", y: " << curr_y);

        node_list.push_back(curr_node);
      }

      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Lane directions: " << (int)lane.lane_attributes.directional_use.lane_direction);
      ...
      if (lane.lane_attributes.directional_use.lane_direction == LANE_DIRECTION::INGRESS)
      {
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Reversed the node list!");
      ...
      if (lane.lane_attributes.directional_use.lane_direction == LANE_DIRECTION::EGRESS)
      {
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Default corresponding_lanelet_id: " << corresponding_lanelet_id <<", in EGRESS");

      }
      else //ingress
      {
        corresponding_lanelet_id = affected_llts.back().id();
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Default corresponding_lanelet_id: " << corresponding_lanelet_id <<", in INGRESS");
      }

      for (auto llt : affected_llts) // filter out intersection lanelets
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Checking if we can get entry/exit from lanelet " << llt.id());
            ...
            llt.lanelet().get().attribute("turn_direction").value().compare("right") == 0))
        {
          ...
          if (lane.lane_attributes.directional_use.lane_direction == LANE_DIRECTION::EGRESS)
          {
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "lanelet " << llt.id() << " is actually part of the intersecion. Trying to detect EGRESS...");
            connecting_llts = current_routing_graph->following(llt.lanelet().get());
          }
          else
          {
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "lanelet " << llt.id() << " is actually part of the intersecion. Trying to detect INGRESS...");
      ...
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Found existing lanelet id: " << corresponding_lanelet_id);

      if (lane.lane_attributes.directional_use.lane_direction == LANE_DIRECTION::INGRESS)
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Detected INGRESS, " << (int)lane.lane_id);
      ...
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Detected EGRESS, " << (int)lane.lane_id);
    ...
    for (auto iter = signal_group_to_exit_lanes.begin(); iter != signal_group_to_exit_lanes.end(); iter++)
    {
      for (auto exit_lane : iter->second)
      {
        ...
        {
          RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Adding exit_lane id: " << exit_lane);
    ...
    for (auto iter = signal_group_to_entry_lanes.begin(); iter != signal_group_to_entry_lanes.end(); iter++)
    {
      ...
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Adding entry_lane id: " << entry_lane);
                                                                      ...
                                                                      const std::shared_ptr<lanelet::LaneletMap>& map, std::shared_ptr<const lanelet::routing::RoutingGraph> routing_graph)
  {
    ...
    if (target_frame_ == "")
    {
      logWarnOnce("Logging Once: Georeference is not loaded yet. Returning without processing this MAP msg.");
      return;
    }
    ...
    for (auto const& intersection : map_msg.intersections)
    {
      ...
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "No existing intersection found. Creating a new one...");
    ...
    {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm::SignalizedIntersectionManager"), "Creating signal for: " << (int)sig_grp_pair.first);
  ...
  void SignalizedIntersectionManager::processSpatFromMsg(const carma_v2x_msgs::msg::SPAT &spat_msg, const std::shared_ptr<lanelet::LaneletMap>& semantic_map)
  {
    ...
    {
      logWarnOnce("Logging Once: No intersection_state_list in the newly received SPAT msg. Returning...");
      return;
    }
   ...
   */
  void SignalizedIntersectionManager::logInfoOnce(const std::string& message) const {
    static std::mutex log_mutex;

    std::lock_guard<std::mutex> lock(log_mutex);
    if (previous_busy_log_streams_.find(message) == previous_busy_log_streams_.end()) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm"), message);
   ...
   * @param message The message to log
   */
  void SignalizedIntersectionManager::logWarnOnce(const std::string& message) const {
    static std::mutex log_mutex;
  ...
  lanelet::Id SignalizedIntersectionManager::getTrafficSignalId(uint16_t intersection_id, uint8_t signal_group_id)
  {
    ...
    if (intersection_id_to_regem_id_.find(intersection_id) == intersection_id_to_regem_id_.end())
    {
      ...
      logWarnOnce("Logging Once: Intersection id: "
    ...
    {
      logWarnOnce("Logging Once: Signal group id: "
    ...
    lanelet::Id& id, const std::shared_ptr<lanelet::LaneletMap>& semantic_map) const
  {
    ...
    {
      logWarnOnce(
        "Logging Once: There was an error querying lanelet for traffic light with id: "
    ...
    {
      logWarnOnce(
        "Logging Once: There was an error querying traffic light with id: "
        + std::to_string((int)id));
      return nullptr;
    }
    ...
    {
      logWarnOnce(
        "Logging Once: Was not able to find traffic signal with id: "
        + std::to_string((int)id) + ", ignoring...");
      return nullptr;
    }
      ...
      const carma_v2x_msgs::msg::MovementState& current_movement_state)
  {
    ...
    for(const auto& current_movement_event:current_movement_state.movement_event_list)
    {
      ...
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm"), "intersection id: "
