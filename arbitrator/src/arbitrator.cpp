...
void Arbitrator::run()
{
  if (
    ...
    switch (sm_->get_state()) {
      ...
      case SHUTDOWN:
        RCLCPP_INFO_STREAM(
          nh_->get_logger(), "Arbitrator shutting down after being commanded to shutdown!");
        rclcpp::shutdown();
        exit(0);
        break;
      default:
...
void Arbitrator::guidance_state_cb(carma_planning_msgs::msg::GuidanceState::UniquePtr msg)
{
  switch (msg->state) {
    ...
    case carma_planning_msgs::msg::GuidanceState::DRIVERS_READY:
      if (
        sm_->get_state() == ArbitratorState::PLANNING ||
...
void Arbitrator::initial_state()
{
  if (!initialized_) {
    ...
    final_plan_pub_ =
      nh_->create_publisher<carma_planning_msgs::msg::ManeuverPlan>("final_maneuver_plan", 5);
    guidance_state_sub_ = nh_->create_subscription<carma_planning_msgs::msg::GuidanceState>(
      "guidance_state", 5, std::bind(&Arbitrator::guidance_state_cb, this, std::placeholders::_1));
...
void Arbitrator::planning_state()
{
  ...
  if (!plan.maneuvers.empty()) {
    ...
    if (plan_duration < min_plan_duration_) {
      ...
      RCLCPP_INFO_STREAM(
        nh_->get_logger(), "Arbitrator is publishing plan "
                             << std::string(plan.maneuver_plan_id) << " of duration "
                             << plan_duration.seconds() << " as current maneuver plan");
    }
...
void Arbitrator::paused_state() { std::this_thread::sleep_for(std::chrono::milliseconds(100)); }
...
void Arbitrator::bumper_pose_cb()
{
  ...
  if (wm_->getRoute()) {
    ...
    auto lanelets =
      wm_->getLaneletsBetween(vehicle_state_.downtrack, vehicle_state_.downtrack, true);
