...
class GeofenceScheduler
{
  ...
  std::function<void(std::shared_ptr<carma_wm_ctrl::Geofence>)> active_callback_;
  std::function<void(std::shared_ptr<carma_wm_ctrl::Geofence>)> inactive_callback_;
...
public:
   ...
   */
  void addGeofence(std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr);
   ...
   */
  void onGeofenceActive(std::function<void(std::shared_ptr<carma_wm_ctrl::Geofence>)> active_callback);
  /**
   ...
   */
  void onGeofenceInactive(std::function<void(std::shared_ptr<carma_wm_ctrl::Geofence>)> inactive_callback);
...
private:
   ...
   */
  void startGeofenceCallback(const ros::TimerEvent& event, std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr, const int32_t timer_id);
  /**
   ...
   * @param timer_id The id of the timer which caused this callback to occur
   */
  void endGeofenceCallback(const ros::TimerEvent& event, std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr, const int32_t timer_id);
};
}  // namespace carma_wm_ctrl