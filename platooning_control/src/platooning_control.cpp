...
: carma_guidance_plugins::ControlPlugin(options),
  ...
  pcw_(PlatooningControlWorker())
{
  // Declare parameters
  config_.stand_still_headway_m =
    declare_parameter<double>("stand_still_headway_m", config_.stand_still_headway_m);
  ...
  config_.max_delta_speed_per_timestep =
    declare_parameter<double>("max_delta_speed_per_timestep", config_.max_delta_speed_per_timestep);
  config_.min_delta_speed_per_timestep =
    declare_parameter<double>("min_delta_speed_per_timestep", config_.min_delta_speed_per_timestep);
  config_.adjustment_cap_mps =
    declare_parameter<double>("adjustment_cap_mps", config_.adjustment_cap_mps);
  ...
  config_.vehicle_response_lag =
    declare_parameter<double>("vehicle_response_lag", config_.vehicle_response_lag);
  config_.max_lookahead_dist =
    declare_parameter<double>("maximum_lookahead_distance", config_.max_lookahead_dist);
  config_.min_lookahead_dist =
    declare_parameter<double>("minimum_lookahead_distance", config_.min_lookahead_dist);
  config_.speed_to_lookahead_ratio =
    declare_parameter<double>("speed_to_lookahead_ratio", config_.speed_to_lookahead_ratio);
  config_.is_interpolate_lookahead_point = declare_parameter<bool>(
    "is_interpolate_lookahead_point", config_.is_interpolate_lookahead_point);
  config_.is_delay_compensation =
    declare_parameter<bool>("is_delay_compensation", config_.is_delay_compensation);
  config_.emergency_stop_distance =
    declare_parameter<double>("emergency_stop_distance", config_.emergency_stop_distance);
  config_.speed_thres_traveling_direction = declare_parameter<double>(
    "speed_thres_traveling_direction", config_.speed_thres_traveling_direction);
  config_.dist_front_rear_wheels =
    declare_parameter<double>("dist_front_rear_wheels", config_.dist_front_rear_wheels);

  config_.dt = declare_parameter<double>("dt", config_.dt);
  config_.integrator_max_pp =
    declare_parameter<double>("integrator_max_pp", config_.integrator_max_pp);
  config_.integrator_min_pp =
    declare_parameter<double>("integrator_min_pp", config_.integrator_min_pp);
  config_.ki_pp = declare_parameter<double>("Ki_pp", config_.ki_pp);
  config_.is_integrator_enabled =
    declare_parameter<bool>("is_integrator_enabled", config_.is_integrator_enabled);
  config_.enable_max_adjustment_filter =
    declare_parameter<bool>("enable_max_adjustment_filter", config_.enable_max_adjustment_filter);
  config_.enable_max_accel_filter =
    declare_parameter<bool>("enable_max_accel_filter", config_.enable_max_accel_filter);
  ...
  config_.shutdown_timeout =
    declare_parameter<int>("control_plugin_shutdown_timeout", config_.shutdown_timeout);
  config_.ignore_initial_inputs =
    declare_parameter<int>("control_plugin_ignore_initial_inputs", config_.ignore_initial_inputs);

  pcw_.ctrl_config_ = std::make_shared<PlatooningControlPluginConfig>(config_);
}

rcl_interfaces::msg::SetParametersResult PlatooningControlPlugin::parameter_update_callback(
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  // vehicle_id, control_plugin_shutdown_timeout and control_plugin_ignore_initial_inputs are not
  // updated as they are global params
...
carma_ros2_utils::CallbackReturn PlatooningControlPlugin::on_configure_plugin()
{
  ...
  add_on_set_parameters_callback(
    std::bind(&PlatooningControlPlugin::parameter_update_callback, this, std_ph::_1));

  // Trajectory Plan Subscriber
  trajectory_plan_sub_ = create_subscription<carma_planning_msgs::msg::TrajectoryPlan>(
  ...
  platoon_info_sub_ = create_subscription<carma_planning_msgs::msg::PlatooningInfo>(
    "platoon_info", 1, std::bind(&PlatooningControlPlugin::platoon_info_cb, this, std_ph::_1));

  // Control Publishers
  platoon_info_pub_ =
    create_publisher<carma_planning_msgs::msg::PlatooningInfo>("platooning_info", 1);
...
autoware_msgs::msg::ControlCommandStamped PlatooningControlPlugin::generate_command()
{
  ...
  if (!current_trajectory_ || !current_pose_ || !current_twist_) return ctrl_msg;
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "current_time_ms = " << current_time_ms << ", prev_input_time_ms_ = " << prev_input_time_ms_
                         << ", input counter = " << consecutive_input_counter_);
  ...
  if (consecutive_input_counter_ <= config_.ignore_initial_inputs) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_control"), "returning due to first data input");
    return ctrl_msg;
  }

  carma_planning_msgs::msg::TrajectoryPlanPoint second_trajectory_point =
  ...
  ctrl_msg =
    generate_control_signals(second_trajectory_point, current_pose_.get(), current_twist_.get());

  return ctrl_msg;
}

void PlatooningControlPlugin::platoon_info_cb(
  const carma_planning_msgs::msg::PlatooningInfo::SharedPtr msg)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_control"),
    "Platoon leader leader id:  " << platoon_leader_.staticId);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_control"),
    "Platoon leader leader pose:  " << platoon_leader_.vehiclePosition);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_control"),
    "Platoon leader leader cmd speed:  " << platoon_leader_.commandSpeed);
...
autoware_msgs::msg::ControlCommandStamped PlatooningControlPlugin::generate_control_signals(
...
{
  pcw_.set_current_speed(trajectory_speed_);  // TODO why this and not the actual vehicle speed?
                                              // Method name suggests different use than this.
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "Forced from frame_id: " << state_tf.header.frame_id
                             << ", into: " << current_trajectory_.get().header.frame_id);

  current_trajectory_.get().header.frame_id = state_tf.header.frame_id;

  auto autoware_traj_plan = basic_autonomy::waypoint_generation::process_trajectory_plan(
    current_trajectory_.get(), config_.vehicle_response_lag);
  ...
  auto steer_cmd =
    cmd
      .front_wheel_angle_rad;  // autoware sets the front wheel angle as the calculated steer.
                               // https://github.com/usdot-fhwa-stol/autoware.auto/blob/3450f94fa694f51b00de272d412722d65a2c2d3e/AutowareAuto/src/control/pure_pursuit/src/pure_pursuit.cpp#L88
...
motion::motion_common::State PlatooningControlPlugin::convert_state(
  const geometry_msgs::msg::PoseStamped & pose,
  const geometry_msgs::msg::TwistStamped & twist) const
{
...
void PlatooningControlPlugin::current_trajectory_callback(
  const carma_planning_msgs::msg::TrajectoryPlan::UniquePtr tp)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_control"),
    "New trajectory plan #" << consecutive_input_counter_ << " at time " << prev_input_time_ms_);
...
geometry_msgs::msg::TwistStamped PlatooningControlPlugin::compose_twist_cmd(
...
autoware_msgs::msg::ControlCommandStamped PlatooningControlPlugin::compose_ctrl_cmd(
...
std::string PlatooningControlPlugin::get_version_id() { return "v1.0"; }

// extract maximum speed of trajectory
double PlatooningControlPlugin::get_trajectory_speed(
  const std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> & trajectory_points)
{
  ...
  double t1 =
    (rclcpp::Time((trajectory_points[trajectory_points.size() - 1].target_time)).nanoseconds() -
     rclcpp::Time(trajectory_points[0].target_time).nanoseconds()) /
    1e9;

  double avg_speed = d1 / t1;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_control"),
    "trajectory_points size = " << trajectory_points.size() << ", d1 = " << d1 << ", t1 = " << t1
                                << ", avg_speed = " << avg_speed);
  ...
  for (size_t i = 0; i < trajectory_points.size() - 2; i++) {
    ...
    double t = rclcpp::Time((trajectory_points[i + 1].target_time)).seconds() -
               rclcpp::Time(trajectory_points[i].target_time).seconds();
    double v = d / t;
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_control"), "trajectory speed: " << trajectory_speed);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_control"), "avg trajectory speed: " << avg_speed);
