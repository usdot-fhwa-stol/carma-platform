...
: carma_guidance_plugins::TacticalPlugin(options)
{
  ...
  config_.trajectory_time_length =
    declare_parameter<double>("trajectory_time_length", config_.trajectory_time_length);
  config_.control_plugin_name =
    declare_parameter<std::string>("control_plugin_name", config_.control_plugin_name);
  ...
  config_.minimum_lookahead_distance =
    declare_parameter<double>("minimum_lookahead_distance", config_.minimum_lookahead_distance);
  config_.maximum_lookahead_distance =
    declare_parameter<double>("maximum_lookahead_distance", config_.maximum_lookahead_distance);
  config_.minimum_lookahead_speed =
    declare_parameter<double>("minimum_lookahead_speed", config_.minimum_lookahead_speed);
  config_.maximum_lookahead_speed =
    declare_parameter<double>("maximum_lookahead_speed", config_.maximum_lookahead_speed);
  config_.lateral_accel_limit =
    declare_parameter<double>("lateral_accel_limit", config_.lateral_accel_limit);
  config_.speed_moving_average_window_size = declare_parameter<int>(
    "speed_moving_average_window_size", config_.speed_moving_average_window_size);
  config_.curvature_moving_average_window_size = declare_parameter<int>(
    "curvature_moving_average_window_size", config_.curvature_moving_average_window_size);
  config_.curvature_calc_lookahead_count = declare_parameter<int>(
  ...
  config_.destination_range =
    declare_parameter<double>("destination_range", config_.destination_range);
  config_.lanechange_time_out =
    declare_parameter<double>("lanechange_time_out", config_.lanechange_time_out);
  config_.min_timestep = declare_parameter<double>("min_timestep", config_.min_timestep);
  config_.starting_downtrack_range =
    declare_parameter<double>("starting_downtrack_range", config_.starting_downtrack_range);
  config_.starting_fraction =
    declare_parameter<double>("starting_fraction", config_.starting_fraction);
  ...
  config_.desired_time_gap =
    declare_parameter<double>("desired_time_gap", config_.desired_time_gap);
  config_.turn_downsample_ratio =
    declare_parameter<int>("turn_downsample_ratio", config_.turn_downsample_ratio);
  config_.curve_resample_step_size =
    declare_parameter<double>("curve_resample_step_size", config_.curve_resample_step_size);
  config_.back_distance = declare_parameter<double>("back_distance", config_.back_distance);
  config_.buffer_ending_downtrack =
    declare_parameter<double>("buffer_ending_downtrack", config_.buffer_ending_downtrack);
  config_.vehicle_id = declare_parameter<std::string>("vehicle_id", config_.vehicle_id);
}

rcl_interfaces::msg::SetParametersResult CooperativeLaneChangePlugin::parameter_update_callback(
  const std::vector<rclcpp::Parameter> & parameters)
{
  auto error = update_params<std::string>(
    {{"control_plugin_name", config_.control_plugin_name}, {"vehicle_id", config_.vehicle_id}},
    parameters);
  ...
  auto error_2 = update_params<double>(
    {{"trajectory_time_length", config_.trajectory_time_length},
     ...
     {"desired_time_gap", config_.desired_time_gap}},
    parameters);
...
carma_ros2_utils::CallbackReturn CooperativeLaneChangePlugin::on_configure_plugin()
{
  ...
  add_on_set_parameters_callback(
    std::bind(&CooperativeLaneChangePlugin::parameter_update_callback, this, std_ph::_1));

  RCLCPP_INFO_STREAM(get_logger(), "Loaded params: " << config_);

  // Setup subscribers
  pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "current_pose", 1, std::bind(&CooperativeLaneChangePlugin::pose_cb, this, std_ph::_1));
  twist_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>(
    "current_velocity", 1, std::bind(&CooperativeLaneChangePlugin::twist_cb, this, std_ph::_1));
  incoming_mobility_response_sub_ = create_subscription<carma_v2x_msgs::msg::MobilityResponse>(
    "incoming_mobility_response", 1,
  ...
  georeference_sub_ = create_subscription<std_msgs::msg::String>(
    "georeference", 1, std::bind(&CooperativeLaneChangePlugin::georeference_cb, this, std_ph::_1));
  bsm_sub_ = create_subscription<carma_v2x_msgs::msg::BSM>(
    "bsm_outbound", 1, std::bind(&CooperativeLaneChangePlugin::bsm_cb, this, std_ph::_1));

  // Setup publishers
  outgoing_mobility_request_pub_ = create_publisher<carma_v2x_msgs::msg::MobilityRequest>(
    "outgoing_mobility_request", 5);  // Rate from yield plugin
  lanechange_status_pub_ = create_publisher<carma_planning_msgs::msg::LaneChangeStatus>(
...
void CooperativeLaneChangePlugin::mobilityresponse_cb(
...
double CooperativeLaneChangePlugin::find_current_gap(
  long veh2_lanelet_id, double veh2_downtrack,
  carma_planning_msgs::msg::VehicleState & ego_state) const
{
  ...
  RCLCPP_DEBUG_STREAM(
    get_logger(),
    "Veh1 current downtrack: " << veh1_current_downtrack << " veh2 downtrack: " << veh2_downtrack);
  ...
  carma_planning_msgs::srv::PlanTrajectory::Response::SharedPtr resp)
{
  ...
  if (
    req->maneuver_plan.maneuvers[req->maneuver_index_to_plan].type !=
    carma_planning_msgs::msg::Maneuver::LANE_CHANGE) {
  ...
  RCLCPP_DEBUG_STREAM(
    get_logger(), "Starting CLC downtrack: " << maneuver_plan[0].lane_change_maneuver.start_dist);

  if (
    current_downtrack <
    maneuver_plan[0].lane_change_maneuver.start_dist - config_.starting_downtrack_range) {
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Lane change trajectory will not be planned. current_downtrack is more than "
                      << config_.starting_downtrack_range
  ...
  if (
    current_lanelet_id == target_lanelet_id &&
    current_downtrack >= target_downtrack - config_.destination_range) {
    carma_planning_msgs::msg::LaneChangeStatus lc_status_msg;
  ...
  for (int i = 0; i < rwol.size(); i++) {
    if (
      rwol[i].connected_vehicle_type.type ==
  ...
  if (foundRoadwayObject) {
    ...
    RCLCPP_DEBUG_STREAM(
      get_logger(),
      "veh2_lanelet_id: " << veh2_lanelet_id << ", veh2_downtrack: " << veh2_downtrack);
  ...
  if (original_lc_maneuver_values_.find(maneuver_id) == original_lc_maneuver_values_.end()) {
    // If this lane change maneuver ID is being received for this first time, store its original
    // start_dist and starting_lane_id locally
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Received maneuver id " << maneuver_id << " for the first time");
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Original start dist is " << maneuver_plan[0].lane_change_maneuver.start_dist);
    RCLCPP_DEBUG_STREAM(
      get_logger(),
      "Original starting_lane_id is " << maneuver_plan[0].lane_change_maneuver.starting_lane_id);

    // Create LaneChangeManeuverOriginalValues object for this lane change maneuver and add it to
    ...
    original_lc_values.original_starting_lane_id =
      maneuver_plan[0].lane_change_maneuver.starting_lane_id;
    ...
    // If the vehicle has just started this lane change, store its initial velocity locally; this
    // velocity will be maintained throughout the lane change
    if (
      current_downtrack >= (original_lc_maneuver_values_[maneuver_id]).original_start_dist &&
      !(original_lc_maneuver_values_[maneuver_id]).has_started) {
      ...
      original_lc_maneuver_values_[maneuver_id].original_longitudinal_vel_ms =
        std::max(req->vehicle_state.longitudinal_vel, config_.minimum_speed);

      RCLCPP_DEBUG_STREAM(
        get_logger(), "Lane change maneuver "
                        << maneuver_id << " has started, maintaining speed (in m/s): "
                        << original_lc_maneuver_values_[maneuver_id].original_longitudinal_vel_ms
                        << " throughout lane change");
    }
  }

  std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> planned_trajectory_points =
    plan_lanechange(req);
...
void CooperativeLaneChangePlugin::add_trajectory_to_response(
  carma_planning_msgs::srv::PlanTrajectory::Request::SharedPtr req,
  carma_planning_msgs::srv::PlanTrajectory::Response::SharedPtr resp,
  const std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> & planned_trajectory_points)
{
  ...
  trajectory_plan.initial_longitudinal_velocity =
    std::max(req->vehicle_state.longitudinal_vel, config_.minimum_speed);
...
carma_v2x_msgs::msg::MobilityRequest CooperativeLaneChangePlugin::create_mobility_request(
  ...
  carma_planning_msgs::msg::Maneuver & maneuver)
{
  ...
  RCLCPP_DEBUG_STREAM(
    get_logger(), "start_lanelet_id: " << maneuver.lane_change_maneuver.starting_lane_id);
  RCLCPP_DEBUG_STREAM(
    get_logger(), "end_lanelet_id: " << maneuver.lane_change_maneuver.ending_lane_id);
  ...
  if (map_projector_) {
    ...
    RCLCPP_ERROR_STREAM(
      get_logger(), "Map projection not available to be used with request message");
  }

  request_msg.trajectory = trajectory;
  request_msg.expiration = rclcpp::Time(trajectory_plan.back().target_time).seconds();
  RCLCPP_DEBUG_STREAM(
    get_logger(), "request_msg.expiration: " << request_msg.expiration << " of which string size: "
                                             << std::to_string(request_msg.expiration).size());

  return request_msg;
}

carma_v2x_msgs::msg::Trajectory CooperativeLaneChangePlugin::trajectory_plan_to_trajectory(
  const std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> & traj_points) const
{
  ...
  if (traj_points.size() < 2) {
    ...
    for (size_t i = 1; i < traj_points.size(); i++) {
      ...
      carma_v2x_msgs::msg::LocationECEF new_point =
        trajectory_point_to_ecef(traj_points[i]);  // m to cm to fit the msg standard
...
carma_v2x_msgs::msg::LocationECEF CooperativeLaneChangePlugin::trajectory_point_to_ecef(
  const carma_planning_msgs::msg::TrajectoryPlanPoint & traj_point) const
{
  if (!map_projector_) {
...
CooperativeLaneChangePlugin::plan_lanechange(
  carma_planning_msgs::srv::PlanTrajectory::Request::SharedPtr req)
{
  lanelet::BasicPoint2d veh_pos(req->vehicle_state.x_pos_global, req->vehicle_state.y_pos_global);
  double current_downtrack = wm_->routeTrackPos(veh_pos).downtrack;
  ...
  wpg_general_config = basic_autonomy::waypoint_generation::compose_general_trajectory_config(
    "cooperative_lanechange", config_.downsample_ratio, config_.turn_downsample_ratio);

  wpg_detail_config = basic_autonomy::waypoint_generation::compose_detailed_trajectory_config(
    config_.trajectory_time_length, config_.curve_resample_step_size, config_.minimum_speed,
    config_.max_accel, config_.lateral_accel_limit, config_.speed_moving_average_window_size,
  ...
  double original_start_dist = current_downtrack;  // Initialize so original_start_dist cannot be
                                                   // less than the current downtrack
  ...
  if (original_lc_maneuver_values_.find(maneuver_id) != original_lc_maneuver_values_.end()) {
    ...
    RCLCPP_DEBUG_STREAM(
      get_logger(),
      "Maneuver id " << maneuver_id << " original start_dist is " << original_start_dist);

    // Set this maneuver's starting_lane_id to the original starting_lane_id associated with this
    // lane change maneuver
    maneuver_plan.front().lane_change_maneuver.starting_lane_id =
      original_lc_maneuver_values_[maneuver_id].original_starting_lane_id;
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Updated maneuver id "
                      << maneuver_id << " starting_lane_id to its original value of "
                      << original_lc_maneuver_values_[maneuver_id].original_starting_lane_id);

    // If the vehicle has started this lane change, set the request's vehicle_state.longitudinal_vel
    // to the velocity that the vehicle began this lane change at
    if (original_lc_maneuver_values_[maneuver_id].has_started) {
      req->vehicle_state.longitudinal_vel =
        original_lc_maneuver_values_[maneuver_id].original_longitudinal_vel_ms;
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Updating vehicle_state.longitudinal_vel to the initial lane change value of "
                        << original_lc_maneuver_values_[maneuver_id].original_longitudinal_vel_ms);
    }
  } else {
  ...
  auto points_and_target_speeds = basic_autonomy::waypoint_generation::create_geometry_profile(
    maneuver_plan, starting_downtrack, wm_, ending_state_before_buffer_, req->vehicle_state,
  ...
  maneuver_fraction_completed_ =
    (maneuver_start_dist - current_downtrack) / (maneuver_end_dist - maneuver_start_dist);

  RCLCPP_DEBUG_STREAM(
    get_logger(), "Maneuvers to points size: " << points_and_target_speeds.size());
  auto downsampled_points = carma_ros2_utils::containers::downsample_vector(
    points_and_target_speeds, config_.downsample_ratio);
  ...
  std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> trajectory_points =
    basic_autonomy::waypoint_generation::compose_lanechange_trajectory_from_path(
      downsampled_points, req->vehicle_state, req->header.stamp, wm_, ending_state_before_buffer_,
      wpg_detail_config);
...
void CooperativeLaneChangePlugin::georeference_cb(const std_msgs::msg::String::UniquePtr msg)
{
  if (map_georeference_ != msg->data) {
    ...
    map_projector_ = std::make_shared<lanelet::projection::LocalFrameProjector>(
      msg->data.c_str());  // Build projector from proj string
  }
}
...
bool CooperativeLaneChangePlugin::get_availability() { return true; }
