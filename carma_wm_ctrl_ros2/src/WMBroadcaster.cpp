...
void WMBroadcaster::baseMapCallback(autoware_lanelet2_msgs::msg::MapBin::UniquePtr map_msg)
{
  ...
  {
    firstCall = false;
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "WMBroadcaster::baseMapCallback called for first time with new map message");
  }
  else
  {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "WMBroadcaster::baseMapCallback called multiple times in the same node");
  }
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Building routing graph for base map");
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Done building routing graph for base map");
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Creating routing graph message.");
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Done creating routing graph message.");
...
std::vector<std::shared_ptr<Geofence>> WMBroadcaster::geofenceFromMapMsg(std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::MapData& map_msg)
{
  ...
  {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), ">>> Detected no change from previous, ignoring duplicate message! with gf id: " << gf_ptr->id_);
    return {};
  }
...
void WMBroadcaster::geofenceFromMsg(std::shared_ptr<Geofence> gf_ptr, const carma_v2x_msgs::msg::TrafficControlMessageV01& msg_v01)
{
  ...
  if (gf_ptr->affected_parts_.size() == 0) {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "There is no applicable component in map for the new geofence message received by WMBroadcaster with id: " << gf_ptr->id_);
  ...
  {  
    ...
    if(sL > 80_mph )
    {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Digital maximum speed limit is invalid. Value capped at max speed limit."); //Output warning message
    sL = 80_mph; //Cap the speed limit to the predetermined maximum value

    }
    if(sL < 0_mph)
    {
          RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Digital  speed limit is invalid. Value set to 0mph.");
      sL = 0_mph;
    }// @SONAR_START@
  ...
  if (msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::MINSPEED_CHOICE) 
  {
    ...
    if(sL > 80_mph )
    {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Digital speed limit is invalid. Value capped at max speed limit.");
    sL = 80_mph;
    }
    if(sL < 0_mph)
    {
  ...
  if (msg_detail.choice == carma_v2x_msgs::msg::TrafficControlDetail::MINHDWY_CHOICE) 
  {
    ...
    if(min_gap < 0)
    {
      RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Digital min gap is invalid. Value set to 0 meter.");
      min_gap = 0;
    }
...
std::shared_ptr<Geofence> WMBroadcaster::createWorkzoneGeofence(std::unordered_map<uint8_t, std::shared_ptr<Geofence>> work_zone_geofence_cache)
{
  ...
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Workzone geofence finished processing. Therefore following geofence id is being dropped from cache as it is processed as part of it: " << pair.second->id_);
  }
                                                    ...
                                                    std::shared_ptr<std::vector<lanelet::Lanelet>> middle_opposite_lanelets)
{
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Created diag front_llt_diag id:" << front_llt_diag.id());
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Created back_llt_diag diag id:" << back_llt_diag.id());
  ...
  for (int i = middle_opposite_lanelets->size() - 1; i >= 0; i--) //do no use size_t as it might overflow
  {
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Created matching direction of middle_llt id:" << middle_llt.id());
  }
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Created TF_LIGHT of Id: " << tfl_parallel->id() << ", to parallel_llt_front id:" << parallel_llt_front.id());
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Created TF_LIGHT of Id: " << tfl_opposite->id() << ", to opposite_llts_with_stop_line->front() id:" << opposite_llts_with_stop_line->front().id());

  //////////////////////////////
  //ADD ALL NEWLY CREATED LANELETS INTO GEOFENCE 
  //OBJECTS TO BE PROCESSED LATER BY SCHEDULER
  //////////////////////////////
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Added parallel_llt_front id:" << parallel_llt_front.id());
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Added parallel_llt_back id:" << parallel_llt_back.id());
...
void WMBroadcaster::preprocessWorkzoneGeometry(std::unordered_map<uint8_t, std::shared_ptr<Geofence>> work_zone_geofence_cache, std::shared_ptr<std::vector<lanelet::Lanelet>> parallel_llts, std::shared_ptr<std::vector<lanelet::Lanelet>> opposite_llts)
{
  ...
  {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Creating duplicate lanelet of 'previous lanelet' due to TAPERRIGHT using entire lanelet...");
    ...
    {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Workzone area starts from lanelet with no previous lanelet (Id : " << work_zone_geofence_cache[WorkZoneSection::TAPERRIGHT]->affected_parts_.front().lanelet().get().id()
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Finished TAPERRIGHT processing of size: " << new_taper_right_llts.size());
  ...
  {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Creating duplicate lanelet of 'next lanelet' due to OPENRIGHT using entire lanelet...");
    ...
    {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Workzone area ends at lanelet with no following lanelet (Id : " << work_zone_geofence_cache[WorkZoneSection::OPENRIGHT]->affected_parts_.back().lanelet().get().id()
                      << ". This case is rare and not supported at the moment.");
      return;
    }
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Finished OPENRIGHT processing of size: " << new_open_right_llts.size());
  ...
  if (reverse_back_llt.id() == reverse_front_llt.id()) //means there is only 1 middle lanelet, which needs to be split into 3 lanelets
  {
    ...
    if (temp_llts.size() < 2) // if there is only 1 lanelet
    {
      ...
      RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Ended preprocessWorkzoneGeometry with opposite_llts.size()" << opposite_llts->size() << ", and parallel_llts.size()" << parallel_llts->size());
      return;
    }
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Finished REVERSE processing of size: " << opposite_llts->size() << " from original of 1 REVERSE lanelet size");
  }
  else //if there are two or more lanelets
  {
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Finished REVERSE processing of size: " << opposite_llts->size() << " from original of more than one REVERSE lanelet size");
  }
...
std::vector<lanelet::Lanelet> WMBroadcaster::splitLaneletWithPoint(const std::vector<lanelet::BasicPoint2d>& input_pts, const lanelet::Lanelet& input_llt, double error_distance)
{
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "splitLaneletWithPoint returning lanelets size: " << parallel_llts.size());
...
lanelet::Lanelets WMBroadcaster::splitOppositeLaneletWithPoint(std::shared_ptr<std::vector<lanelet::Lanelet>> opposite_llts, const lanelet::BasicPoint2d& input_pt, const lanelet::Lanelet& input_llt, double error_distance)
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "WMBroadcaster was not able to find opposing lane for given point in geofence related to Work Zone! Returning");
    return {};
  }
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "splitOppositeLaneletWithPoint returning lanelets size: " << opposite_llts->size());
...
std::vector<lanelet::Lanelet> WMBroadcaster::splitLaneletWithRatio(std::vector<double> ratios, lanelet::Lanelet input_lanelet, double error_distance) const
{
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "splitLaneletWithRatio evaluating input ratios of size: " << ratios.size());
  ...
  for (int i = 0 ; i < ratios.size(); i ++)
  {
    ...
    if (lanelet::geometry::distance2d(input_lanelet.leftBound2d().front().basicPoint2d(), input_lanelet.leftBound2d()[left_next_pt_idx].basicPoint2d()) <= error_distance)
    {
      ...
      RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Ratio: " << ratios[i] << ", is too close to the lanelet's front boundary! Therefore, ignoring... Allowed error_distance: " << error_distance << ", Distance: " 
    ...
    {
      RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Ratio: " << ratios[i] << ", is too close to the lanelet's back boundary! Therefore, ignoring... Allowed error_distance: " << error_distance << ", Distance: " 
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "splitLaneletWithRatio returning lanelets size: " << created_llts.size());
...
std::vector<std::string> WMBroadcaster::combineParticipantsToVehicle(const std::vector<std::string>& input_participants) const
{
  ...
  {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Detected participants to cover all possible vehicle types");
    participants.emplace_back(lanelet::Participants::Vehicle);
  }
  else
  {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Not making any changes to the participants list");
...
void WMBroadcaster::externalMapMsgCallback(carma_v2x_msgs::msg::MapData::UniquePtr map_msg)
{
  ...
  if (!current_map_ || current_map_->laneletLayer.size() == 0)
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Map is not available yet. Skipping MAP msg");
    return;
  }
...
void WMBroadcaster::geofenceCallback(carma_v2x_msgs::msg::TrafficControlMessage::UniquePtr geofence_msg)
{
  ...
  if (geofence_msg->choice != carma_v2x_msgs::msg::TrafficControlMessage::TCMV01) {
    ...
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), reason_ss.str());
  ...
  if (checked_geofence_ids_.find(boost::uuids::to_string(id)) != checked_geofence_ids_.end()) { 
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), reason_ss.str());
  ...
  {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "CARMA_WM_CTRL received a TrafficControlMessage with unknown TrafficControlRequest ID (reqid): " << reqid);
    return;
  }
...
{
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Scheduling new geofence message received by WMBroadcaster with id: " << gf_ptr->id_);
  ...
  if (detected_workzone_signal && msg_detail.choice != carma_v2x_msgs::msg::TrafficControlDetail::MAXSPEED_CHOICE) // if workzone message detected, save to cache to process later
  {
    ...
    {
      RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Received 'SIG_WZ' signal. Waiting for the rest of the messages, returning for now...");
      return;
    }
  }
...
{
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Getting affected lanelets");
  if (!current_map_ || current_map_->laneletLayer.size() == 0)
  {
    throw lanelet::InvalidObjectStateError(std::string("Base lanelet map is not loaded to the WMBroadcaster"));
  }
  ...
  if (datum.empty()) {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Datum field not populated. Attempting to use WGS84");
    datum = "WGS84";
  }

  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Projection field: " << projection);
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Datum field: " << datum);
  
  std::string universal_frame = datum; //lat/long included in TCM is in this datum


  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Traffic Control heading provided: " << tcm_v01.geometry.heading << " System understanding is that this value will not affect the projection and is only provided for supporting derivative calculations.");
  ...
  if (universal_to_target == nullptr) { // proj_create_crs_to_crs returns 0 when there is an error in the projection
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Failed to generate projection between geofence and map with error number: " <<  proj_context_errno(PJ_DEFAULT_CTX) 
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "In TCM's frame, initial Point X "<< prev_pt.x<<" Before conversion: Point Y "<< prev_pt.y );
  for (auto pt : tcm_v01.geometry.nodes)
  { 
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Before conversion in TCM frame: Point X "<< pt.x <<" Before conversion: Point Y "<< pt.y);
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "After conversion in Map frame: Point X "<< gf_pts.back().x() <<" After conversion: Point Y "<< gf_pts.back().y());
  }
...
void WMBroadcaster::addRegulatoryComponent(std::shared_ptr<Geofence> gf_ptr) const
{
  ...
  for (auto el: gf_ptr->affected_parts_)
  {
    ...
    if (gf_ptr->regulatory_element_->id() != lanelet::InvalId)
    {
      ...
      RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Regulatory element with invalid id in geofence cannot be added to the map");
    }
  }
...
void WMBroadcaster::addGeofence(std::shared_ptr<Geofence> gf_ptr)
{
  ...
  std::lock_guard<std::mutex> guard(map_mutex_);
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Adding active geofence to the map with geofence id: " << gf_ptr->id_);
  ...
  for (auto update : updates_to_send)
  {
    ...
    if (update->invalidate_route_) {
      ...
      RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Rebuilding routing graph after is was invalidated by geofence");

      lanelet::traffic_rules::TrafficRulesUPtr traffic_rules_car = lanelet::traffic_rules::TrafficRulesFactory::create(
      lanelet::traffic_rules::CarmaUSTrafficRules::Location, participant_);
      current_routing_graph_ = lanelet::routing::RoutingGraph::build(*current_map_, *traffic_rules_car);

      RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Done rebuilding routing graph after is was invalidated by geofence");
...
void WMBroadcaster::removeGeofence(std::shared_ptr<Geofence> gf_ptr)
{
  ...
  if (gf_ptr->invalidate_route_) { // If a geofence initially invalidated the route it stands to reason its removal should as well
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Rebuilding routing graph after is was invalidated by geofence removal");
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Done rebuilding routing graph after is was invalidated by geofence removal");

    // Populate routing graph structure
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Creating routing graph message for geofence removal");
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Done creating routing graph message for geofence removal");
  }
...
carma_v2x_msgs::msg::TrafficControlRequest WMBroadcaster::controlRequestFromRoute(const carma_planning_msgs::msg::Route& route_msg, std::shared_ptr<j2735_v2x_msgs::msg::Id64b> req_id_for_testing)
{
  ...
  if (!current_map_ || current_map_->laneletLayer.size() == 0)
  {
   // Return / log warning etc.
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Value 'current_map_' does not exist.");
    throw lanelet::InvalidObjectStateError(std::string("Base lanelet map is not loaded to the WMBroadcaster"));
  ...
  {
   // Return / log warning etc.
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Value 'target_frame' is empty.");
  ...
  if (tmerc_proj == nullptr) { // proj_create_crs_to_crs returns 0 when there is an error in the projection
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Failed to generate projection between request bounds frame and map with error number: " <<  proj_context_errno(PJ_DEFAULT_CTX) 
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Before conversion: Top Left: ("<< minX <<", "<<maxY<<")");
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Before conversion: Top Right: ("<< maxX <<", "<<maxY<<")");
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Before conversion: Bottom Left: ("<< minX <<", "<<minY<<")");
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Before conversion: Bottom Right: ("<< maxX <<", "<<minY<<")");
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "After conversion: MinPoint ( "<< pj_min_tmerc.xyz.x <<", " << pj_min_tmerc.xyz.y <<" )");
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "After conversion: MaxPoint ( "<< pj_max_tmerc.xyz.x <<", " << pj_max_tmerc.xyz.y <<" )");
  ...
  cB.oldest =rclcpp::Time(0.0,0.0,RCL_SYSTEM_TIME); // TODO this needs to be set to 0 or an older value as otherwise this will filter out all controls
...
carma_v2x_msgs::msg::TrafficControlRequestPolygon WMBroadcaster::composeTCRStatus(const lanelet::BasicPoint3d& localPoint, const carma_v2x_msgs::msg::TrafficControlBounds& cB, const lanelet::projection::LocalFrameProjector& local_projector)
{
  ...
  while (i < 3) // three offsets; offsets.size() doesn't return accurate size
  {
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "TCR Vertex Lat: "<< std::to_string(gps_vertex.lat));
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "TCR Vertex Lon: "<<std::to_string(gps_vertex.lon));
...
void WMBroadcaster::currentLocationCallback(geometry_msgs::msg::PoseStamped::UniquePtr current_pos)
{
  if (current_map_ && current_map_->laneletLayer.size() != 0) {
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Could not check active geofence logic because map was not loaded");
  }
}
...
void WMBroadcaster::publishLightId()
{
  ...
  for(auto id : current_route.route_path_lanelet_ids) 
  {
    ...
    {
      RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Found Traffic Light Regulatory Element id: " << traffic_lights.front()->id());
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Found Traffic Light with Intersection id: " << intersection_id << " Group id:" << group_id);
...
carma_perception_msgs::msg::CheckActiveGeofence WMBroadcaster::checkActiveGeofenceLogic(const geometry_msgs::msg::PoseStamped& current_pos)
{
  ...
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "No active geofence llt ids are loaded to the WMBroadcaster");
  ...
  {         
    ...
    for(auto id : active_geofence_llt_ids_) 
    {
      ...
      {           
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Vehicle is on Lanelet " << current_llt.id() << ", which has an active geofence");
        ...
        for (auto regem: current_llt.regulatoryElements())
        {
          ...
          if (regem->attribute(lanelet::AttributeName::Subtype).value().compare(lanelet::DigitalSpeedLimit::RuleName) == 0)
          {
            ...
            RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Active geofence has a speed limit of " << speed->speed_limit_.value());
          ...
          if(regem->attribute(lanelet::AttributeName::Subtype).value().compare(lanelet::DigitalMinimumGap::RuleName) == 0)
          {
            ...
            RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Active geofence has a minimum gap of " << min_gap->getMinimumGap());
          }
          ...
          if(regem->attribute(lanelet::AttributeName::Subtype).value().compare(lanelet::RegionAccessRule::RuleName) == 0)
          {
            ...
            {
              RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Active geofence is a closed lane.");
              RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Closed lane reason: " << accessRuleReg->getReason());
                  ...
                  if(lanelet.id() != current_llt.id())
                  {
                    for (auto rightRegem: lanelet.regulatoryElements())
                    {
                      if(rightRegem->attribute(lanelet::AttributeName::Subtype).value().compare(lanelet::RegionAccessRule::RuleName) == 0)
                      {
                        ...
                        {
                          RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Right adjacent Lanelet " << lanelet.id() << " is CLOSED");
                          RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Assigning LANE_CLOSED type to active geofence");
                          RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Assigning reason " << rightAccessRuleReg->getReason());
                  ...
                  if(lanelet.id() != current_llt.id())
                  {
                    for (auto leftRegem: lanelet.regulatoryElements())
                    {
                      if(leftRegem->attribute(lanelet::AttributeName::Subtype).value().compare(lanelet::RegionAccessRule::RuleName) == 0)
                      {
                        ...
                        {
                          RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Left adjacent Lanelet " << lanelet.id() << " is CLOSED");
                          RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Assigning LANE_CLOSED type to active geofence");
                          RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "Assigning reason " << leftAccessRuleReg->getReason());
...
void WMBroadcaster::updateUpcomingSGIntersectionIds()
{
  ...
  if(isLightFound)
  {
    ...
    {     
     ...
     RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "NO matching Traffic lights along the route");
  }//END Traffic signals
  ...
  if (intersections.empty())
  {
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "NO matching intersection for current lanelet. lanelet id = " << route_lanelet.id());
  }
  else
  {
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("carma_wm_ctrl::WMBroadcaster"), "MAP msg: Intersection ID = " <<  map_msg_intersection_id << ", Signal Group ID =" << cur_signal_group_id );
