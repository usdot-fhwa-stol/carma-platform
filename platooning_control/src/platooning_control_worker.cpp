...
namespace platooning_control
{
    ...
    void PlatooningControlWorker::generate_speed(const carma_planning_msgs::msg::TrajectoryPlanPoint& point)
    {
    	...
    	if(!leader.staticId.empty() && leader.staticId != ctrl_config_->vehicle_id)
        {
	        ...
	        RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "The current leader position is " << leaderCurrentPosition);

            double desiredHostPosition = leaderCurrentPosition - desired_gap_;
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "desiredHostPosition = " << desiredHostPosition);

            double hostVehiclePosition = leaderCurrentPosition - actual_gap_;
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "hostVehiclePosition = " << hostVehiclePosition);
	        ...
	        RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "Adjusted Speed Cmd = " << adjSpeedCmd << "; Controller Output = " << controllerOutput
            ...
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "A speed command is generated from command generator: " << speed_cmd << " m/s");
            ...
            if(ctrl_config_->enable_max_adjustment_filter)
            {
                ...
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "The adjusted cmd speed after max adjustment cap is " << speed_cmd << " m/s");
            }
        ...
        {
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "Host vehicle is the leader");
            ...
            if(ctrl_config_->enable_max_adjustment_filter)
            {
                ...
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "The adjusted leader cmd speed after max adjustment cap is " << speed_cmd << " m/s");
            }
        ...
        else
        {
            ...
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "There is no leader available");
            speed_cmd = 0.0;
            pid_ctrl_.reset();
        }
        ...
        if(ctrl_config_->enable_max_accel_filter) {
                ...
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("platooning_control"), "The speed command after max accel cap is: " << speed_cmd << " m/s");
        }
