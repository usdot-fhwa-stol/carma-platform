...
{
  // NOTE: IMU messages could be added once imu_pipeline package is ported to noetic and added as a
  // dependency
  ...
  if (type == "geometry_msgs/Vector3Stamped")
    return std::make_unique<Transformer<geometry_msgs::msg::Vector3Stamped>>(
      config_, buffer_, shared_from_this());

  else if (type == "geometry_msgs/PointStamped")
    return std::make_unique<Transformer<geometry_msgs::msg::PointStamped>>(
      config_, buffer_, shared_from_this());

  else if (type == "geometry_msgs/PoseStamped")
    return std::make_unique<Transformer<geometry_msgs::msg::PoseStamped>>(
      config_, buffer_, shared_from_this());

  else if (type == "geometry_msgs/PoseWithCovarianceStamped")
    return std::make_unique<Transformer<geometry_msgs::msg::PoseWithCovarianceStamped>>(
      config_, buffer_, shared_from_this());

  else if (type == "geometry_msgs/QuaternionStamped")
    return std::make_unique<Transformer<geometry_msgs::msg::QuaternionStamped>>(
      config_, buffer_, shared_from_this());

  else if (type == "geometry_msgs/TransformStamped")
    return std::make_unique<Transformer<geometry_msgs::msg::TransformStamped>>(
      config_, buffer_, shared_from_this());

  else if (type == "sensor_msgs/PointCloud2")
    return std::make_unique<Transformer<sensor_msgs::msg::PointCloud2>>(
      config_, buffer_, shared_from_this());

  else if (type == "autoware_auto_msgs/BoundingBoxArray")
    return std::make_unique<Transformer<autoware_auto_msgs::msg::BoundingBoxArray>>(
      config_, buffer_, shared_from_this());

  else if (type == "autoware_auto_msgs/DetectedObjects")
    return std::make_unique<Transformer<autoware_auto_msgs::msg::DetectedObjects>>(
      config_, buffer_, shared_from_this());
...
carma_ros2_utils::CallbackReturn Node::handle_on_configure(const rclcpp_lifecycle::State &)
{
  ...
  //       Dynamically changing the parameters is not recommended. Therefore a parameter callback is
  //       not implemented.
  ...
  if (!transformer_) {
    RCLCPP_ERROR_STREAM(
      get_logger(),
      "Failed to build transformer for specified message_type: " << config_.message_type);
