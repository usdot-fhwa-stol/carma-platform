...
namespace trajectory_executor 
{
    ...
    std::map<std::string, std::string> TrajectoryExecutor::queryControlPlugins()
    {
        ...
        ROS_ERROR("Executing stub behavior for plugin discovery MVP...");
    ...
    void TrajectoryExecutor::onNewTrajectoryPlan(const cav_msgs::TrajectoryPlan& msg)
    {
        ...
        ROS_ERROR("Received new trajectory plan!");
        ROS_ERROR_STREAM("New Trajectory plan ID: " << msg.trajectory_id);
        ROS_ERROR_STREAM("New plan contains " << msg.trajectory_points.size() << " points");

        _cur_traj = std::unique_ptr<cav_msgs::TrajectoryPlan>(new cav_msgs::TrajectoryPlan(msg));
        _timesteps_since_last_traj = 0;
        ROS_ERROR_STREAM("Successfully swapped trajectories!");
    }
    ...
    {
        std::unique_lock<std::mutex> lock(_cur_traj_mutex);
        ROS_ERROR("TrajectoryExecutor tick start!");
        ...
        if (_cur_traj != nullptr) {
            ...
            if (!_cur_traj->trajectory_points.empty()) {
                ...
                if (it != _traj_publisher_map.end()) {
                    ROS_ERROR("Found match for control plugin %s at point %d in current trajectory!",
            ...
            ROS_ERROR("Awaiting initial trajectory publication...");
        }
        ROS_ERROR("TrajectoryExecutor tick completed succesfully!");
    }

    void TrajectoryExecutor::run()
    {
        ROS_ERROR("Starting operations for TrajectoryExecutor component...");
        _timer = _private_nh->createTimer(
            ros::Duration(ros::Rate(this->_min_traj_publish_tickrate_hz)),
            &TrajectoryExecutor::onTrajEmitTick, 
            this);

        ROS_ERROR("TrajectoryExecutor component started succesfully! Starting to spin.");
    ...
    {
        ROS_ERROR("Initializing TrajectoryExecutor node...");
        ...
        ROS_ERROR("Initialized all node handles");
        ...
        ROS_ERROR_STREAM("Initalized params with default_spin_rate " << _default_spin_rate 
        ...
        ROS_ERROR("Subscribed to inbound trajectory plans.");

        ROS_ERROR("Setting up publishers for control plugin topics...");
        ...
        {
            ROS_ERROR("Trajectory executor discovered control plugin %s listening on topic %s.", it->first.c_str(), it->second.c_str());
        ...
        ROS_ERROR("TrajectoryExecutor component initialized succesfully!");
