...
public:
  ...
  using ManagedInitialPosePublisher =
    std::function<void(const geometry_msgs::msg::PoseWithCovarianceStamped &)>;
  using StatePublisher =
    std::function<void(const carma_localization_msgs::msg::LocalizationStatusReport &)>;
  ...
  /**
   * \brief Callback for when a new state was triggered. Allows creation of entry/exit behavior for
   * states
   *
   ...
   *
   * \param origin_state The state that was the current state when the timer that triggered this
   * callback was setup
   */
...
private:
  ...
  static const std::unordered_set<std::string>
    LIDAR_FAILURE_STRINGS;  // Static const container defined in cpp file
