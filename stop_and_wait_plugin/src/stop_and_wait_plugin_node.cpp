  ...
  plugin_name_(get_plugin_name_and_ns())
{
  ...
  // Declare parameters
  config_.minimal_trajectory_duration =
    declare_parameter<double>("minimal_trajectory_duration", config_.minimal_trajectory_duration);
  config_.stop_timestep = declare_parameter<double>("stop_timestep", config_.stop_timestep);
  config_.trajectory_step_size =
    declare_parameter<double>("trajectory_step_size", config_.trajectory_step_size);
  config_.accel_limit_multiplier =
    declare_parameter<double>("accel_limit_multiplier", config_.accel_limit_multiplier);
  config_.accel_limit =
    declare_parameter<double>("vehicle_acceleration_limit", config_.accel_limit);
  ...
  config_.default_stopping_buffer =
    declare_parameter<double>("default_stopping_buffer", config_.default_stopping_buffer);
...
rcl_interfaces::msg::SetParametersResult StopandWaitNode::parameter_update_callback(
...
carma_ros2_utils::CallbackReturn StopandWaitNode::on_configure_plugin()
{
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("stop_and_wait_plugin"), "Done loading parameters: " << config_);

  // Register runtime parameter update callback
  add_on_set_parameters_callback(
    std::bind(&StopandWaitNode::parameter_update_callback, this, std_ph::_1));

  plugin_ = std::make_shared<StopandWait>(
    shared_from_this(), get_world_model(), config_, plugin_name_, version_id_);
...
bool StopandWaitNode::get_availability() { return true; }

std::string StopandWaitNode::get_version_id() { return version_id_; }
