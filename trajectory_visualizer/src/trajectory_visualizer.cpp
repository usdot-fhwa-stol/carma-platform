...
carma_ros2_utils::CallbackReturn TrajectoryVisualizer::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  // Setup subscribers
  traj_sub_ = create_subscription<carma_planning_msgs::msg::TrajectoryPlan>(
    "plan_trajectory", 50,
    std::bind(&TrajectoryVisualizer::callbackPlanTrajectory, this, std_ph::_1));
  // Setup publisher
  traj_marker_pub_ =
    create_publisher<visualization_msgs::msg::MarkerArray>("trajectory_visualizer", 1);
...
void TrajectoryVisualizer::callbackPlanTrajectory(
  carma_planning_msgs::msg::TrajectoryPlan::UniquePtr msg)
{
  ...
  for (size_t i = 1; i < count; i++) {
    ...
    RCLCPP_DEBUG_STREAM(
      this->get_logger(), "Speed:" << speed << "ms, max_speed:" << max_speed << "ms");
    if (speed > max_speed) {
