...
public:
   ...
   * \param via A vector of lanelet 2D points in map frame which contains points we want to visit
   * along the route \param end Lanelet 2D point in map frame indicates the final destination of
   * selected route \param map_pointer A constant pointer to lanelet vector map \param graph_pointer
   * A constant pointer to lanelet vector map routing graph
   */
  lanelet::Optional<lanelet::routing::Route> routing(
  ...
  /**
   * \brief Get_available_route service callback. Calls to this service will respond with a list of
   * route names for user to select \param req An empty
   * carma_planning_msgs::srv::GetAvailableRoutes::Request \param resp A
   * carma_planning_msgs::srv::GetAvailableRoutes::Response msg contains a list of empty Route
   * messages with only route name populated
   */
  bool getAvailableRouteCb(
  ...
  /**
   * \brief Set_active_route service callback. User can select a route to start following by calling
   * this service \param req A carma_planning_msgs::srv::SetActiveRoute::Request msg which contains
   * either a route name a user wants to select or an array of carma_v2x_msgs::msg::Position3D
   * destination points to generate a route from. \param resp A
   * carma_planning_msgs::srv::SetActiveRoute::Response msg contains error status indicating whether
   * the routing succeeded
   */
  bool setActiveRouteCb(
    const std::shared_ptr<rmw_request_id_t>,
  ...
  /**
   * \brief Abort_active_route service callback. User can call this service to abort a current
   * following route and return to route selection stage \param req A
   * carma_planning_msgs::srv::AbortActiveRoute::Request msg which contains the route name user
   * wants to stop following \param resp A carma_planning_msgs::srv::AbortActiveRoute::Response msg
   * contains error status showing if there is an active route
   */
  bool abortActiveRouteCb(
    const std::shared_ptr<rmw_request_id_t>,
  ...
  /**
   * \brief Helper function to check whether a route's shortest path contains any duplicate Lanelet
   * IDs. 'true' indicates that the route's shortest path contains duplicate Lanelet IDs. \param
  ...
  /**
   * \brief Function to take route destination points from a vector of 3D points and convert them
   * from lat/lon values to to coordinates in map frame based on the projection string \param
   * destinations A vector of carma_v2x_msgs::msg::Position3D points containing destination points
   * provided as lat/long values
   */
  std::vector<lanelet::BasicPoint3d> loadRouteDestinationsInMapFrame(
    const std::vector<carma_v2x_msgs::msg::Position3D> & destinations) const;

  /**
   * \brief Function to load route destination points from a route file and store them in a vector
   * of 3D points \param route_id This function will read the route file with provided route_id
   */
  std::vector<carma_v2x_msgs::msg::Position3D> loadRouteDestinationGpsPointsFromRouteId(
   ...
   */
  carma_planning_msgs::msg::Route composeRouteMsg(
   ...
   * \param route Route object from lanelet2 lib routing function
   */
  visualization_msgs::msg::Marker composeRouteMarkerMsg(
    const lanelet::Optional<lanelet::routing::Route> & route);

  /**
   * \brief crosstrackErrorCheck is a function that determines when the vehicle has left the route
   * and reports when a crosstrack error has taken place
   *
   *  \param msg Msg that contains the vehicle's current position
   *  \param current_llt The lanelet that the vehicle is currently in
   * */
  bool crosstrackErrorCheck(
   ...
   *
   *  \param cte_dist maximum distance value (specified in the this package's parameters.yaml
   * configuration file)
   */
   ...
   * If the input list does not contain lanelets on the route, still closest lanelet from the route
   * will be returned
   *
  ...
  /**
   * \brief After route is invalidated, this function returns a new route based on the destinations
   * points. \param destination_points_in_map vector of destination points \note Destination points
   * will be removed if the current pose is past those points.
   */
  lanelet::Optional<lanelet::routing::Route> rerouteAfterRouteInvalidation(
...
private:
  ...
  // a bool flag indicates a new route has been generated such that a local copy of Route message
