  ...
  -> carma_cooperative_perception_interfaces::msg::DetectionList
{
  ...
  for (const auto & object_data : sdsm.objects.detected_object_data) {
    ...
    detection.pose.pose.position = to_point_msg(detection_pos_utm);
...
{ 
  ...
  for (const auto & external_object : external_object_list.objects) {
    ...
    sdsm.objects.detected_object_data.push_back(std::move(sdsm_detected_object));
  }
  ...
  -> carma_v2x_msgs::msg::DetectedObjectData
{
  ...
  if (external_object.presence_vector & external_object.POSE_PRESENCE_VECTOR){
    detected_object_common_data.pos.offset_x.object_distance = static_cast<float>(external_object.pose.pose.position.x);
    detected_object_common_data.pos.offset_y.object_distance = static_cast<float>(external_object.pose.pose.position.y);
    detected_object_common_data.pos.offset_z.object_distance = static_cast<float>(external_object.pose.pose.position.z);
  }
