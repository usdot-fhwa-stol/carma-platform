...
namespace carma_wm
{
   ...
   *
   * The position of a point relative to a reference line. The perpendicular distance from the point to the reference line is the crosstrack
   * The distance to the intersection of the perpendicular from the start of the line is the downtrack distance
   * If an x,y point would be located to the right of the reference line then the crosstrack is positive. It is negative if on the left side of the line.
   * NOTE: The positive/negative crosstrack side is flipped relative to a lanelet::ArcCoordinate
   * Most utility functions will assume that the downtrack/crosstrack values are specified in meters
   */
  class TrackPos
  {
    public:
      ...
      : downtrack(down_track), crosstrack(cross_track) {}
      /*! \brief Constructor from lanelet ArcCoordinates 
        *         which are converted to TrackPos where downtrack = ArcCoordinates.length and crosstrack = -ArcCoordinates.distance
        * 
        * \param arc_coord ArcCoordinate to copy
        */
      TrackPos(const lanelet::ArcCoordinates& arc_coord) 
      : downtrack(arc_coord.length), crosstrack(-arc_coord.distance) {}
      ...
      bool operator==(const TrackPos &other) const {
        return this == &other 
          || (this->downtrack == other.downtrack
            && this->crosstrack == other.crosstrack);
      }
  ...
  class WorldModel
  {
    ...
    public:
      ...
      /*! \brief Returns a pair of TrackPos, computed in 2d, of the provided area relative to the current route.
        ...
        * 
        * \return A pair where the first element contains the TrackPos area's earliest downtrack vertex and the second element contains the area's latest downtrack point
        */
      ...
      /*! \brief Returns the TrackPos, computed in 2d, of the provided point relative to the centerline of the provided lanelet. 
        *        Positive crosstrack will be to the right. Points occuring before the segment will have negative downtrack.
        *        See the matchSegment function for a description of the edge cases associated with the max_crosstrack parameter
        * 
        * \param lanelet The lanelet whose centerline will serve as the TrackPos reference line
        * \param point The lanelet2 point which will have its distance computed
        * \param max_crosstrack The maximum crosstrack distance allowed for a perfect centerline segment match. See the matchSegment function for details
        * 
       ...
       */  
      virtual TrackPos trackPos(const lanelet::BasicPoint2d& p, const lanelet::BasicPoint2d& seg_start, const lanelet::BasicPoint2d& seg_end) const = 0;
      
      /**
       * \brief Get the TrackPos of the provided point along the linestring and the segment of the line string which the provided point should be considered in.
       *
       ...
       * Once the nearest point has been found next step is to determine which segment it should go with using the following rules. 
       ...
       * If the minimum point is within the downtrack bounds of both segments then use the one with the smallest crosstrack distance
       * If the minimum point is within the downtrack bounds of both segments and has exactly equal crosstrack bounds with each segment then use the first one 
       * 
       ...
       * 
       * \return An std::tuple where the first element is the TrackPos of the point and the second element is the matched segment
       */
      virtual std::tuple<TrackPos, lanelet::BasicSegment2d> matchSegment(const lanelet::BasicPoint2d& p, const lanelet::BasicLineString2d& line_string) const = 0;

      /*! \brief Returns a list of lanelets which are part of the route and whose downtrack bounds exist within the provided start and end distances 
        *        The bounds are included so areas which end exactly at start or start exactly at end are included
        * 
        ...
        * 
        * \return A list of lanelets which contain regions that lie between start and end along the route. This function will not return lanelets which are not part of the route
        */
      virtual std::vector<lanelet::ConstLanelet> getLaneletsBetween(double start, double end) const = 0;

      /*! \brief Returns a list of lists of local (3-point) curvatures, computed in 2d. Each continuous segment of the lanelets' centerlines is one elemtent in the first list. 
        *        Where each lane change occurs along the list of lanelets a new list of curvatures is started. 
        *        
        * Each elemnent in the first list contains a tuple where the first element is the index of the lanelet which is the starting point of that segment. 
        * The second element of the tuple contains the list of local curvatures for that segment. These map to the points on the lanlet centerlines excluding first and last point of the continuous centerline segments
        * 
        ...
        */
      virtual std::vector<std::tuple<size_t, std::vector<double>>> getLocalCurvatures(const std::vector<lanelet::ConstLanelet>& lanelets) const = 0;
      
      /*! \brief Get a pointer to the current map. If the underlying map has changed the pointer will also need to be reacquired
        *
      ...
      /*! \brief Get a pointer to the current route. If the underlying route has changed the pointer will also need to be reacquired
        *
        * \return Shared pointer to underlying lanelet route. Pointer will return false on boolean check if no route or map is loaded
        */ 
      virtual LaneletRouteConstPtr getRoute() const = 0;

      /*! \brief Get a pointer to the routing graph for the current map. If the underlying map has changed the pointer will also need to be reacquired
        *
        * \return Shared pointer to underlying lanelet route graph. Pointer will return false on boolean check if no map is loaded
        */ 
      virtual LaneletRoutingGraphConstPtr getMapRoutingGraph() const = 0;

      /*! \brief Function for computing curvature from 3 points.
       * 
       * This function is a direct copy of the function by the same name found in the lanelet2_validation package which was not exposed for use
       * The origianal function can be found here https://github.com/fzi-forschungszentrum-informatik/Lanelet2/blob/master/lanelet2_validation/src/validators/CurvatureTooBig.cpp
       * The source function is copyrighted under BSD 3-Clause "New" or "Revised" License a copy of that notice has been included with this package
       * 
       ...
       */ 
      virtual double computeCurvature(const lanelet::BasicPoint2d& p1, const lanelet::BasicPoint2d& p2, const lanelet::BasicPoint2d& p3) const = 0;
