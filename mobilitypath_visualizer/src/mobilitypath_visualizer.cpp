...
double compute_2d_distance(
  const geometry_msgs::msg::Point & pt1, const geometry_msgs::msg::Point & pt2)
{
...
carma_ros2_utils::CallbackReturn MobilityPathVisualizer::handle_on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  // init subscribers
  host_mob_path_sub_ = create_subscription<carma_v2x_msgs::msg::MobilityPath>(
    "mobility_path_msg", 10,
    std::bind(&MobilityPathVisualizer::callbackMobilityPath, this, std_ph::_1));

  cav_mob_path_sub_ = create_subscription<carma_v2x_msgs::msg::MobilityPath>(
    "incoming_mobility_path", 10,
    std::bind(&MobilityPathVisualizer::callbackMobilityPath, this, std_ph::_1));

  georeference_sub_ = create_subscription<std_msgs::msg::String>(
    "georeference", 10, std::bind(&MobilityPathVisualizer::georeferenceCallback, this, std_ph::_1));

  // Return success if everthing initialized successfully
  return CallbackReturn::SUCCESS;
}

carma_ros2_utils::CallbackReturn MobilityPathVisualizer::handle_on_activate(
  const rclcpp_lifecycle::State &)
{
  // Setup timer
  timer_ = create_timer(
    get_clock(), std::chrono::milliseconds(config_.timer_cb_rate),
...
{
  RCLCPP_DEBUG_STREAM(
    get_logger(), "Received a msg from sender: "
                    << msg->m_header.sender_id << ", and plan id:" << msg->m_header.plan_id
                    << ", for receiver:" << msg->m_header.recipient_id
                    << ", time:" << msg->m_header.timestamp
                    << ", at now: " << std::to_string(this->now().seconds()));

  if (
    latest_cav_mob_path_msg_.find(msg->m_header.sender_id) != latest_cav_mob_path_msg_.end() &&
    msg->m_header.plan_id.compare(
      latest_cav_mob_path_msg_[msg->m_header.sender_id].m_header.plan_id) == 0 &&
    msg->m_header.timestamp ==
      latest_cav_mob_path_msg_[msg->m_header.sender_id].m_header.timestamp) {
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Already received this plan id:" << msg->m_header.plan_id << "from sender_id: "
  ...
  if (!map_projector_) {
    RCLCPP_ERROR(
      get_logger(), "Cannot visualize mobility path as map projection not yet available");
    return;
  }
  ...
  if (msg->m_header.sender_id.compare(config_.host_id) == 0) {
    ...
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Composed cav marker successfuly! with sender_id: " << msg->m_header.sender_id);
  }
}

visualization_msgs::msg::MarkerArray MobilityPathVisualizer::composeVisualizationMarker(
  const carma_v2x_msgs::msg::MobilityPath & msg, const MarkerColor & color)
{
  ...
  size_t count =
    std::max(prev_marker_list_size_[msg.m_header.sender_id], msg.trajectory.offsets.size());

  auto curr_location_msg =
    msg;  // variable to update on each iteration as offsets are measured since last traj point
  ...
  if (msg.trajectory.offsets.empty()) {
    ...
    RCLCPP_DEBUG_STREAM(
      get_logger(), "ECEF point x: " << curr_location_msg.trajectory.location.ecef_x
                                     << ", y:" << curr_location_msg.trajectory.location.ecef_y);
    arrow_start =
      ECEFToMapPoint(curr_location_msg.trajectory.location);  // also convert from cm to m
  ...
  for (size_t i = 1; i < count; i++)  // start id = 1 to comply with arrow marker type
  {
    ...
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Map Point- DEBUG x: " << arrow_start.x << ", y:" << arrow_start.y);
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "Last ECEF Point- DEBUG x: " << curr_location_msg.trajectory.location.ecef_x
                                 << ", y:" << curr_location_msg.trajectory.location.ecef_y);
  RCLCPP_DEBUG_STREAM(
    get_logger(), "Last Map Point- DEBUG x: " << arrow_end.x << ", y:" << arrow_end.y);
  prev_marker_list_size_[msg.m_header.sender_id] = msg.trajectory.offsets.size();

  return output;
}

geometry_msgs::msg::Point MobilityPathVisualizer::ECEFToMapPoint(
  const carma_v2x_msgs::msg::LocationECEF & ecef_point) const
{
  if (!map_projector_) {
...
visualization_msgs::msg::MarkerArray MobilityPathVisualizer::composeLabelMarker(
  ...
  const std::vector<visualization_msgs::msg::MarkerArray> & cav_markers) const
{
  ...
  marker.frame_locked = true;
  marker.lifetime =
    builtin_interfaces::msg::Duration((rclcpp::Duration::from_nanoseconds(config_.t * 1e9)));
  ...
  for (auto const & cav_marker : cav_markers) {
    ...
    while (idx < host_marker.markers.size() && idx < cav_marker.markers.size()) {
      if (
        compute_2d_distance(
          cav_marker.markers[idx].points[0], host_marker.markers[idx].points[0]) <=
        1.0)  // within 1 meter
      {
        ...
        std::string collision_time =
          std::to_string((rclcpp::Time(cav_marker.markers[idx].header.stamp) -
                          rclcpp::Time(host_marker.markers[0].header.stamp))
                           .seconds());
        collision_time =
          collision_time.substr(0, collision_time.size() - 4);  // reduce precision to 2 decimals
    ...
    if (
      compute_2d_distance(
        cav_marker.markers[idx - 1].points[1], host_marker.markers[idx - 1].points[1]) <=
      1.0)  // within 1 meter
    {
      // last point
      marker.id = (int32_t)idx;
...
std::vector<visualization_msgs::msg::MarkerArray> MobilityPathVisualizer::matchTrajectoryTimestamps(
  const visualization_msgs::msg::MarkerArray & host_marker,
  const std::vector<visualization_msgs::msg::MarkerArray> & cav_markers) const
{
  if (
    host_marker.markers.empty() ||
    host_marker.markers[0].action == visualization_msgs::msg::Marker::DELETE)
    return cav_markers;
  ...
  for (auto const & curr_cav : cav_markers) {
    ...
    if (
      rclcpp::Time(curr_cav.markers[0].header.stamp) >
        rclcpp::Time(host_marker.markers[0].header.stamp) ||
    ...
    if (
      curr_cav_marker_stamp + rclcpp::Duration::from_nanoseconds(time_step * 1e9) <
      host_marker_stamp) {
      continue;
    }
    ...
    while (curr_idx < curr_cav.markers.size() &&
           rclcpp::Time(curr_cav.markers[curr_idx].header.stamp) <=
             rclcpp::Time(host_marker.markers[0].header.stamp)) {
      curr_idx++;
    }
    ...
    double dt = (rclcpp::Time(host_marker.markers[0].header.stamp) -
                 rclcpp::Time(curr_cav.markers[curr_idx].header.stamp))
                  .seconds();
    ...
    while (curr_idx < curr_cav.markers.size()) {
      if (curr_cav.markers[curr_idx].action == visualization_msgs::msg::Marker::DELETE) break;
      ...
      {
        double dx =
          (curr_cav.markers[curr_idx].points[1].x - curr_cav.markers[curr_idx].points[0].x) /
          time_step * dt;
        double dy =
          (curr_cav.markers[curr_idx].points[1].y - curr_cav.markers[curr_idx].points[0].y) /
          time_step * dt;
        double dz =
          (curr_cav.markers[curr_idx].points[1].z - curr_cav.markers[curr_idx].points[0].z) /
          time_step * dt;
    ...
    while (curr_idx < synchronized_marker_array.markers.size() - 1) {
      ...
      if (!synchronized_marker_array.markers[curr_idx].points.empty())
        synchronized_marker_array.markers[curr_idx].points[1] =
          synchronized_marker_array.markers[curr_idx + 1].points[0];
      curr_idx++;
    }

    if (!synchronized_marker_array.markers[curr_idx].points.empty()) {
      // extrapolate the last point just to conform with 0.1s interval between points
      double dx = (synchronized_marker_array.markers[curr_idx].points[1].x -
                   synchronized_marker_array.markers[curr_idx].points[0].x) /
                  (time_step - dt) * time_step;
      double dy = (synchronized_marker_array.markers[curr_idx].points[1].y -
                   synchronized_marker_array.markers[curr_idx].points[0].y) /
                  (time_step - dt) * time_step;
      double dz = (synchronized_marker_array.markers[curr_idx].points[1].z -
                   synchronized_marker_array.markers[curr_idx].points[0].z) /
                  (time_step - dt) * time_step;

      synchronized_marker_array.markers[curr_idx].points[1].x =
        synchronized_marker_array.markers[curr_idx].points[0].x + dx;
      synchronized_marker_array.markers[curr_idx].points[1].y =
        synchronized_marker_array.markers[curr_idx].points[0].y + dy;
      synchronized_marker_array.markers[curr_idx].points[1].z =
        synchronized_marker_array.markers[curr_idx].points[0].z + dz;
    }
