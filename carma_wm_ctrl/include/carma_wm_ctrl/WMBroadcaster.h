...
public:
   ...
   */
  void addGeofenceHelper(std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr);

  /*!
   * \brief Removes a geofence from the current map and publishes the ROS msg
   */
  void removeGeofence(std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr);

    /*!
   * \brief Determines the type of geofence and removes it from the current map 
   */
  void removeGeofenceHelper(std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr);
...
private:
  void addSpeedLimit(std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr);
  void addBackSpeedLimit(std::shared_ptr<carma_wm_ctrl::Geofence> gf_ptr);
  std::shared_ptr<carma_wm_ctrl::Geofence> geofenceFromMsg(const cav_msgs::ControlMessage& geofence_msg);
