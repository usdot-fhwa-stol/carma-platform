...
{
/**
 * \brief Anonymous function to set the starting_lane_id for all maneuver types except lane
 * following. This maneuver parameter cannot be set with SET_MANEUVER_PROPERTY calls since it is not
 * included in LANE_FOLLOW maneuvers.
 */
void setManeuverStartingLaneletId(carma_planning_msgs::msg::Maneuver & mvr, lanelet::Id start_id)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"), "Updating maneuver starting_lane_id to " << start_id);
...
/**
 * \brief Anonymous function to set the ending_lane_id for all maneuver types except lane following.
 * This maneuver parameter cannot be set with SET_MANEUVER_PROPERTY calls since it is not included
 * in LANE_FOLLOW maneuvers.
 */
void setManeuverEndingLaneletId(carma_planning_msgs::msg::Maneuver & mvr, lanelet::Id end_id)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"), "Updating maneuver ending_lane_id to " << end_id);
...
/**
 * \brief Anonymous function to get the starting lanelet id for all maneuver types except lane
 * following. This maneuver parameters cannot be obtained with GET_MANEUVER_PROPERTY calls since
 * they are not included in LANE_FOLLOW maneuvers.
 */
...
/**
 * \brief Anonymous function to get the ending lanelet id for all maneuver types except lane
 * following. This maneuver parameters cannot be obtained with GET_MANEUVER_PROPERTY calls since
 * they are not included in LANE_FOLLOW maneuvers.
 */
    ...
    this->get_node_topics_interface(), this->get_node_parameters_interface())
{
  ...
  config_.planning_topic_prefix =
    declare_parameter<std::string>("planning_topic_prefix", config_.planning_topic_prefix);
  config_.planning_topic_suffix =
    declare_parameter<std::string>("planning_topic_suffix", config_.planning_topic_suffix);
  config_.trajectory_planning_rate =
    declare_parameter<double>("trajectory_planning_rate", config_.trajectory_planning_rate);
  config_.max_trajectory_duration =
    declare_parameter<double>("trajectory_duration_threshold", config_.max_trajectory_duration);
  config_.min_crawl_speed = declare_parameter<double>("min_speed", config_.min_crawl_speed);
  config_.duration_to_signal_before_lane_change = declare_parameter<double>(
    "duration_to_signal_before_lane_change", config_.duration_to_signal_before_lane_change);
  config_.tactical_plugin_service_call_timeout = declare_parameter<int>(
    "tactical_plugin_service_call_timeout", config_.tactical_plugin_service_call_timeout);
}
...
carma_ros2_utils::CallbackReturn PlanDelegator::handle_on_configure(const rclcpp_lifecycle::State &)
{
  ...
  upcoming_lane_change_status_pub_ =
    create_publisher<carma_planning_msgs::msg::UpcomingLaneChangeStatus>(
      "upcoming_lane_change_status", 1);
  turn_signal_command_pub_ = create_publisher<autoware_msgs::msg::LampCmd>("lamp_cmd", 1);

  // Setup subscribers
  plan_sub_ = create_subscription<carma_planning_msgs::msg::ManeuverPlan>(
    "final_maneuver_plan", 5, std::bind(&PlanDelegator::maneuverPlanCallback, this, std_ph::_1));
  ...
  pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "current_pose", 5, std::bind(&PlanDelegator::poseCallback, this, std_ph::_1));
  guidance_state_sub_ = create_subscription<carma_planning_msgs::msg::GuidanceState>(
    "guidance_state", 5, std::bind(&PlanDelegator::guidanceStateCallback, this, std_ph::_1));
...
{
  traj_timer_ = create_timer(
    get_clock(), std::chrono::milliseconds((int)(1 / config_.trajectory_planning_rate * 1000)),
...
{
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("plan_delegator"),
  ...
  if (isManeuverPlanValid(copy_plan)) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("plan_delegator"),
      "Received plan with " << latest_maneuver_plan_.maneuvers.size() << " maneuvers");
  ...
  // Update upcoming_lane_change_information_ and current_lane_change_information_ based on the
  // received maneuver plan
  if (!latest_maneuver_plan_.maneuvers.empty()) {
    ...
    // Set upcoming_lane_change_information_ based on the first found lane change in the plan that
    // begins after current_downtrack, if one exists
    upcoming_lane_change_information_ =
      boost::optional<LaneChangeInformation>();  // Reset to empty optional
    for (const auto & maneuver : latest_maneuver_plan_.maneuvers) {
      if (maneuver.type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE) {
        if (current_downtrack >= maneuver.lane_change_maneuver.start_dist) {
          // Skip this lane change maneuver since ego vehicle has passed the lane change start point
          // (this is not an 'upcoming' lane change)
          continue;
        } else {
          LaneChangeInformation upcoming_lane_change_information =
            getLaneChangeInformation(maneuver);
          upcoming_lane_change_information_ =
            boost::optional<LaneChangeInformation>(upcoming_lane_change_information);
          break;
        }
      }
    }
    ...
    current_lane_change_information_ =
      boost::optional<LaneChangeInformation>();  // Reset to empty optional
    if (
      latest_maneuver_plan_.maneuvers[0].type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE) {
      LaneChangeInformation current_lane_change_information =
        getLaneChangeInformation(latest_maneuver_plan_.maneuvers[0]);
      current_lane_change_information_ =
        boost::optional<LaneChangeInformation>(current_lane_change_information);
    }
  }
}
...
LaneChangeInformation PlanDelegator::getLaneChangeInformation(
  const carma_planning_msgs::msg::Maneuver & lane_change_maneuver)
{
  ...
  lanelet::ConstLanelet starting_lanelet = wm_->getMap()->laneletLayer.get(
    std::stoi(lane_change_maneuver.lane_change_maneuver.starting_lane_id));
  lanelet::ConstLanelet ending_lanelet = wm_->getMap()->laneletLayer.get(
    std::stoi(lane_change_maneuver.lane_change_maneuver.ending_lane_id));

  // Determine if lane change is a left or right lane change and update lane_change_information
  // accordingly
  ...
  while (!shared_boundary_found) {
    ...
    if (current_lanelet.leftBound() == ending_lanelet.rightBound()) {
      // If current lanelet's left lane boundary matches the ending lanelet's right lane boundary,
      // it is a left lane change
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("plan_delegator"), "Lanelet " << std::to_string(current_lanelet.id())
      ...
      // If current lanelet's right lane boundary matches the ending lanelet's left lane boundary,
      // it is a right lane change
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("plan_delegator"), "Lanelet " << std::to_string(current_lanelet.id())
      ...
      if (wm_->getMapRoutingGraph()->following(current_lanelet, false).empty()) {
        // Maneuver requires we travel further before completing lane change, but there is no
        // routable lanelet directly ahead; in this case we have reached a lanelet which does not
        // have a routable lanelet ahead and isn't adjacent to the lanelet where lane change ends.
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("plan_delegator"), "Now checking for shared lane boundary with lanelet "
...
void PlanDelegator::publishUpcomingLaneChangeStatus(
  const boost::optional<LaneChangeInformation> & upcoming_lane_change_information)
{
  // Initialize an UpcomingLaneChangeStatus message, which will be populated based on
  ...
  if (upcoming_lane_change_information) {
    // Get the downtrack distance between the ego vehicle and the start of the upcoming lane change
    // maneuver
    lanelet::BasicPoint2d current_loc(latest_pose_.pose.position.x, latest_pose_.pose.position.y);
    double current_downtrack = wm_->routeTrackPos(current_loc).downtrack;
    upcoming_lane_change_status.downtrack_until_lanechange =
      std::max(0.0, upcoming_lane_change_information.get().starting_downtrack - current_downtrack);
    ...
    if (upcoming_lane_change_information.get().is_right_lane_change) {
      upcoming_lane_change_status.lane_change =
        carma_planning_msgs::msg::UpcomingLaneChangeStatus::RIGHT;
    } else {
      upcoming_lane_change_status.lane_change =
        carma_planning_msgs::msg::UpcomingLaneChangeStatus::LEFT;
    }
  } else {
    upcoming_lane_change_status.lane_change =
      carma_planning_msgs::msg::UpcomingLaneChangeStatus::NONE;
  }
...
void PlanDelegator::publishTurnSignalCommand(
  ...
  const carma_planning_msgs::msg::UpcomingLaneChangeStatus & upcoming_lane_change_status)
{
  ...
  // NOTE: A LampCmd message can have its 'r' OR 'l' field set to 1 to indicate an activated right
  // or left turn signal, respectively. Both fields cannot be set to 1 at the same time.
  ...
  if (current_lane_change_information) {
    ...
    upcoming_lane_change_status.lane_change !=
    carma_planning_msgs::msg::UpcomingLaneChangeStatus::NONE) {
    // Only publish turn signal command for upcoming lane change if it will begin in less than the
    // time defined by config_.duration_to_signal_before_lane_change
    if (
      (upcoming_lane_change_status.downtrack_until_lanechange / latest_twist_.twist.linear.x) <=
      config_.duration_to_signal_before_lane_change) {
      if (
        upcoming_lane_change_status.lane_change ==
        carma_planning_msgs::msg::UpcomingLaneChangeStatus::RIGHT) {
        turn_signal_command.r = 1;
...
PlanDelegator::getPlannerClientByName(const std::string & planner_name)
{
  ...
  if (trajectory_planners_.find(planner_name) == trajectory_planners_.end()) {
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger("plan_delegator"), "Discovered new trajectory planner: " << planner_name);

    trajectory_planners_.emplace(
      planner_name,
      create_client<carma_planning_msgs::srv::PlanTrajectory>(
        config_.planning_topic_prefix + planner_name + config_.planning_topic_suffix));
  }
  return trajectory_planners_[planner_name];
}

bool PlanDelegator::isManeuverPlanValid(
  const carma_planning_msgs::msg::ManeuverPlan & maneuver_plan) const noexcept
{
  // currently it only checks if maneuver list is empty
  return !maneuver_plan.maneuvers.empty();
}

bool PlanDelegator::isTrajectoryValid(
  const carma_planning_msgs::msg::TrajectoryPlan & trajectory_plan) const noexcept
{
  // currently it only checks if trajectory contains less than 2 points
  return !(trajectory_plan.trajectory_points.size() < 2);
}

bool PlanDelegator::isManeuverExpired(
  const carma_planning_msgs::msg::Maneuver & maneuver, rclcpp::Time current_time) const
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"),
    "maneuver start time:" << std::to_string(
      rclcpp::Time(GET_MANEUVER_PROPERTY(maneuver, start_time)).seconds()));
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"),
    "maneuver end time:" << std::to_string(
      rclcpp::Time(GET_MANEUVER_PROPERTY(maneuver, end_time)).seconds()));
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"), "current time:" << std::to_string(now().seconds()));
  bool isexpired =
    rclcpp::Time(GET_MANEUVER_PROPERTY(maneuver, end_time), get_clock()->get_clock_type()) <=
    current_time;  // TODO maneuver expiration should maybe be based off of distance not time?
                   // https://github.com/usdot-fhwa-stol/carma-platform/issues/1107
  ...
  const uint16_t & current_maneuver_index) const
{
  ...
  if (latest_trajectory_plan.trajectory_points.empty()) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("plan_delegator"), "latest_pose_.header.stamp: " << std::to_string(
                                              rclcpp::Time(latest_pose_.header.stamp).seconds()));
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("plan_delegator"),
      "plan_req->header.stamp: " << std::to_string(rclcpp::Time(plan_req->header.stamp).seconds()));
  ...
  // set vehicle state based on last two planned trajectory points
    ...
    carma_planning_msgs::msg::TrajectoryPlanPoint last_point =
      latest_trajectory_plan.trajectory_points.back();
    carma_planning_msgs::msg::TrajectoryPlanPoint second_last_point =
      *(latest_trajectory_plan.trajectory_points.rbegin() + 1);
    plan_req->vehicle_state.x_pos_global = last_point.x;
    plan_req->vehicle_state.y_pos_global = last_point.y;
    auto distance_diff = std::sqrt(
      std::pow(last_point.x - second_last_point.x, 2) +
      std::pow(last_point.y - second_last_point.y, 2));
    rclcpp::Duration time_diff =
      rclcpp::Time(last_point.target_time) - rclcpp::Time(second_last_point.target_time);
...
bool PlanDelegator::isTrajectoryLongEnough(
  const carma_planning_msgs::msg::TrajectoryPlan & plan) const noexcept
{
  rclcpp::Duration time_diff = rclcpp::Time(plan.trajectory_points.back().target_time) -
                               rclcpp::Time(plan.trajectory_points.front().target_time);
...
void PlanDelegator::updateManeuverParameters(carma_planning_msgs::msg::Maneuver & maneuver)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"),
    "Changing maneuver distances for planner: " << GET_MANEUVER_PROPERTY(
      maneuver, parameters.planning_tactical_plugin));
  double adjusted_start_dist = original_start_dist - length_to_front_bumper_;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"), "original_start_dist:" << original_start_dist);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"), "adjusted_start_dist:" << adjusted_start_dist);
  double adjusted_end_dist = original_end_dist - length_to_front_bumper_;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"), "original_end_dist:" << original_end_dist);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("plan_delegator"), "adjusted_end_dist:" << adjusted_end_dist);
  ...
  // NOTE: Assumes that maneuver start and end downtrack distances have not been shifted by more
  // than one lanelet
  if (
    maneuver.type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING &&
    !maneuver.lane_following_maneuver.lane_ids.empty()) {
    // (1) Add new beginning lanelet to maneuver if necessary and (2) remove ending lanelet from
    // maneuver if necessary

    // Obtain the original starting lanelet from the maneuver
    lanelet::Id original_starting_lanelet_id =
      std::stoi(maneuver.lane_following_maneuver.lane_ids.front());
    lanelet::ConstLanelet original_starting_lanelet =
      wm_->getMap()->laneletLayer.get(original_starting_lanelet_id);

    // Get the downtrack of the start of the original starting lanelet
    lanelet::BasicPoint2d original_starting_lanelet_centerline_start_point =
      lanelet::utils::to2D(original_starting_lanelet.centerline()).front();
    double original_starting_lanelet_centerline_start_point_dt =
    ...
    if (adjusted_start_dist < original_starting_lanelet_centerline_start_point_dt) {
      auto previous_lanelets =
        wm_->getMapRoutingGraph()->previous(original_starting_lanelet, false);
      ...
      if (!previous_lanelets.empty()) {
        ...
        if (llt_on_route_optional) {
          ...
          RCLCPP_WARN_STREAM(
            rclcpp::get_logger("plan_delegator"),
            "When adjusting maneuver for lane follow, no previous lanelet found on the shortest "
            "path for lanelet "
              << original_starting_lanelet.id()
              << ". Picking arbitrary lanelet: " << previous_lanelets[0].id() << ", instead");
          previous_lanelet_to_add = previous_lanelets[0];
        }

        // lane_ids array is ordered by increasing downtrack, so this new starting lanelet is
        // inserted at the front
        maneuver.lane_following_maneuver.lane_ids.insert(
          maneuver.lane_following_maneuver.lane_ids.begin(),
          std::to_string(previous_lanelet_to_add.id()));

        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("plan_delegator"), "Inserted lanelet "
                                                  << std::to_string(previous_lanelet_to_add.id())
    ...
    lanelet::Id original_ending_lanelet_id =
      std::stoi(maneuver.lane_following_maneuver.lane_ids.back());
    lanelet::ConstLanelet original_ending_lanelet =
      wm_->getMap()->laneletLayer.get(original_ending_lanelet_id);

    // Get the downtrack of the start of the maneuver ending lanelet
    lanelet::BasicPoint2d original_ending_lanelet_centerline_start_point =
      lanelet::utils::to2D(original_ending_lanelet.centerline()).front();
    double original_ending_lanelet_centerline_start_point_dt =
    ...
    if (adjusted_end_dist < original_ending_lanelet_centerline_start_point_dt) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("plan_delegator"),
        "Original ending lanelet "
          << original_ending_lanelet.id()
          << " removed from lane_ids since the updated maneuver no longer crosses it");

      // lane_ids array is ordered by increasing downtrack, so the last element in the array
      // corresponds to the original ending lanelet
      maneuver.lane_following_maneuver.lane_ids.pop_back();
    }
  } else if (maneuver.type != carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING) {
    // (1) Update starting maneuver lanelet if necessary and (2) Update ending maneuver lanelet if
    // necessary
    ...
    if (adjusted_start_dist < original_starting_lanelet_centerline_start_point_dt) {
      ...
      if (!previous_lanelets.empty()) {
        ...
        if (llt_on_route_optional) {
          ...
          RCLCPP_WARN_STREAM(
            rclcpp::get_logger("plan_delegator"),
            "When adjusting non-lane follow maneuver, no previous lanelet found on the shortest "
            "path for lanelet "
              << original_starting_lanelet.id()
              << ". Picking arbitrary lanelet: " << previous_lanelets[0].id() << ", instead");
          previous_lanelet_to_add = previous_lanelets[0];
...
carma_planning_msgs::msg::TrajectoryPlan PlanDelegator::planTrajectory()
{
  ...
  // Track the index of the starting maneuver in the maneuver plan that this trajectory plan service
  // request is for
  ...
  while (current_maneuver_index < latest_maneuver_plan_.maneuvers.size()) {
    ...
    if (isManeuverExpired(maneuver, get_clock()->now())) {
      RCLCPP_INFO_STREAM(
        rclcpp::get_logger("plan_delegator"),
        "Dropping expired maneuver: " << GET_MANEUVER_PROPERTY(maneuver, parameters.maneuver_id));
    ...
    double current_downtrack = wm_->routeTrackPos(current_loc).downtrack;
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("plan_delegator"), "current_downtrack" << current_downtrack);
    double maneuver_end_dist = GET_MANEUVER_PROPERTY(maneuver, end_dist);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("plan_delegator"), "maneuver_end_dist" << maneuver_end_dist);

    // ignore maneuver that is passed.
    if (current_downtrack > maneuver_end_dist) {
      RCLCPP_INFO_STREAM(
        rclcpp::get_logger("plan_delegator"),
        "Dropping passed maneuver: " << GET_MANEUVER_PROPERTY(maneuver, parameters.maneuver_id));
      // Update the maneuver plan index for the next loop
      ++current_maneuver_index;
      continue;
    }
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("plan_delegator"), "Current planner: " << maneuver_planner);
    ...
    auto future_status = plan_response.wait_for(
      std::chrono::milliseconds(config_.tactical_plugin_service_call_timeout));
    ...
    if (future_status == std::future_status::ready) {
      ...
      if (!isTrajectoryValid(plan_response.get()->trajectory_plan)) {
        RCLCPP_WARN_STREAM(
          rclcpp::get_logger("plan_delegator"),
          "Found invalid trajectory with less than 2 trajectory points for "
      ...
      if (latest_trajectory_plan.trajectory_points.size() != 0) {
        if (
          latest_trajectory_plan.trajectory_points.back().target_time ==
          plan_response.get()->trajectory_plan.trajectory_points.front().target_time) {
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("plan_delegator"),
            "Removing duplicate point for planner: " << maneuver_planner);
          plan_response.get()->trajectory_plan.trajectory_points.erase(
            plan_response.get()->trajectory_plan.trajectory_points.begin());
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("plan_delegator"),
            "plan_response.get()->trajectory_plan size: "
              << plan_response.get()->trajectory_plan.trajectory_points.size());
        }
      }
      latest_trajectory_plan.trajectory_points.insert(
        latest_trajectory_plan.trajectory_points.end(),
        plan_response.get()->trajectory_plan.trajectory_points.begin(),
      ...
      // Assign the trajectory plan's initial longitudinal velocity based on the first tactical
      // plugin's response
      if (first_trajectory_plan == true) {
        latest_trajectory_plan.initial_longitudinal_velocity =
          plan_response.get()->trajectory_plan.initial_longitudinal_velocity;
      ...
      if (isTrajectoryLongEnough(latest_trajectory_plan)) {
        RCLCPP_INFO_STREAM(
          rclcpp::get_logger("plan_delegator"),
          "Plan Trajectory completed for " << std::string(latest_maneuver_plan_.maneuver_plan_id));
        break;
      }

      // Update the maneuver plan index based on the last maneuver index converted to a trajectory
      // This is required since inlanecruising_plugin can plan a trajectory over contiguous
      // LANE_FOLLOWING maneuvers
      ...
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("plan_delegator"),
        "Unsuccessful service call to trajectory planner:"
          << maneuver_planner << " for plan ID "
          << std::string(latest_maneuver_plan_.maneuver_plan_id));
      // if one service call fails, it should end plan immediately because it is there is no point
      // to generate plan with empty space
      break;
    }
  }
...
void PlanDelegator::lookupFrontBumperTransform()
{
  ...
  try {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("plan_delegator"), "length_to_front_bumper_: " << length_to_front_bumper_);
