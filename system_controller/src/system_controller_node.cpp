...
void SystemControllerNode::initialize()
{
  try {
    ...
    // NOTE: Currently, intra-process comms must be disabled for a publisher that is
    ...
    // Create a publisher that will send all previously published messages to late-joining
    // subscribers ONLY If the subscriber is transient_local too
    auto pub_qos_transient_local =
      rclcpp::QoS(rclcpp::KeepLast(5));  // A publisher with this QoS will store the last 5 messages
                                         // that it has sent on the topic
    ...
    config_.signal_configure_delay =
      this->declare_parameter<double>("signal_configure_delay", config_.signal_configure_delay);
    config_.service_timeout_ms =
      this->declare_parameter<int64_t>("service_timeout_ms", config_.service_timeout_ms);
    config_.call_timeout_ms =
      this->declare_parameter<int64_t>("call_timeout_ms", config_.call_timeout_ms);
    config_.required_subsystem_nodes = this->declare_parameter<std::vector<std::string>>(
      "required_subsystem_nodes", config_.required_subsystem_nodes);
...
void SystemControllerNode::set_config(SystemControllerConfig config) { config_ = config; }
...
void SystemControllerNode::on_error(const std::exception & e)
{
  ...
  RCLCPP_ERROR_STREAM(get_logger(), reason);  // Log error

  lifecycle_mgr_.shutdown(
    std_msec(config_.service_timeout_ms), std_msec(config_.call_timeout_ms),
    false);  // Trigger shutdown when internal error occurs
...
void SystemControllerNode::startup_delay_callback()
{
  try {
    ...
    if (!lifecycle_mgr_
           .configure(std_msec(config_.service_timeout_ms), std_msec(config_.call_timeout_ms))
           .empty()) {
      ...
      lifecycle_mgr_.shutdown(
        std_msec(config_.service_timeout_ms), std_msec(config_.call_timeout_ms), false);
    ...
    if (!lifecycle_mgr_
           .activate(std_msec(config_.service_timeout_ms), std_msec(config_.call_timeout_ms))
           .empty()) {
      RCLCPP_ERROR_STREAM(get_logger(), "System could not be activated. Shutting down.");
...
void SystemControllerNode::publish_system_alert(carma_msgs::msg::SystemAlert msg)
{
  ...
  msg.source_node =
    get_node_base_interface()->get_fully_qualified_name();  // The the source name for the message
...
void SystemControllerNode::on_system_alert(const carma_msgs::msg::SystemAlert::UniquePtr msg)
{
  try {
    ...
    if (
      msg->type == carma_msgs::msg::SystemAlert::SHUTDOWN   // If a SHUTDOWN signal was received
      || (msg->type == carma_msgs::msg::SystemAlert::FATAL  // OR if a required node notified FATAL
          && std::find(
               config_.required_subsystem_nodes.begin(), config_.required_subsystem_nodes.end(),
               msg->source_node) != config_.required_subsystem_nodes.end())) {
      // TODO might make more sense for external shutdown to be a service call and remove SHUTDOWN
      // from system alert entirely
      ...
      std::this_thread::sleep_for(
        std::chrono::milliseconds(5));  // Add a bit of sleep to allow message publication
