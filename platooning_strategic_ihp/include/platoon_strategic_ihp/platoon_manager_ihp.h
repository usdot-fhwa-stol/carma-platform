...
struct ActionPlan
{
  ...
  std::string
    peerId;  // vehicle ID of the candidtate joining vehicle or leader of platoon we are joining
 ...
 * ACK - indicates that the plugin accepts the MobilityRequest and will handle making any
 * adjustments needed to avoid a conflict NACK - indicates that the plugin rejects the
 * MobilityRequest and would suggest the other vehicle replan NO_RESPONSE - indicates that the
 * plugin is indifferent but sees no conflict
 */
...
struct PlatoonMember
{
  ...
  : staticId(""),
    ...
    vehicleSpeed(0.0),
    vehiclePosition(0.0),
    vehicleCrossTrack(0.0),
    timestamp(0)
  {
  }

  PlatoonMember(
    std::string staticId, double commandSpeed, double vehicleSpeed, double vehiclePosition,
    double vehicleCrossTrack, long timestamp)
  : staticId(staticId),
    commandSpeed(commandSpeed),
    vehicleSpeed(vehicleSpeed),
    vehiclePosition(vehiclePosition),
    vehicleCrossTrack(vehicleCrossTrack),
    timestamp(timestamp)
  {
  }
};
...
/**
 * \brief Class containing the logic for platoon manager. It is responsible for keeping track of the
 * platoon members and role of the host vehicle in the platoon
 */
...
public:
  ...
  /**
   * \brief Update information for members of the host's platoon based on a mobility operation
   * STATUS message
   *
   ...
   */
  void hostMemberUpdates(
    const std::string & senderId, const std::string & platoonId, const std::string & params,
    const double & DtD, const double & CtD);

  /**
   * \brief Update information for members of a neighboring platoon based on a mobility operation
   * STATUS message
   *
   * \param senderId static id of the broadcasting vehicle
   * \param platoonId platoon id
   ...
   * \param CtD crosstrack distance in roadway at host's location, m
   */
  void neighborMemberUpdates(
    const std::string & senderId, const std::string & platoonId, const std::string & params,
    const double & DtD, const double & CtD);

  /**
   * \brief Updates the list of vehicles in the specified platoon, based on info available from a
   * mobility operation STATUS message from one of that platoon's vehicles.  It ensures the list of
   *vehicles is properly sorted in order of appearance from front to rear in the platoon.  If the
   *host is in the platoon, it will update host info as well.
   *
   ...
   * \param dtDistance the downtrack location (from beginning of host's route) of the sending
   *vehicle, m \param ctDistance the crosstrack location (from center of roadway at host's current
   *route location) of sending vehicle, m \param curSpeed the current actual speed of the sending
   *vehicle, m/s
   **/
  void updatesOrAddMemberInfo(
  ...
  /**
   * \brief Resets all  variables that might indicate other members of the host's platoon; sets the
   * host back to solo vehicle.
   */
  ...
  /**
   * \brief UCLA: Return follower's desired position (i.e., downtrack, in m) that maintains the
   * desired intra-platoon time gap, based on IHP platoon trajectory regulation algorithm.
   *
  ...
  /**
   * \brief UCLA: Return joiner's desired position in terms of target platoon index to cut into the
   * platoon. CAUTION: ASSUMES that the neighbor platoon info is fully populated!
   *
   * \param joinerDtD: The current downtrack distance (with regards to host vehicle) of the joiner
   * vehicle.
   *
   * \return: cut-in index: The index of the gap-leading vehicle within the platoon. If front join,
   * return -1.
   */
   ...
   *        Note: The origin of the vehicle (for downtrack distance calculation) is located at the
   * rear axle. CAUTION: ASSUMES that the neighbor platoon info is fully populated!
   *
   * \param gap_leading_index: The platoon index of the  gap-leading vehicle.
   * \param joinerDtD: The current downtrack distance (with regards to host vehicle route) of the
   * joiner vehicle.
   *
  ...
  // List of members in the host's own platoon (host will always be represented, so size is never
  // zero)
  ...
  // CAUTION: we can only represent one neighbor platoon in this version, so if multiple platoons
  // are nearby,
  ...
  std::string neighborPlatoonID = dummyID;  // ID of the neighbor platoon that we may be attempting
                                            // to join (dummy if neighbor is a solo vehicle)
...
private:
  ...
  std::string algorithmType_ = "APF_ALGORITHM";  // a string that defines the current algorithm to
                                                 // determine the dynamic leader.
   ...
   */
  std::vector<double> calculateTimeHeadway(
   ...
   * Note that the host will always choose the closest violator (i.e. time headaway too small or too
   * large) to follow.
   *
   * \param timeHeadways: A vector containing the time headaway of all platoon members.
   *
   * \return An index indicating the proper vehicle to follow (i.e., leader). If choose to follow
   * platoon leader, return 0.
   */
  int determineDynamicLeaderBasedOnViolation(std::vector<double> timeHeadways);

  /**
   * \brief Find the closest vehicle to the host vehicle that violates the (time headaway) lower
   * boundary condition.
   *
   * \param timeHeadways: A vector containing the time headaway of all platoon members.
   *
  ...
  /**
   * \brief Find the closest vehicle to the host vehicle that violates the (time headaway) maximum
   * spacing condition.
   *
   * \param timeHeadways: A vector containing the time headaway of all platoon members.
   *
  ...
  /**
   * \brief Return a sub-vector of the platoon-wise time headaways vector that start with a given
   * index.
   *
   * \param timeHeadways: A vector containing the time headaway of all platoon members.
