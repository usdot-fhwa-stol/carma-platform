...
const std::unordered_set<std::string> LocalizationManager::LIDAR_FAILURE_STRINGS(
  {"One LIDAR Failed", "Both LIDARS Failed"});

LocalizationManager::LocalizationManager(
  PosePublisher pose_pub, StatePublisher state_pub, ManagedInitialPosePublisher initialpose_pub,
...
{
  transition_table_.setTransitionCallback(std::bind(
    &LocalizationManager::stateTransitionCallback, this, std::placeholders::_1,
    std::placeholders::_2, std::placeholders::_3));
  timer_clock_type_ = timer_factory_->now().get_clock_type();
}

double LocalizationManager::computeFreq(
  const rclcpp::Time & old_stamp, const rclcpp::Time & new_stamp) const
{
  return 1.0 / (new_stamp - old_stamp).seconds();  // Convert delta to frequency (Hz = 1/s)
}

void LocalizationManager::setConfig(const LocalizationManagerConfig & config) { config_ = config; }

double LocalizationManager::computeNDTFreq(const rclcpp::Time & new_stamp)
{
  if (!prev_ndt_stamp_) {  // Check if this is the first data point
  ...
  return computeFreq(
    rclcpp::Time(prev_ndt_stamp_.get(), timer_clock_type_),
    new_stamp);  // Convert delta to frequency (Hz = 1/s)
}

void LocalizationManager::poseAndStatsCallback(
  ...
  const autoware_msgs::msg::NDTStat::ConstPtr stats)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("localization_manager"), "Received pose resulting in frequency value of "
                                                  << ndt_freq << " with score of " << stats->score);

  if (
    stats->score >= config_.fitness_score_fault_threshold ||
    ndt_freq <= config_.ndt_frequency_fault_threshold) {
    transition_table_.signal(LocalizationSignal::UNUSABLE_NDT_FREQ_OR_FITNESS_SCORE);
  } else if (
    stats->score >= config_.fitness_score_degraded_threshold ||
    ...
    if (!(static_cast<LocalizerMode>(config_.localization_mode) ==
            LocalizerMode::GNSS_WITH_NDT_INIT &&
          transition_table_.getState() == LocalizationState::DEGRADED_NO_LIDAR_FIX &&
          lidar_init_sequential_timesteps_counter_ >=
            config_.sequential_timesteps_until_gps_operation)) {
  ...
  if (
    static_cast<LocalizerMode>(config_.localization_mode) == LocalizerMode::GNSS_WITH_NDT_INIT &&
    state == LocalizationState::OPERATIONAL && last_raw_gnss_value_) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("localization_manager"),
      "lidar_init_sequential_timesteps_counter_: " << lidar_init_sequential_timesteps_counter_);
    if (
      lidar_init_sequential_timesteps_counter_ < config_.sequential_timesteps_until_gps_operation) {
      if (is_sequential_) lidar_init_sequential_timesteps_counter_++;
  ...
  } else if (!(static_cast<LocalizerMode>(config_.localization_mode) ==
                 LocalizerMode::GNSS_WITH_NDT_INIT &&
               state == LocalizationState::DEGRADED_NO_LIDAR_FIX && last_raw_gnss_value_ &&
               lidar_init_sequential_timesteps_counter_ >=
                 config_.sequential_timesteps_until_gps_operation))  // don't reset counter if Lidar
                                                                     // initialized and switched to
                                                                     // GPS as it will reset state
                                                                     // to OPERATIONAL
  {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("localization_manager"),
      "Resetting lidar_init_sequential_timesteps_counter_: "
        << lidar_init_sequential_timesteps_counter_
        << ", with new state: " << transition_table_.getState());
  }

  if (state == LocalizationState::OPERATIONAL && last_raw_gnss_value_) {
    tf2::Vector3 ndt_translation(
      pose->pose.position.x, pose->pose.position.y, pose->pose.position.z);

    tf2::Vector3 gnss_translation(
      last_raw_gnss_value_->pose.position.x, last_raw_gnss_value_->pose.position.y,
      last_raw_gnss_value_->pose.position.z);
  ...
  if (state != LocalizationState::DEGRADED_NO_LIDAR_FIX) {
    RCLCPP_DEBUG_STREAM(
      ...
      "Publishing mixed pose with lidar_init_sequential_timesteps_counter_: "
        << lidar_init_sequential_timesteps_counter_ << ", and state" << state);
    current_pose_ = *pose;
  }
...
void LocalizationManager::gnssPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  ...
  // Just like ndt_matching the gnss pose is treated as an initialize signal if the system is not
  // yet intialized
  ...
  if (transition_table_.getState() == LocalizationState::DEGRADED_NO_LIDAR_FIX) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("localization_manager"),
      "Publishing GNSS pose with lidar_init_sequential_timesteps_counter_: "
        << lidar_init_sequential_timesteps_counter_);
...
void LocalizationManager::stateTransitionCallback(
  LocalizationState prev_state, LocalizationState new_state, LocalizationSignal signal)
{
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("localization_manager"),
  ...
  switch (new_state) {
    case LocalizationState::INITIALIZING:
      ...
      current_timer_ = timer_factory_->buildTimer(
        current_timer_id_,
        rclcpp::Duration::from_nanoseconds(config_.auto_initialization_timeout * 1e6),
        std::bind(&LocalizationManager::timerCallback, this, new_state), true, true);

      timers_[current_timer_id_] =
        std::make_pair(std::move(current_timer_), false);  // Add start timer to map by Id
      break;
    case LocalizationState::DEGRADED_NO_LIDAR_FIX:
      current_timer_id_ = nextId();
      current_timer_ = timer_factory_->buildTimer(
        current_timer_id_,
        rclcpp::Duration::from_nanoseconds(config_.gnss_only_operation_timeout * 1e6),
        std::bind(&LocalizationManager::timerCallback, this, new_state), true, true);
...
void LocalizationManager::posePubTick()
{
  ...
  // This check provides protection against excessively long NDT computation times that do not
  // trigger the callback
  if (prev_ndt_stamp_) {
    double freq =
      computeFreq(rclcpp::Time(prev_ndt_stamp_.get(), timer_clock_type_), timer_factory_->now());
  ...
  if (
    transition_table_.getState() != LocalizationState::OPERATIONAL &&
    transition_table_.getState() != LocalizationState::DEGRADED_NO_LIDAR_FIX) {
    lidar_init_sequential_timesteps_counter_ = 0;
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("localization_manager"),
  ...
  if (current_pose_) {
    ...
    if (
      static_cast<LocalizerMode>(config_.localization_mode) ==
...
LocalizationState LocalizationManager::getState() const { return transition_table_.getState(); }
