...
namespace ukfnode {
    ...
    void UKFNode::imuCallback(const sensor_msgs::Imu &imu_msg) {
        ...
        z_imu_raw_(1) =
                x_previous(3) + (imu_msg.linear_acceleration.x * delta_t_); //Linear acceleration along x-direction
        z_imu_raw_(2) =
                x_previous(4) + (imu_msg.linear_acceleration.y * delta_t_); //Linear acceleration along y-direction
        ...
        R_imu_(1, 1) = P_previous(3, 3) +
                       (delta_t_ * delta_t_ * imu_msg.linear_acceleration_covariance[0]);//linear accelertion x
        R_imu_(2, 2) = P_previous(4, 4) +
                       (delta_t_ * delta_t_ * imu_msg.linear_acceleration_covariance[4]);//linear acceleration y
        R_imu_(3, 3) = imu_msg.angular_velocity_covariance[8];//angular velocity z
        R_imu_(4,
               4) = 0.00000000000000000000000000001;//Adjust the sensor covariance for the addition of number one, we can't assume zero because the matrix will be invertible
    ...
    void UKFNode::gpsCallback(const geometry_msgs::PoseWithCovarianceStamped &gps_msg) {
        ...
        R_gps_(3,
               3) = 0.00000000000000000000000000001;//Adjust the sensor covariance for the addition of number one, we can't assume zero because the matrix will be invertible
        //P Matrix need extra zero
        //Same with vehicle state
