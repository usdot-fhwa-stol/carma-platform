...
namespace arbitrator
{
    ...
    {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "INSIDE AAAAAAA!!!");
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "INSIDE first!!!");
        ...
        while (!open_list_to_evaluate.empty())
        {
            ...
            for (auto it = open_list_to_evaluate.begin(); it != open_list_to_evaluate.end(); it++)
            {
                ...
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "START");
                
                for (auto mvr : cur_plan.maneuvers)
                {
                    RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "Printing cur_plan: mvr: "<< (int)mvr.type);
                }   

                RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "PRINT END");
                ...
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "1 END");
                ...
                if (plan_duration >= target_plan_duration_) 
                {
                    ...
                    RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "Has enough duration, skipping that which has following mvrs..:");
                    for (auto mvr : it->first.maneuvers)
                    {
                        RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "Printing mvr: "<< (int)mvr.type);
                    }  
                    continue;
                }
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "1 END");
                ...
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "2 END");
                // Compute cost for each child and store in open list
                for (auto child = children.begin(); child != children.end(); child++)
                {
                    RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "3 END");
                    
                    if (child->maneuvers.empty())
                    {
                        RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "Child was empty for id: " << child->maneuver_plan_id);
                        continue;   
                    }
                    RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "3a END");

                    temp_open_list.push_back(std::make_pair(*child, cost_function_->compute_cost_per_unit_distance(*child)));
                }
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "4 END");
            }
            
            open_list_to_evaluate = temp_open_list;
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "5 END");
        }
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "INSIDE!ended!!");

        final_open_list = search_strategy_->prioritize_plans(final_open_list);
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "INSIDE!!!");
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("arbitrator"), "INSIDE!!!2");
