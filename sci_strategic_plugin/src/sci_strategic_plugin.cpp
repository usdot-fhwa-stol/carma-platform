           ...
           : ((mvr).type == carma_planning_msgs::msg::Maneuver::INTERSECTION_TRANSIT_STRAIGHT \
                ...
                : ((mvr).type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE              \
                     ...
                     : ((mvr).type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING      \
                          ...
                          : ((mvr).type == carma_planning_msgs::msg::Maneuver::STOP_AND_WAIT  \
                               ? (mvr).stop_and_wait_maneuver.property                        \
 ...
 * GET_MANEUVER_PROPERY calls due to it missing in stop and wait plugin \param mvr input maneuver
 * \return end speed
 */
double getManeuverEndSpeed(const carma_planning_msgs::msg::Maneuver & mvr)
{
  switch (mvr.type) {
    ...
    default:
      RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("sci_strategic_plugin"),
...
: carma_guidance_plugins::StrategicPlugin(options), config_(SCIStrategicPluginConfig())
{
  ...
  config_.stop_line_buffer =
    declare_parameter<double>("stop_line_buffer", config_.stop_line_buffer);
  ...
  config_.intersection_exit_zone_length = declare_parameter<double>(
...
carma_ros2_utils::CallbackReturn SCIStrategicPlugin::on_configure_plugin()
{
  ...
  add_on_set_parameters_callback(
    std::bind(&SCIStrategicPlugin::parameter_update_callback, this, std_ph::_1));

  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "Done loading parameters: " << config_);
  ...
  current_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "current_pose", 1, std::bind(&SCIStrategicPlugin::currentPoseCb, this, std_ph::_1));

  // BSM subscriber
  bsm_sub_ = create_subscription<carma_v2x_msgs::msg::BSM>(
    "bsm_outbound", 1, std::bind(&SCIStrategicPlugin::BSMCb, this, std_ph::_1));
...
rcl_interfaces::msg::SetParametersResult SCIStrategicPlugin::parameter_update_callback(
...
carma_ros2_utils::CallbackReturn SCIStrategicPlugin::on_activate_plugin()
{
  mob_op_pub_timer_ = create_timer(
    get_clock(), std::chrono::duration<double>(0.1),
    std::bind(&SCIStrategicPlugin::publishMobilityOperation, this));

  return CallbackReturn::SUCCESS;
}

SCIStrategicPlugin::VehicleState SCIStrategicPlugin::extractInitialState(
  const carma_planning_msgs::srv::PlanManeuvers::Request & req) const
{
  VehicleState state;
  if (!req.prior_plan.maneuvers.empty()) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "Provided with initial plan...");
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"),
    "state.stamp: " << std::to_string(state.stamp.seconds()));
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "state.downtrack : " << state.downtrack);
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("sci_strategic_plugin"), "state.speed: " << state.speed);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "state.lane_id: " << state.lane_id);
...
void SCIStrategicPlugin::mobilityOperationCb(carma_v2x_msgs::msg::MobilityOperation::UniquePtr msg)
{
  if (msg->strategy == stop_controlled_intersection_strategy_) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"),
      "Approaching Stop Controlled Intersection: " << approaching_stop_controlled_interction_);
...
void SCIStrategicPlugin::guidance_state_cb(
...
void SCIStrategicPlugin::currentPoseCb(geometry_msgs::msg::PoseStamped::UniquePtr msg)
{
  ...
  if (vehicle_engaged_) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"),
      "Downtrack from current pose: " << current_downtrack_);
  }
}
...
void SCIStrategicPlugin::parseStrategyParams(const std::string & strategy_params)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "scheduled_stop_time_: " << scheduled_stop_time_);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "scheduled_enter_time_: " << scheduled_enter_time_);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"),
    "scheduled_depart_time_: " << scheduled_depart_time_);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"),
    "scheduled_departure_position_: " << scheduled_departure_position_);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "is_allowed_int_: " << is_allowed_int_);
}

int SCIStrategicPlugin::determine_speed_profile_case(
  double stop_dist, double current_speed, double schedule_stop_time, double speed_limit)
{
  int case_num = 0;
  double estimated_stop_time = calcEstimatedStopTime(stop_dist, current_speed);

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "estimated_stop_time: " << estimated_stop_time);
  if (estimated_stop_time < schedule_stop_time) {
    case_num = 3;
  } else {
    double speed_before_stop =
      calc_speed_before_decel(estimated_stop_time, stop_dist, current_speed);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "speed_before_stop: " << speed_before_stop);
...
double SCIStrategicPlugin::calc_speed_before_decel(
  double stop_time, double stop_dist, double current_speed) const
{
  ...
  double sqr_term = sqrt(
    pow(1 - (desired_acceleration / desired_deceleration), 2) * pow(stop_dist / stop_time, 2) +
    (1 - (desired_acceleration / desired_deceleration)) *
      (current_speed * current_speed - 2 * current_speed * stop_dist / stop_time));

  speed_before_decel =
    (stop_dist / stop_time) + sqr_term / (1 - (desired_acceleration / desired_deceleration));

  return speed_before_decel;
}

std::vector<lanelet::ConstLanelet> SCIStrategicPlugin::getLaneletsBetweenWithException(
  ...
  carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp)
{
  ...
  bool is_empty_schedule_msg =
    (scheduled_depart_time_ == 0 && scheduled_stop_time_ == 0 && scheduled_enter_time_ == 0);
  if (is_empty_schedule_msg) {
    resp->new_plan.maneuvers = {};
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "Receiving empty schedule message");
    return;
  }
  ...
  for (auto l : nearest_stop_intersection->stopLines()) {
    ...
    double stop_bar_dtd =
      wm_->routeTrackPos(l.front().basicPoint2d()).downtrack - config_.veh_length;
  ...
  double distance_to_stopline =
    stop_intersection_down_track - current_downtrack_ - config_.stop_line_buffer;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "distance_to_stopline  " << distance_to_stopline);
  ...
  std::vector<lanelet::ConstLanelet> intersection_lanelets = getLaneletsBetweenWithException(
    stop_intersection_down_track, intersection_end_downtrack, true, true);
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "time_to_approach_int  " << time_to_approach_int);
  ...
  if (time_to_approach_int) {
    ...
    double desired_deceleration =
      config_.vehicle_decel_limit * config_.vehicle_decel_limit_multiplier;

    double safe_distance = pow(current_state.speed, 2) / (2 * desired_deceleration);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "safe_distance:  " << safe_distance);

    if (distance_to_stopline - safe_distance > config_.stop_line_buffer) {
      int case_num = determine_speed_profile_case(
        distance_to_stopline, current_state.speed, time_to_schedule_stop, speed_limit_);
      ...
      if (case_num < 3) {
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("sci_strategic_plugin"), "Decelerating to stop at the intersection");
        ...
        std::vector<lanelet::ConstLanelet> crossed_lanelets = getLaneletsBetweenWithException(
          current_downtrack_, stop_intersection_down_track, true, true);

        double stopping_accel =
          caseThreeSpeedProfile(distance_to_stopline, current_state.speed, time_to_schedule_stop);
        ...
        maneuver_planned = composeStopAndWaitManeuverMessage(
          current_state.downtrack, stop_intersection_down_track, current_state.speed,
          crossed_lanelets[0].id(), crossed_lanelets[0].id(), stopping_accel, current_state.stamp,
      ...
      maneuver_planned = composeStopAndWaitManeuverMessage(
        current_state.downtrack, stop_intersection_down_track, current_state.speed,
        crossed_lanelets[0].id(), crossed_lanelets[0].id(), desired_deceleration,
        current_state.stamp,
        current_state.stamp + rclcpp::Duration::from_nanoseconds(time_to_schedule_stop * 1e9));
    }
  }
  ...
  bool not_time_to_intersection_traverse =
    int((street_msg_timestamp_) - (scheduled_enter_time_)) < 0;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"),
  ...
  if (time_to_stop_at_stopline) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "stop_duration:  " << stop_duration);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "Planning stop and wait maneuver");
    ...
    maneuver_planned = composeStopAndWaitManeuverMessage(
      current_state.downtrack, stop_intersection_down_track, current_state.speed,
      stop_line_lanelet.id(), stop_line_lanelet.id(), stopping_accel, current_state.stamp,
  ...
  if (!is_allowed_int_) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "current_downtrack_: " << current_downtrack_);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "current state dtd: " << current_state.downtrack);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"),
    ...
    maneuver_planned = composeStopAndWaitManeuverMessage(
      current_state.downtrack, stop_intersection_down_track, current_state.speed,
      current_state.lane_id, current_state.lane_id, stop_acc, current_state.stamp,
      current_state.stamp + rclcpp::Duration::from_nanoseconds(stop_duration * 1e9));
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "time for crossing? " << time_for_crossing);

  if (time_for_crossing && is_allowed_int_) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"),
      "street_msg_timestamp_ - scheduled_enter_time_ = " << street_msg_timestamp_ -
                                                              scheduled_enter_time_);

    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"), "Vehicle is crossing the intersection");
    ...
    intersection_transit_time =
      config_.min_maneuver_planning_period;  // std::max(intersection_transit_time,
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"),
      "turn direction at the intersection is: " << intersection_turn_direction_);
    ...
    double end_of_intersection = std::max(
      config_.intersection_exit_zone_length,
      intersection_end_downtrack - stop_intersection_down_track);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"),
      "Actual length of intersection: " << intersection_end_downtrack -
                                             stop_intersection_down_track);
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("sci_strategic_plugin"),
      "Used length of intersection: " << end_of_intersection);

    maneuver_planned = composeIntersectionTransitMessage(
      current_state.downtrack, current_state.downtrack + end_of_intersection, current_state.speed,
      intersection_speed_limit, current_state.stamp,
      rclcpp::Time(req->header.stamp) +
        rclcpp::Duration::from_nanoseconds(intersection_transit_time * 1e9),
    ...
    if (distance_to_stopline < -end_of_intersection) {
      ...
      // once the vehicle crosses the intersection, reset the flag to stop planning and publishing
      // status/intent
...
TurnDirection SCIStrategicPlugin::getTurnDirectionAtIntersection(
...
void SCIStrategicPlugin::caseOneSpeedProfile(
  double speed_before_decel, double current_speed, double stop_time,
  std::vector<double> * float_metadata_list) const
{
  ...
  double a_acc =
    ((1 - desired_acceleration / desired_deceleration) * speed_before_decel - current_speed) /
    stop_time;
  ...
  double a_dec = ((desired_deceleration - desired_acceleration) * speed_before_decel -
                  desired_deceleration * current_speed) /
                 (desired_acceleration * stop_time);
  ...
  double t_dec =
    -speed_before_decel / a_dec;  // a_dec is negative so a - is used to make the t_dec positive.
...
void SCIStrategicPlugin::caseTwoSpeedProfile(
  double stop_dist, double speed_before_decel, double current_speed, double stop_time,
  double speed_limit, std::vector<double> * float_metadata_list) const
{
  double desired_acceleration =
    config_.vehicle_accel_limit * config_.vehicle_accel_limit_multiplier;
  double desired_deceleration =
  ...
  double t_c_nom =
    2 * stop_dist *
      ((1 - desired_acceleration / desired_deceleration) * speed_limit - current_speed) -
    stop_time * ((1 - desired_acceleration / desired_deceleration) * pow(speed_limit, 2) -
                 pow(current_speed, 2));
  double t_c_den = pow(speed_limit - current_speed, 2) -
                   (desired_acceleration / desired_deceleration) * pow(speed_limit, 2);
  double t_cruise = t_c_nom / t_c_den;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"), "Case two t_cruise: " << t_cruise);

  // Equations obtained from TSMO UC 1 Algorithm draft doc
  double a_acc = ((1 - desired_acceleration / desired_deceleration) * speed_limit - current_speed) /
                 (stop_time - t_cruise);
  ...
  double a_dec = ((desired_deceleration - desired_acceleration) * speed_limit -
                  desired_deceleration * current_speed) /
                 (desired_acceleration * (stop_time - t_cruise));
  ...
  double t_dec =
    -speed_limit / a_dec;  // a_dec is negative so a - is used to make the t_dec positive.
...
double SCIStrategicPlugin::caseThreeSpeedProfile(
  double stop_dist, double current_speed, double stop_time) const
{
  double a_dec =
    (2 * stop_dist - current_speed * (stop_time + config_.delta_t)) / (stop_time * config_.delta_t);
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("sci_strategic_plugin"), "Case three a_dec: " << a_dec);
  return a_dec;
}

carma_planning_msgs::msg::Maneuver SCIStrategicPlugin::composeLaneFollowingManeuverMessage(
  int case_num, double start_dist, double end_dist, double start_speed, double target_speed,
  rclcpp::Time start_time, double time_to_stop, std::vector<lanelet::Id> lane_ids)
{
  ...
  maneuver_msg.lane_following_maneuver.parameters.presence_vector =
    ...
    carma_planning_msgs::msg::ManeuverParameters::HAS_STRING_META_DATA;
  ...
  maneuver_msg.lane_following_maneuver.end_time =
    start_time + rclcpp::Duration::from_nanoseconds(time_to_stop * 1e9);
  maneuver_msg.lane_following_maneuver.lane_ids =
    lanelet::utils::transform(lane_ids, [](auto id) { return std::to_string(id); });

  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"),
    "Creating lane follow start dist: " << start_dist << " end dist: " << end_dist);

  std::vector<double> float_metadata_list;

  double speed_before_decel =
    calc_speed_before_decel(time_to_stop, end_dist - start_dist, start_speed);
  ...
  switch (case_num) {
    ...
    case 2:
      caseTwoSpeedProfile(
        end_dist - start_dist, speed_before_decel, start_speed, time_to_stop, speed_limit_,
        &float_metadata_list);
      break;
    default:
      return empty_msg;
...
carma_v2x_msgs::msg::MobilityOperation SCIStrategicPlugin::generateMobilityOperation()
{
  ...
  mo_.strategy_params = "access: " + std::to_string(is_allowed_int_) +
                        ", max_accel: " + std::to_string(vehicle_acceleration_limit_) +
                        ...
                        ", depart_pos: " + std::to_string(scheduled_departure_position_) +
...
carma_planning_msgs::msg::Maneuver SCIStrategicPlugin::composeStopAndWaitManeuverMessage(
  double current_dist, double end_dist, double start_speed, const lanelet::Id & starting_lane_id,
  const lanelet::Id & ending_lane_id, double stopping_accel, rclcpp::Time start_time,
  rclcpp::Time end_time) const
{
  carma_planning_msgs::msg::Maneuver maneuver_msg;
...
carma_planning_msgs::msg::Maneuver SCIStrategicPlugin::composeIntersectionTransitMessage(
  double start_dist, double end_dist, double start_speed, double target_speed,
  rclcpp::Time start_time, rclcpp::Time end_time, TurnDirection turn_direction,
  const lanelet::Id & starting_lane_id, const lanelet::Id & ending_lane_id) const
{
  ...
  if (turn_direction == TurnDirection::Left) {
    ...
    maneuver_msg.intersection_transit_left_turn_maneuver.starting_lane_id =
      std::to_string(starting_lane_id);
    maneuver_msg.intersection_transit_left_turn_maneuver.ending_lane_id =
    ...
    maneuver_msg.intersection_transit_right_turn_maneuver.starting_lane_id =
      std::to_string(starting_lane_id);
    maneuver_msg.intersection_transit_right_turn_maneuver.ending_lane_id =
      std::to_string(ending_lane_id);
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("sci_strategic_plugin"),
    "Creating IntersectionTransitManeuver start dist: " << start_dist << " end dist: " << end_dist
...
bool SCIStrategicPlugin::get_availability() { return true; }

std::string SCIStrategicPlugin::get_version_id() { return "v1.0"; }
