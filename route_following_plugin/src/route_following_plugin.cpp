...
void setManeuverLaneletIds(carma_planning_msgs::msg::Maneuver& mvr, lanelet::Id start_id, lanelet::Id end_id) {
      ...
      : carma_guidance_plugins::StrategicPlugin(options), tf2_buffer_(get_clock())
  {
    ...
    carma_ros2_utils::CallbackReturn RouteFollowingPlugin::handle_on_configure(const rclcpp_lifecycle::State &)
  {
    ...
    twist_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>("current_velocity", 50,
                                                              std::bind(&RouteFollowingPlugin::twist_cb,std_ph::_1));
    current_maneuver_plan_sub_ = create_subscription<carma_planning_msgs::msg::ManeuverPlan>("maneuver_plan", 50,
                                                              std::bind(&RouteFollowingPlugin::current_maneuver_plan_cb,std_ph::_1));
 ...
 carma_ros2_utils::CallbackReturn RouteFollowingPlugin::handle_on_activate(const rclcpp_lifecycle::State &prev_state)
  {
    // Timer setup
    discovery_pub_timer_ = create_timer(get_clock(),std::chrono::milliseconds(100),
    ...
    void plan_maneuvers_callback(
      ...
      carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp)
      {
        ...
        while (planned_time < min_plan_duration_ && i < latest_maneuver_plan_.size())
        {
            ...
            planned_time += rclcpp::Time(getManeuverDuration(latest_maneuver_plan_[i], epsilon_)).seconds();
