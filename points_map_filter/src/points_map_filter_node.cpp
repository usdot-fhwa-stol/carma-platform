...
//       once the underlying PCL version is updated to 1.12.1 which should occur with a ROS version
//       update
...
Node::Node(const rclcpp::NodeOptions & options) : carma_ros2_utils::CarmaLifecycleNode(options)
{
  ...
  // Declare parameters
  config_.cell_side_length =
    declare_parameter<double>("cell_side_length", config_.cell_side_length);
}
...
carma_ros2_utils::CallbackReturn Node::handle_on_configure(const rclcpp_lifecycle::State &)
{
  ...
  points_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
    "points_raw", 10, std::bind(&Node::points_callback, this, std_ph::_1));

  // NOTE: Currently, intra-process comms must be disabled for subcribers that are transient_local:
  // https://github.com/ros2/rclcpp/issues/1753
  rclcpp::SubscriptionOptions map_sub_options;
  map_sub_options.use_intra_process_comm =
    rclcpp::IntraProcessSetting::Disable;  // Disable intra-process comms for this
                                           // SubscriptionOptions object

  auto map_sub_qos =
    rclcpp::QoS(rclcpp::KeepLast(1));  // Set the queue size for a subscriber with this QoS
  map_sub_qos
  ...
  map_sub_ = create_subscription<autoware_lanelet2_msgs::msg::MapBin>(
    "lanelet2_map", map_sub_qos, std::bind(&Node::map_callback, this, std_ph::_1), map_sub_options);
...
void Node::points_callback(sensor_msgs::msg::PointCloud2::UniquePtr msg)
{
  ...
  pcl::experimental::FilterFunction<PointT> filter = [this](
                                                       const CloudT & cloud, pcl::index_t idx) {
...
void Node::map_callback(autoware_lanelet2_msgs::msg::MapBin::UniquePtr msg)
{
  ...
  // TODO it would be great if lanelet2 already knew the map bounds and this iteration didn't need
  // to happen twice
  ...
  RCLCPP_INFO_STREAM(
    get_logger(), "Found base map bounds of (min_x, max_x, min_y, max_y): "
                    << "( " << min_x << ", " << max_x << ", " << min_y << ", " << max_y << ")");
  RCLCPP_INFO_STREAM(
    get_logger(), "Expanded map bounds to: "
                    << "( " << intersection_config.min_x << ", " << intersection_config.max_x
                    << ", " << intersection_config.min_y << ", " << intersection_config.max_y
                    << ")");
