...
carma_ros2_utils::CallbackReturn Node::on_configure_plugin()
{
  ...
  // Setup publishers
  mob_response_pub =
    create_publisher<carma_v2x_msgs::msg::MobilityResponse>("outgoing_mobility_response", 5);
  mob_request_pub =
    create_publisher<carma_v2x_msgs::msg::MobilityRequest>("outgoing_mobility_request", 5);
  mob_operation_pub =
  ...
  worker_ = std::make_shared<PlatoonStrategicIHPPlugin>(
    get_world_model(), config_, [this](auto msg) { this->mob_response_pub->publish(msg); },
    [this](auto msg) { this->mob_request_pub->publish(msg); },
    [this](auto msg) { this->mob_operation_pub->publish(msg); },
    [this](auto msg) { this->platoon_info_pub->publish(msg); },
    std::make_shared<carma_ros2_utils::timers::ROSTimerFactory>(shared_from_this()));

  // Setup subscribers
  mob_request_sub = create_subscription<carma_v2x_msgs::msg::MobilityRequest>(
    "incoming_mobility_request", 10,
    std::bind(&PlatoonStrategicIHPPlugin::mob_req_cb, worker_.get(), std_ph::_1));

  mob_response_sub = create_subscription<carma_v2x_msgs::msg::MobilityResponse>(
    "incoming_mobility_response", 10,
    std::bind(&PlatoonStrategicIHPPlugin::mob_resp_cb, worker_.get(), std_ph::_1));

  mob_operation_sub = create_subscription<carma_v2x_msgs::msg::MobilityOperation>(
    "incoming_mobility_operation", 10,
    std::bind(&PlatoonStrategicIHPPlugin::mob_op_cb, worker_.get(), std_ph::_1));

  current_pose_sub = create_subscription<geometry_msgs::msg::PoseStamped>(
    "current_pose", 10, std::bind(&PlatoonStrategicIHPPlugin::pose_cb, worker_.get(), std_ph::_1));

  current_twist_sub = create_subscription<geometry_msgs::msg::TwistStamped>(
    "current_velocity", 10,
    std::bind(&PlatoonStrategicIHPPlugin::twist_cb, worker_.get(), std_ph::_1));

  cmd_sub = create_subscription<geometry_msgs::msg::TwistStamped>(
    "twist_raw", 10, std::bind(&PlatoonStrategicIHPPlugin::cmd_cb, worker_.get(), std_ph::_1));

  georeference_sub = create_subscription<std_msgs::msg::String>(
    "georeference", 10,
...
{
  // Ensure subscribers are disconnected incase cleanup is called, we don't want to keep driving the
  // worker
...
{
  if (worker_) worker_->plan_maneuver_cb(*req, *resp);
}

bool Node::get_availability() { return true; }

std::string Node::get_version_id() { return "v4.0"; }
