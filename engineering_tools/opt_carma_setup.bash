...
GRP_ID=1000
USER_ID=1000
# Create the carma user if it doesn't exist
id -u carma &>/dev/null || sudo useradd --uid $USER_ID --gid $GRP_ID carma
# Create the carma group if it doesn't exist
sudo groupadd --gid $GRP_ID carma 2>/dev/null || true
USERNAME=$(whoami)
...
# Set ownership to carma user and group
...
sudo find /opt/carma -type d -exec chmod g+s {} \;
...
if [ "$ADD_SIM_FOLDER" == "true" ]; then
    ...
    # Create and set permissions for simulation directories
    sudo chown -R carma:carma /opt/carma-simulation/
    sudo chmod 775 -R /opt/carma-simulation/
    sudo find /opt/carma-simulation -type d -exec chmod g+s {} \;
    
    sudo chown -R carma:carma /opt/carma-streets/
    sudo chmod 775 -R /opt/carma-streets/
    sudo find /opt/carma-streets -type d -exec chmod g+s {} \;
fi
