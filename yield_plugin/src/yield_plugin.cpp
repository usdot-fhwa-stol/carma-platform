  ...
  carma_planning_msgs::srv::PlanTrajectory::Response::SharedPtr resp)
{
    ...
    RCLCPP_ERROR(nh_->get_logger(),"yield_plugin was ended!");
  ...
  carma_planning_msgs::msg::TrajectoryPlan YieldPlugin::generate_JMT_trajectory(const carma_planning_msgs::msg::TrajectoryPlan& original_tp, double initial_pos, double goal_pos, double initial_velocity, double goal_velocity, double planning_time)
  {
    ...
    std::vector<double> values = quintic_coefficient_calculator::quintic_coefficient_calculator(initial_pos,
                                                                                                ...
                                                                                                initial_time,
                                                                                                planning_time);
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 1 and size for original_tp.trajectory_points: " << original_tp.trajectory_points.size());
    ...
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 2 and size: " << original_tp.trajectory_points.size());
    ...
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 3");
    ...
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 4 and filtered_speeds size: " << filtered_speeds.size());
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 4 and original_tp.trajectory_points.size(): " << original_tp.trajectory_points.size());
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 4 and original_traj_downtracks: " << original_traj_downtracks.size());
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 4 and original_traj_downtracks: " << original_traj_downtracks.size());
    ...
    for(size_t i = 1; i < original_tp.trajectory_points.size(); i++ )
    {
      ...
      RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 4.a and index: " << i);

      double current_speed = filtered_speeds.at(i);
      double traj_target_time = i * planning_time / original_tp.trajectory_points.size();
      if (current_speed >= config_.max_stop_speed)
      {
        RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 4.b and jmt_trajectory_points.size(): " << jmt_trajectory_points.size());
      ...
      {
        RCLCPP_ERROR(nh_->get_logger(),"target speed is zero");
    ...
    RCLCPP_ERROR_STREAM(nh_->get_logger(), "Yield 5 and jmt_trajectory_points size: " << jmt_trajectory_points.size());
  ...
  std::optional<rclcpp::Time> YieldPlugin::detect_collision_time(const carma_planning_msgs::msg::TrajectoryPlan& trajectory1, const std::vector<carma_perception_msgs::msg::PredictedState>& trajectory2, double collision_radius)
  {
    ...
    bool on_route = false;
    int on_route_idx = 0;
    for (const auto& route_id : route_llt_ids_)
    {
        RCLCPP_ERROR_STREAM(nh_->get_logger(), "Route llts: " << route_id);
    }

    for (size_t j = 0; j < trajectory2.size(); j+=4)
    {
      ...
      auto corresponding_lanelets = wm_->getLaneletsFromPoint(curr_point, 10); // get 4 lanelets
  ...
  carma_planning_msgs::msg::TrajectoryPlan YieldPlugin::update_traj_for_object(const carma_planning_msgs::msg::TrajectoryPlan& original_tp, const std::vector<carma_perception_msgs::msg::ExternalObject>& external_objects, double initial_velocity)
  {
    ...
    {
      RCLCPP_ERROR(nh_->get_logger(), "Yield plugin received less than 2 points in update_traj_for_object, returning unchanged...");
      return original_tp;
    }
    ...
    for (const auto& curr_obstacle : external_objects)
    {
      ...
      if (curr_obstacle.object_type != 4) //TODO
      {
        RCLCPP_ERROR_STREAM(nh_->get_logger(), "Skipping not pedestrian");
        continue;
      }

      if (curr_obstacle.velocity.twist.linear.x < 1.0) //TODO
      {
        RCLCPP_ERROR_STREAM(nh_->get_logger(), "pedestrian but not moving!");
        continue;
      }
      RCLCPP_ERROR_STREAM(nh_->get_logger(), "===================== Checking Collision! ====================");
    ...
    {
      RCLCPP_ERROR_STREAM(nh_->get_logger(),"Collision Detected!");
      ...
      RCLCPP_ERROR_STREAM(nh_->get_logger(),"dist_to_object: " << dist_to_object);

      RCLCPP_ERROR_STREAM(nh_->get_logger(),"object speed: " << earliest_collision_obj.value().velocity.twist.linear.x);
      ...
      if (goal_velocity <= config_.min_obstacle_speed){
        RCLCPP_ERROR(nh_->get_logger(),"The obstacle is not moving, goal velocity is set to 0");
        goal_velocity = 0.0;
      }
    ...
    RCLCPP_ERROR(nh_->get_logger(),"No collision detection, so trajectory not modified.");
    return original_tp;
  }
  ...
  std::vector<double> YieldPlugin::get_relative_downtracks(const carma_planning_msgs::msg::TrajectoryPlan& trajectory_plan) const
  {
    ...
    RCLCPP_ERROR_STREAM(nh_->get_logger(),"Sub yield 4 and size: " << downtracks.size());
