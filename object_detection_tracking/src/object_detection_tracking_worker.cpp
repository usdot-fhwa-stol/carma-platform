...
ObjectDetectionTrackingWorker::ObjectDetectionTrackingWorker(
  PublishObjectCallback obj_pub, TransformLookupCallback tf_lookup,
  rclcpp::node_interfaces::NodeLoggingInterface::SharedPtr logger)
: obj_pub_(obj_pub), tf_lookup_(tf_lookup), logger_(logger)
{
}

bool ObjectDetectionTrackingWorker::isClass(
  const autoware_auto_msgs::msg::TrackedObject & obj, uint8_t class_id)
{
  return obj.classification.end() !=
         std::find_if(obj.classification.begin(), obj.classification.end(), [&class_id](auto o) {
           return o.classification == class_id;
         });
}

void ObjectDetectionTrackingWorker::detectedObjectCallback(
  autoware_auto_msgs::msg::TrackedObjects::UniquePtr obj_array)
{
  ...
  if (!transform) {
    RCLCPP_WARN_STREAM(
      logger_->get_logger(), "Ignoring fix message: Could not locate static transforms exception.");
    return;
  }
  ...
  for (size_t i = 0; i < obj_array->objects.size(); i++) {
    ...
    // Object id. Matching ids on a topic should refer to the same object within some time period,
    // expanded
    ...
    obj.pose.covariance =
      covariance_helper::transformCovariance(input_covariance, covariance_transform);
    ...
    // The size of the object aligned along the axis of the object described by the orientation in
    // pose Dimensions are specified in meters Autoware provides a set of 2d bounding boxes and
    // supports articulated shapes. The loop below finds the overall bounding box to ensure
    ...
    if (isClass(obj_array->objects[i], autoware_auto_msgs::msg::ObjectClassification::MOTORCYCLE)) {
      obj.object_type = obj.MOTORCYCLE;
                 ...
                 obj_array->objects[i], autoware_auto_msgs::msg::ObjectClassification::BICYCLE)) {
      ...
      obj.object_type = obj.SMALL_VEHICLE;
                 ...
                 obj_array->objects[i], autoware_auto_msgs::msg::ObjectClassification::TRUCK)) {
      obj.object_type = obj.LARGE_VEHICLE;
    } else if (isClass(
                 obj_array->objects[i], autoware_auto_msgs::msg::ObjectClassification::TRAILER)) {
    ...
    if (
      (fabs(
        obj.velocity.twist.linear.x || obj.velocity.twist.linear.y ||
        obj.velocity.twist.linear.z)) > 0.75) {
      obj.dynamic_obj = 1;
    } else {
      obj.dynamic_obj = 0;
    }
...
void ObjectDetectionTrackingWorker::setMapFrame(std::string map_frame) { map_frame_ = map_frame; }
