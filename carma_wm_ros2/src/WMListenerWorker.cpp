...
void WMListenerWorker::mapCallback(const autoware_lanelet2_msgs::msg::MapBin::UniquePtr map_msg)
{
  ...
  while(!map_update_queue_.empty() && more_updates_to_apply) {
    ...
    if (update->map_version < current_map_version_) { // Drop any so far unapplied updates for the previous map
      RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "There were unapplied updates in carma_wm when a new map was received.");
      continue;
    }
    if (update->map_version == current_map_version_) { // Current update goes with current map
      ...
      RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Done applying updates for new map. However, more updates are waiting for a future map.");
  ...
  if (delayed_route_msg_) {
    if (delayed_route_msg_.get().map_version == current_map_version_) { // If there is a delayed route message to apply then do so
      ...
      RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Dropping delayed route message which was never applied as updated map was not recieved");
      delayed_route_msg_ = boost::none;
    } else {
      RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "There is a delayed route message still waiting to be applied in carma_wm");
    }
  }
}
...
void logSignalizedIntersectionManager(const carma_wm::SignalizedIntersectionManager& sim)
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "inter id: " << (int)pair.first << ", regem id: " << pair.second);
  }
  for (auto pair : sim.signal_group_to_entry_lanelet_ids_)
  {
    for (auto iter = pair.second.begin(); iter != pair.second.end(); iter++)
    {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "signal id: " << (int)pair.first << ", entry llt id: " << *iter);
    }
  }
  for (auto pair : sim.signal_group_to_exit_lanelet_ids_)
  {
    for (auto iter = pair.second.begin(); iter != pair.second.end(); iter++)
    {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "signal id: " << (int)pair.first << ", exit llt id: " << *iter);
    }
  }
  for (auto pair : sim.signal_group_to_traffic_light_id_)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "signal id: " << (int)pair.first << ", regem id: " << pair.second);
  }
}

void WMListenerWorker::mapUpdateCallback(autoware_lanelet2_msgs::msg::MapBin::UniquePtr geofence_msg)
{
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Map Update Being Evaluated. SeqNum: " << geofence_msg->seq_id);
  if (rerouting_flag_) // no update should be applied if rerouting 
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Currently new route is being processed. Queueing this update. Received seq: " << geofence_msg->seq_id << " prev seq: " << most_recent_update_msg_seq_);
    map_update_queue_.emplace(std::move(geofence_msg));
    return;
  }
  if (geofence_msg->seq_id <= most_recent_update_msg_seq_) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Dropping map update which has already been processed. Received seq: " << geofence_msg->seq_id << " prev seq: " << most_recent_update_msg_seq_);
    return;
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Update received for newer map version than available. Queueing update until map is available.");
    map_update_queue_.emplace(std::move(geofence_msg));
    return;
  } else if (current_map_version_ > geofence_msg->map_version) { // If this update is for an older map
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Dropping old map update as newer map is already available.");
    return;
  } else if (most_recent_update_msg_seq_ + 1 < geofence_msg->seq_id) {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Queuing map update as we are waiting on an earlier update to be applied. most_recent_update_msg_seq_: " << most_recent_update_msg_seq_ << "geofence_msg->seq_id: " << geofence_msg->seq_id);
    map_update_queue_.emplace(std::move(geofence_msg));
    return;
  }
  ...
  {  
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Received notice that route has been invalidated in mapUpdateCallback");

    if(route_node_flag_!=true)
    {
     RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Route is not yet available. Therefore queueing the update");
     map_update_queue_.emplace(std::move(geofence_msg));
     return;
    }
  }
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Processing Map Update with Geofence Id:" << gf_ptr->id_);
  
  RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Geofence id" << gf_ptr->id_ << " requests addition of lanelets size: " << gf_ptr->lanelet_additions_.size());
  for (auto llt : gf_ptr->lanelet_additions_)
  {
    // world model here should blindly accept the map update received
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Adding new lanelet with id: " << llt.id());
  ...
  RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Geofence id" << gf_ptr->id_ << " sends record of traffic_lights_id size: " << gf_ptr->traffic_light_id_lookup_.size());
  for (auto pair : gf_ptr->traffic_light_id_lookup_)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Adding new pair for traffic light ids: " << pair.first << ", and lanelet::Id: " << pair.second);
    world_model_->setTrafficLightIds(pair.first, pair.second);
  }

  RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Geofence id" << gf_ptr->id_ << " sends record of intersections size: " << gf_ptr->sim_.intersection_id_to_regem_id_.size());
  ...
  RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Geofence id" << gf_ptr->id_ << " requests removal of size: " << gf_ptr->remove_list_.size());
  for (auto pair : gf_ptr->remove_list_)
  {
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Regems found in lanelet: " << regems_copy_to_check.size());
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Regems left in lanelet after removal: " << parent_llt.regulatoryElements().size());

  }
  
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Geofence id" << gf_ptr->id_ << " requests update of size: " << gf_ptr->update_list_.size());
  ...
  for (auto pair : gf_ptr->update_list_)
  {
    ...
    if (regemptr_it != world_model_->getMutableMap()->regulatoryElementLayer.end())  
    {
      ...
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Reapplying previously existing element for lanelet id:" << parent_llt.id() << ", and regem id: " << regemptr_it->get()->id());
    ...
    {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "New regulatory element at lanelet: " << parent_llt.id() << ", and id: " << pair.second->id());
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Finished Applying the Map Update with Geofence Id:" << gf_ptr->id_); 

  // Call user defined map callback
  if (map_callback_)
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Calling user defined map update callback, but not calling!");
    map_callback_();
  }

  RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "At the end of geofence processing:" << gf_ptr->id_); 

  if (gf_ptr->sim_.intersection_id_to_regem_id_.size() > 0)
  {
    for (auto pair : world_model_->sim_.signal_group_to_traffic_light_id_)
    {
      auto regem_id = pair.second;

      auto general_regem = world_model_->getMutableMap()->regulatoryElementLayer.get(regem_id);

      auto lanelets_general = world_model_->getMutableMap()->laneletLayer.findUsages(general_regem);
      if (lanelets_general.empty())
      {
        RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "There was an error querying lanelet for traffic light with id: " << regem_id);
      }
      else
      {
        RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Found size lanelets: " << lanelets_general.size() << ", for id: " << regem_id << 
          ", lanelet id: " << lanelets_general[0].id());
      }
...
void WMListenerWorker::newRegemUpdateHelper(lanelet::Lanelet parent_llt, lanelet::RegulatoryElement* regem) const
{
  ...
  switch(resolveGeofenceType(regem->attribute(lanelet::AttributeName::Subtype).value()))
  {
    ...
    default:
      RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "World Model instance received an unsupported geofence type in its map update callback!");
      break;
  }
}
...
LaneletRoutingGraphPtr WMListenerWorker::routingGraphFromMsg(const autoware_lanelet2_msgs::msg::RoutingGraph& msg, lanelet::LaneletMapPtr map) const {
  ...
  if (msg.participant_type.compare(getVehicleParticipationType()) != 0) {
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"),"Received routing graph does not have matching vehicle type for world model. WM Type: " 
  ...
  try {
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Received routing graph specifies lanelets which do not match the current map version. Actual exception: " << e.what());
  ...
  for (size_t i = 0; i < msg.lanelet_vertices.size(); ++i) {
    ...
    for (size_t j = 0; j < vertex.lanelet_or_area_ids.size(); ++j) {
      ...
      try {
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"),"Received routing graph specifies lanelets which do not match the current map version. Not found lanelet or area: " 
...
void WMListenerWorker::routeCallback(const carma_planning_msgs::msg::Route::UniquePtr route_msg)
{
  if (route_msg->map_version < current_map_version_) {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Route message rejected as it is for an older map");
    rerouting_flag_ = false; // Clear any blockers on map updates as the route we were waiting for is no longer valid
    return;
  }

  if (route_msg->map_version > current_map_version_) {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Route message received for future map. Delaying application until map is recieved");
...

  {
    ...
    while(!map_update_queue_.empty() && more_updates_to_apply) {
      ...
      if (update->map_version < current_map_version_) { // Drop any so far unapplied updates for the current map
        RCLCPP_WARN_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Apply from reroute: There were unapplied updates in carma_wm when a new map was recieved.");
        continue;
      }
      if (update->map_version == current_map_version_) { // Current update goes with current map which is also the map used by this route
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Applying queued update after route was recieved. ");

        if (update->invalidates_route == true) {
          RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Applied queued map update has invalidated the route.");
        ...
        RCLCPP_INFO_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Apply from reroute: Done applying updates for new map. However, more updates are waiting for a future map.");
        more_updates_to_apply = false; // If there is more updates queued that are not for this map version assume they are for a future map version
      }
  ...
  if (!world_model_->getMap()) { // This check is a bit redundant but still useful from a debugging perspective as the alternative is a segfault
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "WMListener received a route before a map was available. Dropping route message.");
    return;
  }
  ...
  if (route_invalidated_by_queued_map_update && route_node_flag_){
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"),"At least one applied queued map update has invalidated the route. Routing graph will be recomputed.");
    world_model_->setMap(world_model_->getMutableMap(), current_map_version_, route_invalidated_by_queued_map_update);
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"),"Finished recomputing the routing graph for the applied queued map update(s)");
    ...
    if(route_opt.is_initialized()) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("carma_wm_ros2"), "Setting route in world model");
