...
namespace mock_drivers{
    ...
    TEST(MockCommsDriver, Constructor){
        MockCommsDriver d1;
        MockCommsDriver d2(false);
        MockCommsDriver d3(true);
    ...
    TEST(MockCommsDriver, pubCallbacks){
        MockCommsDriver d(true);
        ...
        msg.inbound_binary_msg_flag = true;
        ...
        ASSERT_EQ(test_str_vector[0], "comms/inbound_binary_msg");
        
        ASSERT_EQ(test_time_vector.size(), 1);
        ASSERT_EQ(test_time_vector[0], ros::Time::now());
    }

    TEST(MockCommsDriver, driver_discovery){
        MockCommsDriver d(true);

        ASSERT_TRUE(d.driverDiscovery());
    }

    TEST(MockCommsDriver, run){
        MockCommsDriver d(true);
        ASSERT_EQ(d.run(), 0);
    }
}