...
NOTE: A known issue with this release is packets being sent between Raspberry Pi's are occasionally dropped, causing the automated Port Drayage demonstration to hang while the vehicle waits for a new instruction that it will never receive. This issue will be addressed in the following release.
