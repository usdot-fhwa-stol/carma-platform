...
{
  config_.sensor_nodes =
    declare_parameter<std::vector<std::string>>("sensor_nodes", config_.sensor_nodes);
  config_.sensor_fault_map =
    sensor_fault_map_from_json(declare_parameter<std::string>("sensor_fault_map", ""));
...
carma_ros2_utils::CallbackReturn LocalizationControllerNode::handle_on_configure(
...
void LocalizationControllerNode::on_system_alert(carma_msgs::msg::SystemAlert::UniquePtr msg)
{
  carma_msgs::msg::SystemAlert alert =
    *msg;  // Keep local copy so parent method can be called with std::move
  ...
  if (sensor_status_.find(alert.source_node) == sensor_status_.end()) {
    RCLCPP_INFO_STREAM(
      get_logger(), "Alert not from a sensor node. Alerting node: " << alert.source_node);
    return;
  }
  ...
  switch (alert_status) {
    ...
    case SensorAlertStatus::WARNING:
      ...
      break;
    default:
      return;  // OPERATIONAL case no-op as other subsystems do not need notification of operational
               // status
  }
...
std::unordered_map<std::vector<SensorBooleanStatus>, SensorAlertStatus, VectorHash>
LocalizationControllerNode::sensor_fault_map_from_json(std::string json_string)
{
  ...
  for (rapidjson::SizeType i = 0; i < map_value.Size(); i++) {
    ...
    for (rapidjson::SizeType j = 0; j < map_value[i].Size() - 1; j++) {
      if (!map_value[i][j].IsInt()) {
        RCLCPP_WARN_STREAM(
          get_logger(), "sensor_fault_map row " << i << " element " << j << " is not an int");
        continue;
      }
