...
/**
 * \brief Struct representing trajectory smoothing algorithm parameters using distance and
 * acceleration Based on TSMO USE CASE 2. Chapter 2. Trajectory Smoothing
 */
enum TSCase
{
  CASE_1 = 1,
...
private:
  ...
  /**
   * \brief Method to check if the state is an allowed green movement state. Currently Permissive
   * and Protected green are supported \param state The traffic signal state to check for \return
   ...
   */
  boost::posix_time::time_duration getMovementAllowedDuration(
    lanelet::CarmaTrafficSignalPtr traffic_light);

  /**
   * \brief Method for performing maneuver planning when the current plugin state is
   * TransitState::UNAVAILABLE Therefor no maneuvers are planned but the system checks for the
   * precense of traffic lights ahead of it
   *
   * \param req Plan maneuver request
   ...
   * \param traffic_light The single traffic light along the vehicle's route in the intersection
   * that is relevant and in front of the vehicle \param entry_lanelet The entry lanelet into the
   * intersection \param exit_lanelet The exit lanelet into the intersection \param current_lanelet
   * The current lanelet the vehicle's state is on \throws if given entry_lanelet doesn't have
   * stop_line NOTE: returns empty if the given traffic light is empty
   */
  void planWhenUNAVAILABLE(
    carma_planning_msgs::srv::PlanManeuvers::Request::SharedPtr req,
    carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp,
  ...
  /**
   * \brief Method for performing maneuver planning when the current plugin state is
   * TransitState::APPROACHING Therefore the planned maneuvers deal with approaching a traffic
   * light.
   *
   *
   * \param req Plan maneuver request
   ...
   * intersection \param exit_lanelet The exit lanelet into the intersection \param current_lanelet
   * The current lanelet the vehicle's state is on \throws if given entry_lanelet doesn't have
   * stop_line or speed profile case number was failed to be calculated NOTE: if there is empty
   * traffic_light, it signals that the vehicle has crossed the bar, or if invalid light state is
   * given
   */
  void planWhenAPPROACHING(
    carma_planning_msgs::srv::PlanManeuvers::Request::SharedPtr req,
    carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp,
    const VehicleState & current_state, const lanelet::CarmaTrafficSignalPtr & traffic_light,
    const lanelet::ConstLanelet & entry_lanelet, const lanelet::ConstLanelet & exit_lanelet,
  ...
  /**
   * \brief Method for performing maneuver planning when the current plugin state is
   * TransitState::WAITING Therefor the planned maneuvers deal with waiting at a red light
   *
   * \param req Plan maneuver request
   * \param[out] resp Plan maneuver response with a list of maneuver plan
   * \param current_state The current state of the vehicle at the start of planning
   ...
   * that is relevant and in front of the vehicle \param entry_lanelet The entry lanelet into the
   * intersection \param exit_lanelet The exit lanelet into the intersection \param current_lanelet
   * The current lanelet the vehicle's state is on NOTE: returns empty if the given traffic light is
   * empty
   */
  void planWhenWAITING(
    carma_planning_msgs::srv::PlanManeuvers::Request::SharedPtr req,
    carma_planning_msgs::srv::PlanManeuvers::Response::SharedPtr resp,
    const VehicleState & current_state, const lanelet::CarmaTrafficSignalPtr & traffic_light,
    const lanelet::ConstLanelet & entry_lanelet, const lanelet::ConstLanelet & exit_lanelet,
  ...
  /**
   * \brief Method for performing maneuver planning when the current plugin state is
   * TransitState::DEPARTING Therefor the planned maneuvers deal with the transit of the
   * intersection once the stop bar has been crossed
   *
   * \param req Plan maneuver request
   * \param[out] resp Plan maneuver response with a list of maneuver plan
   * \param current_state The current state of the vehicle at the start of planning
  ...
  boost::optional<bool> canArriveAtGreenWithCertainty(
    ...
    const lanelet::CarmaTrafficSignalPtr & traffic_light, bool check_late, bool check_early) const;
   ...
   *
   * \return A transift maneuver message specifically designed for light controlled intersection
   * tactical plugin
   */
  carma_planning_msgs::msg::Maneuver composeTrajectorySmoothingManeuverMessage(
   ...
   */
  carma_planning_msgs::msg::Maneuver composeStopAndWaitManeuverMessage(
   ...
   */
  carma_planning_msgs::msg::Maneuver composeIntersectionTransitMessage(
  ...
  /**
   * \brief This function returns stopping maneuver if the vehicle is able to stop at red and in
   * safe stopping distance. Given the initial speed, ending speed (which is 0), and
   * remaining_downtrack, this function checks if calculated remaining_time falls on RED \param req
   * Plan maneuver request \param[out] resp Plan maneuver response with a list of maneuver plan
  ...
  void handleStopping(
    ...
    const VehicleState & current_state, const lanelet::CarmaTrafficSignalPtr & traffic_light,
    const lanelet::ConstLanelet & entry_lanelet, const lanelet::ConstLanelet & exit_lanelet,
    const lanelet::ConstLanelet & current_lanelet, double traffic_light_down_track,
    bool is_emergency = false);

  /**
   * \brief This function returns valid maneuvers if the vehicle is able to utilize trajectory
   * smoothing parameters to go through the intersection with certainty It utilizes
   * canMakeWithCertainty function to determine if it is able to make it. Corresponds to TSCase 1-7
   * \param req Plan maneuver request
   * \param[out] resp Plan maneuver response with a list of maneuver plan
   * \param current_state Current state of the vehicle
   ...
   *
   * \return void. if successful, resp is non-empty. Resp is empty if not able to make it with
   * certainty or ts_params.is_successful is false
   */
  ...
  /**
   * \brief When the vehicle is not able to successfully run the algorithm or not able to stop, this
   * function modifies departure speed as close as possible to the original using max comfortable
   * accelerations with given distance.
   *
   * \param req Plan maneuver request
   * \param[out] resp Plan maneuver response with a list of maneuver plan
   * \param current_state Current state of the vehicle
  ...
  /**
   * \brief Helper function to handleFailureCase that modifies ts_params to desired new parameters.
   * See handleFailureCase \param starting_speed starting speed \param departure_speed
   * departure_speed originally planned \param speed_limit speed_limit \param remaining_downtrack
   * remaining_downtrack until the intersection \param traffic_light_downtrack
   * traffic_light_downtrack when vehicle is scheduled to enter
   *
   * \return TSP with parameters that is best available to pass the intersection. Either cruising
   * with starting_speed or sacrifice departure speed to meet time and distance
   */
  TrajectoryParams handleFailureCaseHelper(
    const lanelet::CarmaTrafficSignalPtr & traffic_light, double current_time,
    double starting_speed, double departure_speed, double speed_limit, double remaining_downtrack,
  ...
  /**
   * \brief Helper method that checks both if the input optional light state is set and if the state
   * it contains is supported via supportedLightState.
   *
   * \param optional_state An optional light state and its min_end_time pair. If this is unset the
   * method will return false \param source_time The time used to optain the optional light state.
   * This is used for logging only
   *
   * \return True if the optional is set and the contained state signal is supported. False
   * otherwise
   */
  bool validLightState(
    const boost::optional<std::pair<boost::posix_time::ptime, lanelet::CarmaTrafficSignalState>> &
      optional_state,
    const rclcpp::Time & source_time) const;

  /**
   * \brief Helper method to extract the initial vehicle state from the planning request method
   * based on if the prior_plan was set or not.
   *
   * \param req The maneuver planning request to extract the vehicle state from
   *
   * \return The extracted VehicleState
   */
  VehicleState extractInitialState(
    carma_planning_msgs::srv::PlanManeuvers::Request::SharedPtr req) const;

  /**
   * \brief Helper method which calls carma_wm::WorldModel::getLaneletsBetween(start_downtrack,
   * end_downtrack, shortest_path_only, bounds_inclusive) and throws and exception if the returned
   * list of lanelets is empty. See the referenced method for additional details on parameters.
   */
  std::vector<lanelet::ConstLanelet> getLaneletsBetweenWithException(
    double start_downtrack, double end_downtrack, bool shortest_path_only = false,
    bool bounds_inclusive = true) const;

  /**
   * \brief Get the final entry time from either vehicle's own internal ET calculation (TSMO UC2) or
   * from carma-street (TSMO UC3) This function also applies necessary green_buffer to adjust the
   * ET. \param current_state Current state of the vehicle \param earliest_entry_time Earliest entry
   * time calculated by the vehicle \param traffic_light traffic signal the vehicle is using \return
   * tuple of <final entry time the vehicle uses to enter the intersection,
   *         is_entry_time_within_green_or_tbd: true if ET is in green or TBD (related to UC3,
   * always set to true in UC2), in_tbd: true if ET is in TBD (false if in UC2)>
   */
  std::tuple<rclcpp::Time, bool, bool> get_final_entry_time_and_conditions(
    const VehicleState & current_state, const rclcpp::Time & earliest_entry_time,
    lanelet::CarmaTrafficSignalPtr traffic_light);

  /**
   * \brief Provides the scheduled entry time for the vehicle in the future. This scheduled time is
   * the earliest possible entry time that is during green phase and after timestamps both that is
   * kinematically possible and required times for vehicle in front to pass through first Returns
   * std::nullopt if no valid green entry time found
   *
   ...
   * \param minimum_required_green_time Minimum seconds required by other vehicles in front to pass
   * through the intersection first
   *
   * \return Nearest ROS entry time during green phase if it exists, std::nullopt if no valid green
   * entry time found NOTE: TSMO UC2: Algorithm 2. Entering time estimation algorithm for EVs in
   * cooperation Class A when the SPaT plan is fixed-time (defined for C-ADS-equipped vehicles)
   */

  std::optional<rclcpp::Time> get_nearest_green_entry_time(
    const rclcpp::Time & current_time, const rclcpp::Time & earliest_entry_time,
    const lanelet::CarmaTrafficSignalPtr & signal, double minimum_required_green_time = 0.0) const;

  /**
   * \brief Returns the later of earliest_entry_time or the end of the available signal states in
   * the traffic_signal
   *
   * \param earliest_entry_time Earliest ROS entry time into the intersection kinematically possible
   * \param signal CARMATrafficSignal that is responsible for the intersection along route
   *
   * \return Returns the later of earliest_entry_time or the end of the available signal states in
   * the traffic_signal
   */

  rclcpp::Time get_eet_or_tbd(
    const rclcpp::Time & earliest_entry_time, const lanelet::CarmaTrafficSignalPtr & signal) const;

  /**
   * \brief Gets the earliest entry time into the intersection that is kinematically possible for
   * the vehicle. Designed to get it for motion that has maximum one acceleration, deceleration, or
   * cruise segments until destination.
   *
   * \param remaining_distance Distance to the intersection in meters
   * \param free_flow_speed Free flow speed along the route until the intersection (usually
   * speed_limit) in m/s \param current_speed Current speed in m/s \param departure_speed Speed into
   * the intersection in m/s. A.k.a target speed at the intersection \param max_accel The
   * acceleration in m/s^2 of the acceleration segment \param max_decel The deceleration in m/s^2 of
   * the deceleration segment
   *
   * \return The estimated stopping distance in meters
   * NOTE: TSMO UC2: Algorithm 1. Earliest entering time estimation algorithm for EVs in all
   * cooperation classes (defined for C-ADS-equipped vehicles)
   */

  rclcpp::Duration get_earliest_entry_time(
    double remaining_distance, double free_flow_speed, double current_speed, double departure_speed,
    double max_accel, double max_decel) const;

  /**
   * \brief Gets maximum distance (nearest downtrack) the trajectory smoothing algorithm makes
   * difference than simple lane following within half of one full fixed cycle time boundary
   * compared to free flow arrival time.
   *
   ...
   * \param departure_speed Speed into the intersection in m/s. A.k.a target speed at the
   * intersection \param max_accel The acceleration in m/s^2 of the acceleration segment \param
   * max_decel The deceleration in m/s^2 of the deceleration segment
   *
   * \return the max distance the plugin should activate to support all speed profile cases of the
   * trajectory smoothing algorithm -1 if it is within the distance already
   */
  double get_trajectory_smoothing_activation_distance(
    double time_remaining_at_free_flow, double full_cycle_duration, double current_speed,
    double speed_limit, double departure_speed, double max_accel, double max_decel) const;

  /**
   * \brief Get required distance to accel then decel, or vise versa - each at least once - to reach
   * departure_speed with given speed and acceleration parameters
   *
  ...
  double get_distance_to_accel_or_decel_twice(
    double free_flow_speed, double current_speed, double departure_speed, double max_accel,
   ...
   * \param x1  Required distance to accel then decel, or vise versa-each at least once-to reach
   * departure_speed with given speed and acceleration parameters \param x2  required distance to
   * accel or decel to reach departure_speed with given speed and acceleration parameters \param
   * free_flow_speed Free flow speed in m/s (usually the speed limit) \param current_speed Current
   * speed in m/s \param departure_speed Speed to enter into the intersection in m/s \param
   * max_accel Acceleration rate during acceleration segment in m/s^2 \param max_decel Deceleration
   * rate during acceleration segment in m/s^2 NOTE: note that this returns departure speed if x <
   * x2 since vehicle's trajectory corresponding to its EET would contain only one acceleration or
   * deceleration piece \return The estimated stopping distance in meters
   */

  double get_inflection_speed_value(
    double x, double x1, double x2, double free_flow_speed, double current_speed,
    double departure_speed, double max_accel, double max_decel) const;

  /**
   * \brief Get required distance to accel or decel to reach departure_speed with given speed and
  ...
  double get_distance_to_accel_or_decel_once(
    double current_speed, double departure_speed, double max_accel, double max_decel) const;

  /**
   * \brief Helper method to use basic kinematics to compute an estimated stopping distance from
   * from the inputs
   *
  ...
  /**
   * \brief Helper method to use basic kinematics to compute an estimated stopping time from from
   * the inputs
   *
   ...
   */
  double calc_estimated_entry_time_left(
    double entry_dist, double current_speed, double departure_speed) const;

  /**
   * \brief Get boundary distances used to compare against current state in order to create
  ...
  BoundaryDistances get_delta_x(
    double v0, double v1, double v_max, double v_min, double a_max, double a_min);

  /**
   * \brief Get all possible trajectory smoothing parameters for each segments. Later this will be
   * used to generate a single trajectory \param t current time in seconds \param v0 starting_speed
   * \param v1 departure_speed
   * \param v_max speed_limit
   * \param v_min minimum speed
   * \param a_max max_comfort_acceleration limit
   ...
   */
  std::vector<TrajectoryParams> get_boundary_traj_params(
    double t, double v0, double v1, double v_max, double v_min, double a_max, double a_min,
    double x0, double x_end, double dx, BoundaryDistances boundary_distances);
   ...
   */
  TrajectoryParams ts_case1(
    double t, double et, double v0, double v1, double v_max, double a_max, double a_min, double x0,
    double x_end, double dx);
  TrajectoryParams ts_case2(
    double t, double et, double v0, double v1, double a_max, double a_min, double x0, double x_end,
    double dx);
  TrajectoryParams ts_case3(
    double t, double et, double v0, double v1, double a_max, double a_min, double x0, double x_end,
    double dx);
  TrajectoryParams ts_case4(
    double t, double et, double v0, double v1, double v_min, double a_max, double a_min, double x0,
    double x_end, double dx);
  TrajectoryParams ts_case5(
    double t, double et, double v0, double a_max, double a_min, double x0, double x_end, double dx);
  TrajectoryParams ts_case6(
    double t, double et, double v0, double v_min, double a_min, double x0, double x_end, double dx,
    double dx3, TrajectoryParams traj6);
  TrajectoryParams ts_case7(
    double t, double et, double v0, double v_min, double a_min, double x0, double x_end, double dx);
  ...
  TrajectoryParams boundary_accel_or_decel_incomplete_upper(
    double t, double v0, double v1, double a_max, double a_min, double x0, double x_end, double dx);
  TrajectoryParams boundary_accel_nocruise_notmaxspeed_decel(
    double t, double v0, double v1, double a_max, double a_min, double x0, double x_end, double dx);
  TrajectoryParams boundary_accel_cruise_maxspeed_decel(
    double t, double v0, double v1, double v_max, double a_max, double a_min, double x0,
    double x_end, double dx);
  TrajectoryParams boundary_accel_nocruise_maxspeed_decel(
    double t, double v0, double v1, double v_max, double a_max, double a_min, double x0,
    double x_end, double dx);
  TrajectoryParams boundary_accel_or_decel_complete_upper(
    double t, double v0, double v1, double x0, double x_end, double dx);
  TrajectoryParams boundary_decel_nocruise_notminspeed_accel(
    double t, double v0, double v1, double v_min, double a_max, double a_min, double x0,
    double x_end, double dx);
  TrajectoryParams boundary_decel_nocruise_minspeed_accel_incomplete(
    double t, double v0, double v_min, double a_max, double a_min, double x0, double x_end,
    double dx);
  TrajectoryParams boundary_decel_nocruise_minspeed_accel_complete(
    double t, double v0, double v1, double v_max, double v_min, double a_max, double a_min,
    double x0, double x_end, double dx);
  TrajectoryParams boundary_decel_cruise_minspeed_accel(
    double t, double v0, double v1, double v_min, double a_max, double a_min, double x0,
    double x_end, double dx);
  TrajectoryParams boundary_decel_cruise_minspeed(
    double t, double v0, double v_min, double a_min, double x0, double x_end, double dx);
  TrajectoryParams boundary_decel_incomplete_lower(
    double t, double v0, double a_min, double x0, double x_end, double dx);
  TrajectoryParams boundary_decel_cruise_minspeed_decel(
    double t, double v0, double v_min, double a_min, double x0, double x_end, double dx);

  TrajectoryParams get_ts_case(
    double t, double et, double v0, double v1, double v_max, double v_min, double a_max,
    double a_min, double x0, double x_end, double dx, BoundaryDistances boundary_distances,
