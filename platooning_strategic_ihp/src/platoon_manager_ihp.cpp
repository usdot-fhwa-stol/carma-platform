 ...
 *  A vector of platoon members (vehicles), sorted by downtrack distance in descending order (i.e.,
 * [dtd_1 > dtd_2 > ... > dtd_n] ).
 *
 * 2. speed vector indexing:
 *  A vector that only contains speed (m/s) of each platoon member. Same order with platoon list
 * (i.e., [platoon_front, follwer_1, ..., follower_n]).
 *
 * 3. downtrackDistance vector indexing:
 *  A vector that only contains downtrack distance (m) of each platoon member. Same order with
 * platoon list (i.e., [platoon_front, follwer_1, ..., follower_n])
 *
 * 4. timeHeadway vector indexing
 *  A vector that only contains time headaway (s) behind each platoon member (i.e., the time gap
 * between host vehicle and its following vehicle). Same order with platoon list. For APF, when gap
 * too small, the dynamic leader will be the front vehicle of the small gap. If gap too large, the
 * dynamic leader wil be the rear vehicle of the large gap.
 *
 */
...
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "Top of PlatoonManager ctor.");
}
...
void PlatoonManager::updateHostPose(const double downtrack, const double crosstrack)
{
  RCLCPP_DEBUG_STREAM(
    ...
    "Host (index " << hostPosInPlatoon_ << "): downtrack = " << downtrack
...
void PlatoonManager::hostMemberUpdates(
  const std::string & senderId, const std::string & platoonId, const std::string & params,
  const double & DtD, const double & CtD)
{
  ...
  if (isFollower) {
    ...
    if (platoonLeaderID.compare(host_platoon_[0].staticId) != 0) {
      RCLCPP_DEBUG_STREAM(
        ...
        "///// platoonLeaderID NOT PROPERLY ASSIGNED! Value = "
          << platoonLeaderID << ", host_platoon_[0].staticId = " << host_platoon_[0].staticId);
    }
    ...
    if (needPlatoonIdChange) {
      // TODO: better to have leader explicitly inform us that it is merging into another platoon,
      // rather than require us to deduce it here.
      //       This condition may also result from missing some message traffic, new leader in this
      //       platoon, or other confusion/incorrect code. Consider using a new STATUS msg param
      //       that tells members of new platoon ID and new leader ID when a change is made.
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "It seems that the current leader is joining another platoon.");
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "So the platoon ID is changed from " << currentPlatoonID << " to " << platoonId);
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "The first vehicle in our list is now " << host_platoon_[0].staticId);
    } else  // sender is in a different platoon
    {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "This STATUS message is not from a vehicle we care about. Ignore this message with id: "
          << senderId);
    }
  } else  // host is leader
  {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Host is leader: currentPlatoonID = " << currentPlatoonID
    ...
    if (currentPlatoonID == platoonId) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "Platoon IDs not matched");
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "currentPlatoonID: " << currentPlatoonID);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "incoming platoonId: " << platoonId);
    }
  }

  // update host vehicle information each time new member is updated. Now platoon contains host
  // vehicle.
...
// Update/add one member's information from STATUS messages, update platoon ID if needed.
void PlatoonManager::neighborMemberUpdates(
  const std::string & senderId, const std::string & platoonId, const std::string & params,
  const double & DtD, const double & CtD)
{
  // parse params, read member data
  ...
  if (neighborPlatoonID == platoonId) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "The first vehicle in that platoon is now " << neighbor_platoon_[0].staticId);

    // If we have data on all members of a neighboring platoon, set a complete record flag
    if (
      neighbor_platoon_info_size_ > 1 && neighbor_platoon_.size() == neighbor_platoon_info_size_) {
      is_neighbor_record_complete_ = true;
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "Neighbor record is complete!");
    }
  } else  // sender is in a different platoon
  {
    RCLCPP_DEBUG_STREAM(
...
// Check a new vehicle's existence; add its info to the platoon if not in list, update info if
// already existed.
void PlatoonManager::updatesOrAddMemberInfo(
  std::vector<PlatoonMember> & platoon, std::string senderId, double cmdSpeed, double dtDistance,
  double ctDistance, double curSpeed)
{
  ...
  for (size_t i = 0; i < platoon.size(); ++i) {
    if (platoon[i].staticId == senderId) {
      if (
        abs(dtDistance - platoon[i].vehiclePosition) / (platoon[i].vehiclePosition + 0.01) >
        config_.significantDTDchange) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "DTD of member " << platoon[i].staticId
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "Receive and update platooning info on member " << i << ", ID:" << platoon[i].staticId);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "    CommandSpeed       = " << platoon[i].commandSpeed);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "    Actual Speed       = " << platoon[i].vehicleSpeed);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "    Downtrack Location = " << platoon[i].vehiclePosition);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "    Crosstrack dist    = " << platoon[i].vehicleCrossTrack);

      if (senderId == HostMobilityId) {
        hostPosInPlatoon_ = i;
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "    This is the HOST vehicle");
      }
      isExisted = true;
    }
  }
  ...
  if (sortNeeded) {
    ...
    std::sort(
      std::begin(platoon), std::end(platoon), [](const PlatoonMember & a, const PlatoonMember & b) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "    Platoon order is now:");
    for (size_t i = 0; i < platoon.size(); ++i) {
      std::string hostFlag = " ";
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "    " << platoon[i].staticId << "its DTD: " << platoon[i].vehiclePosition << " "
               << hostFlag);
    }
  }
  ...
  if (!isExisted) {
    ...
    PlatoonMember newMember =
      PlatoonMember(senderId, cmdSpeed, curSpeed, dtDistance, ctDistance, cur_t);
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "Add a new vehicle into our platoon list " << newMember.staticId
...
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "host platoon size: " << host_platoon_.size());
...
PlatoonMember PlatoonManager::getDynamicLeader()
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"), "host_platoon_ size: " << host_platoon_.size());
  if (isFollower) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
    ...
    if (algorithmType_ == "APF_ALGORITHM") {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "dynamic_leader_index_: " << dynamic_leader_index_);
      if (newLeaderIndex < host_platoon_.size()) {  // this must always be true!
        dynamicLeader = host_platoon_[newLeaderIndex];
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "APF output: " << dynamicLeader.staticId);
...
// The implementation of all predecessor following algorithm. Determine the dynamic leader for the
// host vehicle to follow.
int PlatoonManager::allPredecessorFollowing()
{
  ///***** Case Zero *****///
  // If the host vehicle is the second follower of a platoon, it will always follow the platoon
  // leader in the front
  ...
  ///***** Case One *****///
  // If there weren't a leader in the previous time step, follow the first vehicle (i.e., the
  // platoon leader) as default.
  ...
  // Update host vehicle info when update member info, so platoon list include host vehicle, direct
  // use platoon size for downtrack/speed vector. Record downtrack distance (m) of each member
  ...
  // The following line will not throw exception because the length of downtrack array is larger
  // than two in this case
  double distHeadwayWithPredecessor =
    downtrackDistance[hostPosInPlatoon_ - 1] - downtrackDistance[hostPosInPlatoon_];
  gapWithPred_ = distHeadwayWithPredecessor;
  if (insufficientGapWithPredecessor(distHeadwayWithPredecessor)) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "APF algorithm decides there is an issue with the gap with preceding vehicle: "
        << distHeadwayWithPredecessor << " m. Case Two");
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "APF calculate time headways: ");
    for (const auto & value : timeHeadways) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"), "APF time headways: " << value);
    }
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "APF found the previous dynamic leader is " << previousFunctionalDynamicLeaderID_);
    ...
    if (previousFunctionalDynamicLeaderIndex_ == 0) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger("platooning_strategic_ihp"),
        "APF use violations on lower boundary or maximum spacing to choose dynamic leader. Case "
        "Three.");
      return determineDynamicLeaderBasedOnViolation(timeHeadways);
    } else {
      // if the previous dynamic leader is not the first vehicle
      // get the time headway between every consecutive pair of vehicles from index Of Previous
      // dynamic Leader
      std::vector<double> partialTimeHeadways =
        getTimeHeadwayFromIndex(timeHeadways, previousFunctionalDynamicLeaderIndex_);
      for (const auto & value : partialTimeHeadways) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"), "APF partial time headways: " << value);
      }
      int closestLowerBoundaryViolation =
        findLowerBoundaryViolationClosestToTheHostVehicle(partialTimeHeadways);
      int closestMaximumSpacingViolation =
        findMaximumSpacingViolationClosestToTheHostVehicle(partialTimeHeadways);
      // if there are no violations anywhere between the subject vehicle and the current dynamic
      // leader, then depending on the time headways of the ENTIRE platoon, the subject vehicle may
      // switch dynamic leader further downstream. This is because the subject vehicle has
      // determined that there are no time headways between itself and the current dynamic leader
      // which would cause the platoon to be unsafe. if there are violations somewhere betweent the
      // subject vehicle and the current dynamic leader, then rather than assigning dynamic
      // leadership further DOWNSTREAM, we must go further UPSTREAM in the following lines
      if (closestLowerBoundaryViolation == -1 && closestMaximumSpacingViolation == -1) {
        ...
        // introduce a hysteresis in order to eliminate the possibility of a vehicle continually
        // switching back and forth between two dynamic leaders because one of the time headways is
        // hovering right around the "lower_boundary" threshold; second the leading vehicle and its
        // predecessor must have a time headway less than "min_spacing" second. Just as with
        // "upper_boundary", "min_spacing" exists to introduce a hysteresis where dynamic leaders
        // are continually being switched.
        bool condition1 =
          timeHeadways[previousFunctionalDynamicLeaderIndex_] > config_.headawayStableLowerBond;
        bool condition2 =
          timeHeadways[previousFunctionalDynamicLeaderIndex_ - 1] < config_.headawayStableUpperBond;
        ...
        if (condition1 && condition2) {
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
            "APF found two conditions for assigning local dynamic leadership further downstream "
            "are satisfied. Case Four");
          return determineDynamicLeaderBasedOnViolation(timeHeadways);
        } else {
          ///***** Case Five *****///
          // We may not switch dynamic leadership to another vehicle further downstream because some
          // criteria are not satisfied
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
            "APF found two conditions for assigning local dynamic leadership further downstream "
            "are not satisfied. Case Five.");
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
        ...
        // The rest four cases have roughly the same logic: locate the closest violation and assign
        // dynamic leadership accordingly
        ...
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger("platooning_strategic_ihp"),
          "APF found closestMaximumSpacingViolation and closestLowerBoundaryViolation on partial "
          "time headways.");
        if (closestLowerBoundaryViolation > closestMaximumSpacingViolation) {
          ///***** Case Eight *****///
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
            "closest LowerBoundaryViolation is higher than closestMaximumSpacingViolation on "
            "partial time headways. Case Eight.");
          return previousFunctionalDynamicLeaderIndex_ - 1 + closestLowerBoundaryViolation;
        } else if (closestLowerBoundaryViolation < closestMaximumSpacingViolation) {
          ///***** Case Nine *****///
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
            "closestMaximumSpacingViolation is higher than closestLowerBoundaryViolation on "
            "partial time headways. Case Nine.");
          return previousFunctionalDynamicLeaderIndex_ + closestMaximumSpacingViolation;
        } else {
          RCLCPP_DEBUG_STREAM(
            rclcpp::get_logger("platooning_strategic_ihp"),
...
// Find the time headaway (s) sub-list based on the platoon wise comprehensive time headaway list,
// starting index is indicated by the parameter: "start".
std::vector<double> PlatoonManager::getTimeHeadwayFromIndex(
  std::vector<double> timeHeadways, int start) const
{
  std::vector<double> result(timeHeadways.begin() + start - 1, timeHeadways.end());
  return result;
}

// Determine if the gap (m) between host and predecessor is big enough, with regards to minGap_ (m)
// and maxGap_ (m).
bool PlatoonManager::insufficientGapWithPredecessor(double distanceToPredVehicle)
{
  ...
  // Host vehicle was following predecessor vehicle. --> The predecessor vehicle was violating gap
  // threshold.
  bool previousLeaderIsPredecessor =
    previousFunctionalDynamicLeaderID_ == host_platoon_[host_platoon_.size() - 1].staticId;

  // Gap greater than maxGap_ is necessary for host to stop choosing predecessor as dynamic leader.
  bool frontGapIsNotLargeEnough =
    distanceToPredVehicle < config_.maxCutinGap && previousLeaderIsPredecessor;

  return (frontGapIsTooSmall || frontGapIsNotLargeEnough);
}

// Calculate the time headway (s) behind each vehicle of the platoon. If no one behind or following
// car stoped, return infinity.
std::vector<double> PlatoonManager::calculateTimeHeadway(
  std::vector<double> downtrackDistance, std::vector<double> speed) const
{
  std::vector<double> timeHeadways(downtrackDistance.size() - 1);
  // Due to downtrack descending order, the platoon member with smaller index has larger downtrack,
  // hence closer to the front of the platoon.
  for (size_t i = 0; i < timeHeadways.size(); i++) {
    ...
    {
      timeHeadways[i] = (downtrackDistance[i] - downtrackDistance[i + 1]) /
                        speed[i + 1];  // downtrack is in m, speed is in m/s.
    }
...
{
  /**
   *  Note: For both condition, the host will always choose to follow the vechile that has a
   * relatively larger gap in front.
   *
   *
   *  max-vilation (follow veh2):   [*veh3*] ---------- [*veh2*] -------------------------------
   * [*veh1*] ---------- [*veh0*]
   *                                                       ^
  ...
  int closestLowerBoundaryViolation =
    findLowerBoundaryViolationClosestToTheHostVehicle(timeHeadways);
  int closestMaximumSpacingViolation =
    findMaximumSpacingViolationClosestToTheHostVehicle(timeHeadways);

  // Compare the violation locations, always following the closer violation vehicle (larger index)
  // first, then the furthur ones.
  if (closestLowerBoundaryViolation > closestMaximumSpacingViolation) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "APF found violation on closestLowerBoundaryViolation at " << closestLowerBoundaryViolation);
    return closestLowerBoundaryViolation;  // Min violation, following the vehicle that is in font
                                           // of the violating gap.
  } else if (closestLowerBoundaryViolation < closestMaximumSpacingViolation) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "APF found violation on closestMaximumSpacingViolation at "
        << closestMaximumSpacingViolation);
    return closestMaximumSpacingViolation +
           1;  // Max violation, follow the vehicle that is behinf the violating gap.
  } else {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"),
      "APF found no violations on both closestLowerBoundaryViolation and "
      "closestMaximumSpacingViolation");
    return 0;
  }
}

// Find the lower boundary violation vehicle that closest to the host vehicle. If no violation
// found, return -1.
int PlatoonManager::findLowerBoundaryViolationClosestToTheHostVehicle(
  std::vector<double> timeHeadways) const
{
  // Due to descending downtrack order, the search starts from the platoon rear, which corresponds
  // to last in list.
...
// Find the maximum spacing violation vehicle that closest to the host vehicle. If no violation
// found, return -1.
int PlatoonManager::findMaximumSpacingViolationClosestToTheHostVehicle(
  std::vector<double> timeHeadways) const
{
...
// Change the local platoon manager from follower operation state to leader operation state for
// single vehicle status change. This could happen because host is 2nd in line and leader is
// departing, or because APF algorithm decided host's gap is too large and we need to separate from
// front part of platoon.
void PlatoonManager::changeFromFollowerToLeader()
{
  // Get current host info - assumes departing leader or front of platoon hasn't already been
  // removed from the vector
  PlatoonMember hostInfo = host_platoon_[hostPosInPlatoon_];

  // Clear the front part of the platoon info, since we are splitting off from it; leaves host as
  // element 0
...
// Change the local platoon manager from leader operation state to follower operation state for
// single vehicle status change. This could happen because another vehicle did a front join and host
// is now 2nd in line, or because host was a solo vehicle that just completed joining a platoon at
// any position. Note: The platoon list will be firstly reset to only include the new leader and the
// host, then allowed to gradually repopulate via updateMembers() as new MobilityOperation messages
// come in.
void PlatoonManager::changeFromLeaderToFollower(std::string newPlatoonId, std::string newLeaderId)
{
  ...
  // Clear contents of the platoon vector and rebuild it with the two known members at this time,
  // leader & host. Remaining leader info and info about any other members will get populated as
  // messages come in.
  PlatoonMember newLeader = PlatoonMember();
  newLeader.staticId = newLeaderId;
  host_platoon_.clear();
  host_platoon_.push_back(
    newLeader);  // can get location info updated later with a STATUS or INFO message
  host_platoon_.push_back(hostInfo);

  hostPosInPlatoon_ =
    1;  // since host was previously leader it is now guaranteed to be 2nd in the line (index 1)
  isFollower = true;
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "The platoon manager is changed from leader state to follower state. Platoon vector "
    "re-initialized. Plan ID = "
      << newPlatoonId);
}

// Return the number of vehicles in the front of the host vehicle. If host is leader or a single
// vehicle, return 0.
int PlatoonManager::getNumberOfVehicleInFront() { return hostPosInPlatoon_; }

// Return the distance (m) to the predecessor vehicle.
double PlatoonManager::getDistanceToPredVehicle() { return gapWithPred_; }
...
std::string PlatoonManager::getHostStaticID() const { return HostMobilityId; }

// Return the physical length from platoon front vehicle (front bumper) to platoon rear vehicle
// (rear bumper) in m.
double PlatoonManager::getCurrentPlatoonLength()
{
  // this works even if platoon size is 1 (can't be 0)
  return host_platoon_[0].vehiclePosition -
         host_platoon_[host_platoon_.size() - 1].vehiclePosition + config_.vehicleLength;
}
...
double PlatoonManager::getIHPDesPosFollower(double time_step)
{
  ...
  double timeGapAndStepRatio =
    desirePlatoonGap /
    time_step;  // The ratio between desired platoon time gap and the current time_step.
  double totalTimeGap =
    desirePlatoonGap *
    pos_idx;  // The overall time gap from host vehicle to the platoon leader, in s.

  // calcilate pos_gap and pos_headway
  if ((pred_spd <= ego_spd) && (ego_spd <= config_.ss_theta)) {
    // ---> 4.1 pos_g
    pos_g =
      (pred_pos - config_.vehicleLength - config_.standstill + ego_pos * timeGapAndStepRatio) /
      (1 + timeGapAndStepRatio);
    // ---> 4.2 pos_h
    double pos_h_nom =
      (pred_pos - config_.standstill + ego_pos * (totalTimeGap + tmp_time_hdw) / time_step);
    double pos_h_denom = (1 + ((totalTimeGap + tmp_time_hdw) / time_step));
    pos_h = pos_h_nom / pos_h_denom;

  } else {
    // ---> 4.1 pos_g
    pos_g = (pred_pos - config_.vehicleLength + ego_pos * (timeGapAndStepRatio)) /
            (1 + timeGapAndStepRatio);
    // ---> 4.2 pos_h
...
// UCLA: find the index of the closest vehicle in the target platoon that is in front of the host
// vehicle (cut-in joiner). Note: The joiner will cut-in at the back of this vehicle, which make
// this index points to the vehicle that is leading the cut-in gap.
int PlatoonManager::getClosestIndex(double joinerDtD)
{
  /*
      A naive way to find the closest member that is in front of the host that point to the gap to
     join the platoon. Note: The potential gap leading vehicle should be in front of the joiner
     (i.e., gap leading vehicle's dtd > joiner's dtd). If the joiner is already in front of the
     platoon leader, this function will return -1 (i.e., cut-in front).
  */
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("platooning_strategic_ihp"),
    "neighbor_platoon_.size(): " << neighbor_platoon_.size());
  ...
  for (size_t i = 0; i < neighbor_platoon_.size(); i++) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "current_member_dtd: " << current_member_dtd);
    double curent_dtd_diff = current_member_dtd - joinerDtD;
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("platooning_strategic_ihp"), "curent_dtd_diff: " << curent_dtd_diff);
    // update min index
...
// UCLA: find the current cut-in join gap size in downtrack distance (m). The origin of the vehicle
// when calculating DtD is locate at the rear axle.
