...
{
std::vector<PointSpeedPair> create_geometry_profile(
  const std::vector<carma_planning_msgs::msg::Maneuver> & maneuvers, double max_starting_downtrack,
  ...
  const DetailedTrajConfig & detailed_config)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "VehDowntrack:" << max_starting_downtrack);
  for (const auto & maneuver : maneuvers) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Used downtrack: " << starting_downtrack);

    if (maneuver.type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Creating Lane Follow Geometry");
      std::vector<PointSpeedPair> lane_follow_points = create_lanefollow_geometry(
        maneuver, starting_downtrack, wm, general_config, detailed_config, visited_lanelets);
      points_and_target_speeds.insert(
        points_and_target_speeds.end(), lane_follow_points.begin(), lane_follow_points.end());
    } else if (maneuver.type == carma_planning_msgs::msg::Maneuver::LANE_CHANGE) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Creating Lane Change Geometry");
      std::vector<PointSpeedPair> lane_change_points = get_lanechange_points_from_maneuver(
        maneuver, starting_downtrack, wm, ending_state_before_buffer, state, general_config,
        detailed_config);
      points_and_target_speeds.insert(
        points_and_target_speeds.end(), lane_change_points.begin(), lane_change_points.end());
    } else {
  ...
  if (maneuvers.back().type == carma_planning_msgs::msg::Maneuver::LANE_FOLLOWING) {
    points_and_target_speeds = add_lanefollow_buffer(
      wm, points_and_target_speeds, maneuvers, ending_state_before_buffer, detailed_config);
  }
  return points_and_target_speeds;
}

std::vector<PointSpeedPair> create_lanefollow_geometry(
  const carma_planning_msgs::msg::Maneuver & maneuver, double starting_downtrack,
  ...
  const DetailedTrajConfig & detailed_config, std::unordered_set<lanelet::Id> & visited_lanelets)
{
  ...
  carma_planning_msgs::msg::LaneFollowingManeuver lane_following_maneuver =
    maneuver.lane_following_maneuver;
  ...
  std::vector<lanelet::ConstLanelet> lanelets = {wm->getMap()->laneletLayer.get(
    stoi(lane_following_maneuver.lane_ids[0]))};  // Accept first lanelet reguardless
  for (size_t i = 1; i < lane_following_maneuver.lane_ids.size();
       i++)  // Iterate over remaining lanelets and check if they are followers of the previous
             // lanelet
  {
  ...
  if (lanelets.empty()) {
    RCLCPP_ERROR_STREAM(
      ...
      "Detected no lanelets between starting downtrack: "
        << starting_downtrack
  ...
  // 400 value here is an arbitrary attempt at improving inlane-cruising performance by reducing
  // copy operations. Value picked based on annecdotal evidence from STOL system testing
  ...
  if (lanelets.size() <= 1)  // no lane change anyways if only size 1
  {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
      "Detected one straight lanelet Id:" << lanelets[curr_idx].id());
    straight_lanelets = lanelets;
  } else {
    // skip all lanechanges until lane follow starts
    while (
      curr_idx + 1 < lanelets.size() &&
      std::find(following_lanelets.begin(), following_lanelets.end(), lanelets[curr_idx + 1]) ==
        following_lanelets.end()) {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
        "As there were no directly following lanelets after this, skipping lanelet id: "
          << lanelets[curr_idx].id());
      curr_idx++;
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
      "Added lanelet Id for lane follow: " << lanelets[curr_idx].id());
    ...
    while (
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
        "Added lanelet Id forlane follow: " << lanelets[curr_idx].id());
      straight_lanelets.push_back(lanelets[curr_idx]);
      following_lanelets = wm->getMapRoutingGraph()->following(lanelets[curr_idx]);
    }
  }
  ...
  for (auto l : straight_lanelets) {
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Processing lanelet ID: " << l.id());
    if (visited_lanelets.find(l.id()) == visited_lanelets.end()) {
      ...
      if (is_turn) {
        downsampled_points = carma_ros2_utils::containers::downsample_vector(
          centerline, general_config.turn_downsample_ratio);
      ...
      if (
        ...
        downsampled_points.size() != 0  // If this is not the first lanelet and the points are
                                        // closer than 1m drop the first point to prevent overlap
        && lanelet::geometry::distance2d(
             downsampled_points.front(), downsampled_centerline.back()) < 1.2) {
        downsampled_points =
          lanelet::BasicLineString2d(downsampled_points.begin() + 1, downsampled_points.end());
      }

      downsampled_centerline =
        carma_wm::geometry::concatenate_line_strings(downsampled_centerline, downsampled_points);
  ...
  for (auto p : downsampled_centerline) {
    if (first && !points_and_target_speeds.empty()) {
      first = false;
      continue;  // Skip the first point if we have already added points from a previous maneuver to
                 // avoid duplicates
    }
    PointSpeedPair pair;
    pair.point = p;
...
std::vector<PointSpeedPair> add_lanefollow_buffer(
  const carma_wm::WorldModelConstPtr & wm, std::vector<PointSpeedPair> & points_and_target_speeds,
  const std::vector<carma_planning_msgs::msg::Maneuver> & maneuvers,
  carma_planning_msgs::msg::VehicleState & ending_state_before_buffer,
  const DetailedTrajConfig & detailed_config)
{
  double starting_route_downtrack =
    wm->routeTrackPos(points_and_target_speeds.front().point).downtrack;

  // Always try to add the maximum buffer. Even if the route ends it may still be possible to add
  // buffered points. This does mean that downstream components might not be able to assume the
  // buffer points are on the route though this is not likely to be an issue as they are buffer only
  double ending_downtrack =
    maneuvers.back().lane_following_maneuver.end_dist + detailed_config.buffer_ending_downtrack;

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Add lanefollow buffer: ending_downtrack: "
      << ending_downtrack << ", maneuvers.back().lane_following_maneuver.end_dist: "
      << maneuvers.back().lane_following_maneuver.end_dist
  ...
  for (size_t i = 0; i < points_and_target_speeds.size(); ++i) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
      "Index i: " << i << ", delta_d: " << delta_d << ", dist_accumulator:" << dist_accumulator
                  << ", current_point.x():" << current_point.x()
                  << "current_point.y():" << current_point.y());
    if (
      dist_accumulator > maneuvers.back().lane_following_maneuver.end_dist &&
      !found_unbuffered_idx) {
      unbuffered_idx = i - 1;
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    ...
    if (dist_accumulator > ending_downtrack) {
      ...
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
        "Max_i breaking at: i: " << i << ", max_i: " << max_i);
      break;
    }

    // If there are no more points to add but we haven't reached the ending downtrack then
    // construct an extrapolated straight line from the final point and keep adding to this line
    // until the downtrack is met Since this is purely needed to allow for a spline fit edge case,
    // it should have minimal impact on the actual steering behavior of the vehicle
    if (i == points_and_target_speeds.size() - 1)  // dist_accumulator < ending_downtrack is
                                                   // guaranteed by earlier conditional
    {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
      ...
      while (delta_d < epsilon_ && j >= 0 && !delta_point) {
        RCLCPP_DEBUG_STREAM(
          rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
          "Looking at index j: " << j << ", where i: " << i);
        prev_point = points_and_target_speeds.at(j).point;
        j--;
        delta_d = lanelet::geometry::distance2d(prev_point, current_point);
      }

      if (j < 0 && delta_d < epsilon_)  // a very rare case where only duplicate points exist in the
                                        // entire trajectory, so it wasn't possible to extend
        break;

      if (!delta_point) {  // Set the step size based on last two distinct points
        delta_point =
          (current_point - prev_point) *
          0.25;  // Use a smaller step size then default to help ensure enough points are generated;
      }
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "Here ending_state_before_buffer.x_pos_global: " << ending_state_before_buffer.x_pos_global
                                                     << ", and y_pos_global"
                                                     << ending_state_before_buffer.y_pos_global);

  std::vector<PointSpeedPair> constrained_points(
    points_and_target_speeds.begin(), points_and_target_speeds.begin() + max_i);
...
std::vector<lanelet::BasicPoint2d> create_lanechange_geometry(
  lanelet::Id starting_lane_id, lanelet::Id ending_lane_id, double starting_downtrack,
  double ending_downtrack, const carma_wm::WorldModelConstPtr & wm, int downsample_ratio,
  double buffer_ending_downtrack)
{
  ...
  lanelet::BasicLineString2d current_lanelet_centerline =
    starting_lanelet.centerline2d().basicLineString();
  lanelet::ConstLanelet current_lanelet = starting_lanelet;
  reference_centerline.insert(
    reference_centerline.end(), current_lanelet_centerline.begin(),
    current_lanelet_centerline.end());

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Searching for shared boundary with starting lanechange lanelet "
      << std::to_string(current_lanelet.id()) << " and ending lanelet "
  ...
  while (!shared_boundary_found) {
    ...
    if (current_lanelet.leftBound() == ending_lanelet.rightBound()) {
      ...
      if (wm->getMapRoutingGraph()->following(current_lanelet, false).empty()) {
        // Maneuver requires we travel further before completing lane change, but no routable
        // lanelet directly ahead
        // In this case we have reached a lanelet which does not have a routable lanelet ahead +
        // isn't adjacent to the lanelet where lane change ends A lane change should have already
        // happened at this point
        throw(
          std::invalid_argument("No following lanelets from current lanelet reachable without a "
                                "lane change, incorrectly chosen end lanelet"));
      }
      ...
      reference_centerline.insert(
        reference_centerline.end(), current_lanelet_linestring.begin() + 1,
        current_lanelet_linestring.end());
      starting_lane.push_back(current_lanelet);
    }
  }

  // Create the target lane centerline using lanelets adjacent to the lanechange lanelets in the
  // starting lane
  ...
  for (size_t i = 0; i < starting_lane.size(); ++i) {
    ...
    // Concatenate linestring starting from + 1 to avoid overlap
    target_lane_centerline.insert(
      target_lane_centerline.end(), target_lane_linestring.begin() + 1,
      target_lane_linestring.end());
  }
  ...
  int start_index_starting_centerline = waypoint_generation::get_nearest_index_by_downtrack(
    downsampled_starting_centerline, wm, starting_downtrack);
  ...
  int start_index_target_centerline =
    waypoint_generation::get_nearest_point_index(downsampled_target_centerline, start_state);

  int end_index_target_centerline = waypoint_generation::get_nearest_index_by_downtrack(
    downsampled_target_centerline, wm, ending_downtrack);
  ...
  int end_index_starting_centerline =
    waypoint_generation::get_nearest_point_index(downsampled_starting_centerline, end_state);

  std::vector<lanelet::BasicPoint2d> constrained_start_centerline(
    downsampled_starting_centerline.begin() + start_index_starting_centerline,
    downsampled_starting_centerline.begin() + end_index_starting_centerline);
  std::vector<lanelet::BasicPoint2d> constrained_target_centerline(
    downsampled_target_centerline.begin() + start_index_target_centerline,
    downsampled_target_centerline.begin() + end_index_target_centerline);

  // If constrained centerlines are not the same size - resample to ensure same size along both
  // centerlines
  if (constrained_start_centerline.size() != constrained_target_centerline.size()) {
    auto centerlines = resample_linestring_pair_to_same_size(
      constrained_start_centerline, constrained_target_centerline);
  ...
  // Add points from the remaining length of the target lanelet to provide sufficient distance for
  // adding buffer
  double dist_to_target_lane_end =
    lanelet::geometry::distance2d(centerline_points.back(), downsampled_target_centerline.back());
  centerline_points.insert(
    centerline_points.end(), downsampled_target_centerline.begin() + end_index_target_centerline,
    downsampled_target_centerline.end());

  // If the additional distance from the remaining length of the target lanelet does not provide
  // more than the required buffer_ending_downtrack, then also add points from the lanelet following
  // the target lanelet
  if (dist_to_target_lane_end < buffer_ending_downtrack) {
    ...
    if (!following_lanelets.empty()) {
      // Arbitrarily choosing first following lanelet for buffer since points are only being used to
      // fit spline
      auto following_lanelet_centerline =
        following_lanelets.front().centerline2d().basicLineString();
      centerline_points.insert(
        centerline_points.end(), following_lanelet_centerline.begin(),
...
std::vector<std::vector<lanelet::BasicPoint2d>> resample_linestring_pair_to_same_size(
  std::vector<lanelet::BasicPoint2d> & line_1, std::vector<lanelet::BasicPoint2d> & line_2)
{
  auto start_time = std::chrono::high_resolution_clock::now();  // Start timing the execution time
                                                                // for planning so it can be logged

  std::vector<std::vector<lanelet::BasicPoint2d>> output;

  // Fit centerlines to a spline
  std::unique_ptr<smoothing::SplineI> fit_curve_1 =
    compute_fit(line_1);  // Compute splines based on curve points
  if (!fit_curve_1) {
    throw std::invalid_argument(
      "Could not fit a spline curve along the starting_lane centerline points!");
  }

  std::unique_ptr<smoothing::SplineI> fit_curve_2 =
    compute_fit(line_2);  // Compute splines based on curve points
  if (!fit_curve_2) {
    throw std::invalid_argument(
  ...
  // TODO: are we missing some computation here?  step_threshold_line1 and step_threshold_line2 are
  // not used anywhere
  ...
  // TODO: unused variable: int total_step_along_curve2 =
  // static_cast<int>(downtracks_raw_line2.back() / 2.0);
  // TODO: unused variable: double step_threshold_line2 = (double)total_step_along_curve2 /
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "ExecutionTime for resample lane change centerlines: " << duration.count() << " milliseconds");

  return output;
}

std::vector<PointSpeedPair> get_lanechange_points_from_maneuver(
  const carma_planning_msgs::msg::Maneuver & maneuver, double starting_downtrack,
  const carma_wm::WorldModelConstPtr & wm,
  carma_planning_msgs::msg::VehicleState & ending_state_before_buffer,
  const carma_planning_msgs::msg::VehicleState & state, const GeneralTrajConfig & general_config,
  const DetailedTrajConfig & detailed_config)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Maneuver ending downtrack:" << ending_downtrack);
  ...
  // get route between starting and ending downtracks - downtracks should be constant for complete
  // length of maneuver
  std::vector<lanelet::BasicPoint2d> route_geometry = create_lanechange_geometry(
    std::stoi(lane_change_maneuver.starting_lane_id),
    std::stoi(lane_change_maneuver.ending_lane_id), starting_downtrack, ending_downtrack, wm,
    general_config.default_downsample_ratio, detailed_config.buffer_ending_downtrack);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Route geometry size:" << route_geometry.size());
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "ending_state_before_buffer_:" << ending_state_before_buffer.x_pos_global
                                   << ", ending_state_before_buffer_.y_pos_global"
                                   << ending_state_before_buffer.y_pos_global);
  ...
  if (ending_downtrack + detailed_config.buffer_ending_downtrack < route_length) {
    ending_pt_index = get_nearest_index_by_downtrack(
      route_geometry, wm, ending_downtrack + detailed_config.buffer_ending_downtrack);
  } else {
    ending_pt_index = route_geometry.size() - 1;
  }

  lanelet::BasicLineString2d future_route_geometry(
    route_geometry.begin() + nearest_pt_index, route_geometry.begin() + ending_pt_index);
  bool first = true;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Future geom size:" << future_route_geometry.size());
  ...
  for (auto p : future_route_geometry) {
    ...
    PointSpeedPair pair;
    pair.point = p;
    // If current speed is above min speed, keep at current speed. Otherwise use end speed from
    // maneuver.
    pair.speed = (state.longitudinal_vel > detailed_config.minimum_speed)
                   ? state.longitudinal_vel
                   : lane_change_maneuver.end_speed;
    points_and_target_speeds.push_back(pair);
  }
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
...
std::vector<double> apply_speed_limits(
  const std::vector<double> speeds, const std::vector<double> speed_limits)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Speeds list size: " << speeds.size());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "SpeedLimits list size: " << speed_limits.size());
...
Eigen::Isometry2d compute_heading_frame(
...
std::vector<PointSpeedPair> constrain_to_time_boundary(
  const std::vector<PointSpeedPair> & points, double time_span)
{
  ...
  if (time_boundary_exclusive_index == points.size()) {
    time_bound_points.insert(
      time_bound_points.end(), points.begin(),
...
std::pair<double, size_t> min_with_exclusions(
  const std::vector<double> & values, const std::unordered_set<size_t> & excluded)
{
...
std::vector<double> optimize_speed(
  const std::vector<double> & downtracks, const std::vector<double> & curv_speeds,
  double accel_limit)
{
  ...
  while (optimize) {
    ...
    for (int i = min_idx - 1; i > 0;
         i--) {  // NOTE: Do not use size_t for i type here as -- with > 0 will result in overflow
      ...
      if (dv > 0) {
        v_f = std::min(
          v_f, sqrt(
                 v_i * v_i - 2 * accel_limit *
                               dx));  // inverting accel as we are only visiting deceleration case
  ...
  output =
    trajectory_utils::apply_accel_limits_by_distance(downtracks, output, accel_limit, accel_limit);
  ...
  const std::string & desired_controller_plugin)
{
  ...
  for (size_t i = 0; i < points.size(); i++) {
    ...
    rclcpp::Duration relative_time = rclcpp::Duration::from_nanoseconds(
      static_cast<int64_t>(times[i] * 1e9));  // Conversion of times[i] from seconds to nanoseconds
...
std::vector<PointSpeedPair> attach_past_points(
  ...
  const int nearest_pt_index, double back_distance)
{
  ...
  back_and_future.insert(
    back_and_future.end(), points_set.begin() + min_i, points_set.begin() + nearest_pt_index + 1);
...
double compute_curvature_at(
  const basic_autonomy::smoothing::SplineI & fit_curve, double step_along_the_curve)
{
...
std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> compose_lanefollow_trajectory_from_path(
  const std::vector<PointSpeedPair> & points, const carma_planning_msgs::msg::VehicleState & state,
...
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "VehicleState: "
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "NearestPtIndex: " << nearest_pt_index);

  std::vector<PointSpeedPair> future_points(
    points.begin() + nearest_pt_index + 1,
    points.end());  // Points in front of current vehicle position

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Ready to call constrain_to_time_boundary: future_points size = "
      << future_points.size()
      << ", trajectory_time_length = " << detailed_config.trajectory_time_length);

  auto time_bound_points =
    constrain_to_time_boundary(future_points, detailed_config.trajectory_time_length);
  ...
  std::vector<PointSpeedPair> back_and_future =
    attach_past_points(points, time_bound_points, nearest_pt_index, detailed_config.back_distance);
  ...
  std::unique_ptr<smoothing::SplineI> fit_curve =
    compute_fit(curve_points);  // Compute splines based on curve points
  if (!fit_curve) {
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "speed_limits.size() " << speed_limits.size());
  ...
  auto total_step_along_curve =
    static_cast<int>(downtracks_raw.back() / detailed_config.curve_resample_step_size);
  ...
  for (int steps_along_curve = 0; steps_along_curve < total_step_along_curve;
       steps_along_curve++)  // Resample curve at tighter resolution
  {
    ...
    distributed_speed_limits.push_back(
      speed_limits[current_speed_index]);  // Identify speed limits for resampled points
  ...
  std::vector<double> final_yaw_values =
    carma_wm::geometry::compute_tangent_orientations(all_sampling_points);

  log::printDoublesPerLineWithPrefix("raw_curvatures[i]: ", better_curvature);

  std::vector<double> curvatures = smoothing::moving_average_filter(
    better_curvature, detailed_config.curvature_moving_average_window_size, false);
  std::vector<double> ideal_speeds = trajectory_utils::constrained_speeds_for_curvatures(
    curvatures, detailed_config.lateral_accel_limit);
  ...
  std::vector<double> constrained_speed_limits =
    apply_speed_limits(ideal_speeds, distributed_speed_limits);

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Processed all points in computed fit");

  if (all_sampling_points.empty()) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "No trajectory points could be generated");
    return {};
  }
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Curvature right now: " << better_curvature[nearest_pt_index] << ", at state x: "
                            << state.x_pos_global << ", state y: " << state.y_pos_global);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Corresponding to point: x: " << all_sampling_points[nearest_pt_index].x()
                                  << ", y:" << all_sampling_points[nearest_pt_index].y());

  int buffer_pt_index =
    get_nearest_index_by_downtrack(all_sampling_points, wm, ending_state_before_buffer);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Ending state's index before applying buffer (buffer_pt_index): " << buffer_pt_index);
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Corresponding to point: x: " << all_sampling_points[buffer_pt_index].x()
                                  << ", y:" << all_sampling_points[buffer_pt_index].y());

  if (nearest_pt_index + 1 >= buffer_pt_index) {
    lanelet::BasicPoint2d current_pos(state.x_pos_global, state.y_pos_global);
    lanelet::BasicPoint2d ending_pos(
      ending_state_before_buffer.x_pos_global, ending_state_before_buffer.y_pos_global);
    ...
    if (wm->routeTrackPos(ending_pos).downtrack < wm->routeTrackPos(current_pos).downtrack) {
      ...
      std::vector<double> downtracks =
        carma_wm::geometry::compute_arc_lengths(remaining_traj_points);
      std::vector<double> speeds = {
        state.longitudinal_vel, state.longitudinal_vel};  // Keep current speed
  ...
  std::vector<lanelet::BasicPoint2d> future_basic_points(
    all_sampling_points.begin() + nearest_pt_index + 1,
    all_sampling_points.begin() + buffer_pt_index);  // Points in front of current vehicle position

  std::vector<double> future_speeds(
    constrained_speed_limits.begin() + nearest_pt_index + 1,
    constrained_speed_limits.begin() +
      buffer_pt_index);  // Points in front of current vehicle position
  std::vector<double> future_yaw(
    final_yaw_values.begin() + nearest_pt_index + 1,
  ...
  final_actual_speeds = smoothing::moving_average_filter(
    final_actual_speeds, detailed_config.speed_moving_average_window_size);
  ...
  std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> traj_points =
    trajectory_from_points_times_orientations(
      all_sampling_points, times, final_yaw_values, state_time,
      detailed_config.desired_controller_plugin);
  ...
  std::vector<double> aligned_speed_limits(
    constrained_speed_limits.begin() + nearest_pt_index, constrained_speed_limits.end());

  msg.speed_limits = aligned_speed_limits;
  std::vector<double> aligned_curvatures(curvatures.begin() + nearest_pt_index, curvatures.end());
...
DetailedTrajConfig compose_detailed_trajectory_config(
  double trajectory_time_length, double curve_resample_step_size, double minimum_speed,
  double max_accel, double lateral_accel_limit, int speed_moving_average_window_size,
  int curvature_moving_average_window_size, double back_distance, double buffer_ending_downtrack,
...
GeneralTrajConfig compose_general_trajectory_config(
  const std::string & trajectory_type, int default_downsample_ratio, int turn_downsample_ratio)
{
  ...
  const DetailedTrajConfig & detailed_config)
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "nearest_pt_index: " << nearest_pt_index);

  std::vector<PointSpeedPair> future_points(points.begin() + nearest_pt_index + 1, points.end());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "future_points size: " << future_points.size());
  ...
  future_geom_points.insert(
    future_geom_points.begin(),
  ...
  auto total_step_along_curve =
    static_cast<int>(downtracks.back() / detailed_config.curve_resample_step_size);
  ...
  std::vector<double> final_yaw_values =
    carma_wm::geometry::compute_tangent_orientations(future_geom_points);
  if (final_yaw_values.size() > 0) {
    final_yaw_values[0] =
      state.orientation;  // Set the initial yaw value based on the initial state
  }
  ...
  int end_dist_pt_index =
    get_nearest_index_by_downtrack(future_geom_points, wm, ending_state_before_buffer);
  ...
  std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> traj_points =
    trajectory_from_points_times_orientations(
      future_geom_points, times, final_yaw_values, state_time,
      detailed_config.desired_controller_plugin);

  return traj_points;
}

autoware_auto_msgs::msg::Trajectory process_trajectory_plan(
  const carma_planning_msgs::msg::TrajectoryPlan & tp, double vehicle_response_lag)
{
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "Processing latest TrajectoryPlan message");

  std::vector<double> times;
  std::vector<double> downtracks;

  std::vector<carma_planning_msgs::msg::TrajectoryPlanPoint> trajectory_points =
    tp.trajectory_points;

  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
    "Original Trajectory size:" << trajectory_points.size());

  trajectory_utils::conversions::trajectory_to_downtrack_time(
    trajectory_points, &downtracks, &times);
  ...
  for (size_t i = 1; i < times.size(); i++) {
    if (times[i] == times[i - 1])  // if exactly same, it is stopping case
    {
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
        "Detected a stopping case where times is exactly equal: " << times[i - 1]);
      RCLCPP_DEBUG_STREAM(
        rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
        "And index of that is: " << i << ", where size is: " << times.size());
      stopping_index = i;
      break;
    }
  }
  ...
  std::vector<double> speeds;
  try {
    trajectory_utils::conversions::time_to_speed(
      downtracks, times, tp.initial_longitudinal_velocity, &speeds);
  } catch (const std::runtime_error & error) {
    // This can only happen if there was negative speed in trajectory generation which usually
    // happens when intending to stop. The plugin is catching that error and logging it for the user
    // to correct the origin plugin's logic, but continues the operation by forcing the negative
    // values to be 0, which is the intention usually.
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
      "Detected a negative speed from <point,time> to <point,speed> trajectory conversion with "
      "error: "
        << error.what()
        << ". Replacing the negative speed with 0.0 speed, but please revisit the trajectory "
           "logic. "
           "Responsible plugin is: "
        << trajectory_points[std::find(speeds.begin(), speeds.end(), 0.0) - speeds.begin()]
             .planner_plugin_name);
  }
  ...
  lag_speeds = apply_response_lag(
    speeds, downtracks, vehicle_response_lag);  // This call requires that the first speed point be
                                                // current speed to work as expected

  autoware_auto_msgs::msg::Trajectory autoware_trajectory;
  autoware_trajectory.header = tp.header;
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger(BASIC_AUTONOMY_LOGGER), "size: " << trajectory_points.size());

  auto max_size = std::min(
    99,
    (int)trajectory_points
      .size());  // NOTE: more than this size autoware auto raises exception with "Exceeded upper
                 // bound while in ACTIVE state." large portion of the points are not needed anyways
  for (int i = 0; i < max_size; i++) {
    ...
    if (i < max_size - 1) {
      yaw = std::atan2(
        trajectory_points[i + 1].y - trajectory_points[i].y,
        trajectory_points[i + 1].x - trajectory_points[i].x);

    } else {
      yaw = std::atan2(
        trajectory_points[max_size - 1].y - trajectory_points[max_size - 2].y,
        trajectory_points[max_size - 1].x - trajectory_points[max_size - 2].x);
      // last point in the trajectory will have yaw value of its previous point to avoid sudden
      // steering in some conditions
    }
    autoware_point.heading.real = std::cos(yaw / 2);
    autoware_point.heading.imag = std::sin(yaw / 2);

    autoware_point.time_from_start =
      rclcpp::Duration::from_nanoseconds(static_cast<int64_t>(times[i] * 1e9));
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(BASIC_AUTONOMY_LOGGER),
      "Setting waypoint idx: " << i << ", with planner: << "
                               << trajectory_points[i].planner_plugin_name << ", x: "
                               << trajectory_points[i].x << ", y: " << trajectory_points[i].y
...
std::vector<double> apply_response_lag(
  const std::vector<double> & speeds, const std::vector<double> downtracks, double response_lag)
{  // Note first speed is assumed to be vehicle speed
  ...
  size_t lookahead_count =
    std::lower_bound(downtracks.begin(), downtracks.end(), downtrack_cutoff) -
    downtracks.begin();  // Use binary search to find lower bound cutoff point
  output = trajectory_utils::shift_by_lookahead(speeds, (unsigned int)lookahead_count);
  return output;
}

bool is_valid_yield_plan(
  ...
  const carma_planning_msgs::msg::TrajectoryPlan & yield_plan)
{
  ...
  if (
    rclcpp::Time(yield_plan.trajectory_points[0].target_time) + rclcpp::Duration(5.0, 0) >
    node_handler->now()) {
    return true;
  } else {
  ...
  int yield_plugin_service_call_timeout)
{
  ...
  auto future_status =
    yield_resp.wait_for(std::chrono::milliseconds(yield_plugin_service_call_timeout));

  if (future_status != std::future_status::ready) {
    // Sometimes the yield plugin's service call may be unsuccessful due to its computationally
    // expensive logic. However, consecutive calls can be successful, so return original trajectory
    // for now
    RCLCPP_WARN(
      node_handler->get_logger(),
