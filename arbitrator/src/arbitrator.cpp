...
namespace arbitrator
{
    void Arbitrator::run()
    {
        ROS_INFO_STREAM("Aribtrator started, beginning arbitrator state machine.");
        while (!ros::isShuttingDown())
        {
            ...
            {
                case INITIAL:
                    ROS_INFO_STREAM("Aribtrator spinning in INITIAL state.");
                    initial_state();
                    break;
                case PLANNING:
                    ROS_INFO_STREAM("Aribtrator spinning in PLANNING state.");
                    planning_state();
                    break;
                case WAITING:
                    ROS_INFO_STREAM("Aribtrator spinning in WAITING state.");
                    waiting_state();
                    break;
                case PAUSED:
                    ROS_INFO_STREAM("Aribtrator spinning in PAUSED state.");
                    paused_state();
                    break;
                default:
    ...
    void Arbitrator::guidance_state_cb(const cav_msgs::GuidanceState::ConstPtr& msg) 
    {
        switch (msg->state)
        {
            ...
            case cav_msgs::GuidanceState::DRIVERS_READY:
                ...
                {
                    ROS_ERROR_STREAM("Received notice that guidance has been restarted, pausing arbitrator.");
            ...
            case cav_msgs::GuidanceState::ENGAGED:
                ROS_ERROR_STREAM("Received notice that guidance has been engaged!");
            ...
            case cav_msgs::GuidanceState::INACTIVE:
                ROS_ERROR_STREAM("Received notice that guidance has been disengaged, pausing arbitrator.");
                sm_->submit_event(ArbitratorEvent::ARBITRATOR_PAUSED);
                break;
            case cav_msgs::GuidanceState::SHUTDOWN:
                ROS_ERROR_STREAM("Received notice that guidance has been shutdown, shutting down arbitrator.");
    ...
    void Arbitrator::initial_state()
    {
        if(!initialized_)
        {   
            ROS_ERROR_STREAM("Arbitrator initializing on first initial state spin...");
    ...
    {
        ROS_ERROR_STREAM("Aribtrator beginning planning process!");
        ros::Time planning_process_start = ros::Time::now();
        cav_msgs::ManeuverPlan plan = planning_strategy_.generate_plan();
        ROS_ERROR_STREAM("arbitrator: planning state: just genegrated plan");        
        ...
        if (!plan.maneuvers.empty()) 
        {
            ...
            {
                ROS_ERROR_STREAM("Arbitrator is unable to generate a plan with minimum plan duration requirement!");
            } 
            else 
            {
                ROS_ERROR_STREAM("Arbitrator is publishing plan " << plan.maneuver_plan_id << " of duration " << plan_duration << " as current maneuver plan");
            }
            final_plan_pub_.publish(plan);
            ROS_ERROR_STREAM("arbitrator: planning state: just published plan");        

        }
        else
        {
            ROS_ERROR("Arbitrator was unable to generate a plan!");
        }
        ...
        ROS_ERROR_STREAM("arbitrator: planning state: planning ended");        
    ...
    void Arbitrator::waiting_state()
    {
        ...
        ROS_ERROR_STREAM("Arbitrator transitioning from WAITING to PLANNING state.");
    ...
    {
        ROS_ERROR_STREAM("Arbitrator shutting down...");
