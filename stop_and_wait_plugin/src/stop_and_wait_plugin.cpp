...
namespace stop_and_wait_plugin
{
    ...
    bool StopandWait::plan_trajectory_cb(cav_srvs::PlanTrajectoryRequest& req, cav_srvs::PlanTrajectoryResponse& resp)
    {
        ...
        {
            if(maneuver.type == cav_msgs::Maneuver::STOP_AND_WAIT && maneuver.stop_and_wait_maneuver.end_dist > current_downtrack) // Only process maneuvers beyond our starting point
            {
        ...
        trajectory.trajectory_points = compose_trajectory_from_centerline(points_and_target_speeds,curr_state, req.header.stamp);
                                                                      ...
                                                                      const carma_wm::WorldModelConstPtr& wm, const cav_msgs::VehicleState& state)
    {
        ...
        for (const auto& l : lanelets)
        {
            ...
            downsampled_points = carma_utils::containers::downsample_vector(centerline, downsample_ratio_); // Downsample centerline
...
// TODO how are we going to handle crawling to the stop line if stopped before?
    ...
    const std::vector<PointSpeedPair>& points, double starting_downtrack, double starting_speed, double stop_location, ros::Time start_time)
    {
        ...
        double target_accel = accel_multiplier * accel_limit; // TODO mkae limit and multiplier parameters
        double req_dist = starting_speed / (2.0 * target_accel); // Distance needed to go from current speed to 0 at target accel

        if (req_dist > remaining_distance) {
            ROS_DEBUG_STREAM("Target Accel Update From: " << target_accel);
            target_accel = starting_speed / (2.0 * remaining_distance); // If we cannot reach the end point it the required distance update the accel target
        ...
        for (int i = points.size() - 2; i > 0; i--) { // NOTE: Do not use size_t for i type here as -- with > 0 will result in overflow
                                                      //       First point's speed is left unchanged as it is current speed of the vehicle
            double v_i = prev_pair.speed;
            
            if (v_i >= starting_speed) { // We are walking backward, so if the prev speed is greater than or equal to the starting speed then we are done backtracking
                reached_end = true;
        ...
        while(traj.back().target_time - traj.front().target_time < ros::Duration(minimal_trajectory_duration_)) {
            cav_msgs::TrajectoryPlanPoint new_point = traj.back();
            new_point.target_time = new_point.target_time + ros::Duration(min_timestep_);
